    ...
    <>
        {/*
          🔴 §155⑥1호 문화유산 주택 토글은 **명부 행으로 옮겼다**(D-6 · P7-3).
             ...
             입력: `HouseEntryOneHouseFactsSection`(명부 행 편집 모달 ⑤)
             도출: `deriveOneHouseFactsFromHouses` → ④ `culturalHeritageHouse`

          법문이 「문화유산주택과 일반주택을 **각각 1개씩** 소유」라 문화유산주택은 **보유 중인
          다른 주택**이다 ⇒ 세대 단위 boolean이 아니라 「어느 주택인가」가 정본이다.

          ⚠️ 여기서 지우고 행에서 켜는 것을 **같은 PR에서** 한다 — 순서가 갈리면 이중 입력이거나
             유일 입력 경로 제거가 된다([[feedback_ui_gate_removes_sole_input_path]]).
