...
test.describe("종합부동산세 사례4 — 직전연도 다주택 중과 자동계산", () => {
  test("PYM-E1: 사례4 — ⑤ 6,464,123 + 직전 종부세상당액 39,556,223(중과 3.6%)", async ({
    ...
    await page.getByPlaceholder("금액 입력").nth(1).fill("1400000000");
    ...
    // 당해 2주택 → 주택별 직전 공시 입력 (placeholder="0")
    await page.getByPlaceholder("0").nth(0).fill("1200000000");
    await page.getByPlaceholder("0").nth(1).fill("1300000000");
    // 직전연도 조정대상지역 2주택 토글 ON
    //   Step5 switch 순서: [0] 당해 조정2주택(OFF 유지) [1] 직전 조정2주택 [2] 직전 1세대1주택
    await expect(
  ...
  test("REGR: 사례2 단일 주택 자동 — 294,923 (신규 필드 추가 후 회귀)", async ({
    ...
    // Step5: 자동모드 + 단일 직전 공시 9억 (당해 1주택 → 단일 입력 경로 보존)
    await page.getByTestId("cap-mode-auto").click();
