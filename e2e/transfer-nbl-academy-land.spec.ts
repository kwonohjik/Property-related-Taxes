...
test.describe("인천 중구 내동 6-20 학원용 토지 환산취득가액", () => {
  test("단독 양도 → 양도세 계산 결과 캡처", async ({ page }) => {
    ...
    // ─── 최소 검증 (엔진 응답 확인) ─────────────────────────────────
    //
    // ⚠️ **이 spec은 회귀 테스트가 아니라 캡처 하네스다** (COV-3, 2026-09-02 코드리뷰).
    //    위 기대값 10줄은 `console.log`로만 출력되고 아래 3건은 「비어 있지 않다」만 본다 —
    //    값이 전부 0으로 바뀌어도 통과한다. 사례 자체도 비사업용 **OFF**(사업용)라
    //    §104①8호 중과를 관측하지 않는다.
    //
    //    중과세액(+10%p)을 금액으로 단언하는 회귀 테스트는
    //    **`e2e/transfer-nbl-surcharge-amount.spec.ts`**에 있다. 이 파일의 기대값을
    //    단언으로 승격하려면 「엔진은 단계별 floor로 최대 1원 작다」는 헤더 문구대로
    //    1원 tolerance 규약을 먼저 명시할 것.
