...
test.describe("일반건물 × 지분 분할 취득", () => {
  ...
  test("GBF-28: 신고서 취득일이 지분별로 다르고, 자산별 산식이 렌더된다", async ({ page }) => {
     ...
     *
     * 🔄 2026-08-23 정정 — 종전 셀렉터는 `text=/[0-9,]+ × [0-9,]+ \//`였다.
     *    PR #1240(2026-08-13)이 `FormulaParts.renderFormula`를 상세명세서에 배선하면서
     *    **문자열 산식의 `숫자 / 숫자`를 자동으로 `<Frac>`으로 치환**한다 — DOM에서 `/`가
     *    사라지므로 그 정규식은 영구히 매칭될 수 없다. 실제 렌더(실측):
     *
     *      <p>1,000,000,000 × <span>            ← Frac
     *        <span>200,000,000</span>                   분자
     *        <span class="border-t">(200,000,000+200,000,000)</span>   분모
     *      </span> = 200,000,000</p>
     *
     *    ⇒ innerText는 `1,000,000,000 × 200,000,000(200,000,000+200,000,000) = 200,000,000`.
     *    분수 표기에서도 성립하도록 「양도가액 × 분자(분모) = 결과」 형태로 단언한다.
     *    CSS 클래스에는 기대지 않는다(Frac 내부 구현이 바뀌어도 살아남게).
     */
    const formulaRow = page
      .getByText(/[0-9,]+ × [0-9,]+\([0-9,+]+\) = [0-9,]+/)
      .first();
    await expect(
      formulaRow,
