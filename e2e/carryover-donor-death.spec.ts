 ...
 * ## 이 spec이 덮지 않는 것 (명시)
 *
 * 「클릭 → 계산 → 결과 화면」 전 구간을 한 번에 태우지는 **못했다**. 시드와 UI 입력을 섞으면
 * 첫 상호작용에서 store가 `defaultFormData`와 병합되며 폼-전역 `contractTotalPrice`가
 * `""`로 덮인다 — **이 기능과 무관한 기존 동작**이고, 시드만 하고 바로 계산하는 다른
 * spec들은 이 경로를 밟지 않는다.
 *
 * 대신 구간을 나눠 전부 잇는다:
