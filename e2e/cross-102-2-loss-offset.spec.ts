...
test.describe("§102② 크로스 차손 통산", () => {
  ...
  test("CL-1: 🔴 기타자산 차손이 부동산 차익에서 공제되어 **세액이 줄어든다**", async ({ page }) => {
    ...
    // 📌 토지 A 2018-06-01 → 2024-06-01 = 6년(§95④ 초일 산입) → 장특 12% ⇒ 5억 × 0.88 = 4.4억
    await expect(table).toContainText("440,000,000 → 240,000,000");
    await expect(table).toContainText("차손 200,000,000 공제");

    /**
     * ② 🔴 **세액이 그 통산 «후» 금액을 기준으로 나온다.**
     *
     * 과세표준 합계 237,500,000(= 240,000,000 − 기본공제 2,500,000) × 38% − 19,940,000 → 70,310,000.
     * (📌 §95④ 초일 산입 전 기대값은 5년 10% 기준 247,500,000 → 74,110,000이었다.)
     * 배선이 빠지면 부동산이 440,000,000 그대로라 과세표준이 437,500,000이 되고 세액이
    ...
    await expect(page.getByText("과세표준 합계 237,500,000")).toBeVisible();
    await expect(page.getByText("70,310,000").first()).toBeVisible();
  });
