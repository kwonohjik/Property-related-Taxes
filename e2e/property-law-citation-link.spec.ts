 ...
 *
 * PLAW-1: 주택 §122 단서 배지 클릭 → 조문 팝업 헤더 "지방세법 제122조"
...
test.describe("재산세 계산 마법사 법조문 링크 → 조문 팝업", () => {
  test("PLAW-1: 주택 §122 단서 배지 → 지방세법 제122조 팝업 헤더", async ({
    page,
  }) => {
    ...
    await page.getByRole("textbox", { name: /금액 입력/ }).fill("500000000");

    // 다음 → step 3 (주택은 §122 단서로 세부담상한 미적용 → 확인 화면)
    await page.getByRole("button", { name: "다음" }).click();

    // §122 단서 배지 — 버튼 텍스트 "§122 단서 ↗" (legalBasis="지방세법 §122")
    const badge = page.getByRole("button", { name: /§122 단서/ });
