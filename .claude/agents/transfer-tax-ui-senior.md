...
- 엔진 시니어와 함께 사용자 시나리오 검토 (단건·다건·일시적2주택·겸용주택·PHD·1990환산 등)
...
- 시나리오별 분기 (예: 겸용주택 PHD + 용도변경 시 시점별 면적 분리) · 테스트 케이스
...
- 활성화 조건 (e.g., 겸용주택일 때만, 토지 자산일 때만, 1990 이전 취득 시만)
...
- 시점별 면적이 분리되는 경우(겸용주택 PHD + 용도변경) 시점별 값을 모두 표기
...
9. **시점별 분기 누락** → 겸용주택 + 용도변경 시 시점별 면적 분리 미적용
...
4. 다양한 시나리오 (단건·다건·일시적2주택·겸용주택·PHD·1990환산) 회귀
