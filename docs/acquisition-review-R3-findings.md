...
> **수정 이력**
> - ✅ **R3-03·R3-04** (구조적) — PR #639 머지 (origin/master 60d7d9c7).
> - ✅ **R3-01·R3-02** (부가세 산식) — 이 브랜치. 지방세법 §151①1·농특세법 §5①6호/⑤ KoreanLaw 실측 후 rewrite.
>   농특세=(적용세율−adjustedStd+2%)×10% [비중과 0.2%·§13의2 4%기준·사치성 물건표준율], 교육세 사치성 본문 라우팅(1.4/1.8 하드코딩 제거). §15② 간주취득 농특세 0.2%(§5⑤) 정정 포함.
> - ✅ **R3-05·R3-06·R3-07·R3-09·R3-10 + C-3·R3-13** (부가세·세율 정밀·표시) — 이 브랜치. KoreanLaw 실측(§151①1가 ×300%·농특세법 §4 10호) 후 수정.
>   R3-05=법인 §13②⑥⑦ 비주택 교육세 본문×300%(surchargeType `section13_gamok` 신설·비주택 게이트·§13① 제외). R3-06=사치성+증여 §13의2③ 20%(luxury 분기에서 assessGiftSurcharge 배선→multiHouseRate 12%). R3-07=특수관계 130% 초과 과다신고 사실상취득가격 유지(저가만 시가 상향). R3-09=자경 §6① 농지 농특세 §4 10호 비과세(0). R3-10=6~9억 선형보간 §11①8나 4자리 확정세율(부담부증여 유상분 동일). C-3·R3-13=교육세·생애최초 표시 산식 동적화.
>   전체 10,781 통과·tsc0·신규 anchor 7(`r3-remaining-fixes`)·기존 anchor 5 갱신(numeric-precision·zod-sync·legacy).
> - ✅ **C-1·C-2** (구조적 base/surcharge 리라이트) — 독립 라운드. KoreanLaw 실측(§13의2①④·§28의2/§28의5·§15①2호) 후 수정.
>   C-1=부담부증여 유상분 §13의2① 다주택 중과에 메인 assessSurcharge 배제로직 재사용(저가주택·일시적2주택·지정전계약). C-2=§15①2호 상속특례 요건 게이트(가목 1가구1주택·나목 §6① 감면농지) 신설 — dead 게이트 필드 소비. 전체 10,796 통과·tsc0·lint0·신규 anchor 7(`c1-c2-fixes`).
> - ✅ **R3-16·R3-17** (조문 라벨·인용 드리프트, 표시-only) — KoreanLaw 실측(§10의2~§10의6 제목) 후 정정.
>   R3-16=base.ts 헤더/섹션 주석 §10의4(부담부증여→원시취득)·§10의5(연부취득→차량·기계장비 특례) 정정 + 과세표준 formula '천원 미만 절사'→'원 단위'(취득세 절사규정 無). R3-17=간주취득 과세표준 legalBasis §7→§10의6(DEEMED_TAX_BASE 신설). 세액 무영향. anchor 2(`r3-16-17-citation`).
> - ✅ **R3-08·R3-12** (LOW 실계산) — KoreanLaw 실측(§7⑤·지특령 §3①2호) 후 수정.
>   R3-08=합병·분할 주식취득 과점주주 포괄면제 분기 제거(§7⑤ 명문 없음 → 원칙적 과세)+`isMergerOrSplitShare` 필드 전량 제거(type·Zod·UI — 크래프트 POST 도달 차단). R3-12=자경농지 §3①2호 거주요건 OR 모델링 — `residesInSameOrAdjacentJurisdiction` 신규 필드(동일·인접 시군구 거주 시 30km 거리요건 면제, 14지점 배선). 전체 10,801 통과·tsc0·lint0.
> - ⏳ **잔여 LOW 1(유권해석 필요)**: R3-14(생애최초 고급주택 §36의3 — §13⑤ 중과가 §36의3 배제사유인지 유권해석 확정 전 수정 보류. 現 코드는 luxury 세율 가드로 세액영향 0).
