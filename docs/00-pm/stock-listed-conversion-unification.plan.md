...
> **V-1~V-5 · Q-1~Q-2 · S0~S3 전건 종결** (S3는 `77fca8b2`).
>
> 🔴 종전 「남은 것은 **S3** 하나다」는 **stale이었다**(2026-09-22 감사). S3 코드 실측:
> `lib/stores/calc-wizard-stock-form-types.ts:255` `acquisitionStdMode` ·
> `calc-wizard-stock-normalize.ts:26`(구 boolean 역산 마이그레이션) ·
> `components/calc/stock-transfer/AcquisitionStdModeRadio.tsx`.
> ❌ 미결로 재기재 금지. 아래 S3 서술은 **착수 시점 설계 기록**으로 보존한다.
>
...
### S0 — 셀렉터 방탄화 (UI 무변경 · 독립 머지 가능) ✅ **완료 2026-09-10**

> 🔄 **초판 방향 정정** — 초판은 「라벨 문자열 셀렉터를 `name`/`data-testid` 축으로 이관」이었다.
> **그것으로는 못 막는다**: S3(Q-2 3안)은 세 ToggleCard를 **라디오 선택지로 흡수**하므로
> 라벨이 바뀌는 게 아니라 **엘리먼트가 사라진다** — testid를 붙여 둬도 소용이 없다.
>
> ⇒ 실제로 줄일 수 있는 것은 **호출 지점의 수**다. 같은 조작이 13개 지점에 복제돼 있었다.
> **헬퍼로 모아 S3이 파일 하나만 고치게** 한다.

**V-4 종결 — 파손 표면 실측**

초판의 「라벨 의존」 목록은 패턴을 넓게 잡아 주식양도세 전역(부동산 「기준시가」까지)을
세고 있었다. **S3이 실제로 없애는 것**으로 좁혀 다시 셌다:

| 축 | 형태 | 건수 |
|---|---|---|
| ToggleCard 「취득 후 상장」 | `[data-slot="toggle-card"]` → `filter({hasText})` → `getByRole("switch")` | 8건 / 7파일 |
| ToggleCard 「양도·취득일 거래정지」 | 위와 같은 형태 + `getByText(제목).click()` 변형 | 5건 / 3파일 |
| 일반 블록 라벨(분모·분자) | `fillByLabel` · `div:has(> label:has-text(...))` · `getByText` | 13건 / 3파일 |

⇒ **조작 축 13건이 헬퍼 대상**이다. 라벨 13건 중 **단언**(「…이 보인다/안 보인다」)은
S3이 실제로 바꾸는 대상이라 **헬퍼로 감추지 않는다** — 감추면 그 spec이 무엇을 지키는지
읽을 수 없게 된다.

**구현** — `e2e/_helpers/stock-conversion.ts` 신설

| export | 용도 |
|---|---|
| `StockConversionMode` | S3의 `acquisitionStdMode` enum 값을 **미리** 이름으로 쓴다 |
| `setStockConversionMode(page, mode)` | 스위치 클릭 방식 |
| `setStockConversionModeByTitle(page, mode)` | 제목 텍스트 클릭 방식(「switch role 이중토글 회피」 변형) |
| `togglePostListingOff(page)` | 켠 뒤 되돌리는 F-10 회귀 플로우 |
| `postListingCard(page)` | 카드 «안»을 스코프해야 하는 곳 |
| `conversionToggleTitle(mode)` | 제목 문자열이 필요한 **단언**용 |

두 조작 방식을 **둘 다** 헬퍼에 담았다 — 왜 갈리는지는 그 spec들이 초록인 이상 파고들지
않되(Surgical), 지식을 한 파일로 모으는 것이 목적이다.
...
|---|---|
| 프로덕션 코드 변경 | **0줄** |
| `[data-slot="toggle-card"]` + 이 영역 제목 복제 | **13건 → 0건** |
| `npx tsc --noEmit` | 0건 |
| E2E 주식 25 spec | **57건 전건 통과** |

> ⚠️ **S3에서 이관해야 할 spec 1건** — `stock-transfer-halt-extension.spec.ts`의
> 「거래정지 + 취득 후 상장 → 양립 불가 차단」은 **불가 조합을 일부러 만드는** 테스트다.
> Q-2 3안으로 축이 하나가 되면 그 조합을 UI로 만들 수 없으므로, S3에서 ⑧·⑫ 단위 anchor
> (`stock-conversion-branch-matrix.anchor.test.ts` MTX-XA/XA')로 이관해야 한다.
> 해당 지점에 주석으로 남겼다.
...
| ~~V-4~~ | ✅ **종결 2026-09-10** — §5 S0 참조. 네 형태를 전수로 훑어 **조작 축 13건**으로 확정하고 헬퍼로 이관 | |
