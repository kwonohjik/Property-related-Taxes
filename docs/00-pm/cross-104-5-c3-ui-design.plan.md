...
**상태**: **C-3a ✅**(엔진 echo) · **C-3b ✅**(어댑터 + 1호 교차) · **C-3c ✅**(화면 · 1차 범위) ·
...
C-3c  화면 /calc/cross-104-5 ✅ **1차 완료** — 상세는 §8

C-3c 초판 계획 (참고)  — 🔄 **v1.1 재검토로 범위 확정**
...
## 8. ✅ C-3c 구현 결과 (2026-08-03)

### 🔴 착수 중 발견 — **서버 라우트가 필요했다**

초판은 어댑터가 순수 함수라 **클라이언트에서 계산**할 수 있다고 봤다. 실측하니 아니었다:
**§55① 누진표는 `effective_date`가 1990·2018·2021·현행 넷**이다
(`lib/tax-engine/data/transfer-rate-seed-historical.ts`). 클라이언트가 주식 정적 상수
`BASIC_PROGRESSIVE_BRACKETS`(현행)를 쓰면 **과거 연도가 조용히 틀린다**.

⇒ **`POST /api/calc/cross-104-5`** 신설. 다른 라우트와 같은 방식으로 `preloadTaxRates(과세기간 말일)`을
쓰고, §104①8호·9호 표는 **DB 기본표에서 `rate` +10%p로 파생**한다(상수를 따로 두면 연도 드리프트).
라우트 테스트 **R-2가 그 사실을 고정**한다 — 같은 과세표준인데 2020년과 2024년 세액이 다르다.

### 구현물

| 파일 | 역할 |
|---|---|
| `app/api/calc/cross-104-5/route.ts` | Zod → **연도별 세율 로드** → `computeCross1045` (분당 15회) |
| `lib/calc/cross-104-5-history.ts` | 이력 후보 선별 **순수 함수**(연도 추출·후보·중복 감지) |
| `lib/calc/cross-104-5-api.ts` | 클라이언트 → 라우트 |
| `app/calc/cross-104-5/{page,Cross1045Client}.tsx` | 화면 — 연도 → 부동산 → 기타자산 → 결과 |
| `CrossEngine1045Notice` | **「§104⑤ 합산 계산하기」 버튼 추가** — 3곳이 막다른 길에서 벗어났다 |

### 1차 범위에서 뺀 것 (정직하게)

- **재계산 경로**(`mode:"single"`·`"mixed-use"`·구 이력) — 후보 목록에 **사유를 적어 보여주되
  선택은 막았다**. 배관(§7 O-1)은 확인했으나 `MultiTransferFormData` 조립이 폼 기본값 전반을
  요구해 1차에서 제외했다.
- **R-2 배분 2안 재계산**(§4.4 U-1) — **감지·경고까지만** 했다. 초과가 있으면 「한쪽 계산기에서
  기사용액을 입력해 다시 계산하라」로 안내한다.
- ⇒ 둘 다 **재계산 UI**가 전제라 함께 묶어 **C-3d**로 남긴다.

### 검증

- 단위 **19건**(이력 선별 14 + 라우트 5) + 기존 어댑터 13 · 고지 카드 anchor 6 불변
- **E2E 4건 통과**(브라우저 검증) — 이력 시드 → 선택 → 합산 → 도출값 **380,740,000** ·
  차이 **+47,620,000** · 범위 고지 노출 · 🔒 주식(§94①3호) 후보 미노출
- 전체 **13,184건 통과 · 회귀 0** · typecheck 0 · lint 0 errors(신규 warning 0)
