...
### 2-2. ~~범위 밖 — 별건 (사용자 승인)~~ → ✅ **차단 전건 해제됨 (2026-09-03)**

> 🔴 **2026-09-22 감사 — 이 절의 「범위 밖」은 stale이다.**
> `lib/calc/transfer-tax-validate.ts:116` 주석 실독: 「✅ **부담부증여·공익수용 차단은
> 2026-09-03에 모두 해제됐다.** 종전 사유는 「지분 분할 양도가액 = 총양도가 × 지분율이라
> 부담부증여(§159 채무액 기반)·공익수용(보상가액)과 비양립」이었다. **둘 다 전제가 틀렸다**」
> — §159는 총양도가를 쓰지 않고 `양도가액 = A × B/C`로 자체 산정하고, 공익수용은 총계약가를
> 그대로 쓴다. ❌ 「범위 밖」·「별건 필요」로 재기재 금지.
>
> (아래 서술은 **당시 판단 기록**으로 보존한다.)
