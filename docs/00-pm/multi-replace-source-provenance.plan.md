...
| **V-3** | 이미 저장된 record의 **가짜 `sourceCalculationId`를 마이그레이션할 것인가** — 「그 id가 다건이면 제거」는 비동기 조회가 필요해 `buildPropertiesFromMultiRecord`가 async가 된다(선행 계획서가 **금지**한 방향, `:56~62` 주석) | §8 Q-1로 승격 | 🟠 |

**V-3이 해소되기 전에는 착수하지 않는다** — 설계를 가른다.

---

## 8. 결정 게이트 Q-n

### Q-1 🔴 이미 굳은 가짜 id를 어떻게 다룰 것인가

| 선택지 | 결과 |
|---|---|
| **(A) 그대로 둔다** | 구현 최소. 구 record는 계속 가짜 id를 갖지만 **하류 가드가 걸러 내 무해**하다(§2 G-3 표). 위험은 가드가 넓어질 때 — 그때 이 계획서가 근거가 된다 |
| (B) 동기 휴리스틱 제거 | `p.sourceCalculationId === record.id`이면 제거. **조회 불요 = 동기 유지**. 다만 「원본이 정말 그 다건이었던」 경우와 구분 못 하는데, 그런 경우는 **정의상 없다**(다건은 자기 자신의 원본이 아니다) |
| (C) 비동기 마이그레이션 | 정확하지만 `buildPropertiesFromMultiRecord`가 async가 되어 선행 계획서가 금지한 전파가 시작된다 |

**추천 (B)** — 동기를 유지하면서 가짜 표지를 실제로 걷어낸다. (A)보다 1줄 더 쓰고 (C)의 전파 위험이 없다.
⇒ **사용자 결정 필요.**
