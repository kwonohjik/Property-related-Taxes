...
## 10. 🔴 후속 결함 — 배관이 실제로는 끊겨 있었다 (2026-09-02, PR #1411 머지 후 발견)

D-5(결과 배너)를 배관하다 발견했다. **PR #1411이 「일자별 입력」을 고르면 취득가액을 0으로
만들고 있었다.**

| 같은 1개월 평균 10,000 | 취득가액 |
|---|---|
| simple + direct | **40,000,000** |
| simple + **daily** | **0** ← 조용한 오답 |

### 원인 — 게이트 안의 호출은 그 분기에서 0곳이다

`stock-transfer-tax-api.ts:400`의 `adaptFlatToApiBody` 호출은
**`listing_only`·`full`에만** 게이트돼 있다. simple 모드는 그 줄을 타지 않으므로
adapter의 simple 분기(§3.3에서 「읽기 3곳」 중 하나로 센 것)는 **API 경로에서 죽은 코드**였다.
simple에서 body에 종가평균을 싣는 지점은 `:275`의 폼 필드 읽기 **하나뿐**인데,
②의 daily는 그 칸을 **비워 두는 것이 정상**이라 아무것도 실리지 않았다.

### 왜 전부 통과했나 — 뮤테이션이 사각지대를 물려받았다

| 검사 | 결과 | 이유 |
|---|---|---|
| anchor RLA-5 | ✅ 통과 | `adaptFlatToApiBody`를 **직접** 호출 — 위의 게이트가 사각지대 |
| 뮤테이션 M-A | ✅ 빨감 | **같은 leaf를 겨눔** — anchor의 사각지대를 그대로 물려받았다 |
| validate·미리보기 | ✅ 통과 | 헬퍼를 직접 부른다(진짜 3곳 중 2곳은 맞았다) |
| E2E PLD-1 | ✅ 통과 | Step3 **도달**만 단언 — 세액을 안 봤다 |

🔴 **뮤테이션이 빨개진 것은 「구별력」의 증거일 뿐 「도달성」의 증거가 아니다.**

### 조치

- `stock-transfer-tax-api.ts:275` — simple 경로도 `resolveListingClosingAvg`를 거친다
  (0은 「자료 없음」으로 보아 `undefined`를 흘려 엔진의 진단 경고를 살린다)
- **RLA-5의 진입점을 `buildStockTransferApiBody`로 교체** — UI가 실제로 부르는 함수
- **LDB-5 신설** — 「daily의 취득가액 == 같은 평균 direct의 취득가액」 세액 동일성
- **PLD-3 신설** — 결과 화면의 **숫자**(16,286,762)를 단언. 「도달」만으로는 이 층을 못 본다
- 뮤테이션 재실측: 배관 4층 4/4 + E2E 1/1 감지

교훈은 [[feedback_leaf_anchor_skips_zod_layer]]에 재발 사례로 기록했다.
