...
| **A** | 별지10호 dash 통일 + 미포함 고지 (§4) | ✅ **완료** — PR #1449 |
| **B1** | 신고 단위 §47의2 + §48②2호 감면 + §47의3(적용제외 게이트 포함). 증여 → 상속 | ✅ **완료** — PR #1451(증여)·#1453(상속) |
| **B2** | 부정행위 축(40%/60% · 가목·나목 분해) | ✅ **완료** — PR #1456 |
| **B3** | 납부지연 §47의4 | ✅ **완료** — 이 PR |

### 🔴 B2 가 드러낸 것 — §47의3④1호는 **네 목이 같지 않다**

가·나목엔 단서가 없고, 다목·라목엔 「부정행위인 경우는 제외한다」가 붙는다. 그리고
**그 두 단서가 가리키는 부정행위조차 서로 다르다**:

- **다목** 「부정행위로 **상속세 및 증여세**의 과세표준을 과소신고한 경우」 ⇒ `penaltyReason` 축
- **라목** 「부정행위로 인하여 **법인세**의 과세표준 및 세액을 결정·경정하는 경우」 ⇒ 별개 사실
  (`corporateAdjustmentByFraud`)

⚠️ **이 결함은 B1 시점에 이미 있었다** — B1 은 적용제외가 있으면 무조건 0을 돌려줬다.
B2 가 부정행위 축을 들여오면서 드러났다.

### 🔴 B3 — §47의4③에도 상속·증여 전용 적용제외가 둘 있다

| 호 | 내용 | 입력 |
|---|---|---|
| **③4호** | 법인세 경정으로 §45의3~§45의5 증여의제이익 변경 (부정행위 제외) | §47의3④1호 **라목과 문언이 같다** ⇒ `corporateAdjustmentByFraud` 재사용 |
| **③6호** | §67·§68 신고 + §70 **기한 내 납부**한 자가 기한 후 평가로 결정·경정 | 「납부」 요건이 추가돼 다목 입력으로 **대체 불가** ⇒ `paidOnTimeThenRevalued` 신설 |

🔑 **③은 감면(§48②)이 아니라 적용 배제**다 — 감면율이 아니라 0이다.

🔑 **법정납부기한 = 신고기한**(상증법 §70①). 다만 §70①4~6호(연부연납·납부유예·물납
신청분)는 자진납부 대상에서 빠지고 §70② 분납은 2개월 유예가 있으므로 **금액·기한을
파생하지 않고 입력받는다**(⑧이 미입력을 차단).

> ✅ **A안 미포함 고지는 B3 에서 제거했다.** B1(일반율) → B2(부정행위) → B3(납부지연)로
> 남은 축이 사라졌다 — 산출해 놓고 「안 했다」고 말하면 반대 방향의 거짓이 된다.
> 삭제만 하면 재도입될 수 있으므로 **없어야 한다는 사실을 anchor 로 고정**했다(A2-3·A2-3b),
> 그 자리를 산출근거 카드 두 장이 대신한다(A2-4).
