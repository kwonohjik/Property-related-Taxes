...
> ### ✅ 결정 완료 (2026-09-03) — **§8이 정본 스펙**
>
> | # | 결정 |
> |---|---|
> | 1 | 상속세 가산세는 **신고 단위 1회** (상속인별 안분하지 않는다) |
> | 2 | **과소신고(§47의3)도 구현** ⇒ §47의3④1호 적용제외 게이트 필수 |
> | 3 | 입력은 **3-state + 신고일** — 결정 2가 entail 하는 2칸만 `정기신고`일 때 조건부 추가 (§8.1) |
>
> ⇒ **A·B 모두 착수한다.** A를 먼저 머지하고(즉시·무위험) B1을 이어 간다 — 근거는 §3.3.
> 아래 §3~§7은 **결정 전에 작성된 원안**이며, 충돌하는 부분은 **§8이 우선**한다.
...
§8의 열린 결정 3건에 답이 나왔다. 아래가 **정본 스펙**이다.

| # | 결정 | 귀결 |
|---|---|---|
| **1** | 상속세 가산세는 **신고 단위 1회** | 상속인별 안분하지 않는다. 부동산·주식 정본(`filingUnitPenaltyDetail`)과 같은 구조. `inheritance-allocation.ts`는 **건드리지 않는다** |
| **2** | **과소신고(§47의3)도 구현** | §47의3④1호 적용제외 게이트가 **필수**가 된다(R-2). 「당초 신고세액」 입력도 필수 — §8.1 |
| **3** | 입력은 **3-state + 신고일** | 그 최소 위에 결정 2가 **entail** 하는 2칸만 조건부로 더한다 — §8.1 |

### 8.1 🔴 결정 2 ↔ 3 충돌과 그 해소

**「3-state + 신고일」만으로는 과소신고를 계산할 수 없다.** 두 가지 이유다.

1. **base가 없다.** §47의3①의 기준은 「과소신고한 납부세액」이고, 엔진 구현도
   `transfer-tax-penalty.ts:314`에서 `determinedTax − priorPaidTax − originalFiledTax
   − interestSurcharge + excessRefundAmount`로 낸다. **「당초 신고세액」이 없으면 base가 정의되지 않는다.**
2. **3-state가 과소신고를 표현하지 못한다.** `on_time`/`late`/`none`은 **「신고했는가·언제」** 축이고,
   과소신고는 **「적게 신고했는가」** 축이라 직교한다.

⇒ **3-state는 그대로 두고, `on_time`일 때만 조건부로 2칸을 연다.** 정상 신고 사용자는
라디오 하나만 본다(현행과 동일한 체감).

```
[라디오] 신고 상태                       ← 결정 3 (증여는 2-state → 3-state 승격)
   ○ 법정기한 내 신고 (정기신고)
   ○ 기한후신고 (국세기본법 §45의3)
   ○ 무신고
[날짜]   실제 신고일                      ← 결정 3 (late 일 때만 — §48②2호 감면 구간 판정)

  └ 정기신고를 고른 경우에만 ↓
[토글]   과소신고 (국세기본법 §47의3)      ← 결정 2가 요구
[금액]   당초 신고세액                     ← §47의3① base (없으면 계산 불가)
[선택]   §47의3④1호 적용제외 사유          ← R-2. 기본 「해당 없음」
           · 해당 없음
           · 가. 소유권 소송 등으로 상속·증여재산 미확정
           · 나. §18~§24·§53·§53의2·§54 공제 적용 착오
           · 다. §60②③·§66 보충적 평가액으로 과세표준 결정
           · 라. 법인세 경정에 따른 §45의3~§45의5 증여의제이익 변경
```

### 8.2 왜 「기한후신고 + 과소」를 따로 두지 않는가

법정신고기한까지 신고하지 않았으면 §47의2의 base는 「그 **신고로 납부하여야 할 세액**」
**전액**이다 — 그 뒤 기한후신고가 과소였는지는 base를 바꾸지 않는다. 그래서 `late`·`none`에서는
과소신고 축을 열지 않는다. 두 조문은 「기한까지 신고했는가」로 **배타**다.

### 8.3 Phase 분리

| Phase | 내용 | 상태 |
|---|---|---|
| **A** | 별지10호 dash 통일 + 미포함 고지 (§4) | 결정과 무관 — 즉시 착수 |
| **B1** | 신고 단위 §47의2 + §48②2호 감면 + §47의3(적용제외 게이트 포함). 증여 → 상속 | 결정 확정으로 착수 가능 |
| **B2** | 부정행위 축(40%/60% · 가목·나목 분해) | 별도. 미구현 동안은 **일반율**(20%/10%)만 적용된다 |
| **B3** | 납부지연 §47의4 3필드 | 별도. G-05와 함께 |

> ⚠️ **B2·B3 미구현 동안 총액은 여전히 과소다.** A안의 미포함 고지를 **B1 이후에도 유지**하되
> 문구를 「부정행위 가산세·납부지연 가산세는 포함되지 않았습니다」로 좁힌다.

### 8.4 남은 확인 필요

- 상속·증여 가산세에 **국고금 관리법 §47① 10원 미만 절사**가 적용되는가 (주식은 적용).
  확인 전에는 **절사하지 않는다** — 없는 절사를 넣으면 과소가 된다. G-35 anchor 패턴으로
  구별 가능한 픽스처를 함께 둔다.
- 상속 결과뷰 `InheritanceTaxResultView.tsx:233`의 「결정세액」 라벨 — §5.3에 따라 `finalTax`가
  가산세를 포함하게 되면 라벨이 거짓이 된다. **「총 납부세액」 행을 새로 두고 결정세액은 가산세
  전 값으로 분리**한다(양도세 `determinedTax`/`totalTax` 구조와 동형).
