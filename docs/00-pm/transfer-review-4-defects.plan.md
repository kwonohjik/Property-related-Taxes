...
| **F-19** | 부담부증여 GB 상세명세서가 K-4(실지취득가) 경로에서도 「안분 취득가액 × N% (개산공제, §163⑥)」라 적는다 — 그 경로의 필요경비는 개산공제가 아니라 **안분 실비**(자본적지출·양도비)다. §97②2호 swap 경로도 같다. 표시 축(세액 무관) | F-18 조사 | 별건 — 율 echo는 실비 경로에서 `undefined`로 두었으므로 그 신호로 문구를 가를 수 있다 |
| **F-20** | GB 상세명세서 **증축(건물2)** 산식의 base가 `acqExtensionStdTotal`(100% 값)이라 **지분 자산에서 산식이 자기 값을 못 만든다** — 토지·건물1은 `landBase`·`buildingBase` echo로 이미 해결된 축인데 증축분만 빠졌다. 표시 축(세액 무관) · **수치 probe 미수행** | F-18 조사 | 별건 — `GeneralBuildingEstimatedDeduction`에 증축 base echo 추가가 필요하다 |
