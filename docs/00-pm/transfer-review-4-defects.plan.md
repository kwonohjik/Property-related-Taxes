...
| **F-16** | §167의10①3호(부득이 취득)·7호(소송 취득)를 **양도 주택 자신**에는 적용하지 않는다 — 각 호는 「양도하는 주택」을 가리키는데 코드는 다른 주택만 본다. **실측**(조정지역 2주택·2026-08-01): 양도 주택이 소송 취득(2년 전)이면 `surchargeApplicable: true`(중과), 같은 주택이 다른 주택이면 배제. 부득이(3호)도 같다 — 과다 방향 | F-3 조사 | 별건 — 3호는 「1주택을 취득함으로써 2주택이 된 경우의 해당 주택」이라 취득 경위 입력이 필요하다 |
