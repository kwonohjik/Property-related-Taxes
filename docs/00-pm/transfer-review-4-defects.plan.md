...
## 30. F-21 구현 기록 — 2026-09-20

F-19(부담부증여 실비)·F-20(증축 실가)과 같은 축인데 **토지·건물1만 남아 있었다**. 표시 축이다.
...
- **신호는 새로 만들지 않았다.** 계획서에 「별도 플래그가 필요할 수 있다」고 적어 두었으나,
  카드에 `usedEstimatedAcquisition: boolean`이 **이미 있었다**(`general-building.types.ts:457`).
  표시 층이 그 카드를 찾아 읽는다 — **엔진 변경 0**.
  ⚠️ `estimatedDeduction.landBase`(F-18·F-20의 base echo)로 겸용하지 **않았다** — base가 없는 것이
  「실가 파트」인지 「echo 이전에 저장된 옛 이력」인지 구별되지 않는다
  (`feedback_one_field_serving_two_legal_axes`). 뮤테이션 M4가 이 겸용을 시도해 3건이 죽는다.
- **옛 이력 호환**: 카드에 그 필드가 없던 시절의 결과는 `undefined`라 종전대로 개산공제 산식으로
  떨어진다(F21-6). `=== false`로 비교하는 이유다 — `!card?.x`로 쓰면 옛 이력이 전부 실가 파트가
  된다(뮤테이션 M3).
- **anchor 6건**: `gb-actual-part-expense-formula-f21.anchor.test.tsx`.
- **mutation 5/6 KILLED**. ⚠️ **M5(카드 매칭에서 `isSameShare` 제거)는 구별력 0**이다 —
  `buildGeneralBuildingAssetCards`는 지분이 있어도 `land`·`building` 카드를 **한 장씩만** 낸다
  (실측). 공유지분 다건 카드(`#share` suffix)는 상위 경로에서 생기므로 이 진입점의 anchor로는
  구별할 수 없다. 기존 코드(`:285-290`)가 쓰는 관례를 그대로 따랐다 —
