...
| **F-17** | 7호의 3년 기산점이 법문과 다르다 — 「소송으로 인한 **확정판결일**부터 3년」인데 필드·라벨·주석이 모두 「소송 **취득일**」이다(`litigationAcquisitionDate`). 두 날짜가 갈리는 사건에서 사용자가 취득일을 넣으면 기간이 어긋난다 — 방향은 사건에 따라 양방향 | F-16 조사 | 별건 — 필드명 변경은 ①②③④⑫⑭ 전 층을 건드린다. 라벨·주석만 먼저 고칠지 정할 것 |
