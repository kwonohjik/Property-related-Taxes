...
> 결정: **Q-1~Q-4 전건 확정**(2026-09-18) — §5. 미검증 V-1~V-8 — §6. 후속 작업 F-1~F-4 — §10.
...
배제 3을 다음이 **모두** 참일 때만 적용한다(배제 2도 ③④를 함께 고친다):

1. **주택 수 = 2**(`effectiveHouseCount === 2`) — §155④ 문언. 3주택 이상에서 §155①과 중첩돼 §155④가 성립하는 경우는 엔진이 모델링하지 않는다 → **Q-4 ✅ 범위 밖**. 3주택 이상 + 동거봉양·혼인 입력이면 결과 화면에 「일시적 2주택과 겹쳐 합가 특례가 성립하는 경우는 반영하지 않았습니다」 한 줄(후속 F-1).
2. **§154① 충족**(`sellingHouseMeetsOneHouseRequirements ?? true`) — **양도일 ≥ 2023-02-28**에만. 그 전 2주택 양도분은 구 5호(§154① 없음).
3. **합가(혼인) 전 보유**: 양도 주택 취득일 ≤ 합가일(혼인일) — V-5.
4. **10년 경계 포함**: `transferDate <= addYears(mergeDate, 10)`(민법 기간 계산). `differenceInYears < 10` 제거.
5. 먼저 양도하는 주택 — 현행 `isFirstTransferredInMerge` 입력 경로 확인(V-4).
...
| **V-4** | 동거봉양 「먼저 양도하는 주택」 입력 경로(`isFirstTransferredInMerge`)가 중과 경로에 닿는지 | grep + probe | PR-1 |
| **V-5** | 합가 전 보유 비교를 `<=`로 할지 — 서면-2023-부동산-0231(취득일 = 합가일도 §155④) 원문. 비과세 E-3.5는 `<` | taxlaw.nts 원문 | PR-1 |
...
| 순서 | PR | 범위 | 선결 |
|---|---|---|---|
| 1 | **PR-1 D9** | `multi-house-surcharge-exclusion.ts` 배제 2·3 + 시점 상수 + manifest + 문서 정정 | V-4·V-5 |
...
| **F-2** | 비과세 경로 E-3.5(`transfer-tax-exemption.ts:252-274`)의 합가 전 보유 비교 `acquisitionDate < mergeDate`(엄격 부등호) | D9 법령 검증 잔여 의심 | V-5에서 「취득일 = 합가일도 §155④」가 확인되면 PR-1과 같은 기준으로 정정 |
