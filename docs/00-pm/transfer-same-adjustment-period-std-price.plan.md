...
| ~~§80③ 대체 산정 UI 미배선~~ | ✅ **§18에서 해소** |
| ~~`transfer-tax.ts` 800줄 분해~~ | ✅ **§18에서 해소** (857 → 679) |
...
## §18. 별건 3건 처리 (2026-08-25)

§16·§17이 「별건」으로 남긴 세 항목을 순차 처리했다.

### ① §80③ 전기 기준시가 대체 산정 — UI 배선

`calcPriorStdPriceSubstitute`는 P-7에서 구현됐지만 **프로덕션 호출부가 0건**이었다
(참조는 anchor뿐). 화면은 근거 라디오 4택만 주고 *"대체 산정한 값을 위 칸에 입력하세요"*라
안내했다 — **사용자가 법정 산식을 손으로 계산**해야 했다.

⑤에 피연산자 4칸을 열고 값이 바뀔 때마다 **엔진과 같은 leaf**로 파생시킨다.

| 근거 | 피연산자 | 산식 |
|---|---|---|
| §80③2호 | 최초고시 기준시가 · 고시 기준율(%) | 최초고시 × 기준율 |
| §80③3호 | 전기 합계액 · 취득당시 합계액 (+ 기존 취득당시 기준시가) | 취득당시 × 전기합계 ÷ 취득당시합계 |
| §80③1호 | — | **조사**라 계산 대상 아님(직접 입력 유지) |

- 표시 순서를 **근거 → 피연산자 → 산정 결과**로 바꿨다(로직 순서 = 표시 순서).
- 파생 근거에서는 결과 칸을 **잠근다** — 산식과 어긋나는 값을 타이핑할 수 있으면 이중 진실이 된다.
- 파생은 `useEffect → store` 미러링이 아니라 **onChange 단일 배치 patch**다. 키를 나눠
  두 번 patch하면 뒤 patch가 앞을 stale spread로 덮는다.
- ⑧이 피연산자 미완성을 **조문과 함께 차단**한다. 안 막으면 ④가 빈 값을 보고
  `sameAdjustmentPeriod`를 통째로 안 보내 **침묵 no-op**이 된다.
- ③ 종전 저장분은 피연산자가 비었고 값이 있으면 근거를 `direct`로 되돌려 **값을 보존**한다.

> 🔴 **부수 발견 — §80③2호 곱셈이 1원 적게 나올 수 있었다.**
> `Math.floor(가액 × 기준율)`이 부동소수 곱이라, 실측 grid(기준율 50~150%)에서
> **1,185,938건**이 정확값보다 1원 낮았다(예: 15,000,000 × 50.03% → 7,504,499 / 정확
> 7,504,500). `applyRateFraction(가액, round(율×10000), 10000)`로 만분율 분수 연산으로
> 바꿨다(memory `feedback_applyrate_fractional_rate_one_won_error`).

**anchor 12건** + mutation 실측(파생 무력화 → 정확히 3건 실패, 나머지 9건 불변).

### ② L-8 — 증환지 fallback 축 불일치 (**서술 정정**)

기록은 *"사이드바는 종전 값을 보여준다"*였다. **실측하니 그게 아니었다.**

같은 규칙(당초분 ㎡당 × 증가분 면적)이 ⑤·④·⑧에 **각각 복제**돼 있었고 ⑥ 사이드바만
빠뜨린 것은 맞다. 그러나 사이드바의 §164⑧ 프리뷰는 `isSingle` 분기 안에 있어
**다건에서는 프리뷰 자체를 하지 않는다**(「계산 후 표시」). 2자산 probe 결과 두 행 모두
`acqPrice 0 / acqPending true` — 어긋난 값을 보여주는 것이 아니라 **아무 값도 안 보여준다**.

⇒ 어긋남이 관측되는 구성은 **당초분을 지워 증가분만 남은 단건 폼**뿐이다
(`CompanionAssetsSection`이 `form.assets` 전체를 받고 `assets.length > 1`이면 어느 카드든
삭제할 수 있다). 그 구성에서 실측: fallback 미적용 → 0/pending, 적용 → **93,023,255**.

`replotIncrementStdPriceAtTransfer` leaf로 묶어 네 호출부가 같은 값을 본다.
부수로 ⑤의 `standardPriceAtTransfer || fbTotal`이 **문자열 truthiness**라 `"0"` 입력을
fallback보다 우선하던 것을 ④⑧과 같은 `parseAmount > 0`으로 맞췄다.

**anchor 8건** + mutation 실측(사이드바 fallback 제거 → 프리뷰가 0/pending으로 회귀).

> 🔑 **「코드를 읽어 찾은 불일치」와 「관측되는 불일치」는 다르다.** L-8은 정적 대조로
> 발견됐고 서술도 그 수준에 머물렀다. probe를 돌리자 **영향 범위가 좁아지고 원인 구조가
> 달라졌다** — 고칠 값은 같았지만 「무엇이 문제였는지」는 정정이 필요했다.

### ③ `transfer-tax.ts` 857줄 → **679줄**

분리 트리거 800 초과 상태였다(§16 기록 시점 847 → STEP 0.47 배선으로 857).
**함수 양 끝의 결과 조립부**를 잘랐다 — 계산 중간을 자르면 지역변수 의존이 얽히지만
반환부는 읽기 전용이라 기계적 이동으로 끝난다.

| 신설 | 줄 | 내용 |
|---|---|---|
| `transfer-tax-loss-return.ts` | 166 | 양도차손(차익 ≤ 0) 조기반환 — §92③3호 총결정세액 조립 |
| `transfer-tax-normal-return.ts` | 209 | 정상 경로 STEP 7.5~11/12 — `finalizeTransferTax` 호출 + 결과 조립 |

**계산은 옮기지 않았다.** 두 모듈 다 조립만 한다 — 지역변수를 인자로 받는 것 외에
바뀐 것이 없고, 회귀는 엔진 전건으로 확인한다.


---

## §19. 코드 품질 게이트 — §18 리뷰 결과 및 처리 (2026-08-25)

`/code-review high`가 **High 2 · Medium 2 · Low 2**를 냈고 전건 probe로 확인된 지적이었다.
셋 다 **「어느 축에서 값을 구하는가」**가 어긋난 것이었다.

### High — §80③ 파생을 **저장**한 것이 원인이었다

**H-3 · H-4 — 파생값을 써 넣으면 다른 섹션의 편집을 못 따라간다**
§80③3호의 피연산자 중 「취득당시의 기준시가」는 **이 섹션 밖**(취득 정보 카드)에서 편집된다.
onChange 시점에 계산해 `sapPriorStdPrice`에 적어 두니 그쪽을 고쳐도 값이 낡은 채로 남았다 —
⑤는 모르고, ⑧은 통과시키고, **④는 낡은 값을 엔진에 보냈다**(실측: 취득당시 2억→3억 정정 후에도
`priorStandardPrice: 180,000,000` 전송. 정확값 270,000,000).

게다가 결과 칸을 `disabled`로 잠가 두어 **빠져나올 길이 없었다**. `AssetSectionTransfer`가
취득 섹션보다 **먼저** 렌더되므로 자연스러운 입력 순서(근거 → 합계액 → 취득당시)에서는
합계액을 채우는 시점에 취득당시가 비어 있어 파생이 `""`가 되고, 나중에 취득당시를 채워도
다시 계산되지 않았다.

⇒ **저장하지 않는다.** `resolveSapPriorStdPrice`(④ 단일 소스에 신설)를 ④⑤⑧이 **읽는 시점에**
부른다. 파생 근거에서는 `sapPriorStdPrice`를 읽지도 쓰지도 않으므로

- 다른 섹션 편집이 **즉시 반영**되고,
- 근거를 「실제 전기 기준시가」로 되돌리면 **직접 입력값이 그대로 살아 있다**(L-11도 함께 해소 —
  종전에는 라디오를 눌러 설명만 읽어도 조회해 둔 값이 지워졌다).

> 🔑 **파생값을 폼에 적어 넣는 것은 소스가 한 화면 안에 있을 때만 안전하다.** 소스가 흩어져
> 있으면 write-time 파생은 「누가 언제 다시 계산하는가」 문제를 만든다 — read-time이 정답이다.
> (`useEffect → store` 미러링 금지와 같은 뿌리다.)

### Medium — L-8을 **닿지 않는 곳**에 고쳤다

**M-6 — 진짜 도달 경로는 안분이었다**
§18 ②에서 「관측되는 구성은 단건뿐」이라 판정했는데, **안분 축을 안 봤다**. `canApportion`·
`computeApportionedSaleMap`도 raw를 읽고 있었고 이쪽은 **2자산에서 정상 도달**한다.
실측: 증가분 칸이 비면 두 행 모두 `sale: 0, pending: true`인데 ④는 파생해서 보내므로
**엔진은 안분하는데 화면은 아무것도 못 보여주는** 상태였다. ⇒ 안분 축에 파생을 적용했다.

**M-7 — 단건 파생은 오히려 틀렸다**
§18에서 넣은 `previewStdPriceAtTransfer` fallback은 `isSingle` 분기 안이라 **primary가 자기
자신**이 된다. 그런데 ④는 `form.assets.slice(1)`에만 파생하고 primary는 입력값을 그대로 쓴다
(`transfer-tax-api.ts:681`·`:426`) ⇒ 파생하면 **엔진이 재현할 수 없는 금액**을 사이드바가
보여준다. 종전의 0/pending이 오히려 정직했다. ⇒ **철회**.

> 🔑 **미러링의 기준은 「같은 규칙」이 아니라 「같은 축」이다.** ④가 index ≥ 1에만 파생하면
> ⑥도 index ≥ 1에만 파생해야 한다. 규칙만 맞추고 적용 범위를 넓히면 새 불일치가 생긴다.

### Low

- **L-10** 기준율 분모를 1e4 → **1e8**. `DecimalInput`이 소수 자릿수를 제한하지 않아
  `12.345%` 입력이 12.35%로 **조용히 반올림**됐다(1.5억 기준 약 7,500원 차이).
  1e12원 × 1e8도 `safeMultiplyThenDivide`의 BigInt fallback이 정확히 처리함을 실측 확인.
- **L-11** 라디오 클릭으로 값이 지워지던 문제 — H-3·H-4 수정으로 **함께 해소**.

### 리뷰가 확인해 준 것

**분해는 순수 이동이다.** 정상 경로 반환 객체의 키 집합이 master와 동일하고, 텍스트 차이는
`splitDetail` → `splitDetailForRate`(호출부가 `splitDetailForRate: splitDetail`로 넘기는 같은
값)뿐이다. 차손 경로는 들여쓰기와 구조분해 인자를 빼면 **바이트 동일**하다.
`finalizeTransferTax`가 `...args` spread가 아니라 구조분해라 넓어진 인자 객체가 새는 것도 없다.
