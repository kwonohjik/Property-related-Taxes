...
## 항목 #1 — 인구감소지역/관심지역 데이터 보강 ✅ 완료

> **완료 요약**: Vworld PNU 전수 실측(강원 42→51·전북 45→52·시군구 뒷자리 정정) + 다중소스 워크플로(행안부 89 인구감소/18 관심 5소스 일치+적대검증) + **세법 필터 적용**.
> - **세법 필터(워크플로가 놓친 법적 보정)**: 다목 1호 단서 "광역시(군 제외) 제외" → 부산·대구 자치구 미수록(대구 군위**군**·수도권 접경 4 포함) = **다목 84곳**. 라목 1호 "수도권 밖+광역시(군 제외)" → 수도권(동두천·포천·인천 동구)·광역시 자치구 제외 = **라목 9곳**.
> - **원본 오류 정정**: 무안(전남도청)·홍성(충남도청)·계룡(군사도시)·증평은 행안부 89곳 아님 → 제거. 인제는 관심지역(decline 아님). 원본 INTEREST 4곳(삼척·순창·담양·상주)은 전부 decline 오라벨 → 정정.
> - **로직**: classify `{kind}` 반환(M-9 priceLimit 제거), count.ts populationAreaType 자동도출(N-6), 라목 판정 경로 신설(O-1).
> - anchor: depopulation-data.test(D1~D12) + utilities MH-NEW-02 + same-sigungu. tsc0·vitest8823.
