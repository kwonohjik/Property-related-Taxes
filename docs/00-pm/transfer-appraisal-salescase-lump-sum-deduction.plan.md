...
| ID | 내용 | 확인 방법 |
|---|---|---|
| **V-1** | **일반건물(GB)·상업용건물(CB)·겸용주택·재개발** 경로에서 감정·매매사례의 개산공제 base가 어디서 오는지. GB는 카드별 기준시가 3시점을 따로 받아 「개산공제는 이미 카드에 실려 있다」고 주석돼 있으나(`general-building-swap.ts:226`) **실측하지 않았다**. 이 경로들도 결함이면 범위가 커진다. | 각 경로 ④ payload probe + 엔진 결과 대조 |
| **V-2** | **분양권**(§163⑥4호 1%)에서 감정·매매사례 선택이 ⑤에서 가능한지, 가능하면 std가 전송되는지. 승계입주권은 전송 확인됨(`transfer-tax-api.ts:329`), **분양권은 미확인**. | ⑤ 렌더 probe + ④ body 캡처 |
| ~~V-3~~ | ~~E2E 회귀~~ — **닫힘**. D-1 + D-4를 함께 얹은 상태로 전건 실행: 1177 passed / 1 skipped = `--list` 1178과 일치(§7.5.3). | 완료 2026-09-15 |
| **V-5** | **상업용건물(`commercial_building`)** 이 ⑧ salesCase 분기에 **도달한다**(`:167·:176`은 상속·환산만 걸러낸다). ④가 CB에서 `standardPriceAtAcquisition`을 실제로 소비하는지 — 소비하지 않으면 필수화가 **거짓 요구**가 된다. | CB + 매매사례 폼으로 ④ payload probe + 엔진 결과 대조 |
| **V-4** | D-2에서 폼 `assetKind` → 엔진 `propertyType` 변환 leaf의 정확한 위치. | grep |
