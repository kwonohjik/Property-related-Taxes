...
| **V-3** | 이미지 7 "10. 접대비한도초과액" — 현행 법령 용어는 **"기업업무추진비"** | ✅ **해소 (사용자 지시 2026-09-11, 후속 PR)** — 「법인세법」 제25조 제목이 **「기업업무추진비의 손금불산입」**이고 제1항이 *「이 조에서 "기업업무추진비"란 **접대**, 교제, 사례 …」* 로 정의한다(KoreanLaw 실측). 「접대」는 **정의 안의 예시어**이지 항목명이 아니다 | 표시 라벨 2곳 정정 + 회귀 anchor 신설(`entertainment-expense-term.anchor.test.ts` ET-1~3). **필드명은 불변**(저장 데이터 키). 서식 «구조»는 원본을 따르되 법령 «용어»는 현행을 따른다 — 행 23·24가 반대로 현행 유지인 것과 같은 기준의 양면 |
...
| V-n | V-1 ✅ · V-2 ✅ · V-3 ✅ — **전건 해소**(V-3은 후속 PR에서 정정) |
| Q-n | Q-1 ✅확정 |
