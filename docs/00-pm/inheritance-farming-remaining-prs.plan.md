...
**✅ KoreanLaw MCP 검증 완료 — 분리 평가 가능 판정 (2026-05-22, 작업 B)**:

§16⑭ 원문 (상증령 §16⑭, KoreanLaw MCP `get_law_text` mst=283637):
> "제4항에도 불구하고 다음 각 호의 어느 하나에 해당하는 기간에는 **피상속인 또는 상속인**이
> 직접 영농에 종사하지 않은 것으로 본다."

법령 원문 결론:
1. **"피상속인 또는 상속인"** — 둘 중 누구든 결격소득(3,700만 이상) 발생 과세기간만 그 사람의 직접 영농 기간에서 제외
2. **피상속인 결격소득** → §16②1호가(피상속인 8년 요건) 평가에 영향 → 전체 자격 미충족
3. **상속인 결격소득** → §16③1호가(그 상속인 2년 요건) 평가에 영향 → **해당 상속인만 자격 미충족**
4. §16③ 본문 "상속인이 ... 요건을 **충족하는 경우**에 적용" — 상속인 단위 평가 명시
5. §16⑤ 본문 "제3항의 요건을 갖춘 상속인이 받거나 받을 상속재산" — 자격자 분배분 명시 (PR-F `4c9cdaf` 이미 구현)

국세청 해석례 4건 (`search_decisions` domain=nts, 본문은 taxlaw.nts.go.kr 외부): "공동 상속" (267636·228966·300802) + "일부가 영농종사자 외 상속인" (39886) → 분리 평가 실무 정착

**판정: heirAssessments 신규 데이터 모델 진입 가능. 본 부록 A 정식 PR 진행.**
