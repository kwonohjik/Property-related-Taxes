...
| # | 항목 | 현행 (실측) | 스코프 요약 | 규모 |
|---|---|---|---|---|
| B-1 | **해외주식·국외전출세 잔여 하위 갭** ✅**완료(PR-κ DR·#168 / PR-λ 재입국·#169 / PR-μ 이자상당액·#170 / PR-ν 기타자산·#172)** | ① DR §157의3 2호 = 동일 과세(엔진 미분기·stale 주석 정정) ②a 재입국 §118의17①1호 환급/취소 ②b 이자상당액 §118의16④·§178의12③(사용자 입력 일수·rate, box 미렌더) ②c 기타자산 §94①4다·라 = §118의9① 범위·§118의11 동일 세율(엔진 기커버·주석 정정). ③ MarketTypeBlock stale은 B-2(PR-ε)에서 완료 | 완료 |
...
| D-2 | **환원율 고시 이력 검증** ✅**완료(PR-κ)** | **KoreanLaw 축자(상증법 시행규칙 MST 284609 §17)**: "영 §54①의 계산식에서 '재정경제부령으로 정하는 이자율'이란 **연간 100분의 10**" — **기재부 고시 아닌 시행규칙 정액**. 로드맵 "고시 이력" 전제 오해 정정(추적할 고시 없음·연도 테이블화 불요). 최근 개정(2026-03-20) §17 미변경 확인(§10의6·§13·§40·§14·§48만). 환원율은 사용자 입력 가능(UI 노출·override)이라 과거 평가기준일 변동도 사용자 처리. 정정: UI 힌트 "기획재정부장관이 고시하는 이자율" 오기→시행규칙 정액 정정. default 10% anchor 기존(EU-05·06·14b) | 완료 |
...
이후  B-3 ✅완료 → B-4 ✅완료(PR-η) → B-5 ✅완료(PR-θ) → C-2·C-3 ✅완료(PR-ι) → D-2 ✅완료(#167) → B-1①② ✅완료(#168·169·170·172)
★ 주식 양도세 잔존 후속 로드맵 전 항목 종료 (2026-06-13).
```
