...
### F-8b — 사이드바 표식 «범위» 규칙이 취득세와 갈렸다

주식은 `maxVisitedStep`(가 본 적 있는 단계)까지만 표식을 붙이고, **취득세는 무효한 단계를
전부** 붙인다(`AcquisitionSidebar.tsx`). 통일하지 않았다 — 취득세는 빈 폼에서 무효 단계가
**① 하나뿐**이고 그게 `active`라 노이즈가 0이라서, 지금 바꾸면 근거 없는 변경이 된다.

> ⚠️ **취득세에 필수 필드가 하나라도 늘면 그 순간 노이즈가 생긴다** — 빈 폼 첫 화면에
> rose `!`가 뜬다. 그때는 주식과 같은 `maxVisitedStep` 규칙으로 옮길 것.
> 주식의 실측값은 단계별 오류 **6·2·1**이었다(취득세는 1·0·0·0·0·0).
