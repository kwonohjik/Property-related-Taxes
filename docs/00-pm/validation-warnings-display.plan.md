...
| | ~~⚖️ **주식은 별건으로 남긴다** — 조용한 오답이 되지 않는다(빈 폼도 custom refine이 400으로 막는다). UX 등급이다~~ → 🔴 **이 등급 판정은 틀렸다. 아래 F-8b 참조** |

| **F-8b** | ✅ **주식분 해소(2026-09-25). 「UX 등급」이라던 F-8의 기록이 틀렸다 — 취득세보다 나쁘다.** 「빈 폼도 400으로 막는다」는 **빈 폼 하나로 잰 값**이었다. 부분 입력은 Zod를 그대로 통과한다 — 같은 픽스처(대주주 KOSPI·1,000주·양도 110,000/주·취득 10,000/주)로 잰 값: 완전 = 과세표준 97,500,000 · 세액 **19,500,000**, **취득단가만 비움 = Zod PASS · 엔진 200 · 과세표준 107,500,000 · 세액 21,500,000 · 경고 0건**. 취득가액이 조용히 **0원**으로 처리된다(`usedEstimatedAcquisition: false` — 환산으로도 안 넘어간다). 취득세(F-8 본건)는 0원이라 눈에 띄게 틀렸지만 **이쪽은 그럴듯해서 안 띈다** |
| | 📌 **도달 경로** — ②에서 취득단가를 지우고 다시 입력하려다 사이드바 「결과」를 누르면 끝이다. Step4가 마운트 시 **자동 계산**하므로(`steps/Step4.tsx:106-111`) 확인 절차조차 없다 |
| | 📌 **① 점프 게이트** `handleStepJump` — 네비 표면 2곳(`StockTransferTaxCalculator.tsx:258`·`:405`)이 `onStepClick={(i) => setStep(i)}` 생짜였다. 취득세와 달리 **조건부 단계 건너뛰기가 없어**(모든 트랙이 STEPS 4개를 동일하게 거친다) `computeNextStep`/`activeStepIndices` 없이 단순 루프로 족했다. 결과뷰 「수정하러 가기」도 주식엔 없다 |
| | 📌 **② 제출 백스톱** — `validateFilingItems`는 **다종목에만** 붙어 있었고 **단건은 무검증**으로 API에 갔다(`:211`) |
| | 📌 **③ 사이드바 표식** — `done/active/todo`만 쓰던 것에 `attention`(rose `!`, F-5 규약) 배선 |
| | 🔴 **두 층이 서로를 가렸다** — 뮤테이션에서 **게이트·백스톱 둘 다 SURVIVED**. 「결과」 점프는 Step4 자동 계산 → 백스톱이 같은 단계·같은 메시지로 되돌리므로, 게이트를 통째로 들어내도 화면이 같다. ⇒ **층별 격리 경로**로 각각 고정했다: 게이트 = ③으로 점프(Step4 미마운트 → 백스톱 미발화) · 백스톱 = 점프 없이 결과 단계에서 시작. 재측정 후 3층 전부 KILLED |
| | 🔴 **표식 범위는 세 번 바뀌었다(실측으로만 갈렸다)** — ⓐ 무효한 단계 전부 → **빈 폼 첫 화면에서 ②③이 동시에 빨개진다**(단계별 오류 **6·2·1**). ⓑ `i < currentStep`만 → 고치러 뒤로 가는 순간 다시 «todo»가 되어 **표식이 영영 안 뜬다**(게이트가 전진을 막아 그 상태를 만들 수 없다). ⓒ **`maxVisitedStep`**(가 본 적 있는 단계) → 노이즈 0 + 도달 가능. ⚠️ **취득세는 빈 폼 무효 단계가 ① 하나뿐이고 그게 current라 ⓐ의 함정이 드러나지 않았다** — 같은 코드가 세목에 따라 다르게 보인다 |
| | 📌 **부수 정정** — `stock-transfer-tax-schema.ts:446`이 「마법사 UI는 `validateFilingItems`가 먼저 막으므로 발현은 API 직접 호출에 한정된다」고 적고 있었다. **단건 경로에서 그 전제가 거짓**임을 실측해 주석을 고쳤다. 오류 배너에 `role="alert"` 추가(a11y + 셀렉터) |
| | ✅ **검증** — RTL anchor 8건(착수 전 임시 관측으로 `currentStep === 3` + `calcSpy` 1회를 green으로 확인해 진단 확정 후 반전) · 뮤테이션 3층 KILLED · E2E `[SJE-1~3]` 신설(게이트·표식 KILLED) |
...
> 📌 **주식(F-8b)에서는 이 축을 재게 됐다.** 백스톱 단독 경로(점프 없이 결과 단계에서 시작)를
> 찾아 RTL anchor로 고정했다 — 주식 Step4가 **마운트 시 자동 계산**하기 때문에 가능했다.
> 취득세는 마지막 단계에 명시 CTA가 있어 같은 수를 쓸 수 없다. **세목마다 다르다.**

### F-8b/c — 사이드바 표식 «범위» 규칙을 두 마법사에 통일했다 ✅

주식(F-8b)에서 `maxVisitedStep`(가 본 적 있는 단계)까지만 표식을 붙이기로 정했고,
**취득세도 같은 규칙으로 옮겼다(F-8c)**.

처음에는 「취득세는 빈 폼 무효 단계가 ① 하나뿐이라 노이즈 0이니 근거 없는 변경」이라 판단해
미뤘다. **그 판단이 틀렸다** — 빈 폼만 봤기 때문이다(F-8b에서 똑같이 밟은 실수다).
조건부 필수가 걸리는 순간 노이즈가 실재한다(실측):

| ①에서 한 선택 | 결과 |
|---|---|
| 연부취득 토글 ON | ②「물건 상세」 즉시 rose — 「최소 2회차 이상 입력해야 합니다」 |
| 간주취득(과점주주) 선택 | ②「물건 상세」 즉시 rose — 「장부상 총가액을 입력하세요」 |

사용자는 ②를 **본 적도 없는데** 빨개진다.

> 🔑 **추적 배선은 사이드바 anchor로 안 잡힌다.** `visitedUpTo`에 `currentStep` 하한이 있어
> 「지나온 단계」는 추적 없이도 표시된다 — `setMaxVisitedStep`을 통째로 지워도 사이드바를
> 직접 렌더하는 anchor 7건이 전부 초록이었다(SURVIVED). 추적이 일하는 자리는 **되돌아온
> 뒤**뿐이다. ⇒ 주식은 E2E `SJE-3`, 취득세는 오케스트레이터를 거치는 `AQ-4`가 지킨다.
