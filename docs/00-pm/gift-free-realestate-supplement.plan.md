...
- 검증 한계 1건(§81⑨2호 가목 인라인 수식 prose 미추출)은 §6에 기록 — 분자·분모 구성요소는 §81⑤ verbatim 도출로 해소.
...
| R-4 | 담보(§37②) 중단 | 분모 = 12개월 (✅ COL-RECT-1~3 검증) |
...
- **§81⑨2호 가목 계산식 본문 미렌더**: KoreanLaw·법제처 DRF·WebFetch 모두 인라인 수식(이미지/MathML) 텍스트 추출 불가. 단 **산식 입력 구성요소는 전부 verbatim 검증**:
  - **무상사용 분모 60개월**: 이미지4(20/60=18,605,246)로 **실증 완료**.
  - **담보(§37②) 분모 12개월**: ✅ **해소** — §81⑤ verbatim "부동산무상사용기간 = §27③후단 **및 §27⑤후단**" + §27⑤후단 verbatim "1년"에서 **도출(추정 아님)**. 가목이 §79②1호(§37 무상사용·담보 공통) 적용임도 본문 확인. COL-RECT-1~3 anchor 작성·green, 엔진 가드 제거·UI 활성화 완료.
  - 남은 한계: 가목 인라인 수식 prose 자체만 verbatim 미추출(공개 채널 한계). 분자(잔여월수)·분모(기간월수)는 검증됨.
...
| COL-RECT-1~3 | 담보 경정(분모 12) | 25,000,000 · 0 · 29,166,666 | ✅ §81⑤ 도출 |
...
- **Phase A — G1 경정청구 (free_use 한정)**(독립·이미지4 핵심): 타입 `RectificationInput`/결과 + 엔진 분기(분모 60) + RECT-1~3 anchor + UI 경정 섹션(산출세액 직접입력·중단일) + 결과 카드. 8지점 배선. **담보(§37②) 경정은 후속 PR에서 활성화(§81⑤ 도출 분모 12·COL-RECT-1~3).**
- **Phase B — G2 무상사용 다기간**: `periods` 타입 + window 루프(기존 단일계산 재위임) + FRE-MULTI-1·2 anchor + 다기간 토글 UI + window 표.
- **Phase C — G3 담보 다기간 + 담보 경정**: B 구조 재사용(1년·1천만) + COL-MULTI-1. **담보 경정(분모 12)은 §6 별표 캡처로 분모 동결 후** COL-RECT-1 anchor 작성 → 확정 시 활성, COL-RECT-1~3 green·가드 제거·UI 활성화 완료(후속 PR).
...
3. **§37② 담보 경정 분모(12월)**: ✅ 해소 — §81⑤(§27⑤후단 1년) verbatim 도출. COL-RECT-1~3 green·엔진 가드 제거·UI 활성화 완료(후속 PR).
