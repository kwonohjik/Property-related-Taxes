...
| 기준일 | 슬롯 기간 | 거래일 | 평균 | 판정 |
|---|---|---|---|---|
| 2025-06-10 | 2025-05-11 ~ **2025-06-10** | 20 | 56,590 | ✅ 대조군 · **기준일 포함 확인** |
...
> ⚠️ **초판 정정** — 2025-06-10 행의 거래일을 「21」로 적었으나 **20**이다. 그 값은 측정하지 않고
> 적은 숫자였다(응답의 `closingPrices` 비-null 개수를 세면 20이다). Phase 1 실측에서 드러났다.
> 「실측했다」고 적은 표에도 재보지 않은 칸이 섞일 수 있다.
...
## 4-2. Phase 2·1 실행 결과 (2026-08-31 — 완료 · 순서 뒤집어 실행)

### Phase 2 — 버튼이 route의 창·평균을 그대로 쓴다

`KiwoomAutoFetchButton`의 창 재구성·평균 재산정을 제거했다. stale 주석 2건도 정정했다
(`route.ts:14`의 「− 1 day」 · 버튼의 「API slotDates는 양도일 미포함」).

⚠️ **뮤테이션 겨냥을 한 번 틀렸다.** `avg`만 재계산으로 되돌렸더니 **통과했다** —
창이 이미 옳으면 같은 값이 나오기 때문이다. 겨냥을 **창 재구성**으로 옮기자 4건 전부 울렸다.
⇒ 하중을 받는 것은 평균식이 아니라 **창**이다([[feedback_mutation_masked_by_second_override]]).

⚠️ anchor 자체도 한 번 고쳤다. KRW-1의 초판 fixture는 세 날짜가 모두 버튼 창 «안»이라
평균이 우연히 일치해 **구별력이 0**이었다 → 창 밖 날짜(2015-01-19)를 넣어 교체.

### Phase 1 — B′ anchor 재결정 (참조 종목 거래일 달력)

신설: `lib/kiwoom/market-calendar.ts` (순수 함수 4개 · anchor 15건)
배선: `app/api/kiwoom/transfer-1month/route.ts` — **fixture 범위 밖일 때만** 참조 종목 조회

`isKrxTradingDay`·`resolveValuationAnchor`·`buildOneMonthBeforeSlots`는 **전부 무변경**이다
(F-3의 범위 축소를 지켰다).

#### 실 API 실측 4건 (KIWOOM_ENV=prod)

| # | 기준일 | anchor | 슬롯 | 거래일 | 평균 | 판정 |
|---|---|---|---|---|---|---|
| ① | 2015-02-19 (설날) | **2015-02-17** `shifted` | 2015-01-18 ~ 2015-02-17 | **22** | **1,371,500** | ✅ 계획서 예측과 원 단위 일치 |
| ② | 2025-06-10 (fixture 안) | 2025-06-10 | 2025-05-11 ~ 2025-06-10 | 20 | 56,590 | ✅ **불변** (참조 조회 미발동) |
| ③ | 199290 · 2015-06-10 | 2015-06-10 **미이동** | 2015-05-11 ~ 2015-06-10 | 0 | 0 | ✅ `stockSpecificGapAtAnchor: true` |
| ④ | 005930 · 2005-04-20 | 2005-04-20 | 2005-03-21 ~ 2005-04-20 | 22 | 502,818 | ✅ 종전 실측과 일치 |

**③이 B안과 B′안을 가르는 케이스다.** KONEX 종목의 **상장 전** 시점이라 시장은 열렸는데
그 종목만 종가가 없다. B안(「종가 없으면 옮긴다」)이었다면 한도까지 되짚어 엉뚱한 날을
anchor로 삼았을 것이다. B′는 **옮기지 않고** 종목 사유임을 알린다(§52의2③ 영역).

#### 응답 신설 필드 4개

`anchorDate` · `anchorShifted` · `marketCalendarUnavailable` · `stockSpecificGapAtAnchor`
— 화면이 「보정이 일어났다」·「참조 달력을 못 얻었다」·「이 종목만 종가가 없다」를
사용자에게 그대로 전할 수 있게 한다(자동 보정은 하지 않는다).

> 🔑 **참조 종목을 못 받으면 옮기지 않는다**(MA-4). 추정으로 창을 움직이면
