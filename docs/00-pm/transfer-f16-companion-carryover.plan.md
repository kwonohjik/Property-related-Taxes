...
> ---
>
> ## 🔵 2026-08-23 Phase 0 완료 — 사용자 결정 4건
>
> | | 결정 | 근거 |
> |---|---|---|
> | **Q-1** | **(a) 정식 지원** 채택 | (b) 명시 차단은 기각 |
> | **Q-2** | **D-4를 F16과 함께** 진행 | |
> | **V-3 스코프** | **배관만 — 신고단위 전환은 별건 분리** | 전환은 엔진 구조 변경(고정점 탐색)이고 **출하된 GB 파트 이월과세·지분분할 세액이 움직인다**. divergence 16/562(2.8%)·전부 현행 과소 방향 |
> | **classifyRateGroup** | **즉시 별건 수정** | F16과 무관하게 오늘 살아 있고 폭이 크다 |
>
> ### V-1·V-10 해소 (blocksImplementation = false)
> - **V-1 = ratio 적용(스케일 정합)**. 차단·현행유지 기각. 적용 지점은 `lib/calc/transfer-tax-api-carryover.ts`
>   (primary·컴패니언 **단일 소스**). 스케일 대상은 `donorAcquisitionPrice`·`giftDateValuation`·
>   `donorCapitalExpenditure` **3필드뿐**이고 🔴 **`donorStandardPrice*` 2필드는 절대 스케일 금지**
>   (이중 축소·분자 반토막 실측). 「자동 안분 fallback 금지」와 충돌하지 않는다 — 화면이
>   `OwnershipRatioInput.tsx:129`에서 「100% 기준으로 입력하세요, 시스템이 지분율을 자동 적용합니다」라고
>   **선언**하고 있어, 적용하지 않는 **현행이 오히려 UI↔엔진 dual-truth**다.
>   ⚠️ `giftTaxAmount`는 미결 — 착수 전 확정할 것.
> - **V-10 = 후보 ①(컴패니언 전용 안분 키 필드 `standardPriceAtTransferForApportion` 신설)**. ~17줄/4파일.
>   ⚠️ 계획서 전제가 뒤집혔다 — 「primary는 역할이 하나라 명시 삼항으로 푼다」는 **부정확**하다.
>   primary도 두 역할을 지되 **서로 다른 두 필드로 전송**해 충돌이 없다(실측: 같은 payload에
>   안분 키 600,000,000과 환산 분모 222,222,222이 **동시에** 실린다). 이식할 패턴은 삼항이 아니라 **전용 필드**다.
>
> ### V-3 결론 (별건으로 분리하되 기록)
> §97의2②3호 비교 스코프 = **신고단위**(판정은 자산별). 근거는 **§104⑤** — 다자산 양도에서
> 산출세액 자체가 과세기간 단위 단일값이고, 법이 자산 단위를 말할 때는 §104⑤2호처럼 **명시**하며
> 그 대상은 **산출세액**이지 결정세액이 아니다. 위임 체인(영 §163의2)에 스코프 규정 없음·규칙 재위임 없음.
> 선례 부존재(nts 399건 중 3건은 전부 보유기간 계산·조세심판원 9건 전수 무관).
> 현행 부동산 엔진은 **자산별**이라 주식 축(`stock-carryover.ts:11`)과 **불일치**한다.
> ⚠️ memory `project_stock_carryover_97_2_necessary_expense_open.md:169-172`의 「부동산 다자산 갭 없음
> (격자 ~7,500건)」과 **상충**한다 — 그 격자가 세율군 역전 축을 포함했는지 확인 불가(재현 불가).
> 별건 착수 시 이 상충부터 해소할 것.
