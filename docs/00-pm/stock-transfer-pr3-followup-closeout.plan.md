...
## 20. Track C 종결 (2026-08-27) — §47조의3①1호 **가목 + 나목**
...
구현은 **전액에 단일 비율**을 곱했다. 부정행위분이 일부인 신고에서 나머지에도 40%가 붙어
**납세자에게 불리**했다. 부동산·주식이 같은 모듈(`transfer-tax-penalty.ts`)을 쓰므로 **두 세목 공통**이었다.

**무신고(§47조의2①)에는 이 분해가 없다** — 그 조항은 「그 신고로 납부하여야 할 세액에 다음 각
호의 구분에 따른 **비율을 곱한** 금액」이라 각 목 구조 자체가 없다. ⇒ 과소신고·초과환급신고 전용.

### 20.2 ⭐ 「부동산 세액까지 움직인다」는 우려는 설계로 해소됐다

착수 전 판단은 「부동산 세액이 바뀌므로 별도 결정 필요」였다. 그러나 **`fraudulentPortion`
미입력이면 base 전액을 부정행위분으로** 보게 두면 종전 동작과 **완전히 같다**.

**실측이 그것을 확인했다** — 전건 17,499 통과, **기존 가산세 anchor 가 하나도 깨지지 않았다**.
세액은 사용자가 그 칸을 채울 때만 바뀌고, 그 방향은 **납세자에게 유리**하다.

⚠️ **0 과 미입력은 다르다** — 0 은 「부정행위분이 없다」는 유효한 선언(전액 10%)이고 미입력은
「전액 부정」이다. ④ 게이트가 **빈 문자열만** 거르고 0 은 보낸다. 둘을 섞으면 「0을 입력했는데
전액 40%가 붙는」 침묵 오류가 된다(anchor PW-4-3 · TF-1-3 이 고정).

### 20.3 배선

| 축 | 내용 |
|---|---|
| 엔진 | `calculateFilingPenalty` 가목·나목 합산 + `fraudSplit` echo · `penaltyRate` 는 혼합 시 **실효세율** |
| 주식 | `FilingAxisFields`·국내/국외 input·폼·normalize·④(2블록)·⑫(국내/국외)·⑭(2경로)·`PenaltyDetailBlock` |
| 부동산 | `FilingPenaltyInput`·`TransferFormData`·④ `buildPenaltyAmendmentPayload`·⑫ `filingPenaltyDetailsSchema`·Step6 |
| 표시 | steps 를 「가목 … + 나목 …」으로 분해 |

### 20.4 검증

anchor **25건**(엔진 11 · 주식 배선 4 · 부동산 배선 5 · UI 3 · 기존 확장) · 전건 **17,499 통과**.

**뮤테이션 4종 전부 감지**: 분해 무력화 8건 · 무신고에도 분해 2건 · 주식 ④ 0 미전송 1건 ·
부동산 ④ 게이트 제거 1건.

> ⭐ **M-C3 이 처음에 「구별력 0」으로 나왔는데 측정 실패였다** — `perl -0pi` 치환이 개행
> 이스케이프 때문에 매칭되지 않아 **뮤테이션이 적용조차 안 됐다**. python 으로 치환 결과를
> 세어 확인하니 정확히 1건을 잡았다. 「구별력 0이면 단언이 아니라 **측정 대상을 의심하라**」
> (`project_transfer_redev_rights_review_2026_08`)가 그대로 재현됐다.
> ⇒ 뮤테이션 스크립트는 **치환 건수를 출력**해 적용 여부를 확인할 것.
