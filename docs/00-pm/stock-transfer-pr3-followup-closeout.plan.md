...
> 🔴 **이 표는 2026-08-27 시점이다 — 지금은 stale.** A-1~A-8·B-1~B-5 **13건 중 11건이 닫혔다**.
> 실측 재판정과 근거는 **§23**(2026-09-12). 여기 🔴/🟡 표시만 보고 착수하지 말 것.
...
## 21. B-5 종결 (2026-08-27) — 국외전출세 기준환율 · 보유현황 신고서

사용자가 자료를 제공해 두 항목을 함께 종결했다(C1-02·R-2 에 이은 **세 번째** — 도구가 못 주는
자료는 사람에게 물어야 한다).

### 21.1 기준환율 — 사용자 입력

§118의13 외국납부세액은 **외국에서 낸 세금**이라 본래 외화인데 폼은 「원화 환산」만 받았다.
소령 §178의5 의 「기준환율 또는 재정환율」을 직접 넣게 하고 **환산은 엔진이** 한다.
해외주식(PR-4A)이 쓰는 축(`foreignTaxPaidForeign` × `foreignTaxExchangeRate`)을 그대로 재사용.

⚠️ **둘 다 있어야 환산한다** — 하나만 있으면 원화 입력값을 쓴다(조용히 0을 만들지 않는다).
통화 목록은 `currency-options.ts` 공용 leaf 로 추출했다(복사하면 목록이 갈린다).

### 21.2 ⭐ 사용자 PDF 가 **구판**이었다

`get_annexes` 로 검증하니 첨부 PDF(〈개정 2024.12.31.〉)와 최신본(〈개정 **2026.3.20.**〉)이
**세 군데** 다르다:

| | 구판(첨부 PDF) | 최신본 |
|---|---|---|
| 제목 | 국외전출자**국내**주식등 보유현황 신고서 | 국외전출자 **주식등** 보유현황 신고서 |
| 2번 섹션 | 국내주식 등 보유 현황 | **주식등** 보유 현황 |
| ⑩ | 사업자등록번호 | **주식등 종목코드 또는 사업자등록번호(해외주식은 ISIN코드와 국가명)** |
| 하단 | — | 시행령 §178의8② **제외 안내 신설** |

제목에서 「국내」가 빠지고 ⑩ 에 해외주식(ISIN)이 들어온 것은 **실질 변화**다.
⇒ 최신본으로 재현했다(`besshi-form-replica` 스킬의 「PDF 라벨을 그대로 신뢰하지 말 것」이
그대로 적중).
...
- **채움**: ⑧출국(예정)일 · ⑨법인명 · ⑩종목코드/사업자등록번호 · ⑪보유 주식수 ·
  ⑫액면총액(액면가 × 주식수) · ⑬지분율
- **비움**: ①~⑦ 인적사항 — 주민등록번호·주소는 계산기가 보관하지 않는다
- ⑩⑫⑬ 은 **세액 계산에 쓰이지 않아** 종목 폼에 **신고서 표시 전용**으로 받는다
  (`securityName` 이 「엔진 미전달 — 신고서 표시용」인 것과 같은 층위).
  ⚠️ 액면가 미입력이면 ⑫ 를 **추정하지 않고 비운다**(anchor HR-3-4).

anchor 11건 · 뮤테이션 3종 전부 감지(ROWS_FIXED · 액면가 가드 · 구판 제목).

---

## 22. B-3 부분 종결 (2026-08-27) — 증권거래세 2020-04-01 구간 확정

§19 에서 「도구가 시행령 §5 과거 본문을 주지 않아 보류」였는데, **사용자가 화면 2건을 제공해**
한 구간이 열렸다(C1-02 · R-2 · B-5 에 이어 **네 번째** — 도구가 못 주면 사람에게 물어야 한다).

### 22.1 확정된 값

| 근거 | 내용 |
|---|---|
| 법 §8① [시행 2020.04.01.] 제16837호 | 「증권거래세의 세율은 **1만분의 45**로 정한다」 ⇒ 비상장 45 |
| 시행령 §5 (개정 2013.8.27·2017.2.7·**2019.5.28**) | 1호 **1천분의 1** = 유가증권시장(자본시장법령 §176의9①)·코넥스시장(§11②) / 2호 **1천분의 2.5** = 코스닥시장(영 24697호 부칙 §8)·금융투자협회 K-OTC(§178①) |

⇒ 신규 구간 **2020-04-01 ~ 2020-12-31**: 코스피 10 · 코넥스 10 · 코스닥 25 · K-OTC 25 · 비상장 45.
`STX_CUTOFF_DATE` 를 **2021-01-01 → 2020-04-01** 로 당겼다.

### 22.2 ⚠️ 더 이르게 열지 않은 이유

시행령 §5 의 마지막 개정이 2019.5.28 인데 **그 시행일이 화면에 없다**. 본법은 개정 전이
「1천분의 5」임을 `time_travel` 로 확보했지만(§19.1), 상장 3시장 탄력세율의 **적용 시작일**이
없으면 구간 경계를 그을 수 없다 — 추정 금지. ⇒ 본법 시행일이 확정된 **2020-04-01** 부터만 연다.

### 22.3 기존 anchor 재산정

`A-30`(2020-12-31 → fallback + 경고)이 이제 **커버 범위 안**이라 법령 정합값으로 고쳤다:
현행 5/10000 fallback(50,000) → 당시 10/10000(**100,000**) · 경고 없음.
경계·시장별 anchor 5건 신설(A-30b~f). 뮤테이션 3종 전부 감지(cutoff · 코스피 · 비상장).

### 22.4 잔여

**2019.5.28 개정 시행일** 하나만 확인되면 그 이전 구간도 열 수 있다. 필요한 것은
국세법령정보시스템에서 **증권거래세법 시행령을 「연혁」으로 열었을 때 보이는 시행일 목록**이다.

---

## 23. 잔여 인벤토리 재판정 (2026-09-12) — **§1 표는 stale 했다**

「미결 사항을 다시 파악하라」는 지시로 §1 Track A·B 13건을 **전부 실측 재판정**했다.
결론: **8건이 이미 닫혀 있었고**, 대신 §1 표에 없던 **표시 결함 1건**이 살아 있었다.

메모리 `feedback_open_item_audit_stale_rate`(「미결 목록은 실측하면 절반 이상이 이미 닫혀
있다」)가 그대로 재현됐다.

### 23.1 Track A 재판정

| # | §1 판정 | 실측 (2026-09-12) | 근거 |
|---|---|---|---|
| **A-1** | 🔴 잔여 | ✅ **닫힘** | `StockSidebar.tsx` 에 `savedItems` 참조 **6건** |
| **A-2** | 🔴 잔여 | ✅ **닫힘** | `StockAggregateSummaryCard.tsx:283-303` 이 `totalSecuritiesTransactionTax` 를 표시 |
| **A-3** | 🔴 잔여 | ✅ **닫힘** | `validateFilingItems`(`stock-transfer-tax-validate.ts:627`)가 확정 종목 전건에 `validateAllSteps` + `handleCalculate:160` 차단 + 목록 미완성 배지(`incompleteIndexes`) |
| **A-4** | 🔴 잔여 | ✅ **닫힘** | `PrRoadmapCard` → `UnsupportedItemsCard` 로 대체(§20 Phase F) |
| **A-5** | 🔴 잔여 | ✅ **닫힘** | `stock-transfer-finalize.ts:234` `computeStockLatePaymentPenalty` |
| **A-6** | 🔴 잔여 | ✅ **닫힘** | 「과소신고납부세액등」 base + 가목/나목 분해(§20 Track C) |
| **A-7** | 🟡 미결 | ✅ **닫힘** | `computeFilingUnitPenalty`(신고 1건 단위 1회) + `stripItemPenalties`(종목별 0) — Step3 안내 「신고서 1매 단위」와 일치 |
| **A-8** | 🔴 잔여 | ✅ **닫힘** | §17 잔여-1 종결. `pickFilingAxisInput` 이 국내·국외를 가르지 않는다 |

> 🔴 **A-3 판정에서 한 번 헛짚었다** — 1차 실측에서 `grep -c savedItems lib/calc/stock-transfer-tax-validate.ts`
> 가 **0건**이라 「검증 부재」로 읽었다. 그 함수는 `savedItems` 를 **인자로 받는**
> (`validateFilingItems(forms)`) 형태라 그 문자열이 파일에 없었을 뿐이다.
> 「참조를 셀 때 형태 열거 금지」(`feedback_enumerate_forms_vs_conservative_superset`)가
> **호출 측 변수명 축에도** 적용된다 — 정의 파일에 그 이름이 없는 것은 근거가 아니다.

**A-7·A-8 안전망 뮤테이션 실측** (`feedback_pre_change_safety_net_probe`):

| 뮤테이션 | 실패 |
|---|---|
| `computeFilingUnitPenalty(determinedTotal → 0)` (A-8 축) | **10건 RED** |
| `stripItemPenalties` 무력화 (A-7 이중계산 축) | **7건 RED** |

⇒ 「닫혔다」가 주석의 주장이 아니라 **실제로 지켜지고 있음**을 확인했다.
...
|---|---|---|
| **B-1** | §15 Phase G 로 종결 | ✅ |
| **B-2** | §18 로 종결(`listed_otc_non_major` 신규 생성 중단). 세율 `case` fall-through·타입 union 은 저장 이력 호환으로 **의도적 잔존** | ✅ |
| **B-3** | §22 로 **부분 종결**(커버 2020-04-01 확대). 2019.5.28 시행일 미확보 구간만 잔여 | 🟡 (아래 23.3) |
| **B-4** | §16 — 잔여가 아니었음 | ✅ |
| **B-5** | §21 로 종결(기준환율 수동 입력은 **의도적 유지**, 보유현황 신고서는 구현) | ✅ |

### 23.3 🔴 살아 있던 결함 — **커버를 넓히고 소비처를 안 고쳤다**

§22가 `STX_CUTOFF_DATE` 를 2021-01-01 → **2020-04-01** 로 당겼는데, 그 날짜를 **말하는 쪽**
5곳이 옛 값을 그대로 들고 있었다. 「어느 층을 고쳤다 ≠ 그 층이 소비된다」의 거울상이다
...
| 양도일 | 세액 | 세율 | 경고 |
|---|---:|---:|---|
| 2020-03-31 | 200,000 | 5 (현행 fallback) | 있음 |
| **2020-04-01** | **250,000** | **10 (당시)** | **없음** |
| **2020-12-31** | **250,000** | **10 (당시)** | **없음** |
| 2021-01-01 | 230,000 | 8 | 없음 |

⇒ **2020-04-01 ~ 12-31 은 당시 세율로 정확히 계산되고 경고도 안 뜬다.** 그런데 화면과
경고 문구는 그 구간을 「미지원」이라 말하고 있었다.

| 위치 | 성격 | 처리 |
|---|---|---|
| `StockTransferTaxResultViewHelpers.tsx:342` 카드 제목 | **사용자 표시** | `${STX_CUTOFF_DATE}` 보간으로 교체 — 리터럴 제거 |
| `legal-codes/stock.ts:688` `WARNING_UNSUPPORTED_PERIOD` | **사용자 표시** | 날짜 정정. legal-codes → data 계층 의존을 만들지 않으려 리터럴 유지하고 **anchor 로 동기화 강제** |
| `securities-transaction-tax.ts:8·100·116·226` | 주석 4곳 | 상수명 표기로 교체 |

### 23.4 `UnsupportedItemsCard` — 대체한 카드와 **같은 방식으로** stale 해졌다

이 카드는 `PrRoadmapCard`(내부 PR 번호 하드코딩)를 대체하면서 스스로
「**항목이 해소되면 같은 PR 에서 문구를 지운다**」는 규율을 주석에 적었다. **그 규율이 깨졌다** —
4항목 중 **3항목이 거짓 고지**였다.

| 항목 | 판정 | 근거 |
|---|---|---|
| 증권거래세 「2021-01-01 이전」 | 🔴 날짜 오기 | 위 23.3 |
| 국외전출세 기준환율 | ✅ 실재 | §21 — 의도적 유지 |
| 「보유현황 신고서 미생성」 | 🔴 **해소됨** | `ExitTaxHoldingReportForm`(별지 제104호서식, 206줄) + `ExitTaxHoldingReportSection` + Step 4 마운트 + anchor 11건 (§21) |
| 「국외 종목만이면 가산세 미계산」 | 🔴 **해소됨** | `pickFilingAxisInput` ①이 국내·국외를 가르지 않는다 · anchor **FP-2-2**(39,500,000 × 40% = 15,800,000) (§17) |

⇒ 뒤 둘을 **삭제**했다. 남은 항목은 2개다.

**교훈 — 고지 목록은 «기능을 구현할 때» 역방향으로 훑어야 한다.** 항목을 추가할 때만 보는
목록은 반드시 stale 해진다. §17·§21이 각각 그 기능을 구현하고도 이 카드를 열지 않았다.

### 23.5 신규 anchor

| anchor | 무엇을 고정하나 | 뮤테이션 |
|---|---|---|
| **STX-CUTOFF-SYNC** | 경고 문구의 날짜 = `STX_CUTOFF_DATE` | 문구를 `2021-01-01` 로 되돌리면 **RED** ✅ |
| **UN-1-1** | 카드 제목이 상수와 같은 날짜 | (상수 보간이라 구조적으로 보장) |
| **UN-1-1b** | 확대 전 날짜(`2021-01-01`)를 더 이상 말하지 않는다 | — |
| **UN-1-3** | 보유현황 신고서가 고지 목록에 **없다** (짝: `exit-tax-holding-report-form.test.tsx`) | — |
| **UN-1-3b** | 국외 종목만 가산세가 고지 목록에 **없다** (짝: `foreign-penalty-axis.anchor.test.ts` FP-2-2) | — |

> ⚠️ **A-30c 가 왜 못 잡았나** — `expect(r.warning).toContain("미지원")` 로 **낱말만** 봤다.
> 날짜가 틀려도 통과한다. `STX-CUTOFF-SYNC` 가 그 자리를 메운다.
>
> ⚠️ **종전 UN-1-2 는 기준환율과 보유현황 신고서를 한 항목에서 함께 단언**하고 있었다.
> 보유현황 고지를 지우며 그 단언을 통째로 지웠다면 **기준환율 안전망까지 사라졌을 것**이다
> (`feedback_shared_assertion_reversal_erases_sibling_net`) ⇒ 둘을 갈라 두고 지워진 쪽은
> 반전 anchor 로 남겼다.

### 23.6 곁다리 — 국외 가산세 주석이 **정반대**였다

`stock-transfer-aggregate.ts:531-535` 가 「`ForeignStockInput` 에 신고축 필드가 없고 국외 단건
엔진도 가산세를 계산하지 않으므로 0을 유지해 단건과 다종목을 일치시킨다」고 적고 있었다.
**두 전제 모두 지금은 거짓**이다(§17이 둘 다 구현했다). 그 자리가 0인 진짜 이유는
**신고 1건 단위 1회**라 종목에 귀속되지 않기 때문이다 — 사유를 그대로 갱신했다.
그대로 뒀다면 다음 사람이 「국외는 가산세가 없다」로 읽었을 자리다.
...
|---|---|
| `npx tsc --noEmit` | ✅ **0건** |
| `npx eslint` (변경 6파일) | ✅ **exit 0** |
| `vitest __tests__/tax-engine/stock-transfer/ __tests__/calc/ __tests__/components/` | ✅ **732파일 6,734테스트 전건 통과** |
| 뮤테이션 3종 (A-7 · A-8 · STX-CUTOFF-SYNC) | ✅ 전부 **RED 감지** 후 원복 |

**세액 변경 0.** 변경은 표시 문자열·주석·anchor뿐이고, 엔진 산식·④변환·⑫Zod·⑭route 어디도
건드리지 않았다.

> ⚠️ **800줄** — `stock-transfer-aggregate.ts` 750 → **759**(주석 +9). 트리거(800)까지 41줄.
> §10.3의 경보는 유효하다 — **다음에 이 파일을 여는 작업은 분리를 먼저 검토할 것.**

### 23.8 남은 것

| # | 항목 | 성격 |
|---|---|---|
| **B-3 잔여** | 증권거래세 **2020-04-01 이전** 구간 — 시행령 §5 의 2019.5.28 개정 **시행일** 미확보 | 🟡 자료 대기(§22.4). 화면 고지·경고가 이제 정확하므로 사용자 오인은 없다 |
| **B-5① 잔여** | 국외전출세 기준환율 한국은행 API 미연동 | 🟡 의도적 유지(§21). 화면 고지 유지 |
