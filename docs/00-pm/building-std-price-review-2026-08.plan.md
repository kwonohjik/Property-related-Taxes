...
| **F-09** | II 연면적 `{}` 게이트 (±10~20%) | ~~⛔ 착수 불가 — 고시 원문 필요~~ → **✅ 종결** (`b8cc6af0` · 2026-08-27 고시·계산사례 전수 실측) | `residential-usage-auto` 11 · `gazette-adjustment-auto-apply` 9 |
...
- ~~**F-07 잔여**~~ → **✅ 종결 확인 (2026-09-12 실측)**. 별칭 표는 전남·광주 24 + 전주 2 = 26건인데, **판정 집합·`lib/geo` 전체에서 별칭이 없는 구 코드가 0건**이다(`regulated-areas-data.ts`·`population-decline-areas.ts`·`lib/geo/**` 스캔). 강원·전북은 **이미 현행 코드**(51·52)로 적혀 있어 별칭이 필요 없다 — 표의 자체 정책(「필요해지면 여기에 추가한다」)이 충족된 상태다.
  ⚠️ 「강원(42→51)·전북 나머지가 없다」는 종전 기재는 **표의 누락이 아니라 「필요 없음」**을 잘못 읽은 것이다. 잔여로 재기재하지 말 것.
  ⚠️ 첫 스캔은 `factory-area-rates.generated.ts` 의 **KSIC 업종코드**(29111 등)를 시군구 코드로 오인해 「미등재 44건」을 냈다 — 5자리 숫자만 보는 스캔은 모집단을 **넓히는** 방향으로도 틀린다.
  · 남는 것은 `/data/stdprice/` 산출물 부재로 **실배포 manifest 키 미확인**, 필지조인의 **읍면동 재부여 여부 미확인** 둘뿐이다(둘 다 데이터 부재 — 조사 항목이지 결함이 아니다).
...
- ⚠️ `lib/calc/building-std-price-form.ts` 가 **852줄**로 800줄 정책 초과 상태다. 다만 이번 수정 **전에 이미 837줄**이었고 분리는 별건이다.
  → **해소됨**(2026-09-12 확인): `building-std-price-validate.ts` 분리로 현재 `-form.ts` 는 정책 내다.

### 종결 현황 (2026-09-12 전수 대조)

**46건 중 45건 종결 · 1건 착수 불가(F-09 → 이후 `b8cc6af0` 로 종결되어 실질 46/46).**
F-14·F-16 을 2026-09-12 에 마지막으로 닫았다 — 상세는 아래 「F-14~F-46 전수 대조」.

**분기됐던 별건도 전건 종결**(2026-09-12 실측): F-07 잔여(별칭 미등재 0건) · F-41 잔여(없음 —
고시 제2025-39호로 양자화 제거 완료) · 연도 순서 검증(아래) · `-form.ts` 800줄 초과(분리로 해소).

**남은 것은 「결함」이 아니라 조사 항목 2건뿐이다** — 둘 다 **데이터 부재**가 원인이다:
- `/data/stdprice/` 산출물이 워크트리에 없어 **실배포 manifest 키 미확인**
- 필지조인이 시군구 5자리만 정규화한다 — **읍면동 재부여 여부 미확인**

그리고 **신축연도 축**(`builtYear > acquisitionYear|valuationYear` 무검증, 실측 +25%)은
차단의 정당성이 미판정이라 의도적으로 열어 두었다 — 상세는 아래 「연도 순서 검증」.

> ✅ **2026-09-22 정정 — 신축연도 축은 둘로 갈렸다.** 위 한 덩어리 서술이 부정확했다.
> **평가·양도 시점 축은 종결**(`builtYearOrderError` 신설, `residual-rate.ts` · ⑧검증 2곳 ·
> anchor §4 11건 · 뮤테이션 4종 KILLED). **취득 시점 축만 미결**이며 착수 조건은
> **국세청 고시 「건물 기준시가 계산방법」 §8④(완공 전 취득) 본문 확보**다.
> ❌ 「신축연도 축 전체가 미결」로 재기재 금지 — 아래 「신축연도 축」 절 참조.


### F-14~F-46 전수 대조 (2026-09-12) — **medium/low 배치는 실제로 닫혀 있었다**

배치 커밋 메시지가 아니라 **각 건의 결함 술어를 현행 코드에서 직접 재확인**했다
(「배치했다」≠「처리했다」 — 완료 판정은 리뷰 ID 역방향 grep + 술어 대조).

| 건 | 대조 결과 |
|---|---|
| **F-14** | 🔴 **잔존** → 이번에 수정 |
| **F-16** | 🟠 **부분 잔존**(하한은 F-12 가 leaf 로 해결, ⑧검증 중복 술어·토글 끄기 경로가 남음) → 이번에 수정 |
| F-15·F-17~F-46 (31건) | ✅ **전건 종결 확인** |

31건의 대조 근거(현행 코드에서 확인한 것): `ACQ_YEAR_MIN=1985` + `availableYears` 파생(F-15) ·
PDF `acqBase` 참조 7건(F-17) · `usesPriorStdPriceSubstitute` 4층 공유(F-18) ·
`hideApartmentConversion`(F-19) · `NED_FETCH_FAILED` 502 + warn 로그(F-20) ·
인코딩 판별 8,192바이트 + 불완전 시퀀스만 절단(F-21) · `skippedParts` 빌드 단위 기록(F-22) ·
상가 토지/건물 분리 행 조건부 렌더(F-23) · ④ 단일시점 게이트 `!f.compositeMode`(F-24) ·
`sec1648Active` leaf 3층 공유(F-25) · `unassignedAncillary` echo + warning(F-26) ·
⑩ = ⑧ × ⑨ 환산 전 규약(F-27) · `packAdjustmentCells`(F-28) · `nts-bsp-x-4`·`-x-sum`(F-29) ·
`NtsReportInstance.timepointLabel`(F-30) · `floorAreaProduct` BigInt micro-㎡(F-31·F-33) ·
`applyRateFraction`(F-32) · `sameAdjustmentPeriodDerived`(F-34) · 칩 ctx `structureKey`(F-35) ·
`acqIndexYear` ≠ 안내 조건 분리(F-36) · `ERA_B_DURABLE_FIXED`(F-37·F-38) ·
`MODULES` 13개(재수출 2 포함 15) + 디렉터리 열거 가드(F-39) · §164 ③⑤⑧ verbatim(F-40) ·
`Math.round` 제거 → `normalUseRatio * 100`(F-41) · override 잔액 흡수(F-42) ·
`-cb-first` 런처(F-43) · 「원」 접미 0건(F-44) · PDF 「부속토지」(F-45) · hint 병기(F-46).

#### F-14 — 재산식이 §164⑨ 분모를 못 본다 (표시 오분류 266,666,666원)

`transfer-tax-aggregate.ts` 가 표시용 취득가액을 원시 `standardPriceAtTransfer` 분모로
**재산식**하는데, 단건 엔진은 §164⑨ 발동 시 낮아진 분모로 환산한다.

실측(토지 10억 · 취득시 기준시가 2억 · 양도시 5억 · 수용 분모 3억):

| 칸 | 현행 표시 | 엔진(정답) | 괴리 |
|---|---:|---:|---:|
| 취득가액 | 400,000,000 | 666,666,666 | **−266,666,666** |
| 필요경비 | 272,666,666 | 6,000,000 | **+266,666,666** |
| 양도차익·세액 | 327,333,334 | 327,333,334 | **0** |

🔑 **교차검산이 오류를 가렸다** — 「양도가액 − 취득가액 − 필요경비 = 양도차익」이
**두 값 모두에서 성립**한다(필요경비를 취득가액에서 역산하므로).

🔑 **안전망 0건** — 재산식을 `estimatedBase` 로 바꾸는 뮤테이션에
`__tests__/{tax-engine,calc,api}` **1,368파일 15,277건이 전건 통과**했다. 세액이 불변이라
어느 테스트도 이 칸을 보지 않는다 ⇒ 신규 anchor 가 유일한 관측 경로다.

⇒ `r.result.estimatedBase` 를 우선 쓰고 재산식은 fallback 으로 남긴다
(`usedEstimatedAcquisition` 은 입력 플래그, `estimatedBase` 는 `calcTransferGain` 산출이라
두 축이 갈릴 수 있다 — 그때는 종전 표시 유지로 회귀 0).
선례: 바로 위 `adoptedCarryoverAcquisitionPrice` 가 이월과세 축을 같은 방향으로 이미 고쳤다.

#### F-16 — 술어 하나만 leaf 밖에 남아 네 층이 갈렸다

F-12 수정이 `isSameAdjustmentPeriodConversion` 으로 엔진·④·UI·단일시점 ⑧을 묶으면서
하한(양도 ≥ 취득)을 leaf 에 넣었는데, **`building-std-price-validate.ts:221` 한 곳만**
술어를 손으로 다시 적은 채 남았다(같은 파일이 leaf 를 이미 import 하고 있었다).

실측(취득 2022 · 양도 2021 · 토글 ON):

| 층 | 판정 | 결과 |
|---|---|---|
| ⑧ validate(손술어) | **true** | 보유월수를 요구, 양도 구조·용도·공시지가는 면제 |
| ④ `toEngineInput`(leaf) | false | 빈 `transfer: {structureKey:""}` 를 싣는다 |
| 엔진(leaf) | false | **「양도시: 구조 미선택」 throw** |
| UI(leaf) | false | §164⑧ 섹션(보유월수 칸) 미렌더 |

⇒ **화면에 없는 칸을 요구하는 해소 불가 차단**. 토글도 `crossYearWindow` 만 보므로
창을 벗어나면 사라져 켜진 플래그를 끌 수도 없다(정리 지점 0곳).

⇒ ① 손술어를 leaf 로 교체 ② 토글 렌더 조건을 `(crossYearWindow || f.crossYearSameAdjust)` 로 넓힘.

⚠️ **「양도연도 < 취득연도」 자체의 차단은 여전히 별건이다** — 어느 경로도 연도 순서를
검증하지 않아 역순 입력이 일반 2시점 계산으로 조용히 완주한다. 이번 수정은 「역순일 때
네 층이 **같은 답**을 낸다」까지만 보장한다.

### 연도 순서 검증 (2026-09-12) — F-16 별건 종결

「양도연도 < 취득연도」를 **어느 경로도 보지 않아** 역순 입력이 경고 없이 완주했다.
...
주식양도세 `stock-transfer-tax-validate.ts:279`. **이 계산기만 예외**였다.

#### 차단은 「화면에 그 칸이 있을 때」만 성립한다

양도연도 Select 는 **공동주택 환산**·**취득 전용(단일시점 acquisition)** 모드에서 숨는다.
그 상태의 stale 값으로 차단하면 **화면에 없는 칸 때문에 막히는 dead-end** — F-16 의 실패모드다.
⇒ 술어를 손으로 다시 적지 않고 **`transferYearVisible` leaf** 를 신설해 UI 렌더 조건과
⑧검증이 같은 답을 내게 했다(Form 도 leaf 를 쓴다 — `acqOnly` 지역변수는 고아가 되어 제거).

뮤테이션 2종 검출: 가시성 조건 제거(항상 차단) → **3건**(dead-end 가드) · 부등호 반전 → **8건**.

#### ⚠️ 이것이 F-16 손술어 축의 구별력을 흡수했다

새 가드가 **더 앞에서** 막으므로, §164⑧ 손술어를 복원해도 같은 2건이 같은 메시지로 실패한다
(뮤테이션 실측 = **구별력 0**). 해당 anchor 2건을 「어느 층이 먼저 막는가」의 계약으로 다시 적고
그 사실을 주석에 남겼다 — 손술어 잔존을 실제로 잡는 것은 「보유월수를 묻지 않는다」 1건뿐이다.

#### ⚠️ 신축연도 축은 차단하지 않았다 (같은 probe 에서 실측) — **착수 시점 기록(2026-09-12)**

> `builtYear > acquisitionYear` (실측 취득 기준시가 118,800,000 vs 정상 95,000,000, **+25%**)·
> `builtYear > valuationYear` 도 똑같이 무검증 통과한다. 그러나 재개발·입주권처럼 **신축이 취득보다
> 뒤인 정당한 조합**을 배제할 위험이 있어 판단 근거가 부족하다 — 법령 근거 없이 정상 입력을 막는
> 방향이라 **기록만 남기고 별건으로 둔다**.

### 신축연도 축 (2026-09-22) — 평가·양도 시점 축 종결 / 취득 시점 축 착수 조건 확정

위 기재는 두 축을 **한 덩어리로 묶은 것이 부정확**했다. 실측이 갈라 놓았다.

실측(rc · 용도1 · 200㎡ · 취득공시 1,000,000 · 양도공시 1,100,000):

| 입력 | 수정 전 ⑧ | 산출 | 수정 후 |
|---|---|---|---|
| 신축 2000 · 취득 2010 · 양도 2020 | 통과 | acq 95,000,000 (잔가율 0.8) | 통과(불변) |
| **신축 2025 · 양도 2020** | **통과** | trans **168,600,000 (잔가율 1.000)** | **차단** |
| **신축 2025 · 상증 평가 2020** | **통과** | val **168,600,000 (1.000)** | **차단** |
| 신축 2015 · 취득 2010 (취득 ≥2001) | 통과 | acq **118,800,000 (1.000)** vs 95,000,000 | **통과 유지** |
| 신축 1995 · 취득 1990 (취득 ≤2000) | 통과 | **THROW** 「산정기준율 미수록」 | 불변 |

#### ✅ 평가·양도 시점 축 — 종결

「그 시점에 건물이 없다」는 재개발·입주권으로도 설명되지 않는다 — **양도하려면 그 건물이
존재해야** 한다. 그리고 저장소 내부에 이미 같은 규칙이 있었다: 형제 술어 `remodelYearError` 가
「대수선 > 평가연도」를 차단하고 「대수선 ≥ 신축」도 요구하므로, **대수선을 입력한 경우에만**
신축 ≤ 평가가 간접 강제되고 있었다. 그 **비대칭**이 결함의 지문이다.

- `residual-rate.ts` `builtYearOrderError(builtYear, pointYear, pointLabel)` 신설(단일 술어).
- ⑧검증 2곳: 상증은 `remodelYearError` **앞**(근본 원인을 먼저 알린다), 양도는 기존
  `transferYearVisible(f)` 블록 안(양도연도 칸이 숨는 모드의 **dead-end 방지** — F-16 실패모드).
- anchor `building-std-year-order.anchor.test.tsx` §4(11건) + §5(3건). 뮤테이션 4종 전부 KILLED
  (leaf 무력화 8 · 경계 `<=`→`<` 2 · 양도 호출부 6 · 상증 호출부 2).

#### 🛑 취득 시점 축 — 미결. 착수 조건 = **고시 §8④ 본문**

보류 판단이 **옳았다**. 「완공 전 취득」은 고시상 **별도 범주로 실재**한다 —
산정기준율표가 그 셀을 의도적으로 비워 두었고(`acq-base-rate.ts:12` 「값 없는 셀(취득<신축=
완공전취득 **§8④** 등) = 미수록」), 실제로 취득 ≤2000 경로는 「산정기준율 미수록」으로 멈춘다.
취득 ≥2001 경로만 잔가율 1.000 으로 통과한다(+25.05%).

⇒ 일괄 차단은 **법 근거 없이 정당한 입력을 막는 방향**이다. 국세청 고시 「건물 기준시가
계산방법」 §8④ 본문을 확보해 완공 전 취득의 처방(별도 산식인지 · 계산 불가인지)이 정해지면
`anchor §5` 를 그 규칙의 anchor 로 교체한다. 그 전까지 §5 가 **배제 축을 고정**한다
(명시 배제 축은 안전망이 0 이 되므로 — `feedback_anchor_excluded_axis_is_unguarded`).
...
- **✅ 종결(2026-08-27)** — 축 확정 + 선행 조건 제거 + 게이트 해제를 한 PR에서 마쳤다.
  - **「항상 적용」이 옳다.** 고시 제11조 구분 II 적용범위 단서는 「지하층·옥탑 제외 / 통나무조 제외 / **주거용건물은 아파트에 한해 최고층수기준만**」뿐 — **용도 제한이 없다**. 계산사례 13건 중 **상속 9건이 예외 없이** 구분 II 를 받고, 근생(라멘)№41 은 「9. 1천㎡ 이하」**만**, 운동시설№24 는 「10. 1천~5천㎡」**만**으로 붙는다.
  - **종전의 「공식 사례 4건이 반증한다」는 오판이었다** — 그 4건은 **양도** 사례다(원문 「양도 개시일」·조정률 「-(양도세 계산시 미적용)」). 판매시설№11 만 라벨이 「상속」이나 계산값이 미적용이어야 맞아 사례집 오기다.
  - **선행 조건 3건을 없앴다**:
    1. **주거 판정을 용도번호로 자동화**(`isResidentialUsage`) — 조정률 모달을 열지 않아도 판정된다. 경계는 체계마다 다르다: 2001~2002 = 1~2 · 2003~2013 = 1~3 · 2014~2026 = 1~2.
    2. ⭐ **아파트 판정도 짝으로 자동화**(`isApartmentUsage`) — **예상 밖의 발견**이다. 주거인데 아파트가 아니면 단서 「아파트에 한해 최고층수기준만」에 걸려 구분 II 가 **통째로** 빠지므로, 한쪽만 자동화하면 아파트가 「주거·비아파트」로 오판돼 최고층수까지 사라진다(BSP-03 이 그렇게 깨져 발견했다). ⚠️ 2001~2002 는 #1 이 「단독주택·아파트」 통합이라 번호로 못 가른다 ⇒ 사용자 플래그 정본.
    3. **복합은 부분별 용도번호로** 판정 — 건물 단위 플래그 하나로는 혼합 건물이 갈리지 않아 공식 작성례(3) 171,500,000 이 깨졌다.
  - 🔴 **실제 제품 결함이 새로 드러났다**: PHD 3시점 배치는 **양도세 전용**인데 1시점 계산 편의로 `inheritance_gift` 모드를 쓴다. 종전엔 조정률이 1.0 이라 무해했으나 구분 II 자동 적용으로 **양도세 계산에 조정률이 붙는다**(상가 최초공시 실측 −10.1%). 고시 제11조 단서 위반이라 배치·**스냅샷**(계산서 렌더에도 쓰인다) 양쪽에 미적용을 명시했다 — 라운드트립 등가 테스트가 그것을 잡았다.
  - anchor `residential-usage-auto.anchor.test.ts` 11건 + `gazette-adjustment-auto-apply.anchor.test.ts` 9건. 뮤테이션 2종 검출(게이트 복원 · 주거 경계 오프바이원).
  - ⚠️ anchor 갱신은 전부 **법령 정합** 방향이다 — 「상증 모드로 양도를 흉내내며 조정률을 암묵 처리하던」 픽스처를 출처와 함께 **명시**로 바꿨고(그 암묵성이 축 자체를 가렸다), 「주거 용도번호를 써서 검증하려던 축이 가려진」 anchor 는 비주거 번호로 복원했다.
...
- **✅ 종결(2026-08-27)** — 리뷰 처방대로 `calcPriorStdPriceSubstitute` 를 배선했다(2001표 fallback은 채택하지 않았다).
  - 🔑 **실측이 드러낸 것 ①: 2000년 산정기준율은 전 그룹·전 신축연도에서 1 이상이다**(rc 1985 1.022 … 2000 1.016; brick·cement_block·solid_wood·stone 전수 확인). 즉 전기(2000) 기준시가가 취득당시(2001)보다 **커서** delta 가 음수가 되고 **§80①1호 하한 단서가 언제나 발동**한다 ⇒ 취득 2001 제1산식의 양도값은 **항상 취득당시와 같다**. F-18 의 실질 효과는 금액 변화가 아니라 「**오류로 막히던 것이 계산된다**」이다.
  - 🔑 **실측이 드러낸 것 ②: 그래서 대체산정 값이 세액에 드러나지 않는다.** 뮤테이션 `delta = 0` 이 **잡히지 않았다**(하한이 흡수). ⇒ 근거를 **warnings 로 노출**해 사용자에게 「왜 양도 = 취득인가」를 설명하는 동시에 관측 경로를 만들었다 — 대체산정 입력 변조 뮤테이션이 그 경로로 잡힌다.
  - 🔑 「산정기준율표 미수록 구조」 방어는 **취득 2001 에서 도달 불가**다 — 미수록 구조 5종은 전부 2013년 신설이라 2001년 **구조지수표**에 없어 그보다 앞에서 막힌다. 방어는 유지하되 계약은 leaf 층에서 고정했다.
  - 엔진·validate·UI 가 `usesPriorStdPriceSubstitute` **한 술어를 같은 인자로** 공유한다. 취득 2001 에서는 「취득전기 공시지가」 입력이 안내 카드로 대체된다.
  - anchor `prior-std-price-substitute.anchor.test.ts` 13건 · 뮤테이션 3종 중 2종 검출(경계 `<`→`<=` · 대체산정 입력 변조), 1종은 위 ② 사유로 원리상 관측 불가.
...
- **✅ 종결(2026-08-27) — 서식 원문 확인 후 구현 완료**
  - 별지 ※표는 **5칸이 맞다**: (1) 2001.1.1현재 건물 기준시가 / (2) 산정기준율 / (3) 취득당시 [(1)×(2)] / **(4) 토지가액** / 합계 〔(3)+(4)〕.
  - 작성요령 ※4 「**토지가액은 취득당시 토지가액을 계산하여 기재합니다.**」 ⇒ 리뷰의 「(4)는 `inst.landValue`가 아니다」가 **정확히 맞았다**. 계산사례 마. 실측: ※(4) **234,000,000** = 130㎡ × **1,800,000**(1999.6.30 = 취득일 **직전 고시분**) vs Ⅵ⑤ **291,200,000** = 130㎡ × 2,240,000(2001 기준).
  - ⇒ **신규 입력 1개**(`acqAtTimeLandPrice`)를 취득 ≤2000 분기에 신설했다. 폼의 기존 취득 공시지가는 라벨 그대로 「위치지수용 (2001.1.1 기준)」이라 쓸 수 없다.
  - 🔑 **표시 전용이라 14 동기화 지점을 전부 타지 않는다** — 계산서는 스냅샷(폼 상태)에서 직접 조립되므로 ①②⑤ + `buildNtsReportContext`(④) + 어댑터 + 화면 + PDF 뿐이고, Zod·route·엔진(⑨~⑭)은 무관하다. 착수 전 「14지점」으로 잡았던 추정을 실측이 좁혔다.
  - 🔑 **미입력은 차단하지 않는다.** 세액 산식에 개입하지 않는 표시 항목이므로 「자동 안분 fallback 금지」의 대상이 아니고, 0 원으로 단정하지도 않는다 — ※(4)(5)를 **「—」**로 비운다.
  - anchor `nts-report-acq-base-land.anchor.test.tsx` 10건(어댑터 4·화면 3·배선 3). 뮤테이션 2종 검출: (4)를 Ⅵ⑤ 값으로 바꿔치기 → 6건 / ④ 배선 제거 → 1건.
  - 부수 확인: 작성요령 ※2 「2001.1.1현재 기준시가는 … **Ⅵ번 ⑪항목과 일치**합니다」 ⇒ **F-27을 「⑪ = 환산 전」으로 고친 방향이 서식으로 확증**된다. Ⅴ표에 **「9 제외」 행**이 실재해 **F-26**의 미귀속 몫 취급도 서식 구조와 정합한다.
...
~~**F-06 · F-09 · F-26 · F-29 · F-37 · F-38 · F-41**~~ → ~~잔여 F-09 · F-29 뿐~~ → **전건 종결**(F-09 `b8cc6af0` · F-29 `6161030c` · F-18 `e9645023` · F-30 `b99b3df7`. 2026-09-11 이력 확인)(F-06·F-41 = 제2025-39호로 종결 · F-26 = 설계문서로 종결 · **F-37·F-38 = 2026-08-27 과거 연도 고시 7건으로 종결**). 아래 서술은 당시 기록이다. 이 건들은 고시 원문 확보가 착수 조건이었다. 확보 없이 어느 방향으로 고쳐도 국세청 공식 계산사례 anchor가 깨진다.
