...
2. **경고 배지는 전부를 잡지 못한다.** M-3 실측 — 저장된 종목이 기본공제를 온전히 받고 차손 통산도
   ...
   🔴 **초판 판정 정정 (2026-09-17 재실측)** — 초판은 이것을 「판별 불가」로 **뭉뚱그렸다**.
   사각지대를 4종으로 분해해 실측하니 **원리적으로 불가능한 것은 1종뿐**이고, 나머지 3종은
   지금 판별기가 **그냥 놓치고 있을 뿐**이었다.

   | 케이스 | 현재 판별기 | 실측 |
   |---|---|---|
   | (b′) 앞 종목이 공제를 **전액** 소진 → 공제 0 | ✅ 잡음 | diff 5키 |
   | (b) 앞 종목이 **일부** 소진 → **부분공제** | ❌ 놓침 | 공제 300,000(단건이면 2,500,000) · 세액 940,000 vs 500,000 |
   | (d) **국외주식**이 마지막 종목 | ❌ 놓침 | 단건 **29키** vs 합산 **38키** — 결과 **형태가 다르다**(`basicDeductionGroup`이 단건엔 없다) |
   | (c) 마지막 편집 종목이 **양도일 최선** · 이익만 | ❌ 놓침 | diff **`{}`** — 진짜 불가능 |

   (c)가 생기는 이유: 엔진은 §103② 배분을 **양도일 오름차순**으로 하는데
   (`stock-transfer-aggregate.ts:333`) UI가 저장하는 것은 **마지막에 편집하던 종목**이다.
   둘이 어긋나면 그 종목이 공제를 먼저 전액 받아 단건과 구분되지 않는다.

   **고칠 수 있었던 방법**(미채택 — 아래 결정 참조):
   - ① 공제 정합식으로 교체 — 단건 주식 그룹은 `calcBasicDeduction`
     (`stock-transfer-helpers.ts:111`)이 유일한 작성자라
     `basicDeduction = min(transferIncome, 250만)`으로 **완전히 결정**된다.
     현재는 그중 `=== 0`만 본다. **등식 위반 전체**로 바꾸면 (b)·(b′)를 한 조건으로 덮는다.
   - ② 기타자산 그룹 편입 — 지금은 통째로 제외돼 있다. 제외 사유인
     `realEstateGroupBasicDeductionUsed`(금액)는 **record의 `inputData`에 있으므로**
     `remaining = max(0, 250만 − used)`로 기대값을 세울 수 있다.
   - ③ 국외주식은 **형태**로 판정 — `inputData.marketType === "foreign_stock"`인데
     `resultData`에 `basicDeductionGroup`·`transferIncome`이 있으면 어댑터를 거친 것,
     즉 합산 산물이다. 단건은 `ForeignStockResult`라 그 키가 없다.

   **⚖️ 결정 (2026-09-17, 사용자) — ①②③ 미구현.** 2026-09-16 이전 이력을 `/history`의
   **전체 삭제**로 일괄 폐기하기로 해서 판별 대상 자체가 사라진다. 컷오프 정합도 확인했다 —
   PR #1647 머지가 **2026-09-16 22:53**이라 「9.16 이전」이 legacy 전부를 덮는다.
   날짜 기준 일괄 삭제 기능도 만들지 않는다(현재 `/history`는 전체 삭제 + 개별 삭제뿐).

   ⚠️ **판별기가 영구히 죽는 코드는 아니다** — `HistoryBackupActions`의 **가져오기**로 옛 백업을
   복원하면 legacy record가 다시 들어온다. 그때 위 3종이 필요해지면 이 절을 근거로 착수한다.
