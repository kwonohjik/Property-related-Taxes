...
### 1-7. 🆕 **혼동 금지** — §29③의 공모 제외는 **다른 규정이다** (v1.4 위임체인 완주)

법문 3단을 끝까지 따라가면 **이중부정 구조**다:

1. 「상증법」**§39①1호가목**: 「그 포기한 신주(실권주)를 **배정**(**주권상장법인**이 「자본시장과 금융투자업에 관한 법률」 **제9조제7항에 따른 유가증권의 모집방법**(대통령령으로 정하는 경우를 **제외**한다)으로 배정하는 경우는 **제외**한다. **이하 이 항에서 같다**)하는 경우에는 …」
2. 「자본시장법」**§9⑦**: 「"모집"이란 … 산출한 **50인 이상**의 투자자에게 새로 발행되는 증권의 취득의 청약을 권유하는 것」
3. 「상증령」**§29③**: 「법 제39조제1항제1호가목에서 "대통령령으로 정하는 경우"란 「자본시장과 금융투자업에 관한 법률 **시행령」 제11조제3항**에 따라 모집하는 경우」
4. 「자본시장법 시행령」**§11③**: 「청약의 권유를 받는 자의 수가 **50인 미만으로서 증권의 모집에 해당되지 아니할 경우에도** 해당 증권이 발행일부터 1년 이내에 50인 이상에게 양도될 수 있는 경우로서 … **전매기준에 해당하는 경우에는 모집으로 본다**」 = **간주모집**

⇒ **최종 해석**:

| 사실관계 | §39① 적용 |
|---|---|
| 주권상장법인이 **진성 공모**(50인 이상 청약권유)로 실권주 배정 | ❌ **제외 — 과세 없음** |
| 그 「모집」이 **§11③ 간주모집**(실질은 50인 미만 사모인데 전매기준 해당) | ✅ **제외에서 다시 빠짐 — 과세** |

형식적 간주모집으로 과세를 회피하는 것을 막는 구조다. **적용 범위는 「이하 이 항에서 같다」 ⇒ §39① 전체**(1호 가·나·다·라 + 2호 전부), 가목 한정이 **아니다**.

| | §39의3 (본 계획 ⓒ · 구현 완료) | §39 (§29③ · ✅ **구현 완료** — 아래 정정) |
|---|---|---|
| 근거 | 자본시장법 **법 §165의6①3**(일반공모 **방식**) | 자본시장법 **법 §9⑦ 모집방법** + 시행령 §11③ 제외 |
| 판정 기준 | 배정 **방식** | **청약권유 인원 50인** + 전매기준 |
| 효과 | 곱하는 **신주수에서 차감** | **「배정」 개념에서 제외** ⇒ 적용 자체 없음 |
| 적용 범위 | §29의3①1·2호 | **§39① 전체** |

**형태가 전혀 다르다** — §39 증자에 ⓒ를 옮겨 붙이면 오적용이다.

> 🔴 **미구현 확인 + 방향**: `capital-increase.ts`에 공모 제외 로직 **0건**(grep 실측). ⇒ 상장법인이 공모로 실권주를 배정해도 엔진은 **과세한다** = **과대과세(납세자 불리)** 방향이다. CT-6(과소과세·근거 미확보)과 달리 **법 근거가 명확**하므로 `feedback_no_unfavorable_application_without_legal_basis`가 **시정을 요구**한다.
> 📎 실무 쟁점 증거: 서울행정법원 2011구합9676·2011구합10195(2012.4.27.) 「**유가증권의 모집방법에 의한 배정에 해당된다고 볼 수 없음**」 — 납세자가 이 제외를 주장했다가 배척된 사례가 복수 존재한다.
> ⇒ **별건 구현 대상으로 등록**(본 계획 범위 외 §10 유지). 착수 시 UI는 2단 판정(진성 공모 여부 → §11③ 간주모집 여부)이 필요하다.
