...
## 13. 브라우저 확인 (2026-09-08 · Playwright, `E2E_PORT=3200`)

throwaway spec 4건으로 실제 렌더를 촬영해 육안 확인했다(확인 후 삭제 — 배치 회귀는
vitest 앵커가 지킨다).

| 확인 | 결과 |
|---|---|
| V-1 기본 화면 배치 | 기준 박스 → 기준일 → 비거래일 → 특수 토글 → 지분율 → 시총 → 키움 → 대차·PEF → hint(6건) → 합산 → 결과. **의도대로** |
| V-2 §6 분리 | 「거래 구분 · 증권거래세」가 별도 섹션으로, 대주주 판정 **뒤**에 렌더 |
| V-3 D-1·D-2 실증 | 합산 OFF에서 대차 600주 입력 → 배지가 「✗ 비대주주」 → 「✓ 대주주」로 전환 |
| V-4 주식수 모드 | §5에 발행주식 총수 **입력칸 없음**, 「발행주식 총수 100,000주 — 「양도·취득 일자 및 주식수」 단계에서 입력합니다」 읽기 전용 표시 |

### 🔴 브라우저 확인에서 결함 1건 추가 발견 — 판정 사유가 가산 전 값이었다

V-3 스크린샷에서 화면이 **자기모순**을 띄웠다:

```
현재 적용 기준: 지분율 2.0% · 시총 50억
…
대주주 자동 판정: ✓ 지분율 1.50% → 대주주 해당      ← 1.5% < 2.0% 인데 대주주?
```

D-2에서 `judgment` **계산**은 가산을 반영하게 고쳤지만, 결과 박스의 **사유 문자열**이
`parseDecimal(form.selfShareRatio)`로 폼 원본값을 다시 읽고 있었다
([[feedback_engine_result_display_drift]] — 차감값↔결과 표시 드리프트와 같은 패턴).

⇒ `judgment`가 **실제로 비교한 값**(`effectiveSelfRatio`·`ratioAugment`)을 함께 반환하고
표시가 그것을 쓰도록 고쳤다. 가산 내역도 함께 보여준다:

```
대주주 자동 판정: ✓ 지분율 2.10% (본인 1.50% + 대차·사모펀드 0.60%p) → 대주주 해당
```

앵커 **L-13** 3건 추가(가산 후 값 표시 · 내역 표기 · 가산 없을 때 내역 미표기).
⇒ 앵커 총 **29건**.

> 🔑 **교훈**: 계산을 고칠 때 그 값을 **읽는 표시 지점을 함께 세지 않으면** 화면만 옛 값에
> 남는다. D-2는 `judgment` useMemo와 `computeAutoIsMajor` 두 곳을 고쳤지만 세 번째 소비자인
> 결과 박스를 놓쳤고, **vitest 앵커 26건도 그것을 잡지 못했다**(배지·판정 결과의 boolean만
> 보고 사유 문자열은 안 봤다). 브라우저 확인이 유일한 관문이었다.

### C-10 — 시가총액 입력 정렬 (사용자 지시로 추가)

「본인 단독 시가총액」이 `FieldCard` 없이 `CurrencyInput`을 맨몸으로 써서 라벨이 카드 밖에
떠 있었다. `CurrencyInput`이 자체 `<label>`을 위에 그리기 때문이다 — 같은 섹션의 다른 필드는
전부 좌-라벨 `FieldCard`라 정렬이 어긋났다.

전수로 보니 **「합산 시가총액」도 같은 형태**였다(`:606`). 둘 다 저장소 선례
(`PostListingAmountInputSection.tsx:134-142` — `FieldCard` + `CurrencyInput label="" hideUnit`)를
따라 감쌌다. 단위 「원」은 `FieldCard`의 `unit` prop으로 옮겼다.

브라우저 재확인: 지분율 카드와 시가총액 카드가 동일 구조가 됐다
(좌측 라벨 → 입력칸 → 우측 단위 → 하단 hint). 합산 시가총액도 같다.

앵커 **L-14** 3건 추가 ⇒ 앵커 총 **32건**.
