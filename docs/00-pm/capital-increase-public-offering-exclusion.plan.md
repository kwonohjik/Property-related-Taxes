...
⇒ **§39①의 공모 제외와 완전히 같은 이중부정 패턴**이며 「이하 이 항에서 같다」로 **§40① 전체**에 걸린다. `grep` 결과 `convertible-bond.ts`에 관련 로직 **0건**.

> ⚠️ 다만 `convertible-bond.ts`는 §40①1호를 `bondAcquisition`(§30①1 — 시가 − 취득가) **하나로만** 처리한다. 나목·다목의 **별도 요건(최대주주·균등 초과 인수)이 구현돼 있는지 자체가 미실측** — 공모 제외를 얹기 전에 **그 확인이 선행**돼야 한다.
