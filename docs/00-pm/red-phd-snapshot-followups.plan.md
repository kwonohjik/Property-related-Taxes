...
### ✅ 착수 조건 충족 — 조문 확인 결과 (2026-08-24, KoreanLaw MCP)

**「소득세법 시행령」 제164조 제7항 본문**(현행, 시행 2026-07-01) 실측:

> 「부동산 가격공시에 관한 법률」에 따른 개별주택가격 및 공동주택가격(이들에 부수되는
> 토지를 포함한다)이 **공시되기 전에 취득한 주택**의 취득당시의 기준시가는 …

⇒ 요건은 「**공시되기 전에 취득**」이고, **취득시기 자체는 규정하지 않는다** — 일반 규정
(의제취득일 포함)에 맡긴다.

**결정적 근거는 저장소의 기존 입장이었다.** `isPhdEligible`(의제취득일 보정 포함)이 이미
**⑧ `transfer-tax-validate-asset` · ⑩ `transfer-tax-schema-refines`(Zod refine) ·
겸용 `transfer-tax-validate-mixed-use-asset`** 등 **실제 차단 게이트 4곳**에서 쓰이고 있다.
즉 이 저장소는 「§164⑦ 판정에 의제취득일이 미친다」는 해석을 이미 채택했고, **재개발 경로만
빠져 있었다** — 같은 조문을 두 경로가 다르게 본 것이다.

⇒ 이 작업은 **새 해석 도입이 아니라 적용 범위 일치**다. 그래서 착수했다.

### 세액이 갈리는 입력은 제도상 불가능한 값뿐

두 판정이 달라지려면 **최초공시일 ≤ 1985-01-01**이어야 한다. 개별주택가격은 2005-04-30,
공동주택가격은 2006-04-28에 최초 공시됐으므로 정상 입력에서는 결과가 같다(probe 실측 —
`1984-12-31/2005-04-30`·`2003-05-10/2005-04-30` 등 4행 일치).

### 구현 — 날짜 비교를 `isPhdEligible`에 위임

⚠️ **방향이 반대인 지점이 하나 있다**: `isPhdEligible`은 날짜가 비면 `true`(게이트 미발동 —
필수입력 검증이 따로 막는다)지만, 트리거는 `false`(발동 안 함)여야 한다. 그래서 날짜 존재를
트리거에서 **먼저** 확인한 뒤 위임한다.

`transfer-tax-validate-redev.ts`의 인라인 식도 같은 술어로 치환했다 — 그쪽 고유 조건인
`isHousingRightReceiveEstimated` 배제만 남긴다(그 플래그는 validate 안에서 계산되므로
술어 인자로 올리지 않는다). ⇒ **UI · 결과탭 게이트 · ⑧ validate가 완전히 같은 판정**이다.

### 검증 — ✅ 완료

- 의제취득일 보정 2행 · 실무 입력 3행 · **`isPhdEligible`과 6행 전수 일치**
- 판정 불능 방향(날짜 부재 → 미발동)이 `isPhdEligible`과 반대임을 anchor로 고정
- ⑧ validate 동기화 anchor — 보정 케이스에서 §164⑦ 필수입력을 요구하지 않는다
- mutation probe 2종(보정 제거·validate 인라인 원복) → 각각 3건·1건 실패
- 회귀: vitest **전체 16,275건** · 재개발 E2E **9건** · tsc 0 · lint 0 errors
