...
- 검증 깊이: **L2** — 신규 필드 7개 · UI 신설. **세액 산식 무변경**(계산 결과를 기존 4필드에 mirror)
- 상태: **계획 확정 · 구현 미착수**
...
둘째 행: 순자산가액        →  1주당 순자산가치 자동 산정 (첫 행의 주식수 공용)
```
...
calcNetAssetPerShare({ assetTotalRow1: 순자산가액, assetAdd: [], assetSub: [],
                       liabTotalRow8: 0, liabAdd: [], liabSub: [], goodwillRow19: 0, shareCount })
```
...
| `listingYearNetAssetAmount` | 상장일 직전 사업연도 순자산가액 |
| `acquisitionYearNetIncomeAmount` · `acquisitionYearShareCount` · `acquisitionYearNetAssetAmount` | 취득일 축 동일 |
...
| ③ | normalize | `calc-wizard-stock-normalize.ts` — `strField` 6 + `enumField("simpleValueInputMode", ["direct","amounts"], "direct")` |
| ④ | API 변환 | **무변경** (mirror) |
...
2. 계산 헬퍼 위임 래퍼 (UI 전용, lib 아님 — 단일 사용)
   → verify: 유닛 anchor — 500,000,000/10,000 → 500,000 · 50,000,000/10,000 → 5,000
...
5. validation (⑧)
   → verify: amounts 모드 — 원천 3필드 미입력 시 차단 / 입력 시 통과
...
| ID | 항목 | 영향 | 상태 |
|---|---|---|---|
| **V-1** | 입력받는 「순자산가액」이 **영업권을 포함한 최종 순자산가액**인가(자산−부채+영업권), 아니면 영업권 전 금액인가 | 완전재현 모드는 영업권을 **별도 행**(`goodwillRow19`)으로 받는다. 별도로 받으려면 필드가 2개 더 는다 | 🟡 **미해소** — 착수 전 확인 필요 |
...
> **V-1이 해소되기 전에는 착수하지 않는다** — 필드 개수와 라벨이 갈린다.
> 기본안은 「입력값 = 영업권을 포함한 최종 순자산가액」이고, 그 경우 라벨을
> 「순자산가액 (영업권 포함)」으로 명시해 오입력을 막는다.
...
- 환원율 입력·영업권 별도 행이 필요하면 완전재현 모드를 쓴다.
