...
- **정본 이동(D-6)**: 위 「이관」 중 **주택별 사실**(§155⑥ 문화유산 · §155⑦ 농어촌 · §155⑧ 부득이 · §155⑳ 임대주택 목록)은
  ...
  ⚠️ **구현 상태 — P6은 이 결정대로 하지 않았다**(2026-09-21 정리 시 병기). §155⑧·합가는 명부 행이 아니라
  `TemporaryTwoHouseSection`에 **그대로 남았고**(§32 · 중과 축 때문), §155⑳도 명부가 아니라 같은 섹션의
  `calc` 모드로 갈랐다(§34). ⇒ D-6의 「명부 행으로 간다」는 **미구현 설계**다. 결론(계산기에도 입력이 남는다)은
  같지만 **경로가 다르므로**, 이 절을 근거로 「이미 명부로 옮겼다」고 읽지 말 것.
...
- **부득이 주택**: 영 §167의10①3호 = 기준시가 3억 이하 · 1년 이상 거주 · 해소 후 3년(`multi-house-surcharge-exclusion.ts:400-409`) /
  §167의10①4호·§155⑧ = 수도권 밖 · 해소 후 3년(`qualifiesUnavoidableOutsideCapital`). 행에는 **두 요건의 합집합 사실**
  (사유 · 소재 · 거주기간 · 기준시가 · 해소일)을 담는다.
...
3. **행 속성은 조건부 노출**한다. 이미 약 70필드다. 예: 농어촌 속성은 행 소재지가 수도권 밖 읍·면일 때(현행 자동 판정 재사용),
   중과 전용 속성은 세대 주택 수 2 이상일 때. 1주택자 간이 입력은 유지(D-4).
4. **§155⑳ 임대주택 목록은 명부 ③ 행에서 도출**한다. 단 `RentalUnitInput`과 `HouseEntry` ③은 **겹치지만 같지 않다** —
   `rentalMonths` · `rentalAcquisitionType` · `requirementsConfirmed` · `rentalAutoTermination`은 `HouseEntry`에 없고,
   `rentalCategory`↔`rentalType` · `region`(수도권/비수도권)↔`region`(capital/non_capital)은 **enum이 다르다**. 대응표는 **V-18**.
   Q-7의 「판정 사실 / 산식 입력」 분할은 그대로다 — 판정 사실의 **위치**만 명부 행으로 바뀐다.
5. **결과에 두 축의 주택 수 산정 내역을 나란히** 보인다 — 어느 행이 어느 축에서 빠졌는지. 두 축 결론이 갈릴 때 이유가 보여야 한다.

**단계 배치 — 과도기 이중 입력 금지**

- **P4**: `HouseEntryEditor`에 §155 행 속성을 추가하고 판정 메뉴에서 켠다. 같은 컴포넌트가 계산기에도 마운트되므로
  **계산기에서는 표시 모드 prop으로 숨긴다** — 켜면 Step4 세대 단위 토글과 **두 입력이 공존**한다(dual truth).
- **P6**: 계산기에서 행 속성을 켜고 **같은 PR에서** Step4 §155⑥⑦⑧ 토글을 뺀다. 순서가 바뀌면 유일 입력 경로 제거다
