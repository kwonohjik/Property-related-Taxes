...
- 🟠 단건 ④는 `toRentalHousingExceptionApi(primary)`로 **primary만** 보내는데(`transfer-tax-api.ts:696`)
  ⑧은 **자산별로** 검사한다. 컴패니언 자산에 §155⑳을 켜면 검증은 하고 전송은 안 하는
  비대칭이다 — P6-c-2 **이전부터 있던** 것이라 이번 범위 밖. 별건으로 판단할 것.
