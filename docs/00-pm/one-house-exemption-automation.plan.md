...
> 상태: **Do 진행 중 (P0~P6 완료 · 설계 문서 2종 작성 완료)**.
>
> 🔴 ~~「🛑 착수 조건 미충족」~~ 은 **stale이었다**(2026-09-22 감사). 근거:
> `docs/02-design/features/one-house-exemption-automation.{engine,ui}.design.md` **둘 다 존재**하고
> (아래 「산출물 게이트」가 요구한 것), 본문 :733이 「**P6 완료** 시점」으로 적고 있으며,
> §5.10-A·B·C(D-6 후속 3건)가 2026-09-22에 머지됐다. ❌ 「착수 조건 미충족」으로 재기재 금지.
>
> (아래 v2.1 서술은 **구조 결정 시점 기록**으로 보존한다.) **구조 결정 D-1~D-6 확정(2026-09-18, 사용자 승인)** —
> 1세대1주택 비과세 **판정은 별도 메뉴**, 세액 계산기는 **간이 입력 + 판정 결과 소비**. Q-1·Q-2 해소 · Q-3′(**세대 판정은 사용자가 한다**)·Q-4·Q-5 결정 ·
> Q-6 소멸 · **Q-7 결정(§155⑳·입주권도 P4에서 이관)** · V-1·V-6·V-8·V-12·V-13·V-14·V-15 종결 · V-11 부분 해소 · V-2~V-5·V-9·V-10 미결 (**V-7은 2026-09-21 §26.6에서 반증 해소** — 머리말이 stale이었다).
...
| 4 | **§155⑧ 행 이관** — 3호/4호 **분리 유지** + OH-30 카드 | 위 ⛔ 주의 | ✅ P7-5 |
...
~~⏳ 3b에서 남긴 것~~ → **4단계에서 붙였다**(아래).

**4 구현 요약 (P7-5) — D-6 완료**

- `HouseEntry` +3필드 — `oneHouseUnavoidableOutsideCapital` · `unavoidableOutsideCapitalReason` ·
  `unavoidableOutsideCapitalResolvedDate`. 「수도권 밖」은 행 `regionCode`로 판정한다(칸 없음).
- ⛔ **3호와 4호를 행 안에서도 분리했다.** 행에는 이미 `isUnavoidableReason`(영 §167의10①**3호**)이
  있는데 §155⑧은 **4호**다 — 3호는 기준시가 3억↓·1년 거주를 요구하고 4호는 그 요건이 **없다**.
  🔑 `OneHouseFactRow`의 `Pick`에 3호 필드를 **넣지 않아 타입 차원에서** 분리가 강제된다.
  anchor UO-5가 **구별력 시료**(기준시가 3억 초과 + 수도권 밖 → 3호 불성립·4호 성립)를 고정한다.
- 🔴 **유일한 이중 입력이 해소됐다.** §155⑧ 블록은 `full` 가드 **밖**이라 계산기·판정 메뉴
  **양쪽**에 떴고 저장소가 달라 값이 두 벌 존재했다. `ImportedOneHouseFactsCard`가 §155⑧만
  의도적으로 뺐던 이유가 이것이다.
- 🔑 **한 선언이 두 축을 움직인다** — 비과세(`qualifiesUnavoidableOutsideCapital`)와 중과 배제가
  같은 값을 쓴다(`transfer-tax-judgment-steps.ts:55`). ④만 바꾸면 두 축이 함께 따라온다.
- **OH-30 카드 구현** — `HousesListSection`이 도출 leaf의 `fromLegacyOnly`를 읽어
  「어느 주택인지 지정되지 않았습니다」를 띄운다(`one-house-legacy-unassigned`).
  화면이 규칙을 재구현하지 않는다.
- calc 모드 섹션 제목 정정 — §155⑧이 떠나 **합가만** 남으므로 「③ 수도권 밖 부득이·합가 특례」
  → 「③ 합가 특례」.

📌 **실측 발견(별건)**: 「부득이한 사유 취득 주택」 토글이 화면에 **둘**이다 — 다이얼로그 밖은
...
2. **양도 주택 자신의 상속·장기임대 중과배제** — 어댑터가 `isInherited: false`·
   ...
   - ✅ **장기임대(2호) — 완료**(2026-09-22 · 아래 §5.10-C).

⇒ **D-6 후속 3건 전건 종결.** ❌ 미결 재기재 금지.

### 5.10-C 양도 주택 자신의 장기임대 중과배제 — §167의3①2호 (2026-09-22 · D-6 후속 ③)

**착수 전 가설이 법문으로 반증됐다.** 직전 보고는 「9유형 중 양도 주택에 실제로 쓰이는 목만
추려 범위를 좁히자 — 18필드를 복제하면 과복잡해진다」였다. 실독(2026-09-22 · MST 286211) 결과
**좁혀지지 않는다**:

| 목 | 법문 | 함의 |
|---|---|---|
| **사목** | 「등록 말소 이후 1년 이내 **양도하는** 주택」 | 문언 자체가 양도 주택이다 |
| **라목** | 「해당 주택을 **양도하는** 거주자는 … 미분양주택 확인서 사본 … 제출해야 한다」 | 양도를 전제한다 |

⇒ 가~자목 어디에도 「임대주택 **외의** 주택을 양도하는 경우」 제한이 없다. 되레 사목은
**양도 주택 전용 목**에 가깝다(2020.8.18 아파트 임대등록 자동말소분의 정본 경로).

**복제 대신 재사용으로 과복잡을 피했다.** 명부 행의 임대 필드 묶음을 `RentalDeclaration`
(`calc-wizard-asset-nbl.ts`)으로 뽑아 **양도 주택과 공유**하고, 9유형 위젯
`HouseEntryRentalTypeSection`을 그 타입으로 재타이핑해 **같은 컴포넌트를 양쪽이 마운트**한다
(`AssetForm`에 25필드를 붙이지 않았다 — 자리는 기존 `sellingHouseExclusion` 컨테이너).

**세액 실측** (조정지역 · 양도 2026-09-18 · 프로덕션 fallback 세율):

| 시료 | 종전 | 연결 후 |
|---|---|---|
| 3주택, 양도 주택이 등록 장기임대 | 354,541,000 | **141,966,000** (−212,575,000) |
| 2주택, 같은 조건 (§167의10①2호 준용) | 299,816,000 | **141,966,000** (−157,850,000) |
| 사목 — 말소 후 1년 내 양도 | 354,541,000 | **141,966,000** |
| 사목 게이트(5% 미충족 · 2020.8.18 전 말소) | — | 354,541,000 (배제 안 됨) |
| 마목 아파트 2021 등록 | — | 354,541,000 (배제 안 됨) |

주택 수는 3 그대로다 — 2호는 §167의3① 본문 괄호(1호·12호)의 불산입 대상이 아니다(D16).

**⑤ 노출 게이트가 형제와 다르다 — 2채다.** `SellingHouseExclusionSection`은 3주택+인데 2호는
2주택에서도 §167의10①2호가 준용하고 엔진도 `effectiveHouseCount >= 2`에서 판정한다. 3으로
맞췄으면 2주택 사용자가 선언 경로를 잃어 **절반만 해소**됐을 것이다
(`sellingHouseLongTermRentalVisible` · LR-10).

**⚠️ 한 필드가 두 조문 축을 겸한다.** `acquisitionOfficialPrice`를 §167의10①3호(부득이)와 장기임대
나·라목이 함께 쓴다. 속성 순서상 부득이 줄이 spread 뒤에 와서, 부득이 칸이 비면 임대 쪽 값이
`undefined`로 **조용히 덮였다**. 두 칸이 같은 사실을 가리키므로 coalesce로 해소(LR-7·LR-8 ·
[[feedback_one_field_serving_two_legal_axes]]).
...
뮤테이션 **6종 전부 KILL** — 어댑터 하드코딩 복귀(LR-2 외 3건) · 취득기준시가 coalesce 제거(LR-7) ·
아파트 선언 무시(LR-F) · 합산경로 배선 제거(LR-9) · 노출 게이트 2→3(LR-10) · UI 토글 배선 절단(SR-2 외 3건).

⚠️ **역방향 grep으로 셀렉터 충돌을 커밋 전에 확인했다.** 「임대사업자 정식 등록」은 명부 행
모달에도 있으나 `e2e/transfer-multi-house-detail.spec.ts:66`이 `dialog.getByText(…, {exact:true})`로
스코프를 걸어 두어 페이지 레벨인 새 섹션과 부딪히지 않는다. 경고 배너 `data-testid`는
`selling-` 접두사로 분리했고(SR-5), 일반어 「아파트」 칩은 「임대주택이 아파트」로 좁혔다.
