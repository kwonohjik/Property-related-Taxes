...
| Phase | 범위 | 선행 | 규모 |
|---|---|---|---|
| **P0** | 인용 정리: §2.4 오기 수정 · **§89②·§155④·§155⑥·§155⑧·§156의2·§156의3 legal-codes 상수화**(§3.5) · manifest 등록(§5.8) · §155⑥ **직접** anchor 유무 확인 후 필요 시 신설 · 🔴 **근거 문자열 anchor 필수**(P-1 — 안전망 0건) | — | 소 |
| **P1** | **G-5 고가주택 기준 시점 함수** `resolveHighValueHouseThreshold(양도일)` + 판정(`rule.maxExemptPrice`)·안분·하드코딩 4곳 전부 호출 전환 + 다건 route 자산별 양도일 · 입주권 경로 재현 — **확정 결함이라 구조 개편과 무관하게 먼저** | — | 소~중 |
