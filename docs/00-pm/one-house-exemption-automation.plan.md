...
- 🟠 **⑦이 입주권에서 그 값을 표시하지 않는다**(`subject === "apt"` 게이트). 세액을 가르는
  값(실측 11,733,334원)의 산식이 결과 화면에 없다 — ⑤가 보여 주므로 완전한 침묵은 아니지만,
  「계산 메타 폐기」 축이다. **이번 작업 중 발견**했고 엔진 result steps를 바꾸는 별개 축이라
  범위 밖으로 뒀다.
