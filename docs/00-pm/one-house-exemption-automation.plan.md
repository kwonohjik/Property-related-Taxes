...
- 🟠 ③-b `SalePriceTotalPreviewCard`도 `!isRightSubject` 게이트가 없다. 읽기 전용 미리보기라
  중복 편집 문제는 없으나, 입주권에 「분양가액」 미리보기가 맞는지는 **별건**이다(측정 안 함).
...
- 🟠 ③-b `SalePriceTotalPreviewCard`의 `!isRightSubject` 누락(§35.6 이월) — 유일한 잔여 항목.
