...
커밋 `771c11bd`(수정) + `79475dac`(플래그 정리).
...
| 바닥 제거 | 3곳 | 3곳 ✓ |
| 기존 단언 정정 | 9건 | 9건 ✓ |
| anchor | A-0~A-9 (10) | **16건**(A-0~A-10, sub-case 포함) |
| 뮤테이션 | M1~M5 | 전건 실측 ✓ |
| vitest | 세액 회귀 0 | 21,478 passed · 회귀 0 ✓ |

### Q1~Q3 결정

- **Q1 → (A) 제거**. M5로 죽었음을 먼저 실측한 뒤 별도 커밋으로 분리했다.
- **Q2 → 기존 렌더 규약 유지**. 다건 경로가 이미 음수를 렌더하고 있어 신규 규약을 만들지 않았다(`toLocaleString` 기본 음수 표기). E2E가 `/-|−|△|\(/`로 표기 축을 느슨하게 고정한다.
- **Q3 → 넣었다**. `buildRows`의 aggregate 조기반환 **뒤**에 배치해 다건에는 닿지 않는다 — 다건은 이미 합산 중이라 「합산하세요」가 모순이 된다. A-10b가 차익 자산에는 안 붙는 것을 고정해 구별력을 확보했다.

### 계획서가 맞힌 것

**§2-1의 경고가 그대로 적중했다.** 「STEP 2a」 주석이 달린 `transfer-tax.ts:391`만 걷어내면 제보 케이스가 여전히 0이라는 예측은 실측으로 확인됐고, 뮤테이션 M1(①만 복원)이 anchor 7건을 빨갛게 만들어 ①이 진짜 발원지임을 다시 보였다.

**§6-3의 「두 카드가 갈린다」도 적중했다.** 엔진만 고쳤다면 신고서(−20,000,000)와 상세명세서(0)가 어긋난 채 머지됐을 것이다 — 뮤테이션 M4가 이 한 줄의 구별력을 실측으로 보였다.

### 계획서가 빗나간 것

**A-4c(신고서 자기일관성)는 구별력이 0이었다.** 계획서는 이것을 결함 탐지 단언으로 적었으나, 취득가액이 역산되므로 항등식은 **어떤 값에서도 자동 성립**한다 — 결함 상태(100,000,000 + 0 + 0 = 100,000,000)에서도 통과했다. 테스트 주석에 그 사실을 명시하고 ✅(회귀 안전망)로 강등했다. 결함 탐지는 A-4(절대값)가 담당한다.

**A-5·A-6 픽스처를 두 번 고쳤다.** 첫 시도는 양도가액을 구분 기재해 §100③ 가드(`TaxCalculationError`)에 막혔고, 두 번째는 `isSeparateAcquisition`이 없어 `splitDetail`이 생성되지 않아 전체 차손으로 떨어졌다. **anchor가 「엉뚱한 단계」를 보고 있었던 것**이다(memory `feedback_anchor_observes_wrong_stage`).

### 부수 발견 — §164⑧ anchor 주석의 산술 오류

4건이 「분자=분모 → 차익 0」이라 적어 두었으나 참값은 **−4,830,000**(= 취득당시 기준시가 161,000,000 × 3% 필요경비개산공제, 소득세법 시행령 §163⑥)이다. 주석이 개산공제를 빠뜨렸고 **엔진의 0 바닥이 그 오차를 5개월간 가려 왔다**. 상수 `GAIN_WHEN_NUMERATOR_EQUALS_DENOMINATOR`로 근거와 함께 고정했다.

### E2E는 결함 상태에서 red를 확인했다

`e2e/transfer-single-loss-gain.spec.ts` — 바닥 ①을 복원한 채 돌려 **취득가액 단언 줄에서 정확히 실패**함을 확인한 뒤 초록으로 되돌렸다. 통과하는 것만 보고 안전망이라 부르지 않았다.
