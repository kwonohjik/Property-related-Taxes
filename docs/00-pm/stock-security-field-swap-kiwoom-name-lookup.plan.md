...
> 상태: **Do 완료** — 코드 A~E 전건 구현·검증 (2026-08-28). **잔여는 층 1(앱키 재발급)뿐** — 사용자 조치.
>
...
**mock·prod 양쪽 모두 8001로 거부**되므로 「`KIWOOM_ENV` 설정이 잘못됐다」가 아니라 **자격증명 자체가 무효**(만료·폐기·오타)다. `searchStockMaster()` 직접 호출 프로브도 같은 결론:
...
⇒ **코드를 아무리 고쳐도 유효한 앱키 없이는 조회가 되지 않는다.** → Q-1.
