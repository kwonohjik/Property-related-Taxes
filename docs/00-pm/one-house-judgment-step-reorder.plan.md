...
| # | 항목 |
|---|---|
| **F-1** | ✅ **해소(2026-09-25) — `docs/00-pm/validation-warnings-display.plan.md`.** ❌ 미결 재기재 금지. 착수 시 실측하니 **모집단이 3건이 아니라 13건**이었다(판정 3 + 주식 10) — 아래 기술은 판정분만 센 것이다. 두 마법사 모두 공용 `<ValidationWarnings>`로 표시 경로가 생겼다. ↓ 이하 당시 기록 |
