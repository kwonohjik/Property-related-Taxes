...
## 9-D. ✅ PR-2·PR-3 완료 (2026-08-25)

### PR-2 — 적용 불가 시 사유 표시 (D-8) · commit `1b3ab2be`

🔴 **초판 D-8 정정**: 「적용 불가여도 **계산 과정**을 표시」는 **엔진 구조상 불가능**하다.
적용 불가 경로는 전 조문이 `formulaSteps: []`를 반환한다(계산 자체를 하지 않는다 —
`new-99.ts:123`·`new-99-3.ts:393`·`unsold-98-8.ts:137`·`unsold-hybrid.ts:175` 실측).
⇒ 적용 불가에서는 **사유 표시가 정본**이다.

전수 확인 결과 실질 갭은 **§77 하나**였다 — `ReductionDetailCards`가 `?.isEligible` 게이트로
카드를 통째로 숨겨, 감면을 입력했는데 요건 미충족이면 **아무 설명 없이 사라졌다**.
같은 계열 §77의2·§77의3은 게이트가 없고 카드가 `notEligibleReason`을 표시한다(일관성 붕괴).

§69는 `qualifies=false` 분기에서 `breakdown`을 사유로 보여주고 있었다(초판이 `isEligible`만
grep해 「분기 없음」으로 오판 → 정정).

### PR-3 — 3단계 명세 행 값 인라인 (D-1·D-2·D-3·D-4)

| 항목 | 내용 |
|---|---|
| **엔진 echo** | 하이브리드 8조문(`UnsoldHybridResult`)·§99·§98의8에 `transferIncomeApplied`·3시점 기준시가 추가. §99는 재개발 변형 분모용 `previousHouseStdPriceApplied`까지 **4번째 시점** |
| **공용 빌더** | `buildIncomeDeductionReducibleFormula` 신설 — §99의3 전용을 8조문 공용으로. **조문을 분기하지 않고 분자·분모·라벨을 받는다**(§99만 분모가 달라 if-체인으로는 안 된다는 자가검토 #3 반영). `buildNew993ReducibleFormula`는 위임 래퍼로 유지(호출부 무변경) |
| **0인 사유** | `describeZeroReason` — 「5년이 되는 날의 기준시가가 취득 당시보다 낮아 5년간 발생한 양도소득금액이 없다」. **값은 그대로 보이고** 사유를 덧붙인다 |
| **배선** | `DetailedStatementHelpers.ts` — `result.new993Detail` 단독 분기를 `pickIncomeDeductionFormulaSource(result)`로 교체(§127⑦로 적용은 1건이므로 첫 detail 선택, 라벨 배열과 순서 공유) |

제보 케이스 결과 — 종전 값 없는 상수 문구가 다음으로 바뀐다:

```
양도소득금액 11,210,000 × (5년시점 기준시가 112,969,780 − 취득시 121,191,049 = -8,221,269)
                        ÷ (양도시 기준시가 126,887,420 − 취득시 121,191,049 = 5,696,371)
= 0 (§99의2 — 5년이 되는 날의 기준시가가 취득 당시 기준시가보다 낮아 5년간 발생한 양도소득금액이 없다)
```

### 안전망 — mutation으로 사각지대 재확인

PR-1에서 드러난 것과 **같은 사각지대**가 PR-3에도 있었다: 빌더 단위 anchor는 source를
fixture로 주입하므로 엔진 echo 주입을 지키지 못한다. 엔진 echo를 제거하는 mutation에서
**빌더 anchor 7건은 전건 통과**했고 엔진 anchor만 실패했다 ⇒ 두 층 모두 필요하다.

| anchor | 건수 | mutation |
|---|---|---|
| 빌더 산식 | 7 | (엔진 echo 제거에 **무반응** — 설계상 정상) |
| 엔진 echo 배선 | 2 | echo 제거 → **1건 실패** ✅ |

### 검증

- `npx tsc --noEmit` 0건
- `npm run test:transfer` — **636파일 6976건 전건 통과** (세액 무변동)
- legacy dead code 37줄 제거(공용 빌더로 대체된 §99의3 전용 구현)
