...
- 결과: vitest **17,331건 통과 · 실패 0** · 주식 E2E **9건 통과**(신규 A-2 포함) · typecheck/lint 0 error
- 잔여: **없음** — A·B·C·E-1·E-2·E-3·E-4 전건 완료 (Q-4 결정으로 E-2 보류 해제)
...
### Q-4 ✅ 결정 완료 (2026-08-27) — 「공휴일·납회 포함」으로 확대

**질문**: 비거래일 양도일의 anchor 시프트를 「주말만」에서 「공휴일·납회 포함」으로 넓힐 것인가?

**결정**: 넓힌다. 사용자 지시(2026-08-27).

**근거**: 상증법 §63①1가목 괄호 「평가기준일이 **공휴일 등** 매매가 없는 날인 경우에는 **그 전일**을
기준으로 한다」 + 상증령 §52의2④(공휴일·대체공휴일·토요일). 소득세법 §99①3이 이 가목을 준용한다.

**구현 — 세 번째 복제 대신 재사용**:
같은 조문에서 나온 상증세 평가용 `resolveValuationAnchor`(`lib/kiwoom/calendar.ts:90-112`)가
**이미** 납회 jump + `isKrxTradingDay` 역탐색 + 30 iteration 가드를 갖추고 있었다.
두 세목이 갈려 있던 것을 그 헬퍼로 합쳤다 — `buildOneMonthBeforeSlots`와
`resolvePreTransferAnchor` 둘 다 위임한다.

**갱신한 기존 anchor 1건**: `__tests__/kiwoom/calendar.test.ts` K-LEAP-01 —
2024-03-01(삼일절)이 anchor 그대로였던 것을 직전 거래일 2024-02-29로.
윈도우도 [2024-02-02 ~ 2024-03-01] 29일 → **[2024-01-30 ~ 2024-02-29] 31일**로 바뀐다.
정정 사유를 테스트 주석에 조문과 함께 남겼다. 대조군 K-LEAP-02(2024-02-29 거래일 양도) 신설.

**픽스처 범위 밖**은 여전히 시프트하지 않는다 — `isKrxTradingDay`가 2020~2026 밖 평일을
거래일로 보기 때문이다(A-9가 이 경계를 고정). 「모르는 것을 아는 척하지 않는다」.
