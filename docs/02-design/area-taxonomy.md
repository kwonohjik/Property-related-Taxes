...
#### 예외 — 시점 축이 하나뿐인 필드는 시점어를 생략한다 (2026-08-12)

**취득·양도 양쪽에 같은 값이 쓰이는 단일 필드**는 `[기준 시점]`을 라벨에서 뺀다.
`취득·양도 당시 토지 면적` → **`토지 면적`**(주택·건물·재개발·일반건물 4곳).

시점어는 **구별할 시점이 둘 이상일 때만 정보**다. 한 필드가 두 시점 모두의 곱셈 인자이면
`취득·양도 당시`는 구별하는 바가 없으면서 라벨만 길게 만들고, 같은 줄의
`건물 연면적`·`건축물 바닥면적`과 나란히 놓일 때 3열 그리드의 균형을 깬다(사용자 지적).

⚠️ 시점이 실제로 갈리는 필드(`acquisitionArea` vs `transferArea`)에는 이 예외가 적용되지 않는다 —
거기서는 시점어가 **유일한 구별 수단**이다.
