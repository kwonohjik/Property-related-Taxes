...
배제가 성립하면 **토지·건물 두 파트 모두** 시나리오 B로 간다.

🔴 **구현 중 드러난 함정** — legacy 경로는 건물 파트에 `asset.buildingCarryover ?? c`를 넘긴다. 건물 전용 폼에는 관계·사망 입력이 **없으므로** 그대로 두면 건물만 기본값(미선택·false)이 되어 **토지만 배제된다**. ⇒ `buildEngineShaped(part, relationSource)`로 **관계 정본을 별도 인자**로 고정하고 항상 토지 쪽 `c`를 넘긴다. 회귀 방어는 DD-W-05.

UI에서도 라디오 `name`을 `assetId`로 고유화해야 한다 — 자산이 여럿이면 name이 겹쳐 **하나만 선택된다**.
...
**직계존비속에는 부정확하다**(혼인관계가 없다). 그리고 「단서」라고 부르는 것도 틀렸다 — **괄호**다.

#### 구현 결정 (2026-08-10 — 초안 수정)

초안은 **관계별 2문구**로 갈랐다. 그러려면 `CarryoverTaxationDetail`에 `donorRelation`을 실어야 하는데, 결과 타입·엔진 반환·표시까지 배선이 늘어나는 데 비해 사용자가 얻는 정보가 없다 — 관계는 **입력할 때 그 문언으로 이미 물었다**.

⇒ **관계 무관 단일 문구**로 간다:

> 「§97조의2 ① — 증여자 사망으로 이월과세 대상에서 제외」

배우자 사별도 「증여자 사망」이 맞고 직계존비속도 맞다. 결과 타입 변경 0.
...
| 관계 필수화(⑧)가 **기존 입력을 차단**한다 | 🔑 **관계를 단독 필수화하지 않는다**(구현 결정). 「이월과세를 새로 여는 경우」를 코드로 구분할 방법이 없어 초안대로 하면 구형 sessionStorage 입력이 전부 막힌다. ⇒ **사망을 선언했을 때만** 관계를 요구한다 — 사망 미선언이면 관계는 판정에 영향이 없다(`isCarryoverRelationExcluded`). 회귀 방어는 DD-W-09 |
