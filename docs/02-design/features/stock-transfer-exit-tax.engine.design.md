...
| 소득세법 §118의12 | 조정공제 | 실양도가액 < 출국일 시가 시: **[출국일 양도가 − 실양도가] × §118의11에 따른 세율**. §118의11의 세율은 누진표이므로 「출국일 기준 산출세액 − 실양도가 기준 산출세액」으로 계산한다 (구 기재 「산출세액 × 차액 / 양도차익」은 법문에 없는 실효세율 — 2026-08-28 정정) |
...
STEP 7: 경정청구 계산 (납부유예 후 실양도 시 — exit-tax-deferred.ts)
  조정공제 (§118의12):
    if actualTransferPricePerShare < departureDayPricePerShare:
      priceDrop = departureDayValue − actualTransferValue (전체 종목 합산)
      adjustmentDeduction = incomeTax − applyExitTaxRate(max(0, taxBase − priceDrop))
      # 「차액 × §118의11 세율」 — §118의11은 누진표라 차액이 얹힌 위치에 적용해야 한다.
      # 취득가·기본공제가 두 계산에서 상쇄되므로 과세표준 차 = 양도가액 차와 같다.
      # ⚠️ 「차액 × 단일 한계세율」 축약 금지 — 구간을 걸치면 아랫부분에 20%가 적용된다.
  외국납부세액공제 (§118의13):
...
| R3-06 | CRITICAL 정정 | ET-anchor-02 조정공제액 확정: 244,791,666 → **2026-08-28 폐기**(리뷰 #15 — §118의12①은 「차액 × §118의11 세율」. 정본 250,000,000) | §9.2 |
