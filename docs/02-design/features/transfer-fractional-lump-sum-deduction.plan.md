...
| # | 지적 | 조치 |
|---|---|---|
| 1 | **인용 오기** — "§166⑥이 토지·건물을 구분해 각각 산출하도록 규정"은 사실과 다름. §166⑥ 원문은 "가액의 **구분이 불분명한 때**"의 안분방법만 규정. "각각 구분하여 기장" 원칙의 근거는 **본법 §100②** | ✅ **정정 완료**(법제처 원문 직접 재확인). 코드 2 · 테스트 2 · 설계문서 1. 결론(성분별 독립)은 불변이며 근거는 오히려 강해짐 |
| 2 | **사이드바 floor 순서 불일치 실재** — `calc-wizard-store.ts:485-486,503-506`이 여전히 `floor(floor(std×rate)×ratio)`(rate 먼저). 엔진 순서 A와 0.96%에서 1원 차 | 📌 **P4 예정 항목**(누락 아님). P4에서 `computeEstimatedDeduction` 경유로 통일 |
| 3 | **P3c 미착지 4경로 ↔ UI 순서 의존성** — 다필지·재개발 2경로·상가 §164⑥·일반건물은 타입 도달만 완료, 산출식 미참조. 회귀는 아니나(이전에도 지분 미반영) **UI가 이 asset kind에 지분 입력을 노출하기 전에 P3c가 반드시 선행**해야 함 | 📌 **P3c·P5 전제조건으로 명시**. UI 시니어 인계 시 순서 의존성 전달 |
| 4 | **§104③ 미등기 0.3% 미적용** — split·PHD·겸용·상가·재개발 경로가 3% 고정. **pre-existing**(이번 PR은 rate 인자 무변경, ratio만 추가) | 📌 §10 범위 밖에 이미 기록됨. **별도 티켓 필요** |

**미검증(리뷰 명시)**: §163⑥ 성분별 독립 floor를 직접 다룬 재결례는 조회에서 **찾지 못함**.
조문 구조(1호·2호가목 별개 호 열거) + §100② + Excel 정본 anchor로 판정했다.
...
## 12. 확정된 결정 (2026-07-28 사용자 확인)

| # | 항목 | 결정 |
|---|---|---|
| 1 | **floor 순서**(§6.4) | **A 채택** — `floor(floor(std × 지분) × 3%)`. 지분 기준시가를 result에 echo해 표시 산식을 정합시키는 것이 우선. 단일 floor(C) 대비 0.96%에서 1원 작은 점은 이 엔진 전체의 정수 절사 규약(`applyRatio` = `Math.floor`)과 일관하므로 수용 |
| 2 | **부담부증여**(§4.2) | **별건 분리 확정** — `burdened-gift-apportionment.ts:171` 범위 밖. 경로 전체가 지분 미인지이므로 여기만 고치면 혼합 스케일 결함을 신설한다 |
| 3 | ⑦ 표시 9+ 지점 | **별도 PR**(판단) — `lumpSumDeductionBase` echo가 P2에서 나와야 표시를 고칠 수 있어 의존 순서상 P4를 분리하는 것이 자연스럽다 |
