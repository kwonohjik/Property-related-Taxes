...
- **R-3 확정 (2026-06-11)**: §97의4 추가율 표 = 사용자(세무 전문가) 제공값 = 코드 기재값(`RENTAL_97_4_ADDITIONAL_RATE_TABLE`: 6년 2%/7년 4%/8년 6%/9년 8%/10년 10%) **일치** → `isFullyImplemented=true`, `Rental974InputForm` 폼·렌더분기 추가. F-3(§97의3②)는 rental 라디오 단일 선택으로 자연 배제. ⚠️ 본문 내장 표 API 누락(별표 `NOT_FOUND`)으로 KoreanLaw 직접인용 추후 보강.
...
| 15 | §97의4 — 6~7년 2%·…·10년+ 10% 추가율 (R-3 확정) | §97의4 ✅ R-3 | 법정 표 직접 | `rental-97-evaluators.test.ts`(케이스 #15~17) | ☑ |
| 16 | §97의4 — 일반공제율 + 추가율 가산 (통합) | §97의4 ✅ R-3 | 상대 검증 | `rental-97-4-integration.test.ts` | ☑ |
...
| 23 | 중복배제 — §97의3 + §97의4 동시 → rental 라디오 단일 선택 자연 배제 (F-3 ✅, `toggleGroupRadio`) | §97의3② ✅ | UI 자연배제 | UI(`UnifiedReductionPanel`) | ☑ |
