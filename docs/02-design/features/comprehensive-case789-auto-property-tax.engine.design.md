...
★ R-9 확인 필요(설계 검토 중 KoreanLaw 재확인): 지방세법 시행령 §118 "세부담의 상한" —
  "직전 연도의 해당 재산에 대한 재산세액 상당액"이 감면전 표준세율인지 감면후 실부과액인지.
  동일 감면율 가정 시 결과 동일(아래 알고리즘 §B 대수 증명) — 본 설계는 감면전 표준세율 기준.
```
...
- ★ **cap 기준 = 감면전 표준세율, 감면은 cap 후곱**. 동일 감면율 g 가정 시
  ...
  직전≠당해 감면율 시 결과 상이 → 직전 감면율 별도 입력 안 함(당해 `reductionRate` 재사용 가정). R-9 §118 본문 확인 후 확정.
