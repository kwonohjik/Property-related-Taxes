...
- ✅ **해소(상속·증여 후속 완료, 2026-07-14)**: **상속·증여는 버그 없음 — 이미 전체 PNU 보존.** `EstateBodyRealEstate`의 주소 onChange → `buildAddressPatch(v)`(`EstateBodyHelpers.ts:63`)가 `estateAddress.pnu: v.pnu || undefined`로 **전체 19자리 PNU 보존** → `addrValue.pnu`(`:146` 복원) → 모달 `initialAddress={addrValue}`(`EstateBodySupplementaryValuation:235`) → 버튼 활성. 양도세 자산(PNU를 `slice(0,10)`로 폐기)과 달리 상속은 애초에 보존. **정정 불요**, 회귀 가드 anchor(`inheritance-estate-address-pnu-preserve.test.ts`) 추가로 보존 잠금.
