...
- 상태: ✅ **P1~P4 구현 완료(2026-08-12)** · 구현 중 **O-2 발견·수정** · 🟡 **O-3 미해결**(§9)
...
### 9.2 🟡 O-3 — O-2를 고친 뒤에도 결과 화면이 뜨지 않는다 (미해결·별건)

O-2 수정 후 4자산 모두 **차단 메시지가 사라졌고 `/api/calc/transfer`가 200**을 반환한다.
그런데 결과 화면(「양도소득세 계산 결과」)으로 전환되지 않았다(20초 대기).

- 기존 부담부증여 계산 E2E(`transfer-burdened-gift-fractional.spec.ts` 등)는 **통과**한다
  ⇒ 어떤 조건에서는 정상 렌더된다.
- ⇒ probe seed가 불완전했을 가능성이 크다(자산별 필수 필드 누락 등). **결함이라고 단정하지 않는다.**

**다음 조사**: 통과하는 기존 spec의 seed와 내 probe seed를 diff해 무엇이 다른지 좁힐 것.
