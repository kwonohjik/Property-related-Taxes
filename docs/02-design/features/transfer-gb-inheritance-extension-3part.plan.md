...
### 10-6. ✅ 부담부증여 × 이월과세(증여) — 차단 (2026-08-08)

취득원인 라디오의 **「이월과세(증여)」**(`carryover_gift`)는 다른 셋과 성격이 다르다 — 칸 노출 문제가 아니라 **성립 여부 자체가 미판정**이다.

#### 법령 조사 — 법문상으로는 **연결된다**

「소득세법 시행령」 제159조 제1항 제1호의 **A**는 「법 **제97조제1항제1호**에 따른 가액」이다(verbatim 확인). 그리고 「소득세법」 제97조의2 제1항 제1호는 「취득가액은 … 배우자 또는 직계존비속이 해당 자산을 취득할 당시의 **제97조제1항제1호에 따른 금액**으로 한다」 — **같은 금액을 치환한다**. 부담부증여의 「양도로 보는 부분」도 §88①1호 후단에 따른 **양도**이므로 §97의2①의 「양도하는 경우」에 해당할 여지가 있다.

⇒ 「§159가 있으니 §97의2는 배제된다」는 **자동으로 성립하지 않는다**. 두 조문은 A를 통해 이어진다.

> ⚠️ `burdened-gift-apportionment.ts:12`의 「양도자 = 증여자 본인이므로 §97의2 미적용」은 **이번 부담부증여가 이월과세를 촉발하지 않는다**(수증자가 나중에 팔 때의 이야기)는 뜻이지, 증여자의 **선행 취득**에 §97의2가 붙지 않는다는 판정이 아니다. 두 문장을 같은 것으로 읽지 말 것.

#### 🛑 그래서 세액은 바꾸지 않았다

국세청 해석 **2건**이 존재한다 — 「부담부증여로 취득한 자산 중 양도로 보는 부분의 배우자 이월과세 적용여부」(2006-10-30 · 2006-11-02). 그러나 **본문을 확인하지 못했다**: 법제처 OPEN API는 `ntsCgmExpc` 목록만 제공하고 본문 조회를 지원하지 않으며, `taxlaw.nts.go.kr` 상세 페이지는 JS 렌더라 fetch로 읽히지 않는다. 조세심판례·판례는 0건이다.

⇒ 근거 미확인 상태에서 **세액을 바꾸지 않는다**(memory `feedback_unverified_authority_blocks_tax_change`). 적용하는 쪽이 일반적으로 **불리**하므로(§97의2②3호가 세액이 큰 쪽을 채택한다) 명문 확인 전까지 현행(미적용)이 안전측이다(`feedback_no_unfavorable_application_without_legal_basis`).

#### 현행 구현 실측 — 「요구하는데 무시」

두 스텝이 **함께 돈다**. STEP 0.475(이월과세)가 `workingInput`을 시나리오 결과로 바꾼 뒤, STEP 0.48(부담부증여)이 `transferPrice`·`acquisitionPrice`·`expenses`를 §159 안분값으로 **덮어쓴다**.

| | 증여자 취득가액 1억 | 7억 |
|---|---|---|
| 이월과세 **단독**(대조군) | 204,930,000 | 93,110,000 |
| **부담부증여를 얹으면** | **71,260,000** | **71,260,000** |

부담부증여 단독도 71,260,000 — **완전히 같다**. 그런데 ⑧ validate는 그 무시되는 값을 **요구**했다(carryover 블록 (c) 증여 당시 평가액 · (d) 증여자 취득가액).

> 🔑 **대조군이 없었으면 오진했다.** probe 첫 시도는 증여일을 2020으로 잡았는데 그건 2023.1.1. 전 증여라 **5년 룰**이 적용돼 애초에 이월과세 대상이 아니었다 — 「무반응」이 픽스처 탓이었다. 대조군을 세워 반응(204,930,000 ↔ 93,110,000)을 확인하고서야 CO-2의 무반응이 의미를 가졌다. anchor에 CO-1로 **영구 편입**했다.

#### 차단이 손해가 없는 이유

취득원인을 무엇으로 골라도 §159가 취득가액을 정하므로 **세액이 같다** — 매매(미선택)·증여·상속·이월과세 **네 경우 모두 71,260,000**. ⇒ 「증여로 선택하세요」 안내가 가리키는 곳에서 사용자는 **정확히 같은 답**을 얻는다.

| 층 | 파일 | 변경 |
|---|---|---|
| ⑧ validate | `transfer-tax-validate-bg.ts` | 부담부증여 × `carryover_gift` 차단 + 「증여」 안내 (자산종류 분기보다 **앞**에서 도는 모듈이라 전 자산종류 커버) |
| anchor | `burdened-gift-carryover-block.anchor.test.ts` (6건) | CO-1 대조군 · CO-2 무반응 · 취득원인 4종 동일 · CO-3 차단/통과/회귀 |
| E2E | `transfer-burdened-gift-carryover-block.spec.ts` (3건) | 차단 문구 · 「증여」 목적지 도달 · **일반 양도 대조군** |

> ⚠️ **이 차단은 「§97의2 미적용」 판정이 아니다.** 적용해야 한다는 근거가 확인되면 **엔진을 먼저 고치고**(STEP 0.48이 0.475를 덮지 않도록) 차단과 anchor를 함께 손본다. 그 순서를 뒤집으면 검증 없는 세액 변경이 된다.
