...
> **상태**: ✅ **구현 완료** (PR#731·#733·#734 · 2026-07-21) — 2026-08-04 코드 실측 · 2026-08-05 인용 PR·커밋 재검증(종전 헤더는 stale이었음).
> ~~종전 표기: Plan (Do 미착수)~~
...
- **🛑 Do 조사 결과 = 보류 확정(2026-07-21, 사용자 결정)**: #2② Do 착수 시 실측으로 **계획서 범위보다 큰 systemic 갭** 발견 → 사용자가 "정본 앵커 확보 후 보류" 선택. 재개 조건 아래.
  - **실측 사실**: 겸용 `buildTotalTax`는 `calculateProgressiveTax`(누진)만 적용, route handler(`route.ts:678-714`)도 사후 단기 보정 없음 → **겸용은 housing·commercial·nonBiz 어디에도 단기세율 미적용**(단건 rate-calc 미경유). 즉 단기보유 겸용은 주택분(§104①2·3호 70%/60%)·상가분(50%/40%)·nonBiz(§104후단)이 **전부** 누락.
  - **nonBiz-only의 결함**: nonBiz 토지가 단기이려면 전체 물건이 단기 → 주택·상가분도 동시 단기 미처리. nonBiz만 고쳐도 **어떤 실제 시나리오도 정확해지지 않음**(주택·상가분 여전히 과소).
  - **정본 부재(핵심 게이트)**: 겸용은 3부분 **합산 누진**(aggregate `taxBase`)이라 §104후단 비교의 nonBiz 과세표준 기준이 **합산 한계세율 귀속분 vs standalone 누진** 중 무엇인지 불확정 + **§104⑤ 비교과세**(합산누진 vs 자산별 max)까지 얽힘. 코드베이스·교재 앵커 **전무** → 추정 구현 금지(no-guess·anchor-first).
  - **재개 조건**: 교재/NTS **겸용 단기보유 예제**(3부분 세율 + §104⑤ 비교과세 산식) 확보. 확보 시 nonBiz-only(scoped)가 아니라 **full 겸용 단기+비교과세**로 진행(부분 수정은 무의미). 도달성 매우 낮음(단기보유 겸용 + nonBiz 초과토지 동시).
