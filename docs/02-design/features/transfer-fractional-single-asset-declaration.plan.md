...
### 3.8 근거 확보 — 두 설계 중 하나가 **법문으로 갈렸다**

종전 판단은 「두 설계가 모두 성립하므로 근거를 얻기 전까지 차단」이었다.
2026-09-03 1차 자료를 읽어 **필드별로** 갈렸다.

#### 「소득세법 시행령」 §166 (KoreanLaw 실측 · mst=286211)

- **①1호**: `양도가액 − (기존건물과 그 부수토지의 **평가액** + **납부한** 청산금) − 필요경비`
- **④1호**: 「기존건물과 그 부수토지의 평가액」 = **「관리처분계획등에 따라 정하여진 가격」**

#### 「도시 및 주거환경정비법」 §39①1호 (KoreanLaw 실측)

> 토지 또는 건축물의 소유권과 지상권이 **여러 명의 공유에 속하는 때** … 그 여러 명을
> **대표하는 1명을 조합원으로 본다**

⇒ 두 조문이 결합해 **필드별로 결론이 갈린다**:

| 필드 | 판정 | 근거 |
|---|---|---|
| **평가액(권리가액)** | **스케일 O** | §166④1호가 「계획에 **정하여진** 가격」이라 하고, §39①1호로 계획은 **물건(대표조합원) 단위**로 가격을 정한다 ⇒ 공유자별 몫은 **지분율로 귀속** |
| **청산금** | 🔴 **스케일 X** | §166①1호가 「**납부한** 청산금」이라 **사실**을 지목한다. 조합은 대표 1인에게 부과하고 공유자 간 분담은 **내부 약정** ⇒ 지분율로 파생되지 않는다 |
| 인가전·인가후 필요경비 | 스케일 O | §97①2·3호. 화면 규약 「필요경비는 100% 기준 입력」 |
| 기준시가·면적 | 스케일 X | §166③ 환산에서 분자·분모로 함께 나타나 **약분**된다. 줄이면 이중 축소 |

**청산금은 부담부증여 인수채무와 정확히 같은 구조다** — 「계약(약정)으로 정해지는 금액이지
지분율로 파생되는 값이 아니다」(`burdened-gift-valuation.ts` 「스케일하지 않는 것」).
엔진이 ×지분율로 쪼개면 **자동 안분 fallback**이 되어 정책 위반이다.

⇒ 종전에 「(가)냐 (나)냐」로 놓았던 것이 **틀린 이분법**이었다. 정답은 **혼합**이다.

> ⚠️ 기준시가 스케일 X는 **§166⑦(기준시가 과세)이 미구현**이라는 전제 위에 선다(실측 확인).
> ⑦은 기준시가를 **절대값으로 차감**해 약분되지 않으므로, 구현하게 되면 이 판단을 **다시 해야 한다**.

### 3.9 구현 — 차단 해제

`buildRedevelopmentPayload(asset, ownershipRatio)`가 절대금액 성분만 `share()`로 감싼다.
UI는 지분 모드에서 청산금 라벨을 「(지분 해당분)」으로 바꾸고 안내를 붙인다 —
판정 술어는 ④와 **같은 소스**(`isFractionalOwnership`)다.

**정합 실측 (P: 지분율 40% + 물건 전체 · Q: 100% + 지분분 직접입력, 청산금은 양쪽 지분 해당분)**:

| 자산종류 × 변형 | P | Q | |
|---|---|---|---|
| 재개발APT × {원조합원 납부·수령, 환산(§166③), 승계조합원} | — | — | ✅ 4/4 |
| 입주권 × 동일 4변형 | — | — | ✅ 4/4 |

수정 전 170,000,000 → **302,000,000**(= Q와 일치). Gate-A 차단은 **해제**했다.

> 🔑 **교훈**: 「두 설계가 모두 성립한다」는 판단은 **1차 자료를 읽기 전**의 것이었다.
> §166①1호의 「납부한」과 §39①1호의 「대표하는 1명」을 읽자 **필드별로** 갈렸다.
> 「근거가 없다」가 아니라 **「아직 안 읽었다」**였다(`feedback_no_statute_claim_needs_requirement_article`).
...
| **5** | 재개발·입주권 스케일 정합 재조사 → 결함 확인 | ✅ L 6건 · §3.6~3.7 |
| **6** | §166·도시정비법 1차 자료 → **필드별 스케일 구현** | ✅ RV 10건 · E2E 2건 (§3.8~3.9) |
...
| `lib/calc/transfer-tax-api-redev.ts` | ④ | §166 필드별 지분 스케일 (§3.8~3.9) |
| `components/calc/transfer/RedevelopmentBlock.tsx` | ⑤ | 청산금 「(지분 해당분)」 라벨·안내 + 권리가액 100% 안내 |
...
| M6 `rightsValue` 스케일 제거 | RV 7건 |
| M7 **청산금을 (잘못) 스케일** | RV 7건 ← 「스케일 X」 결정을 능동적으로 지킨다 |
| M8 필요경비 스케일 제거 | RV 9건 |
