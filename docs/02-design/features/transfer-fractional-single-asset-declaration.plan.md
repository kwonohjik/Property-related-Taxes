...
### 3.6 [실측 · Phase 0b] 자산종류 매트릭스 — 스케일 정합

단건 지분 40% + 물건 전체 금액 **vs** 100/100 + 지분분 금액 (비-1세대1주택 = 문턱 효과 제거).
두 경로의 `transferGain`·`totalTax`가 같으면 지분 스케일이 정합한다는 뜻이다.

| assetKind | 결과 |
|---|---|
| housing · land · building · commercial_building · general_building · presale_right | **✅ 6종 전부 일치** (gain 240,000,000 / tax 47,245,000·156,750,000) |
| redevelopment_apt · right_to_move_in | ⚠️ **미검증** — probe 픽스처에 `redevelopment.approvalDate`가 없어 400. 결함 관측이 아니라 **픽스처 한계**다 |

> 🟠 **남은 확인**: 재개발·입주권 2종은 축 A 스케일 정합이 아직 실측되지 않았다.
> Gate-A는 열렸으므로 계산 자체는 가능하다. 별건으로 픽스처를 갖춰 확인할 것.
