...
> **라운딩 결정 = (A-2) 확정 (Task #10 anchor 실측 재잠금 2026-05-11)**: 우리 엔진 실측값(토지 흡수 24,190,751 / 건물2 흡수 2,336,344)을 anchor로 잠금. 양도코리아 PDF(24,190,750 / 2,336,344)와 토지 1원 차이는 자동계산기 간 잔액 흡수 순서 차이로 정상. **UI 결과 카드는 우리 엔진 값만 표시.** 통산 후 합계 53,503,968, 산출세액 6,480,952·지방세 648,095는 양도코리아와 1원 단위 완전 일치. **(A-2) 결정으로 우리 엔진 값을 anchor로 잠금**
...
- **라운딩 결정 = (A-2) 확정 (Task #10 anchor 실측 재잠금 2026-05-11)**: 엔진 실측값(24,190,751 / 2,336,344)을 anchor로 잠금. 양도코리아 PDF(24,190,750 / 2,336,344)와 토지 1원 차이는 엔진 §1.4에 명시된 잔액 흡수 순서 차이. UI는 우리 엔진 값만 표시. 산출세액 6,480,952·지방세 648,095는 1원 단위 완전 일치. 코드 변경 0.
...
#    ※ 양도코리아 PDF(24,190,750 / 2,336,344)와 토지 1원 차이 — 정상(§1.4 확정)
...
- [ ] 브라우저 결과 카드: 통산 흡수 토지 24,190,751 / 건물2 2,336,344 표시(우리 엔진 값 — Task #10 실측) 확인. 양도코리아 PDF(24,190,750 / 2,336,344)와 토지 1원 차이 정상.
...
1. **§9.2 통산 흡수액** — Task #10 이후 §9.3에 우리 엔진 실측값(토지 흡수 24,190,751 / 건물2 흡수 2,336,344) + 라운딩 주석으로 재잠금 완료. 양도코리아 PDF(24,190,750 / 2,336,344)와 토지 1원 차이는 설계 문서 §1.4에 명시된 잔액 흡수 순서 차이. 별도 수정 불필요.
