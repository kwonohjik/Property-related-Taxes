...
| 필요경비 | 6,595,233 | 1,404,767 | 1,218,126 (개산공제) | 9,218,126 |
| 양도차익 | 104,260,596 | -26,527,094 | 10,069,492 | 87,802,994 |
| 장기보유특별공제 | 31,278,178 | 0 | 3,020,847 | 34,299,025 |
| 양도소득금액 (income) | 72,982,418 | -26,527,094 | 7,048,645 | 53,503,969 |
| 영 §102② 통산 흡수 | **-24,190,748** | +26,527,094 (결손) | **-2,336,346** | 0 |
| **통산 후 양도소득금액** | **48,791,670** | **0** | **4,712,299** | **53,503,969** |

> **라운딩 결정 = (A) 확정 (2026-05-11 사용자 결정)**: 우리 엔진 결과(토지 흡수 24,190,748 / 건물2 흡수 2,336,346)를 anchor로 잠금. 양도코리아 PDF(24,190,750 / 2,336,344)와 ±2원 차이는 자동계산기 간 라운딩 방식 차이로 정상. **UI 결과 카드는 우리 엔진 값만 표시.** 통산 후 합계(53,503,969)와 산출세액(6,480,952)·지방세(648,095)는 1원 단위 완전 일치. **(A) 결정으로 우리 엔진 값을 anchor로 잠금**

> **통산 전후 시각적 보조 (정정 #7)**: 양도소득금액 합계(53,503,969)는 통산 전후 동일하지만, **자산별 분포가 변경**됩니다. 건물1 결손 -26,527,094가 토지·건물2 양수에 흡수되어 세율 적용 기준 양도소득금액이 자산 단위로 재분배됩니다 (영 §102②). 통산 전: 토지 72,982,418 · 건물1 -26,527,094 · 건물2 7,048,645 → 통산 후: 토지 48,791,670 · 건물1 0 · 건물2 4,712,299. 이 안내 단락은 결과 카드 "통산 후 양도소득금액" 행 아래에 작은 회색 텍스트(`text-xs text-muted-foreground`)로 표시하여 합계가 같아 보여도 통산이 필요한 이유를 사용자가 파악할 수 있도록 한다.
...
**영 §102② 통산 산식**:
```
흡수 풀 = min(양수 income 합계, 결손 income 절대값) = min(80,031,063, 26,527,094) = 26,527,094
토지 흡수 = 26,527,094 × (토지 income 72,982,418 / 양수 합계 80,031,063) → 우리 엔진: 24,190,748
건물2 흡수 = 26,527,094 × (건물2 income 7,048,645 / 양수 합계 80,031,063) → 우리 엔진: 2,336,346
```
- "원" 단위 접미 금지 (`feedback_no_won_suffix.md`)
- 법조문 링크: LawArticleModal "소득세법 시행령 §102②"
- **라운딩 결정 = (A) 확정 (2026-05-11 사용자 결정)**: 엔진 결과(24,190,748 / 2,336,346)를 anchor로 잠금. 양도코리아 PDF(24,190,750 / 2,336,344)와 ±2원 차이는 엔진 설계 문서 §1.4에 명시됨. UI는 우리 엔진 값만 표시. 산출세액 6,480,952·지방세 648,095는 1원 단위 완전 일치. 코드 변경 0.

### 9.4 결과 카드 최종 세액 라인

| 항목 | 표시 |
|---|---|
| 통산 후 양도소득금액 | 53,503,969 |
| 기본공제 | -2,500,000 |
| 과세표준 | 51,003,969 |
...
# → 통산 흡수 확인: 토지 24,190,748 / 건물2 2,336,346 (우리 엔진 값)
#    ※ 양도코리아 PDF(24,190,750 / 2,336,344)와 ±2원 차이 — 정상(정정 #3 §1.4 확정)
...
- [ ] anchor 25개 `toBe()` 원단위 정확 통과 — **(A) 결정에 따라 33-18=24,190,748(흡수 합 26,527,094) · 33-19=48,791,670 · 33-20=4,712,299 · 33-21=0 적용** (2026-05-11 확정)
...
- [ ] 브라우저 결과 카드: 통산 흡수 24,190,748 / 2,336,346 표시(양도코리아 PDF 24,190,750 / 2,336,344 아님) 확인 (정정 #3)
...
1. **§9.2 통산 흡수액** — 이미 파트2 §9.3에 우리 엔진 값(24,190,748 / 2,336,346) + 라운딩 주석으로 정정 #3 적용 완료. 양도코리아 PDF(24,190,750 / 2,336,344)와의 ±2원 차이는 설계 문서 §1.4에 명시된 엔진 비율 안분 라운딩 순서 차이. 별도 수정 불필요.
