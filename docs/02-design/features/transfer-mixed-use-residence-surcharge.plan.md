# 겸용주택 — 영 §154① 거주요건 + 법 §104⑦ 다주택 중과 — v1.7
...
### 5.9-R Phase B3 구현 결과 ✅ (2026-07-31)

**§104⑦ 세율 가산 + 후단 MAX. 계획 단계 손계산이 원 단위까지 맞았다.**

| 케이스 | 목표(§3 E-2 손계산) | 엔진 실측 |
|---|---:|---:|
| 2주택 +20%p | 1,137,370,975 | **1,137,370,975** ✅ |
| 3주택+ +30%p | 1,288,352,475 | **1,288,352,475** ✅ |

1호(B2에서 889,767,350 확인)·2호 모두 손계산과 일치 — §104⑤ 구현이 법문대로다.

**Phase 누적 효과** (CASE14 · 다주택 · 조정지역 · 2026-06-01):

| 단계 | 산출세액 | 증분 |
|---|---:|---:|
| B1 이전 | 685,604,826 | — |
| B2 (§95② 장특 배제) | 889,767,350 | +204,162,524 |
| **B3 (§104⑦ 2주택)** | **1,137,370,975** | **+247,603,625** |
| B3 (3주택+) | 1,288,352,475 | +398,585,125 |

**§104⑦ 후단 MAX — 양방향 실측**:

| 보유 | 중과 한계 | 단기 | 채택 | 표시율 |
|---|---|---|---|---|
| 1년 | 45%+20%p = 65% | **70%** | 단기(§104①3호) | 0.70 |
| 장기 | **45%+20%p** | 없음 | 중과 | 0.65 |
| 장기·3주택+ | **45%+30%p** | 없음 | 중과 | 0.75 |

B-B17/B-B17b가 **양쪽 다 고를 수 있음**을 반증 쌍으로 고정한다.

**구현**:
- `MixedUseRatePart.surchargeAddon?` 신설 — **주택 파트에만** 부착.
- `buildTotalTax`의 IIFE를 `shortTermTax` → **`clause2`**(`{tax, maxRate}`)로 확장.
  중과 파트는 §104①1호와 **다른 호**라 누진 그룹에 합류하지 않고 단독 계산한다
  (§104⑤2호 단서 — 겸용의 주택 파트는 하나뿐이라 항상 단독).
- 조기 반환 조건 확장: 전 파트가 2년 이상이어도 **중과가 있으면 진입**한다.
- 기본공제 귀속 순위에 가산율 반영 — `effRate = addon > 0 ? max(TOP+addon, 단기율) : 단기율 ?? TOP`.

**술어 — 세율과 장특이 다르다**:

| | 술어 | 2008 위기취득 | 한시 유예 |
|---|---|---|---|
| 세율 가산(§104⑦) | `surchargeApplicable` | ❌ 배제 | ❌ 배제 |
| 장특 배제(§95②) | `surchargeType!=="none" && !isSuspended` | ✅ **존속** | ❌ 배제 |

B-B13이 2008 위기취득에서 「세율 0 · 장특 0 → 889,767,350(=1호)」을 고정한다.

**§5.5 경계 — 침묵하지 않는다**: 배율초과 비사토 동반 시 세율 가산을 보류하고
`warnings`에 사유·가산율·「장특 배제는 반영됨」을 명시한다(B-B18).

**anchor +9** — B-B11 · B-B12 · B-B13 · B-B14 · B-B15 · B-B16(2018-04-01 이전) ·
B-B17 · B-B17b · B-B18. 파일 누적 **24건**.

**검증**: `tsc` 0 · `eslint` **0 error** · 전체 **1,130파일 12,694건 통과 · 회귀 0**(12,686 → +8).

**파일 크기**: `transfer-tax-mixed-use.ts` 719 → **744** · `-totals.ts` 172 → **184**.
계획서 §5.6의 `-surcharge.ts` 분리는 **하지 않았다** — 744는 800 트리거와 750 기회주의
구간 **둘 다 미달**이고, 정책이 선제 분리를 금지한다. **B4에서 재확인**(누적 664→744이라
추세는 분명하다).

---

### 5.10-R Phase B4 구현 결과 ✅ (2026-07-31) — **브라우저 확인 포함**

**⑦ 결과 표시 + 이 세션 최초의 브라우저 검증.**

**표시 — 판정을 재도출하지 않는다**:

| 표시 | 소스 |
|---|---|
| 「장기보유공제 (배제)」 + 사유 | `multiHouseSurcharge`를 그대로 읽음(재판정 금지) |
| 「산출세액 (자산별 합계)」 + 산식 | 신규 `total.rateBasis` · `total.surchargeAddon` |

**🔴 P3b부터 있던 표시-계산 drift 2건을 함께 잡았다**:

§104⑤2호가 채택되면 `transferTax ≠ taxByBasicRate`인데,

1. 결과 카드 「양도소득세」 산식이 **1호 값**(`taxByBasicRate`)을 인용했다.
2. `mixedUseToFilingResult` 어댑터(`:87`)가 `calculatedTax: t.taxByBasicRate`를 넣어
   **신고서 표의 산출세액이 1호**인데 **지방소득세는 2호 기준**이었다.

브라우저에서 실제로 관측된 불일치:
```
산출세액        1,262,079,827   ← 1호
지방소득세 산출세액  156,701,913   ← 2호(1,567,019,136) × 10%
```
공용 헬퍼 `adoptedCalculatedTax(t)` 단일 소스로 통일 → 1,567,019,136 / 156,701,913 일치.

**브라우저 검증 — `e2e/mixed-use-multi-house-surcharge.spec.ts` 3건**

배관이 끊기면 세액이 조용히 낮아질 뿐 화면은 정상으로 보인다 → request body의 `houses`를
직접 가로채 확인한다.

| test | 확인 |
|---|---|
| 조정 2주택 | `houses` 전송 · 「장기보유공제 (배제)」 · 「산출세액 (자산별 합계)」 · 산식의 `+ 20.00%` |
| **2008 위기취득** | 「장특 배제」는 **표시되나** 「산출세액 (기본세율)」 — 세율·장특 술어 분리가 **화면에서** 갈림 |
| 비조정 | 배제 라벨 0건 · 「산출세액 (기본세율)」 |

> ⚠️ **E2E 작성 중 실측으로 배운 것 2가지** (추정했다면 틀렸을 것):
> - CASE14류 시드의 취득일 **2010-03-15가 2008 위기취득 구간**(2009-03-16~2012-12-31)이라
>   세율이 배제돼 2호 화면이 나오지 않았다. 구간 밖(2014-03-15)으로 옮겨 확인했고,
>   그 분기 자체를 **세 번째 test로 승격**했다.
> - `houses` 시드에 `isInherited`·`isLongTermRental` 등 필수 boolean이 빠져 Zod가 400을 냈다.
>   화면은 그냥 결과가 안 뜰 뿐이라 **원인이 보이지 않는다** — probe로 응답 본문을 찍어 확인.

**anchor +3** — B-B19(rateBasis·surchargeAddon echo) · B-B20 · B-B21. 파일 누적 **30건**.

**파일 분리** — `MixedUseResultCard.tsx`가 **797줄**(cap 800 3줄 앞)에 도달해
어댑터·표시 파생 헬퍼를 `MixedUseResultCardAdapter.ts`(신규 101줄)로 분리 → **718줄**.
착지목표 ≤700에 18줄 못 미치나 남은 본문은 카드 섹션 JSX라 더 쪼개면 억지 조각화가 된다.

**검증**: `tsc` 0 · `eslint` **0 error** · 전체 **1,130파일 12,698건 통과 · 회귀 0**(12,694 → +4) ·
**Playwright 3건 통과**.
...
| U-7 ✅**확정 → 후속 계획서로 이관** | **별건 결함(범위 밖)** — 조사 결과 배관 누락이 아니라 설계 이슈로 확인되어 [`transfer-surcharge-155-deeming-coverage.plan.md`](./transfer-surcharge-155-deeming-coverage.plan.md)로 분리(과다과세 +137,130,000). 원래 서술:  `multiHouseTemporaryTwoHouse`가 저장소 전체에서 정의·소비 2곳뿐이고 **route가 채우지 않는다**. 단건 중과 판정의 일시적2주택 입력이 상시 `undefined`일 가능성 | 🔶 미확인 · **이 계획서 범위 밖**. 사실이면 단건 경로 결함이므로 별도 조사 필요 |
...
| 버전 | 일자 | 내용 |
|---|---|---|
| **v1.7** | 2026-07-31 | **Phase B4 완료 — 계획서 전 Phase 종결**(§5.10-R). ⑦ 표시 + **브라우저 E2E 3건**(이 세션 최초). P3b부터 있던 **표시-계산 drift 2건** 동반 정정 — §104⑤2호 채택 시 결과 카드 산식과 신고서 어댑터가 1호 값을 인용해 「산출세액 1,262,079,827 vs 지방소득세 156,701,913(=2호×10%)」 불일치가 화면에 있었다. `adoptedCalculatedTax` 단일 소스로 통일. `MixedUseResultCard.tsx` 797→718(어댑터 분리). anchor +3(누적 30) · 12,698건 회귀 0 |
| **v1.6** | 2026-07-31 | **Phase B3 완료**(§5.9-R) — §104⑦ 세율 가산 + 후단 MAX. **계획 단계 손계산 2호가 원 단위까지 적중**(2주택 1,137,370,975 · 3주택+ 1,288,352,475). 후단 MAX 양방향 실측(보유1년 단기 70% 채택 / 장기 중과 0.65·0.75 채택). 세율은 `surchargeApplicable`, 장특은 다른 술어 — B-B13이 2008위기취득에서 분리 고정. anchor +9(누적 24) · 12,694건 회귀 0 |
