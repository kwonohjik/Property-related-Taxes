...
### 9.1 ✅ KoreanLaw 동결 (2026-06-27, 시행령 MST 283637·시행규칙 MST 284609)
- [x] **배당차액 본칙**: §57③ → **상증칙 §18②** 본문 확인 동결. 이미지3 산식 + anchor 6,164,383 일치. 단서(정관상 신주 배당기산일이 기존주식과 동일 시 배당차액 제외) → §9.2 scope-out.
- [x] **적정할인율 §18의3**: 본문 "연간 100분의 8" = **8%** (영 §58의2②1호가목 직접 근거). 현행 동결. 과거 시작일(2010.11.5 vs 2011.07.26)은 receivable 테이블 추종(잠정) — CB anchor(2009~2016)는 전 구간 영향 無(min(R,r)=R이거나 8%).
- [x] **§22 금융재산공제 적격**: **상증령 §19①** — "금융회사등이 취급하는 …주식·채권·…유가증권". 전환사채등은 유가증권이나 "금융회사등 취급" 요건 → **금융회사 보유분만 적격** → `financialDeduction: "hidden_expandable"` 유지(기본숨김+확장선택). receivable과 동일 정책 확정.
- [x] **만기상환금액 정의**: §58의2②1호 "만기 전에 발생하는 이자상당액 포함" — 만기상환금액=원금+상환할증금, 발생이자 별도 가산 구조로 반영.
- [x] **유효이자율(R) 처리**: PV(R)=발행가액 고정(A#1). R은 min(R,r) 판정용. 발행가·현금흐름 역산 scope-out.
- [x] **신주인수권증서 라목**(C#1): 교재 단독 사례 부재 → 자체 산식 anchor(cb-D)로 동결. 실제 집행례 확인 권장(잔여).

### 9.2 미구현 edge case (scope-out, 명시)
- **배당차액 단서**(상증칙 §18② 단서): 정관상 신주 배당기산일을 기존 상장주식과 동일하게 정한 경우 배당차액 제외(0) → 미구현. 해당 시 `cbDividendDifferenceOverride: 0` 직접입력으로 회피.
