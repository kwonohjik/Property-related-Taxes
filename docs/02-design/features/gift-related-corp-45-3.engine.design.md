...
### §45의3①2호 계산식 (legal_research 박스 verbatim 확보 ✅ 2026-06-26)

`get_law_text`는 계산식 박스 미렌더(빈 텍스트)였으나, **`legal_research` task=law_system이 박스를 verbatim 렌더**:

```
가목(중소): 세후영업이익 × 정상거래비율을 초과하는 거래비율 × 한계보유비율을 초과하는 주식보유비율
나목(중견): 세후영업이익 × 정상거래비율의 100분의50을 초과하는 거래비율 × 한계보유비율의 100분의50을 초과하는 주식보유비율
다목(일반): 세후영업이익 × 100분의5를 초과하는 거래비율 × 주식보유비율
```
...
→ **거래비율차감**: 중소=정상거래비율(§⑦ 50%) / 중견=정상거래비율의 50%(40×50=20) / 일반=고정 5.
→ **보유비율차감**: 중소=한계보유비율(§⑨ 10%) / 중견=한계보유비율의 50%(10×50=5) / 일반=0(차감 없음).

**중견·일반 전부 법령 확정** — RC-MEDIUM(486,000,000)·RC-LARGE(702,000,000) anchor 검증. SCOPE_OUT 해소.
...
**⑬ 증여의제이익 출자관계별 구분**:
- 간접보유비율 1/1000 미만 제외. ✅ 구현(2026-09-18 · W10 — `partitionSec13Paths`. 설계에 명시돼 있었으나 Do 단계에서 빠져 있었다)
...
/**
 * 계산식 거래비율차감 (§45의3①2호 — legal_research 박스 verbatim 검증 ✅)
 */
const TRADE_RATIO_DEDUCTION = {
  small:  50,  // 가목 "정상거래비율(§⑦ 50%)을 초과"
  medium: 20,  // 나목 "정상거래비율의 100분의50을 초과" (40×50%)
  large:   5,  // 다목 "100분의5를 초과" (고정)
} as const;
...
- ~~**나목(일반) 1천억 초과 특수관계매출** 요건 (§45의3①1호나목2) — `taxRequirementMet`에서 일반 케이스 진입 시 경고 출력~~
  → **해소(2026-09-18 · W8)**: 경고가 아니라 **요건 자체를 구현**했다. `taxRequirementMet = clauseAMet || clauseB2Met`이고
  `clauseB2Met = large && 거래비율 > 정상거래비율의 3분의 2 && (특수관계매출 − 과세제외매출) > 1천억원`이다.
  1천억원은 상증령 §34의3(「…1천억원을 말한다」) 본문이며, **같은 항 단서**(사업부문별 회계 구분 시 매출 비례 안분)는
  법 §45의3① 각 호 외 부분 후단(사업부문별 계산)이 미구현이라 적용하지 않는다.
  ⚠️ 약속됐던 「경고 출력」은 끝내 구현되지 않은 채 `rc-size-large` 입력 경로만 열려 있었고, 결과 화면은
  「과세요건 미충족」을 단정 표시했다 — 설계가 SCOPE_OUT의 **대가로 건 가드**가 코드에 없으면 SCOPE_OUT은
  「미지원」이 아니라 **오답**이 된다(memory `feedback_plan_exclusion_decision_needs_a_code_gate`).
- **§34의3⑭ 2호·4호** (2026-09-18 · W9에서 **명시 선언**. 종전에는 SCOPE_OUT 기재조차 없어
  「구현된 것」으로 읽혔다) — 2호는 지주회사의 다른 자회사·손자회사 관계를, 4호는 간접출자법인의
  다른 자법인 관계와 가·나·다목 3요건을 요구하는데 입력 모델이 그 관계를 표현하지 못한다.
  ⚠️ 미구현 방향이 **과세제외 과소 = 과대과세**라 「법 근거 없이 불리 적용 금지」와 부딪힌다.
  ⇒ 침묵하지 않는다. `related-corp.ts`가 **⑭1호가 걸리지 않은 ⑩ 미해당 특수관계 매출처가
  남아 있을 때만** `sec14ScopeNotice`를 내고 결과뷰가 amber로 표시한다(여지가 없으면 사라진다).
  같은 ⑭의 **1호·3호는 구현돼 있다**(1호 전액 · 3호 × 보유비율 · 후단 「더 큰 금액」은 매출처 단위 max).
- **다단계 간접출자 = §34의3⑱ 2호·3호** (3단계 이상) — `RcIntermediaryCorpItem.owners`가 개인만
  받아 「법인이 법인을 보유하는」 구조를 표현할 수 없다.
  ⚠️ 미구현 방향은 **과소과세**다 — ⑱1호를 못 넘긴 간접출자법인은 간접보유비율에서 통째로
  빠지는데, 2호(지배주주등 및 1호 법인이 합산 50% 이상)·3호(개재법인)라면 살아나야 한다.
  ⇒ (2026-09-18 · W10) YAGNI 주석에 그치지 않고 **⑱1호 미충족 법인이 실제로 있을 때만**
  `sec18ScopeNotice`를 내고 결과뷰가 amber로 표시한다(빠진 것이 없으면 고지도 없다).
- **간접 2경로 정렬** (작은 것부터 차감, §34의3⑬ 후단) — 사례 1경로라 무영향, YAGNI 주석

**✅ 해소(2026-06-26) — 중견·일반 거래비율차감**: `legal_research` task=law_system이 §45의3①2호 가나다목 박스를 verbatim 렌더(`get_law_text` 미렌더 우회). 나목=정상거래비율의 100분의50(20%)·다목=고정 5% 법령 확정. RC-MEDIUM(486,000,000)·RC-LARGE(702,000,000) anchor 추가 검증. SCOPE_OUT 해제.
