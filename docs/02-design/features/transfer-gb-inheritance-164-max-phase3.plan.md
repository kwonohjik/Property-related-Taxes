...
~~**증여**의 §163⑨1·2호 max는 넣지 않았다~~ → **§9에서 해소**(같은 세션 후속).

---

## §9. 증여 확장 (2026-08-07 — §8-6에서 이월)

### 9-1. 조문은 **같은 문장**이다

§163⑨ 본문은 「**상속 또는 증여**받은 자산」이 대상이고, 단서 1호·2호도 각각 「상속 또는 **증여**받은 토지/건물」이라고 쓴다. ⇒ 증여도 같은 max를 받는다. 별도 근거를 찾을 필요가 없었다.
...
### 9-3. 🔴 「분리 OFF에서 취득가액 0」은 **결함이 아니었다**

첫 probe가 `dispatchGeneralBuilding`에 `actualAcquisitionPrice`를 **0으로** 넘겨 「분리 OFF + 증여 → 취득가액 0·세액 443,235,000」이라는 결과를 냈다. 그대로 보고했다면 없는 결함을 고쳤을 것이다.

정상 전달(70,000,000)로 재측정하니 **40,422,535 / 29,577,465**로 §166⑥ 안분이 제대로 됐다. 관측 지점이 틀렸던 것이다([[feedback_anchor_observes_wrong_stage]]).

### 9-4. ①의 소스가 상속과 다르다 — 그래서 분리 ON이 전제다

상속은 전용 필드(`publishedValueAtInheritance`·`gbBuildingInheritedValue`)가 있지만, 증여 신고가액은 **분리 ON이면 파트 칸**, 분리 OFF면 **자산 단위 칸**이다.

단서가 요구하는 비교는 **자산별**(토지·건물 각각)이므로 파트별 ①이 있어야 성립한다. ⇒ **분리 OFF + 게이트 안은 ⑧이 안내로 차단**한다(「파트별로 나누어 입력하세요 — 「토지·건물 취득일 다름」을 켜면 파트별 칸이 열립니다」).

> 검토했다가 버린 대안: 분리 OFF에서 안분값을 ①로 보고 파트별 max. 안분 basis가 §164 가액과 같아 **수학적으로는 자산 단위 max와 결과가 같지만**(`max(T·sᵢ/S, sᵢ) = sᵢ·max(T/S,1)` — 파트별로 갈리지 않는다), 「증여 신고가액」이 아닌 파생값을 조문상 ①의 자리에 놓게 된다. 한 파트만 게이트 안인 조합(1995년)에서는 그 등가성도 깨진다.

### 9-5. 변경 지점

| # | 파일 | 내용 |
|---|---|---|
| ④ | `transfer-tax-api-gb.ts` | 게이트를 `(상속 \|\| 증여)`로 확장 · 증여 파트 슬롯에 max |
| ⑧ | `transfer-tax-validate-gb.ts` | **V-7g** — 분리 ON 요구 + ② 비교값 요구 |

⑤ UI는 무변경이다 — §164④ 등급환산 섹션은 상속 전용 게이트(`isGbLandPre1990Inheritance`)를 쓴다. **증여 토지의 등급환산은 이번 범위 밖**이며, 취득시 공시지가 칸으로 직접 입력할 수 있다(1990 이전 증여는 상속과 달리 `Pre1990LandValuationInput` 경로가 매매 블록에도 있어 별도 판단이 필요하다 — §10).

### 9-6. 게이트

| | 결과 |
|---|---|
| **Pre-Do anchor** | 13건 중 **7건 실패 · 6건 통과**(회귀 가드) |
| **mutation probe** ①증여를 게이트에서 제외 ②파트 게이트를 자산 단위로 | **2건 · 1건** 실패 |
| `npm test` | **14,358건** — 회귀 **0** |
| **E2E** | 신규 4건 + GB 계열 **16건** |
| `tsc` · `lint` | **0** · **0 errors** |
...
- ~~**증여 토지의 §164④ 등급환산 UI**~~ → **§11에서 해소**. 겹침 확인 결과 **없었다**(매매/상속/증여 블록이 `acquisitionCause`로 배타 렌더).
- **pre-1985 증여** — 기존 §163⑨ 증여 게이트가 `>= 1985-01-01`로 잘려 있다(§176의2④ 의제취득 영역). 그 경계를 여는 것은 환산 fallback 경로와 함께 봐야 한다.
- 상속 × 증축 · 상속 × 부담부증여 — Phase 2 §7 그대로.

---

## §11. 증여 토지 §164④ 등급환산 UI (2026-08-07 — §10에서 이월)

### 11-1. 「확인 필요」 두 건의 답

| 확인 항목 | 결과 |
|---|---|
| 매매 블록 `showPre1990`과 **겹치는가** | **아니다** — GB 카드가 `acquisitionCause`로 매매(`:291`)·상속(`:400`)·증여(`:427`) 블록을 **배타 렌더**한다 |
| 게이트를 어디까지 열 것인가 | **1985-01-01 하한을 함께 둔다** — ④의 `acquisitionByInheritance`·`isLandGift`가 그 하한을 갖는다 |

게이트 함수를 `isGbLandPre1990Inheritance` → **`isGbLandPre1990Sec163_9`**(상속·증여 공통 + 1985 하한)로 바꿨다.

### 11-2. 🔴 함께 드러난 것 — **pre-1985 상속 GB의 취득가액이 0이다**

1985 하한을 확인하다 발견했다. 실측(1980년 상속, 양도가 16.2억):
...
Phase 1(#1080 계열)은 「pre-1985는 **기존 환산 fallback**(회귀-safe)」을 전제하고 1985 하한을 뒀다. 그런데 **O-3(2026-08-06)이 상속 파트의 환산을 1985 하한 없이 차단**하면서(V2) 그 fallback이 사라졌다 — 전제가 3주 뒤에 무효가 됐고 아무도 알아채지 못했다.

> ⚠️ **본 PR 범위 밖이다.** 고치려면 §176의2④ 의제취득 경로(환산 or `calcPreDeemed` 3자 max)를 GB에 어떻게 연결할지 정해야 하고, 그것은 Phase 1이 의도적으로 미룬 축이다. 게이트 함수 주석과 anchor에 남겼다.

### 11-3. 변경 지점

| # | 파일 | 내용 |
|---|---|---|
| bridge | `transfer-pre1990-gb-bridge.ts` | 게이트 → `isGbLandPre1990Sec163_9` (상속·증여 + 1985 하한) |
| ⑤ | `GeneralBuildingAcquisitionCards.tsx` | 섹션 제목·안내를 취득원인별로 (「상속개시일 평가액」/「증여 신고가액」) |

④·⑧은 **무변경**이다 — 둘 다 `effectiveGbLandPriceAtAcq`를 이미 쓰므로 게이트를 넓히면 파생이 자동으로 따라온다(3중 fallback의 효과).

### 11-4. 게이트

| | 결과 |
|---|---|
| anchor | 증여 20건 + 상속 16건 = **36건** 통과 |
| `npm test` | **14,365건** — 회귀 **0** |
| **E2E** | 증여 6건 + 상속 4건 |
| `tsc` · `lint` | **0** · **0 errors** |
