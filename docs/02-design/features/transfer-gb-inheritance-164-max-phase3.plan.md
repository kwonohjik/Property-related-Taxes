...
- **증여 토지의 §164④ 등급환산 UI** — 상속에는 붙였으나(§8-3) 증여에는 없다. 1990.8.30. 이전 **증여** 토지도 개별공시지가가 없으므로 같은 문제가 있다. 다만 증여 경로는 `CompanionAcqGiftBlock`이라 배치 지점이 다르고, 매매 블록의 `showPre1990`과 겹치는지 확인이 필요하다.
