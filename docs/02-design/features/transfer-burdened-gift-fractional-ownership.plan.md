...
**라우트 하네스 실측(단건 ↔ 함께양도 대조)**:

| 기능 | 단건 | 함께양도 | 판정 |
|---|---|---|---|
| 겸용주택 | `mode=mixed-use`, `housingPart` 有 | `mode=bundled`, **소실**(primary가 `assetKind=land`로 강등) | 🔴 |
| 재개발 | `redevelopment` 산출물 有 | **소실** | 🔴 |
| 일반건물 | 토지·건물 분리 안분 有 | **소실** | 🔴 |
| 부담부증여 | `debtRatio` 有 | **소실** | 🔴 (대조군) |

**결정적 증거**: 일반건물은 단건에서 `zoneType` 미입력 시 **500으로 막히는데**,
함께양도에서는 그 검증조차 타지 않고 **200이 나온다** — 분기 미실행의 직접 증명.

**정정**: `transfer-tax-validate.ts`의 차단을 4종으로 확장.
회귀 방어선 `__tests__/api/transfer.route.bundled-swallows-special.test.ts`(5건)가
"차단이 풀리면 무슨 일이 일어나는지"를 대조 구조로 고정한다.

> **상가(`commercial_building`)는 미확인** — 5-a-3 전용 분기를 타지 않아 동일 결함인지
> 검증되지 않았다. **근거 없이 막지 않는다**(잘못된 차단도 해악). 별도 검증 후 판단.
