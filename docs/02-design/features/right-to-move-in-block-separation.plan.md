...
| 4 (D-4) | ⛔ **미해소** | 예규 본문 확보 실패 ⇒ 계획서 규칙대로 **현행 유지** |
...
- **D-4** — ③-c 「비과세 보유 요건」(`exemptionEligibleAtApproval`)은 입주권에서도 LTHD 표1 강등
  효과가 있다(`redevelopment-lthd.ts` subject 분기 **이전**). 근거 예규
  **서면2016-법령해석재산-2705** 본문을 법제처 `interpretation`·국세청 `nts` 검색으로 확보하지
  못해 **판정을 유보**했다. 본문 확인 전에는 코드를 바꾸지 않는다
  ([[feedback-unverified-authority-blocks-tax-change]]).
