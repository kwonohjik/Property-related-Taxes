...
### 남은 것 → **후속 항목 R8로 이관** (2026-08-14 사용자 결정)
...
- **예외**: 관리처분계획인가일 **이후에도 주택인 상태가 유지**되면 양도일 기준을 적용한 사례가 있다
  ...
  **사용자 override가 자동값보다 우선하므로 예외 케이스도 정확한 세액 산출은 가능하다.**
- 자동 판정 분기 신설은 **적용 사례의 범위가 확정되지 않아** 하지 않는다. 착수 조건·검증 기준은
  **`docs/00-pm/transfer-open-items.plan.md` §R8**에 기록했다.
  ⚠️ 서면-2016-법령해석재산-2705은 **원칙(인가일 기준)** 쪽 근거이므로 R8의 근거가 아니다 —
  예외를 인정한 별개 사례의 본문을 먼저 특정해야 한다.
