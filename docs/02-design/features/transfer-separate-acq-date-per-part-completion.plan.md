# 토지·건물 취득시기 상이 — 취득가액 축 재설계 계획서 (rev.2)

> 상태: **Plan rev.2 (STEP 1 자가검토 반영)** · 작성 2026-07-28 · 대상: 양도소득세 토지·건물 **별개 취득** 자산
...
단 `housing`은 §2-D의 결합 공시 구조 때문에 결론이 다르다 — rev.2의 핵심 정정.
...
### 2-D. ⚠️ 주택(라목)은 부수토지 포함 **결합** 기준시가 — rev.2 핵심
...
**R2의 근거인 §163⑥2호나목 본문이 조회되지 않아 확정 불가.** rev.2는 회귀 0이고 라목 문언에 직접 부합하는 **R1을 채택**하며, R2 검토는 §14 별건 과제로 이관한다.

---

## 3. 현행 구현 실측 진단 (rev.2 — 유효한 것만)
...
### 3.4 적용 대상 플래그가 3경로로 오염 ⚠️ **rev.2 신규 발견**
...
### 3.5 지분 모드 스케일 혼재 ⚠️ **rev.2 신규 발견**
...
## 5. 목표 모델 — propertyType 이원화

### 5.0 적용 게이트 (rev.2 신설 — §3.4 해소)
...
| **P1.5** | propertyType 분기 헬퍼(`isSeparateAcquisition`) 확정 + 겸용·selfOwns 제외 | H4·H5 green |
| **P2a** | **축 A 파트별 완결(E2·E3) — 양 propertyType** | B1~B10 · H2·H3·H5 green |
