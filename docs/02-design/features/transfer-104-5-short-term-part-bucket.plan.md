# §104⑤ `short_term` 그룹의 **파트 단위 버킷** — v1.0
...
> | ID | 결함 | 방향 | 실측 | 상태 |
> |---|---|---|---:|---|
> | **P13** | `short_term` 그룹만 **자산 단위**로 버킷을 만든다 — 파트가 있는 자산(`isAssetLevelClause5`)이 통째로 `solo`로 빠져 **같은 호인 다른 자산과 합산되지 않는다** | **과소** | **23,000,000**(①) · **17,000,000**(③) | 🔴 미착수 |
>
...
| Phase | 내용 | 세액 변경 |
|---|---|---|
| **P13** | `short_term` 버킷 멤버를 파트로 교체 + `isAssetLevelClause5` 제거 | **있음** — 과소 23,000,000 / 17,000,000 해소 |
