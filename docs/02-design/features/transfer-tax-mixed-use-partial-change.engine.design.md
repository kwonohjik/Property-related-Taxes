> 🔴 **2026-08-10 — 이 문서의 「용도변경일 기반 LTHD 시간비례 분할」은 폐지됐다.**
> 근거로 삼았던 「집행기준 89-154-24」는 **존재하지 않는 문서**다. 「소득세법」 §95④는
> 보유기간을 「취득일부터 양도일까지」로 정하고, 사전-2021-법령해석재산-0333(겸용주택
> 주택부분→상가 용도변경 시 **기산일 = 취득일**)·사전-2022-법규재산-0427(고가 겸용주택
> 보유기간 = 취득일~양도일, 표2는 **주택 부분에 한정**)이 이를 확인한다.
> ❌ 이 문서를 근거로 시간비례 분할을 재구현하지 말 것.
> 현행 동작 anchor: `__tests__/tax-engine/transfer-tax/mixed-use-usage-change-lthd.anchor.test.ts`
