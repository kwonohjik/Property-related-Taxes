...
### 1.2 양도코리아 정답표 (anchor 기준 — Task #10 실측값으로 재잠금 2026-05-11)
...
> ★ Task #10 사후 정정 — QA 실측값으로 매트릭스 재잠금 (2026-05-11). 산출세액 6,480,952·지방세 648,095는 양도코리아 PDF와 1원 단위 완전 일치 — 사용자 관점 최종 결과는 변동 없음.
...
산출세액 = floor(51,003,968 × 0.24) - 5,760,000
         ...
         = 6,480,952  ← anchor (양도코리아와 정확 일치)

지방소득세 = floor(6,480,952 × 0.10)
           = floor(648,095.2)
           = 648,095  ← anchor (양도코리아와 정확 일치)
```
...
>
> `transfer-tax-aggregate-helpers.ts:147-194`의 단순 floor + 마지막 자산 잔액 흡수 패턴. 건물2가 마지막 자산이므로 건물2 흡수액 = floor 패턴(2,336,344), 토지 흡수액 = 잔액(26,527,095 - 2,336,344 = 24,190,751). 양도코리아 PDF(토지 24,190,750 / 건물2 2,336,344)와 토지 흡수액 1원 차이는 자동계산기 간 라운딩 방식 차이로 정상. UI 결과 카드는 우리 엔진 값만 표시. 산출세액(6,480,952)·지방세(648,095)는 양도코리아와 1원 단위 완전 일치.
>
...
>
> ★ 필요경비 안분 결과: landExp = 6,595,232 (실측). 양도코리아 표기(6,595,233)와 1원 차이는 잔액 보정 순서 차이로 정상.
...
**§1.4 검증 결과 (Task #10 실측 확정)**: sort 순서([토지, 건물1, 건물2])에서 건물2가 마지막 → 건물2 흡수액 = floor(26,527,095 × 7,048,645 / 80,031,063) = **2,336,344** (floor 패턴), 토지 흡수액 = 잔액 26,527,095 - 2,336,344 = **24,190,751** (잔액 흡수). 양도코리아 PDF(토지 24,190,750 / 건물2 2,336,344)와 토지 1원 차이. §1.4 sort 순서 의존 분석은 그대로 유지.
...
| # | 검증 대상 | 기댓값 | 출처 |
|---|---|---:|---|
| 33-1 | 토지 양도가 | **275,736,648** | 양도코리아 사례 33 정답표 + 실측 |
| 33-2 | 건물1 양도가 | **9,996,854** | 양도코리아 사례 33 정답표 + 실측 |
...
| 33-15 | 건물2 양도차익 | **10,069,492** | 양도코리아 + 실측 일치 |
| 33-16 | 건물2 LTHD | **3,020,847** | floor(10,069,492 × 0.30) 실측 |
| 33-17 | 건물2 양도소득금액 (통산 전) | **7,048,645** | 양도코리아 + 실측 일치 |
...
| 33-23 | 산출세액 | **6,480,952** | §55 직접 계산 — 양도코리아와 정확 일치 |
| 33-24 | 지방소득세 | **648,095** | §103조의3 × 10% — 양도코리아와 정확 일치 |

> ★ 산출세액 6,480,952·지방세 648,095는 양도코리아 PDF와 1원 단위 완전 일치 — 사용자 관점 최종 결과는 변동 없음.
...
| 항목 | 결과 |
|---|---|
| aggregate offsetLosses 라운딩 | **(A-2) 결정으로 닫음 — 코드 변경 0, anchor는 Task #10 실측값** (`transfer-tax-aggregate-helpers.ts:147-148, 192-194` 단순 floor + 잔액 흡수 패턴 그대로). 건물2 흡수 2,336,344(floor) / 토지 흡수 24,190,751(잔액) / 토지 통산 후 48,791,667 / 건물2 통산 후 4,712,301. 양도코리아 PDF(토지 24,190,750)와 토지 흡수액 1원 차이는 잔액 흡수 순서 차이로 정상. (Task #10 실측 재잠금 2026-05-11) |
