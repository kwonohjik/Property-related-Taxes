...
예제 정답: 산출세액 **6,480,952** / 지방소득세 **648,095**.

3개 소득 라인(토지·건물1·건물2)을 생성하고 건물1 차손(-26,527,095)을 영 §102② income 기준 pro-rata로 통산하여 도달.
...
### 1.2 예제 정답표 (anchor 기준 — Task #10 실측값으로 재잠금 2026-05-11)

| 라인 | 양도가 | 취득가 | 필요경비 | 양도차익 | LTHD | 양도소득금액 (income) | 통산 후 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 토지 (1001) | 275,736,648 | 164,880,819 (실가 안분) | 6,595,232 | **104,260,597** | **31,278,179** | 72,982,418 | **48,791,667** |
| 건물1 (3001) | 9,996,854 | 35,119,181 (실가 안분) | 1,404,768 | **-26,527,095** | 0 | **-26,527,095** | 0 |
| 건물2 (3002) | 44,266,498 | 32,978,880 (환산) | 1,218,126 (개산공제) | 10,069,492 | 3,020,847 | 7,048,645 | **4,712,301** |
| **합계** | **330,000,000** | **232,978,880** | **9,218,126** | **87,802,994** | **34,299,025** | **53,503,968** | **53,503,968** |

> ★ Task #10 사후 정정 — QA 실측값으로 매트릭스 재잠금 (2026-05-11). 산출세액 6,480,952·지방세 648,095는 예제 PDF와 1원 단위 완전 일치 — 사용자 관점 최종 결과는 변동 없음.

### 1.3 세액 검산 (2023년 §55 기본세율)

```
통산 후 양도소득금액 합계: 53,503,968  ← 실측값 (Task #10 정정)
기본공제: 2,500,000
과세표준: 51,003,968 (원 단위 그대로 — 천원 절사 없음)

2023년 누진세율 (§55 직접 계산):
  51,003,968원 → 5,000만~8,800만 구간 (세율 24%, 누진공제 5,760,000)

산출세액 = floor(51,003,968 × 0.24) - 5,760,000
         = floor(12,240,952.32) - 5,760,000
         = 12,240,952 - 5,760,000
         = 6,480,952  ← anchor (예제와 정확 일치)

지방소득세 = floor(6,480,952 × 0.10)
           = floor(648,095.2)
           = 648,095  ← anchor (예제와 정확 일치)
```

> ★★★ `feedback_transfer_year_tax_rate.md` 적용: 산출세액·지방세는 §55·§103조의3 직접 계산.
> 외부 자료(예제 표기값) 산출세액 그대로 추종 금지. 위 검산값이 anchor.

### 1.4 영 §102② 결손 통산 검산 (income 기준 pro-rata — Task #10 실측 재잠금)

```
양수 income 합: 72,982,418 + 7,048,645 = 80,031,063
손실 절대값:    26,527,095  ← 실측값 (건물1 양도차익 -26,527,095 기준, Task #10 정정)

토지 안분 비율: 72,982,418 / 80,031,063 = 91.193%
건물2 안분 비율: 7,048,645 / 80,031,063 = 8.807%

토지 흡수액: 24,190,751  (잔액 흡수 패턴 — 건물2 floor 후 잔액)
건물2 흡수액: 2,336,344  (우리 엔진 결과 — floor 패턴)
흡수 합 검증: 24,190,751 + 2,336,344 = 26,527,095  ✓ (anchor T-18)

토지 통산 후: 72,982,418 - 24,190,751 = 48,791,667  ✓ (anchor T-20)
건물2 통산 후: 7,048,645 - 2,336,344 = 4,712,301   ✓ (anchor T-21)
건물1 통산 후: 0                                     ✓ (anchor)
합계: 48,791,667 + 4,712,301 + 0 = 53,503,968       ✓ (anchor T-22)
```

> **라운딩 결정 = (A-2) 확정 (2026-05-11 Task #10 사후 정정)**
>
> `transfer-tax-aggregate-helpers.ts:147-194`의 단순 floor + 마지막 자산 잔액 흡수 패턴. 건물2가 마지막 자산이므로 건물2 흡수액 = floor 패턴(2,336,344), 토지 흡수액 = 잔액(26,527,095 - 2,336,344 = 24,190,751). 예제 PDF(토지 24,190,750 / 건물2 2,336,344)와 토지 흡수액 1원 차이는 자동계산기 간 라운딩 방식 차이로 정상. UI 결과 카드는 우리 엔진 값만 표시. 산출세액(6,480,952)·지방세(648,095)는 예제와 1원 단위 완전 일치.
>
> 코드 변경 0 — anchor는 Task #10 실측값(건물2 흡수 2,336,344 / 토지 흡수 24,190,751 / 토지 통산 후 48,791,667 / 건물2 통산 후 4,712,301)으로 재잠금.
...
### Step 2 — 일괄 취득가 2-way 안분 (토지+건물1만, 취득시 기준시가 비율)

```
// 200,000,000은 토지+건물1 일괄. 건물2는 별도 증축이므로 분배 대상 아님.
// 안분 비율은 취득시 기준시가 비율 — Step 1 양도가 안분(양도시 비율)과 구분.
// 취득시 시가: acqLandStd = 1,400,000 × 57 = 79,800,000
//             acquisitionBuildingStdPrice (건물1 취득시) = 16,997,190
landBuildingDenomAtAcq = acqLandStd + acquisitionBuildingStdPrice
                       = 79,800,000 + 16,997,190 = 96,797,190

landAcq      = floor(200,000,000 × 79,800,000 / 96,797,190) = 164,880,819
building1Acq = 200,000,000 - 164,880,819                    =  35,119,181  (잔액 보정)
// 합 검증: 164,880,819 + 35,119,181 = 200,000,000  ✓

landExp      = floor(  8,000,000 × 79,800,000 / 96,797,190) =   6,595,232  ← 실측값
building1Exp =   8,000,000 - 6,595,232                      =   1,404,768  (잔액 보정)
// 합 검증: 6,595,232 + 1,404,768 = 8,000,000  ✓
```

> ★ Task #10 사후 정정 — QA 수학적 검증으로 발견 (2026-05-11). Step 1 양도가 안분은 §166⑥ **양도시** 비율이지만, Step 2 일괄 취득가 안분은 **취득시** 기준시가 비율 — 안분 기준 시점이 분배 대상 시점(취득가)과 일치해야 함. 양도시 비율로는 T-01(275,736,648)과 T-05(164,880,819)를 수학적으로 동시 만족 불가능. 코드 위치: `lib/tax-engine/general-building-extension.ts:79-110`.
>
> ★ 필요경비 안분 결과: landExp = 6,595,232 (실측). 예제 표기(6,595,233)와 1원 차이는 잔액 보정 순서 차이로 정상.
...
|---|---|---|---:|---:|---|---|---|---|
| 토지 | "land" | "land" | 164,880,819 | 6,595,232 | false | 2003-03-17 | — | — |
| 건물1 | "building1" | "general_building_unit" | 35,119,181 | 1,404,768 | false | 2003-03-17 | false | false |
...
[assetRecords 조립] transferGain < 0 → lthd = 0 강제 (건물1 차손 -26,527,095)
...
- 건물1 차손: `income = -26,527,095` → `lthd = 0` 강제 (aggregate L.129)
- 양수 income 풀: 토지(72,982,418) + 건물2(7,048,645) = 80,031,063
- `offsetLosses()` pro-rata: 손실 26,527,095를 income 비율로 분배 흡수
- 결과: 토지 48,791,667 / 건물2 4,712,301 / 건물1 0 (A-2 확정 — Task #10 실측)
...
### 6.3 라운딩 동작 상세 — (A-2) 확정 (Task #10 anchor 실측 재잠금 2026-05-11)
...
**§1.4 검증 결과 (Task #10 실측 확정)**: sort 순서([토지, 건물1, 건물2])에서 건물2가 마지막 → 건물2 흡수액 = floor(26,527,095 × 7,048,645 / 80,031,063) = **2,336,344** (floor 패턴), 토지 흡수액 = 잔액 26,527,095 - 2,336,344 = **24,190,751** (잔액 흡수). 예제 PDF(토지 24,190,750 / 건물2 2,336,344)와 토지 1원 차이. §1.4 sort 순서 의존 분석은 그대로 유지.

**사례 31은 손실 카드가 없어 이 경로를 통과하지 않음** — 사례 33이 multi-asset loss 경로를 처음 노출하는 케이스.

**(A-2) 결정으로 닫음**: anchor 33-18~33-22는 우리 엔진 실제 결과값(Task #10 실측)으로 잠금. 코드 변경 없음. Task #10에서 anchor 실측 재잠금 완료 (2026-05-11).
...
## 10. anchor 테스트 25개 매트릭스 (Task #10 실측 재잠금 — 2026-05-11)

`__tests__/tax-engine/transfer-tax/general-building-extension-case-33.test.ts` 신규.

> ★ Task #10 사후 정정 — QA 실측값으로 매트릭스 전수 재잠금 (2026-05-11). anchor 테스트 파일을 직접 읽어 추출한 값으로 디자인 매트릭스 1대1 동기화 완료.

### 10.1 안분·환산 (8개)

| # | 검증 대상 | 기댓값 | 출처 |
|---|---|---:|---|
| 33-1 | 토지 양도가 | **275,736,648** | 예제 사례 33 정답표 + 실측 |
| 33-2 | 건물1 양도가 | **9,996,854** | 예제 사례 33 정답표 + 실측 |
| 33-3 | 건물2 양도가 | **44,266,498** | 잔액 보정 실측 |
| 33-4 | 3항 합계 검증 | **330,000,000** | 잔액 보정 정합성 |
| 33-5 | 토지 실가 안분 | **164,880,819** | 취득시 비율 안분 실측 (Task #10 정정) |
| 33-6 | 건물1 실가 안분 | **35,119,181** | 잔액 보정 실측 |
| 33-7 | 합 검증 (토지+건물1) | **200,000,000** | 일괄 취득가 정합성 |
| 33-8 | 건물2 환산취득가 | **32,978,880** | §176의2② 환산 실측 |

### 10.2 양도차익·LTHD·income (9개)

| # | 검증 대상 | 기댓값 | 출처 |
|---|---|---:|---|
| 33-9 | 토지 양도차익 | **104,260,597** | 실측 (landExp=6,595,232 기준, Task #10 정정) |
| 33-10 | 토지 LTHD | **31,278,179** | floor(104,260,597 × 0.30) 실측 |
| 33-11 | 토지 양도소득금액 (통산 전) | **72,982,418** | 104,260,597 - 31,278,179 실측 |
| 33-12 | 건물1 양도차익 | **-26,527,095** | 실측 (b1Exp=1,404,768 기준, Task #10 정정) |
| 33-13 | 건물1 LTHD (차손 → 0) | **0** | aggregate 차손 LTHD 강제 0 |
| 33-14 | 건물1 양도소득금액 (통산 전) | **-26,527,095** | 실측 |
| 33-15 | 건물2 양도차익 | **10,069,492** | 예제 + 실측 일치 |
| 33-16 | 건물2 LTHD | **3,020,847** | floor(10,069,492 × 0.30) 실측 |
| 33-17 | 건물2 양도소득금액 (통산 전) | **7,048,645** | 예제 + 실측 일치 |

### 10.3 통산·세액 (7개)

| # | 검증 대상 | 기댓값 | 출처 |
|---|---|---:|---|
| 33-18 | 흡수 합 검증 (토지흡수+건물2흡수) | **26,527,095** | 건물1 income 절대값 (Task #10 정정 +1) |
| 33-19 | 건물2 흡수액 | **2,336,344** | floor 패턴 실측 (Task #10 확정) |
| 33-20 | 토지 통산 후 | **48,791,667** | 72,982,418 - 24,190,751 실측 (Task #10 정정) |
| 33-21 | 건물2 통산 후 | **4,712,301** | 7,048,645 - 2,336,344 실측 (Task #10 정정) |
| 33-22 | 통산 후 합계 | **53,503,968** | 48,791,667 + 4,712,301 실측 (Task #10 정정) |
| 33-23 | 산출세액 | **6,480,952** | §55 직접 계산 — 예제와 정확 일치 |
| 33-24 | 지방소득세 | **648,095** | §103조의3 × 10% — 예제와 정확 일치 |

> ★ 산출세액 6,480,952·지방세 648,095는 예제 PDF와 1원 단위 완전 일치 — 사용자 관점 최종 결과는 변동 없음.
...
| 항목 | 결과 |
|---|---|
| aggregate offsetLosses 라운딩 | **(A-2) 결정으로 닫음 — 코드 변경 0, anchor는 Task #10 실측값** (`transfer-tax-aggregate-helpers.ts:147-148, 192-194` 단순 floor + 잔액 흡수 패턴 그대로). 건물2 흡수 2,336,344(floor) / 토지 흡수 24,190,751(잔액) / 토지 통산 후 48,791,667 / 건물2 통산 후 4,712,301. 예제 PDF(토지 24,190,750)와 토지 흡수액 1원 차이는 잔액 흡수 순서 차이로 정상. (Task #10 실측 재잠금 2026-05-11) |
...
- [x] **라운딩 결정 = (A-2) 확정 (Task #10 실측 재잠금 2026-05-11)** — anchor T-18 흡수 합=26,527,095 · 건물2 흡수=2,336,344 · T-20 토지 통산 후=48,791,667 · T-21 건물2 통산 후=4,712,301 · T-22 합계=53,503,968. 코드 변경 없음.
