...
**⚖️ 판단 보류 — 사용자 결정 필요.** 독법을 바꾸면 **세액이 움직인다**(위 표에서 B의 한도
9,715,910 → 17,100,000). 명문·서식·예규가 **모두 없는** 상태에서 세액을 바꾸는 것은
[[feedback_unverified_authority_blocks_tax_change]]에 걸린다 ⇒ **현행(자산별) 유지 + FA-5로 고정**하고,
결정이 서면 FA-5-4를 갱신한다.
