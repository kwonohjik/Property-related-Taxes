...
### 0.4 ~~🔴 겸용주택은 형태가 다르다~~ ✅ **해소 (2026-09-04)**

> 「세액이 엔진 내부에서 완결돼 합칠 수 없다」가 차단 사유였는데, **엔진을 1회 돌리고 `total`을
> 버린 뒤 그 엔진만이 아는 값**(§166⑥ 안분·§97 환산·§163⑥ 개산공제·§167의5 배율초과 비율)만
> 꺼내 aggregate item으로 되먹이면 **단건 겸용과 세액이 완전히 일치**한다(5케이스 실측).
> 세율·장특·비과세·중과는 item이 다시 판정한다 — aggregate가 item마다 단건 엔진을 돌리기 때문이다.
>
> 🔑 **설계 §9.2의 「주택 1카드」는 틀렸다** — 주택분 장특은 토지·건물 각각의 보유기간으로
> 계산해 더하므로 2카드가 필요하고, §89① 12억 판정은 `totalPropertyTransferPrice`가 담당한다.
>
> ⚠️ **primary 겸용은 계속 차단**이다(5-a의 primary는 `{...engineInput}` 스프레드).
> 상세: `transfer-bundled-subengine-hosting.design.md` §10.

<details><summary>종전 기재 (차단 근거 — 보존)</summary>
...
| 4 | 겸용주택 | 세액 계산이 엔진 내부 완결 | ✅ **개방 (2026-09-04)** — 파트 카드 4~5장으로 되먹여 해소 |
