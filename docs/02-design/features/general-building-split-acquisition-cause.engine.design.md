...
사례 32 작업 후 사용자 피드백 — 일반건물 자산종류에서 "취득원인" 단일 필드는 토지/건물의 다른 취득원인(예: 토지 매매 + 건물 신축)을 표현 불가. 양도코리아 UI 패턴(토지/건물 각각 분리 입력)에 정렬한다.
...
**대안 #0-A (분할 선행 PR) 폐기 사유**: validate 파일은 도메인 경계가 명확하지 않음 — 일반건물·검용주택·다필지 등 cross-cutting 검증이 많아 분리 시 helper 공유 코드 발생 → 분할 효과 제한적. 임계 초과(800+) 시점에 강제 분할이 더 효율적.
