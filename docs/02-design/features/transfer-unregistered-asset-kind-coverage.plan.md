...
>
> ✅ **2026-09-18 정정(D15)**: 별도 근거는 **조세특례제한법 §129②**다 — 「「소득세법」 제104조제3항에 따른 미등기양도자산에 대해서는 양도소득세의 비과세 및 감면에 관한 규정을 적용하지 아니한다.」(KoreanLaw MST 284389). 감면 배제는 법 근거가 있는 적용이며, 게이트는 `docs/00-pm/transfer-review-4-defects.plan.md` §2(PR-2)에서 구현했다.
...
| ~~**C-6d**~~ ✅ | **판정(2026-08-11): 현행 유지.** 조기 반환이 **§104⑦ 다주택 중과**(최고 **75%**)와 겹치면 70%를 골라 과소로 보였으나, **문언이 현행을 지지한다**(§7-1 상세·anchor `unregistered-104-7-precedence`) | — |
| **C-6c** | 미등기 + 환산 취득의 동시 성립이 §168① 제외 사유(장기할부·법원 결정 등)에 걸리는 경우 UI가 판단하지 않는다 | 현행 정책 유지(사용자 입력 그대로) — Q4와 동일 축 |

#### C-6d 상세 — 미등기 70%(§104①10호) × 다주택 중과(§104⑦)의 우열 ✅ 현행 유지

**종전 서술이 틀렸다.** 「⑦항과의 우열은 명문이 없다 → 미판정」으로 적어 두었으나, §104 전문을
실측하니 **명문이 있고 방향이 반대**였다([[feedback_no_statute_claim_needs_requirement_article]] —
「명문 없음」 단언은 요건 조항 본문을 읽어야 성립).

**후단(비교) 규정은 셋이고, 어느 것도 10호를 넣지 않는다** (소득세법 §104 전문, 2026-08-11 실측):
...
미등기에는 §55①이 아니라 §104①10호 70%가 적용되므로 **가산의 base가 없다**.
(⑦ 후단이 2·3호 단일세율과의 비교를 규정한 것은 반대 정황이나, 그 열거에 10호는 없다.)

**발현 조건 실측** (anchor `__tests__/tax-engine/transfer/unregistered-104-7-precedence.anchor.test.ts` 4건):

- **B-1** 과세표준 20억 — 현행 70% **1,400,000,000** vs ⑦3호 75% **1,434,060,000** ⇒ 차 **34,060,000**
- **B-2** 미등기의 `candidateClauses`는 `["104-1-10"]` 하나뿐 — ⑦ 호를 싣지 않는다(등기 대조군은 싣는다)
- **B-3** 분기점 **과세표준 1,318,800,000**(0.45T − 65,940,000 + 0.3T = 0.7T). 그 아래는 70%가 커서 미발현
- **B-4** 중과 **한시 유예 ~2026-05-09**라 그 이전 양도분은 ⑦ 자체가 없다

⇒ 발현하려면 **2026-05-10 이후 양도 + 미등기 주택 + 조정지역 다주택 + 과세표준 13.19억 초과**가
동시에 성립해야 한다. 유예가 재연장되면 발현 구간이 사라진다.

⇒ **현행 유지**. 바꾸면 세액이 **오르는** 방향이라
[[feedback_unverified_authority_blocks_tax_change]]가 그대로 걸리고, 여기서는 문언까지 현행 편이다.
❌ 「⑦이 더 크니 MAX를 넣자」로 재제안하지 말 것 — 막고 있는 것은 근거 부재가 아니라 **한정 열거**다.

> ⚠️ memory `project_transfer_104_1_latter_short_term`(§104① 후단 × 일반 단기)과는 **결론 방향이
> 반대**다. 거기는 문언이 **바꾸는 쪽**인데 권위가 없어 막혀 있다. 두 건을 한 묶음으로 다루지 말 것 —
> 접점은 `candidateClauses`/`clauseBucketKey` 한 곳뿐이다.
...
### ✅ C-6c 완료 (2026-08-11) — §168① 제외 사유 안내

엔진은 제외 사유를 **판정하지 않는다**(입력이 없다). 사용자가 제외 대상인데 토글을 켜면 70%가 그대로 적용돼 **과대 계산**되므로, 켠 시점에 확인을 유도하는 안내를 넣었다(`SpecialSituationSection.tsx`).

- **노출 조건**: 미등기 축이 하나라도 ON. 일반건물은 토지·건물 2축이라 폼-전역 값만 보면 안 된다(`anyUnregisteredOn`).
- **내용**: §168① 각 호(1·2·3·4·6·7 — **5호는 삭제**)를 읽기 쉬운 말로 나열 + `LawArticleModal` 조문 링크.
- **판정은 하지 않는다** — 「해당한다면 토글을 끄고 계산하세요」로 사용자 확인에 맡긴다. 자동 판정에 필요한 입력(장기할부 계약조건·법원 결정·도시개발사업 종료 여부 등)이 폼에 없고, 임의 추정은 자동 안분 fallback 금지 원칙과 같은 축이다.

E2E 2건(대조군 포함 — 켜기 전 부재 → 켠 뒤 표시 + 각 호 본문 존재).

### ⏭️ 남은 작업

**없다.** 마지막까지 열려 있던 🟡 §104⑦ 우열도 2026-08-11에 **C-6d ✅ 현행 유지**로 닫혔다
(§7 C-6d 상세 · anchor `unregistered-104-7-precedence` 4건).
「명문이 없어 미판정」이라던 종전 서술이 틀렸고, **④·⑦ 후단의 한정 열거**가 현행을 지지한다.
