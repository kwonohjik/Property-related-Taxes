# ❌ [폐기] Phase 3 — pre-deemed 취득가액 §2 「양도차익 기준 유리선택」

작성일: 2026-07-08 · **폐기: 2026-08-07** · 대상: 양도세 상속·증여 `calcPreDeemed` (PR#539·#540 후속)

> ## ❌ **2026-08-07 — §2 「양도차익 기준 유리선택」 폐기**
>
> **법적 근거가 없다.** §8.5가 「✅ 확증된 것」으로 든 2건을 **원문으로 재검증한 결과 둘 다
> 반대 방향**이었고, 그 밖에 유리선택을 뒷받침하는 심판례·판례를 찾지 못했다.
>
> | 근거 | 본 계획의 주장 | 원문 |
> |---|---|---|
> | **국심2003서3266** | gain-level 유리선택 = 법적 정당 | ❌ **가목 우선** — 처분청의 환산 경정을 배척하고 §163⑨(가목 단서) 적용. 실제 필요경비는 그 **결과**다(법 §97③1호). 「유리해서」가 이유가 아니다 |
> | **대법원 2006두1326** | 〃 | ❌ 가목 ↔ 나목 **배타** |
> | 국심2003부0627·0626 | (미인용) | 환산 경정 **취소** — 가목 우선 |
>
> ⇒ **pre-deemed 정면 결정 2건이 모두 「가목 우선」**이다. 현행(#1089 `af74d907`)이 법령·실무와
> 일치하고, §2는 그것을 **되돌리는** 방향이다.
>
> **⚠️ 「원문 미확보」 기록이 이 오독을 4개월간 살려 뒀다.** §8.5는 2003서3266을 확증 근거로 들었고
> 코드 주석은 같은 결정을 「원문을 확보하지 못했다」고 적었다 — 아무도 대조하지 못한 채 양쪽이
> 공존했다. 실제로는 **검색으로 바로 나온다**(조세심판원 ID 112396).
>
> ### 되살리려면
>
> ①②③ 우열에 유리선택 여지가 있다는 **새로운 근거**(예규·정면 결정)가 필요하다. 그 전에는 착수 금지.
> 논점 자체는 지우지 않기 위해 본문을 남긴다.
>
> ### ✅ 살아 있는 것 — 폐기 대상이 아니다
>
> | | 상태 |
> |---|---|
> | **② §164 후보 추가**(제목 앞부분) | ✅ **#1080에서 완료** — `clauseA`의 `sec164` |
> | **decedent 청소**(CPI 분기 제거) | ✅ 완료 |
> | **§8.5 BLOCKER 1** — ②·③ 분자 시점 = 의제취득일 | 🟠 **별건 유효**(V-2). `…164-max.plan.md` §6 V-2로 승계 |
> | **§8.5 BLOCKER 2** — ②와 ③ 분자가 같은 값(`housePriceAtInheritanceUsed`)으로 얽힘 | 🟠 **별건 유효**. 가목 우선에서는 안전하지만(③ 미도달) 시점 논점은 남는다 — `inheritance-acquisition-helpers.ts:161-168` 주석 참조 |
> | **§8.5 BLOCKER 3** — pre-1990 토지 `useEstimatedAcquisition` 강제 | 🟠 **별건 유효**. `transfer-tax.ts:90` 실측 확인(2026-08-07) |
>
> 관련: [`…clause-a-b-separation.plan.md`](inheritance-pre-deemed-clause-a-b-separation.plan.md) ·
> [`…confirmation-criteria.engine.design.md`](pre-deemed-clause-a-confirmation-criteria.engine.design.md)(U2-E)

---

> ⚠️ **이하 본문은 2026-07-08 작성 시점 그대로다.** §2·§4.2의 유리선택 설계는 위 사유로 **폐기**됐다.
...
- ~~**gain-level 유리선택 = 법적 정당**(위법 아님): 대법원 2006두1326·**국심2003서3266**(상속토지에 가목단서 실지거래가액[의제취득일 §164④ 환산] + 실제 필요경비[철거비·이주보상비] 인정 = 납세자 유리 채택). "…있으나"=허용 표현. → 설계 개념 유효.~~
  - > 🔴 **2026-08-06 정정 — 이 항목은 「확증」이 아니다.** 근거 2건이 모두 무너졌다.
    > · **국심2003서3266**: ~~원문 미확보~~ → ✅ **2026-08-07 확보**(아래 실측). 내용이 **정반대**다.
    > · **대법원 2006두1326**: [`…clause-a-b-separation.plan.md`](inheritance-pre-deemed-clause-a-b-separation.plan.md) §6과 코드 주석은 이 판결을 **가목 ↔ 나목 배타(=유리선택 불가)** 의 근거로 쓴다. **같은 판결이 정반대 방향으로 인용되고 있다.**

    > ### ✅ **국심2003서3266 원문 실측 (2026-08-07)** — 유리선택 근거가 **아니다**
    >
    > 조세심판원 ID **112396** 「필요경비 해당 여부(경정)」, 의결 **2004.4.29.**
    > (종전 주석의 "원문을 확보하지 못했다"는 사실이 아니었다 — 검색으로 바로 나온다.)
    >
    > **사실관계**: 1969·1977 **공동상속**(pre-deemed) 토지를 2001 양도. 처분청이 §97①1호
    > **다목**(현행 나목)으로 **환산** 경정(2,313,478,921원).
    >
    > **판단**: 「쟁점토지의 취득가액을 같은조 제1항 제1호 **가목 단서**의 규정을 적용하여
    > **실지거래가액(의제취득일 현재 기준시가)**으로 함에 따라 같은조 제3항 **제1호**의 규정에 의해
    > … 필요경비로 인정할 수 있다」 ⇒ **환산을 배척하고 §163⑨(가목)을 적용**했다.
    >
    > ⚠️ **「유리해서」 고른 것이 아니다.** 이유는 **가목 단서 적용**이고, 실제 필요경비(철거비
    > 362,500,000 · 이주보상비 926,125,000)는 그 **결과**다(법 §97③1호가 가목 단서일 때 실제
    > 필요경비를 정한다). 「gain-level 유리선택」의 근거로 드는 것은 **오독**이다.
    >
    > ⇒ 이 결정은 §2를 뒷받침하지 않는다. **#1089(가목 우선)를 뒷받침한다.**
    > pre-deemed 정면 결정이 **국심2003부0627·0626 + 2003서3266 = 2건**이 됐고 **둘 다 같은 방향**이다.
    > ⇒ **§2 유리선택의 법적 근거는 남아 있지 않다.** 폐기 판단이 가능해졌다(위 🛑 참조).
