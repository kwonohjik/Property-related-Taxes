...
> ## 🛑 2026-08-06 — **착수 금지. §2 유리선택은 현행 구현과 정면 충돌한다**
>
> **② §164 후보 추가는 #1080에서 완료**됐다(`clauseA`의 `sec164`). 그러나 **§2 「양도차익 기준
> 유리선택」은 미구현**이고(`sec164ValueAtInheritance`·`actualNecessaryExpense`·
> `estimatedDeductionRate` 전부 코드 0건), **구현하면 #1089를 되돌리게 된다**.
>
> | | §2(본 계획) | #1089 `af74d907`(현행) |
> |---|---|---|
> | ③의 지위 | **후보** — 총공제가 크면 `취득가액 = ③` | **나목** — 가목이 0일 때만 도달 |
> | 기준 | 납세자에게 **유리한 쪽** | 법 §97①1호 단서 「가목을 확인할 수 없는 경우에 **한정**하여」 |
> | 근거 | 아래 §8.5 ✅(⚠️ 하나는 **원문 미확보**) | 명문 + **국심2003부0627·0626**(pre-deemed 정면 결정) |
>
> ⚠️ **§8.5의 「gain-level 유리선택 = 법적 정당」 근거 2건이 모두 흔들린다** — §8.5 정정 참조.
>
> ⇒ **폐기 여부는 별도 법령 판단**으로 남긴다(「①②③ 우열에 유리선택 여지가 있는가」라는 논점 자체를
> 지우지 않기 위해). 다만 **재검토 전에는 Do에 착수하지 말 것.**
>
> ### 🔵 **2026-08-07 갱신 — 폐기 쪽으로 기운다**
>
> §8.5의 근거 2건을 원문으로 재검증한 결과, **유리선택을 뒷받침하는 심판례·판례는 없다**:
> · **국심2003서3266**(원문 확보) — 「가목 단서 적용」이 이유이고 실제 필요경비는 그 **결과**다
> · **국심2003부0627·0626** — 환산 경정을 **취소**하고 §163⑨ 적용
> · **대법원 2006두1326** — 가목 ↔ 나목 **배타**
>
> ⇒ pre-deemed 정면 결정 **2건이 모두 「가목 우선」**이다. §2를 되살리려면 **새로운 근거**가 필요하다.
> 최종 폐기는 사용자 판단으로 남기되, 현재 상태에서 착수할 근거는 **없다**.
