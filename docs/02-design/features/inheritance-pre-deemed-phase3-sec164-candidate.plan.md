...
- ~~**gain-level 유리선택 = 법적 정당**(위법 아님): 대법원 2006두1326·**국심2003서3266**(상속토지에 가목단서 실지거래가액[의제취득일 §164④ 환산] + 실제 필요경비[철거비·이주보상비] 인정 = 납세자 유리 채택). "…있으나"=허용 표현. → 설계 개념 유효.~~
  - > 🔴 **2026-08-06 정정 — 이 항목은 「확증」이 아니다.** 근거 2건이 모두 무너졌다.
    > · **국심2003서3266**: `inheritance-acquisition-price.ts:76`이 **「원문을 확보하지 못했다」** 고 명시한다. 같은 주석 `:71`은 이 번호가 **오인용**임을 판정했다(실제는 국심2003부0627·0626). ⇒ **본문 미확인 근거로 세액 판정을 가를 수 없다**(memory `feedback_unverified_authority_blocks_tax_change`).
    ...
    > ⇒ 유리선택을 되살리려면 **먼저 이 두 인용부터 원문으로 재검증**할 것.
