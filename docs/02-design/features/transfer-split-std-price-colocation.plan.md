...
> ⚠️ **아래 주택 예외는 2026-07-30 폐기됐다** — KoreanLaw MCP로 §163⑥·§99①1호를 실측한 결과
> "라목 항등성" 논리가 **동시 취득 주택에만** 타당함이 확인됐다. §163⑥2호가목은 "라목의 주택
> **취득당시**의 라목 가액 × 3/100"을 규정하는데, 토지를 먼저 취득하고 건물을 나중에 신축·취득한
> 경우 **토지 취득 당시엔 주택이 없어** 라목 결합 공시가 존재하지 않는다. 그 경우 각 파트에
> §163⑥1호(토지: 취득당시 가목 개별공시지가 × 3/100)·2호(건물)가 **따로** 적용되므로
> **별개취득은 자산 종류와 무관하게 파트별 독립**이 정본이다. 결합 총액 역산은 오히려 건물분에
> **토지 취득시점**을 섞어 §164③(직전 고시분)을 위반한다.
> → 변경: 엔진 `calcAcqStdPair`·API 전송·validate V3/V6·UI 3개 게이트에서 `building` 제한 제거,
> 상단 결합 총액 블록은 별개취득이면 자산 종류 무관 읽기 전용 파생 표시로 전환.
> **동시 취득 주택은 종전 그대로**(라목 결합 공시가 실재 → 역산이 정본, 회귀 가드 존치).

| 자산 (별개취득) | 건물 섹션 취득시 | 건물 섹션 양도시 |
|---|---|---|
| 주택(housing) | **입력 칸 + 「취득시 건물 기준시가 계산」 런처**(2026-07-30 전환) | **신규 노출** |
| 일반건물(building) | 계산서 있음(현행) | **신규 노출** → 요구대로 "취득시 + 양도시" 둘 다 |

<details><summary>초안(폐기) — 주택 라목 역산 예외</summary>
...
**최종 동작(2026-07-30 확정)**: 별개취득이면 자산 종류와 무관하게
① 상단 결합 총액 블록 → 읽기 전용 파생 표시(`SplitAcqStdReadonlyPanel`),
② 건물 섹션에 **취득시 건물기준시가 입력 칸 + 「취득시 건물 기준시가 계산」 런처**(`applyTimePoint="acquisition"`
→ 모달이 `acqOnly` 모드로 취득당시 구조·용도·공시지가를 표시하고 양도 시점은 숨긴다),
③ 환산이면 **양도시 건물 기준시가 카드**도 같은 섹션에 → 이미지 10·11 요구 충족.
`calcDerivedBuildingStdAtAcq`는 **동시 취득 역산 경로**의 단일 소스로 유지된다.
