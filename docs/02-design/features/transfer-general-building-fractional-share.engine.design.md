...
⚠️ **같은 뿌리가 UI 계층에 남아 있다 — 이 PR에서 고치지 않았다.** `#k`를 모르는 정확 비교:

- `components/calc/results/transfer/DetailedStatementFormulaBuilders.ts` — `isLandProp()`(:41~48)
  외 `p.propertyId === "land" || "land_business" || "land_nbl"` 인라인 비교 **8곳**
  ⇒ 지분 분할 결과 화면에서 자산별 산식이 **표시되지 않는다**(값이 틀리는 방향은 아니다).
- `components/calc/results/transfer/FilingFormTableAggregateHelpers.ts:62` — 같은 비교로
  `formData.assets[0]` 메타를 찾는다. 실패 시 `assetId` 직접 조회 → `basePid`는
  `__(appurtenant|excess)` 접미사만 벗기므로 `#k`는 못 벗긴다 ⇒ 신고서 행 메타 누락.

두 곳 모두 **자체 anchor·E2E가 필요한 표시 계층**이라 별건으로 남긴다.
