...
| C-04 | 2지분: 60% 매매 + 40% **증여** | 영 §163⑨ · 단서 max | GBF-23 | ✅ |
| C-05 | 3지분 (1/3 × 3) — floor 잔액 흡수 | Σ = 총양도가 불변식 | GBF-03 | ✅ |
| C-06 | 지분 × **토지·건물 취득일 분리**(M-1a) | 영 §166⑥ · 법 §95④ | GBF-24 | ✅ |
...
| C-11 | 지분 × §99-164-10 최초공시 — 지분별 취득기준시가로 **산식이 자동 분기** | 집행기준 99-164-10 | GBF-25 | ✅ |
| C-12 | 지분 × **NBL 부수토지 초과** — 지분마다 토지 카드 2장 | 지방세령 §101①2호·② | GBF-26 | ✅ 🔴 |
| C-13 | **§97②2호 단서 × 혼합 지분** — 단서는 **환산 지분에만** | 법 §97②2호 단서 (계획 §3-4) | GBF-11 | ✅ |
| C-14 | 지분 × **이월과세**(토지 파트) | 법 §97의2 | GBF-27 | ✅ ⚠️UI부재 |
| C-15 | 개산공제 **지분율 축소** — `ownershipRatio` 도달 | 영 §163⑥ | GBF-04 | ✅ |
| C-16 | **파트별 실가 취득가액 × 지분율 스케일** | 계획 §3-5 | GBF-05 | ✅ |
| C-17 | **차단**: 지분율 합계 ≠ 100% | 기존 `transfer-tax-validate.ts:161~173` 재사용 | GBF-15 | ✅ |
| C-18 | **차단**: 상속 지분에 환산·감정·매매사례 선택 (**지분 인덱스**로 보고) | 법 §97①1호 단서·영 §163⑨ | GBF-12 | ✅ |
| C-19 | **차단**: 지분 분할 × 부담부증여·공익수용 (범위 밖 유지) | 계획 §2-2 | GBF-14 | ✅ |
...
### 2026-08-10 잔여 anchor 착지 — C-04·C-06·C-11·C-12·C-14

PR #1161이 「공용 경로를 그대로 타므로 동작할 것으로 보지만 **검증하지 않았다**」고 명시하고
남긴 5행을 `__tests__/api/transfer.route.gb-fractional-cases.anchor.test.ts`(25건)로 닫았다.
4행은 서술대로 동작했고, **C-12에서 표시 드리프트 1건이 나왔다.**

🔴 **C-12 — `buildApportionment`의 지분 접미사 미인식** (정정 완료)

`card.propertyId === "land_business"`로 비교하는데 지분 카드는 `land_business#0`이라 **항상
false** → 사업용 토지 카드가 비사업용 비율로 표시됐다. 실측(바닥면적 20㎡·일반주거 4배·초과 0.2):

| 항목 | 정정 전 | 정정 후 |
|---|---|---|
| `land_business#0` 기준시가 | 40,000,000 | **160,000,000** |
| `land_business#0` displayRatio | 0.1 | **0.4** |
| 지분당 displayRatio 합 | 0.7 | **1.0** |

세액은 이 값을 쓰지 않아 **표시만** 틀렸다(메모리 `feedback_engine_result_display_drift`).
`general-building-route-cards.ts`에 `SHARE_ID_SEPARATOR`·`baseCardId()`를 두고 접미사를 벗겨
비교하도록 정정했다. `general-building-fractional.ts`의 `tagId`도 같은 상수를 쓴다.

### ✅ UI 계층의 같은 뿌리 — 후속 정정 완료 (2026-08-10)

위 라우트 정정 직후 조사에서 **같은 정확 비교가 UI 계층에도** 있었다. 증상은 라우트와 달랐다:

| 위치 | 증상 |
|---|---|
| `DetailedStatementFormulaBuilders.ts` — `isLandProp()` 외 인라인 비교 8곳 | 자산별 산식 빌더가 전부 `undefined`를 돌려 **화면에 산식이 안 뜬다** |
| `FilingFormTableAggregateHelpers.ts` — `findAssetByPropertyId` | `land#1`이 어느 분기에도 안 걸려 **신고서 행 메타(취득일 등) 누락** |
| `FilingFormTableHelpers.ts` — `getAcqDateForCard` | `building2#0`이 default로 떨어져 **증축일 대신 원건물 취득일** 표시 |

정정 요지:

1. **규약을 leaf로 승격** — `lib/tax-engine/general-building-share-id.ts`(무의존)에 `SHARE_ID_SEPARATOR`·
   `baseCardId`·`shareIndexOf`·`isSameShare`. `use client` 컴포넌트가 직접 import한다.
   `general-building-route-cards.ts`는 하위 호환 재수출만 남겼다.
2. **인라인 3-way OR 체인을 헬퍼로 흡수** — 중복이 드리프트의 원인이었으므로 비교를 한 곳으로 모았다.
3. 🔑 **신고서는 `assets[0]`이 아니라 `assets[shareIndexOf(pid)]`** — 접미사 인덱스가 곧 `assets`의
   인덱스다(`transfer-tax-api-gb-shares.ts:168`이 같은 배열을 같은 순서로 순회해 태깅). 지분마다
   취득일이 다른 것이 이 기능의 존재 이유라, 전부 `assets[0]`로 보내면 신고서 취득일이 **전 행 동일하게 틀린다**.
4. 🔑 **짝 카드는 `isSameShare`로 같은 지분 안에서** — base id만 보고 `find`하면 지분 0의 건물 카드가
   지분 1의 토지 산식에 끌려 들어간다(일괄 실가 취득가 안분: 지분마다 일괄 취득가가 다르다).
   이건 「산식이 안 뜬다」가 아니라 **틀린 숫자가 뜨는** 방향이라 더 위험하다.

anchor: `__tests__/lib/general-building-share-id.anchor.test.ts`(10건) ·
`__tests__/components/gb-fractional-display-share-suffix.anchor.test.ts`(12건) ·
E2E `GBF-28`(mutation probe로 「고치기 전엔 실패한다」 확인).
