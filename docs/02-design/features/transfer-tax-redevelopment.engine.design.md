...
본 설계는 예제 11개 사례(36~46) + PDF 6페이지(제1~8절) 학습 결과를 바탕으로 매트릭스 전체를 단일 엔진 모듈로 구현한다. UI는 사례 44만 노출하되 엔진 일반화로 후속 PR 확장이 가능하도록 한다.
...
|---|---|---|---|---|---|
| 1 | 사례 36 — 입주권 양도 / 실가 / 납부 | **시행령 §166①1호** + 소법 §95②·§166⑤1호, 도정법 §74 | 예제 xlsx `권리-실가-납부` | `__tests__/tax-engine/transfer/redevelopment.spec.ts` | ☐ TODO |
| 2 | 사례 37 — 입주권 양도 / 환산 / 납부 | **시행령 §166①1호·§166③** + §176의2②2호 + §95② 단서 | 예제 xlsx `권리-환산-납부` | 동일 | ☐ TODO |
| 3 | 사례 38 — 입주권 양도 / 실가 / 수령 | **시행령 §166①2호** + §95②·§166⑤1호 | 예제 xlsx `권리-실가-수령` | 동일 | ☐ TODO |
| 4 | 사례 39 — 입주권 양도 / 환산 / 수령 | **시행령 §166①2호·§166③** + §176의2②2호 | 예제 xlsx `권리-환산-수령` | 동일 | ☐ TODO |
| 5 | 사례 40 — 완공 APT 양도 / 토지출자 / 실가 / 납부 | **시행령 §166②1호** + §166⑤2호 | 예제 xlsx `APT-실가-납부` | 동일 | ☐ TODO |
| 6 | 사례 41 — 완공 APT 양도 / 토지출자 / 환산 / 납부 | **시행령 §166②1호·§166③** + §176의2②2호 | 예제 xlsx `APT-환산-납부` | 동일 | ☐ TODO |
| 7 | 사례 42 — 완공 APT 양도 / 토지출자 / 실가 / 수령 | **시행령 §166②2호·§166①2호 + 분할양도(§88·§95①)** | ✅ **종결 (2026-08-27)** — 예제 자료가 청산금분 68,400,000을 누락. 국세청 해석 3건(재일46014-2870·2104·법규재산2012-358)으로 정정 | total.gain 370,600,000 → **439,000,000** | ✅ DONE |
| 8 | 사례 43 — 완공 APT 양도 / 토지출자 / 환산 / 수령 | **시행령 §166②2호·§166③** + 분할양도(§88·§95①·§163⑥) | ✅ **종결 (2026-08-27)** — 엔진은 이미 정확했고 ⑧ validate가 **오차단**하고 있었다. xlsx 시트가 없어 기대값은 **조문에서 직접 도출**(사례 42 선례) | 나목 268,810,400 · 가목 139,000,000 · 청산금분 79,389,600 | ✅ DONE |
| 9 | **사례 44 — 완공 APT 양도 / 주택출자 / 환산 / 납부** ★ UI 구현 대상 | **시행령 §166②1호·§166③·§166⑤2호나목** + §176의2②2호 + §164⑦ 단서 | 예제 xlsx `APT-환산-납부-주택출자` — 산출 56,799,400 / 지방 5,679,940 / 합 62,479,340 | 동일 (필수 9개 toBe anchor) | ☐ TODO |
| 10 | 사례 45 — 완공 APT 양도 / 주택출자 / 실가 / 납부 (1세대1주택 12억 안분 + LTHD 표2) | 소법 §89①3호 단서 (12억 초과 고가주택) + **소법 §95③·시행령 §160** (12억 안분 위임 산식) + §95② 표2 | 예제 xlsx `APT-실가-납부-주택출자` | 동일 | ☐ TODO |
| 11 | 사례 46 — 완공 APT 양도 / 주택출자 / 실가 / 수령 — **본 PR 범위: 과세 산출 anchor만 등록 (§166②2호·①2호 산식 검증)**. 시행령 §154 비과세 미달 자체 적용 로직은 후속 PR | **시행령 §166②2호·§166①2호** + 소법 §95④ + NTS 집행기준 "settlementSaleDate = 소유권이전 고시일 다음날" | 예제 xlsx `APT-실가-수령-주택출자` | 동일 | ☐ TODO |
...
소득세법 시행령 §164⑦ (원문 — 사용자 검토 반영 정정):
  ...
  ★ 단서 트리거 정정 (이전 안 폐기):
    - ❌ 이전 안 "취득시 기준시가 ≤ 취득전기 기준시가" (예제 xlsx 운영팁이지 시행령 본문 아님)
...
- 사례 38·39·42·43 예제 xlsx 보유기간 종료일 컬럼 매핑 사전 확인 필요
...
- `__tests__/tax-engine/transfer/redevelopment/_helpers.ts` — 공통 fixture (예제 xlsx 입력 데이터 표준화)
...
**보류 (snapshot only)**: ~~사례 42~~·43 — 후속 PR에서 국세청 해석례·집행기준 추가 자료로 확정 후 등록.

> ✅ **사례 42는 2026-08-27 종결**. 예정대로 **국세청 해석례**가 답이었다 —
> 재일46014-2870(1997, 「토지 등」)·재일46014-2104(1999, 「토지·건물」)·법규재산2012-358(2012).
> 청산금 상당분은 **분할양도**로 과세되며 **자산 종류를 가리지 않는다**(§166①이 「건물 또는
> 토지만을 제공한 경우를 포함한다」를 명시).
> ⚠️ 이 행의 「snapshot only」 기재는 **부정확했다** — 실제 테스트는 `toBe`로 settlement=0을
> 고정하고 있었다(즉 보류가 아니라 **틀린 값을 못박고 있었다**).
>
> ✅ **사례 43도 2026-08-27 종결**. 다만 함정은 **달랐다** — 테스트 파일 자체가 없었고
> ⑧ validate가 계산을 막고 있었다(못박을 값이 없었다). 「미구현」이라던 주석의 진단도 틀렸다:
> `runLandContribEstimated`는 `subject === "right"` 전용이라 완공APT는 애초에 그 경로가 아니다.
> ⇒ **「보류/HOLD」 표기는 매번 실측해야 한다 — 함정의 모양이 행마다 다르다.**
