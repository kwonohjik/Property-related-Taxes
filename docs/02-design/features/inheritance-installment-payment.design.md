...
### 4.4-B 모드 B(grace10) 정밀 검증 결과 (2026-06-04)

KoreanLaw 조회: 국세청 해석례 본문은 OPEN API 미제공·공식 PDF 추출 실패 → **법령 텍스트로 확정**:
- **현행 §71②1가** = "허가일부터 **20년** 또는 허가 후 10년이 되는 날부터 **10년**"(단순). 웹의 "3년거치7년/5년거치15년·50% 기준"은 **개정 전 舊법** — 현행 미적용.
- **§68①1호 산식** = `대상금액 ÷ (연부연납기간 + 1)`. 모드 B 연부연납기간 = **10** → **÷11**. (3호 모드A: 신고기한 anchor가 "+1"=21회 ↔ 1호 모드B: "허가 후 10년이 되는 날" anchor가 "+1"=11회)
- **정정 적용**: 거치 회차 **0~9**(10년, 가산금만) + 원금 분할 회차 **10~20**(11회, 각 ÷11, 마지막 잔액 흡수). (현행 코드 `÷10`·거치 0~10·납부 11~20 → **법령 부정합 결함** 수정)
- **잔여 caveat(미해결)**: 거치기간 가산금을 (실무) **매년 이자 계상**으로 구현. §72 본문 문리(처음 분할납부세액에 신고기한~첫 납부 일수 합산 = 첫 회 일괄)와 다를 수 있음 → 결과 notes + 세무사 확인 안내로 노출. 공식 워크드 예시 확보 시 재확정.
- anchor: **INS-12**(거치 0~9 원금0·첫 원금 10년차·÷11·합계 전액) · **INS-13**(거치 가산금 매년 계상) · **INS-14**(최대 회차 20).
...
- **확인 필요 ②** ✅ **해결(2026-06-04)** — `decedentType: resident|non_resident` 필드 존재 확인 → 비거주자 신고기한 **9개월**(§67④) 구현. 카드 `filingMonths = non_resident ? 9 : 6` + 안내 note. anchor INS-16 · E2E INST-E2E-3.
- **확인 필요 ③** ✅ 가산금 회차 단위 절사 = 소구간 합산 후 1회 floor(BigInt). anchor 동결.
- **확인 필요 ④** API 미경유(결과뷰 계산) 채택 — 이력 영속 요구 시 result 확장(미발생).
- **확인 필요 ⑤** ✅ **해결(2026-06-04)** — §68② 분모 "총상속재산가액" = `result.grossEstateValue`(= `valuationResults` 합 = **본래+간주 상속재산 평가액**, `inheritance-tax.ts:305` 주석). **추정상속(presumedTotal)·사전증여·채무는 제외** — 가업상속재산가액(§68④)도 동일하게 채무 차감 전 평가액이라 분자·분모 정합. 가업상속공제액은 양쪽 차감(교재 사례 나 산식).
- **확인 필요 ⑥** ✅ **해결(2026-06-04)** — §68① 단서 "매년 1천만원 초과"는 **"각 호"별**(1호 가업거치·3호 일반/모드A) 적용 → **분별 독립 판정** 법령 정합. 일반분은 자동 단축, 가업분(기간 법정 고정)은 1회 ≤1천만 시 경고 note(자동 단축 안 함).
- **확인 필요 (가산율 시행일)** ✅ **해결(2026-06-04)** — 현행 가산율 **연 3.1%(1천분의 31)** 국기칙 §19의3(시행 2026.03.20) 검증. 과거 시행일 경계는 §19의3 개정 이력(교재 연혁표)로 동결. anchor INS-15.
- **잔여 미해결** — 모드 B 거치기간 가산금 §72 본문 문리(첫 회 일괄) vs 실무(매년 이자) — 공식 워크드 예시 미확보(국세청 해석례 본문 API 미제공·PDF 추출 실패). 현재 실무 기준 계상 + notes 경고. 공식 예시 확보 시 재확정.
