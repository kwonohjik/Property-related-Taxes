# §164④·⑥·⑤~⑦ 부분 입력이 조용히 무시된다 (all-or-nothing opt-in의 침묵 실패)

> **상태**: ✅ **구현 완료** (2026-08-06) — anchor 22건 · E2E 전건 919 통과 · 회귀 0
> **세목**: 양도소득세 — 「소득세법 시행령」 §163⑨1호·2호 · §164④·⑤·⑥·⑦
> **선행**: [`gift-163-9-clause-1-2-max.plan.md`](gift-163-9-clause-1-2-max.plan.md) §12(후속으로 기록) · #1103

---

## 1. 한 줄 요약

§163⑨1호·2호의 **②(§164④~⑦ 취득당시 기준시가)** 는 필수 필드가 **모두** 채워져야 payload가 생성된다.
일부만 입력하면 빌더가 `{}`를 반환해 **②가 계산에서 사라지고 ① 단독으로 계산되는데, 아무 경고도 없다**.

사용자에게는 **"입력했는데 반영되지 않는"** 상태다. 화면·결과 어디에도 그 사실이 드러나지 않는다.

---

## 2. 법령 — ②는 선택이 아니라 max의 한쪽이다

| 조항 | 문언 |
|---|---|
| **§163⑨1호** | "1990년 8월 30일 개별공시지가가 고시되기 전에 상속 또는 증여받은 **토지** … 평가한 가액과 **제164조제4항의 규정에 의한 가액 중 많은 금액**" |
| **§163⑨2호** | "…기준시가가 고시되기 전에 상속 또는 증여받은 **건물** … 평가한 가액과 **제164조제5항 내지 제7항의 규정에 의한 가액 중 많은 금액**" |

⇒ ②는 **비교 대상 자체**다. 누락되면 max의 한쪽이 사라져 **취득가액이 과소 산정**될 수 있다
(②가 ①보다 큰 경우). 「법령상 요구되는 비교를 수행하지 않는 것」이므로 정확성 문제다.

⚠️ **세액 방향으로 정당화하지 않는다** — ②가 크면 취득가액이 늘어 납세자에게 유리하지만,
판단 기준은 **법규정 정합**이다(memory `feedback_no_unfavorable_application_without_legal_basis`의 역방향도 같다).

---

## 3. 실태 — 실측 매트릭스 (2026-08-06 probe)

throwaway probe로 각 경로에 **필수 필드를 1개만 비워** 넣고 payload·validate를 동시에 측정했다.

> 🔴 **1차 측정을 정정했다.** 토지 3케이스를 `pre1990Enabled: false`로 넣었는데 **그 상태는 UI로
> 만들 수 없다** — 등급 입력칸은 `Pre1990LandValuationInput`의 자체 토글(`:177`)을 켜야 보이고,
> 켜는 순간 `pre1990Enabled = true`가 된다. 아래는 **토글 ON(실제 도달 가능 상태)** 재측정이다.

| # | 경로 | 취득원인 | payload | validate | 판정 |
|---|---|---|---|---|---|
| 1 | 주택 §164⑤~⑦ | 상속(1998) | `{}` | `null` | 🔴 **침묵** |
| 1b | 주택 §164⑤~⑦ | **증여**(1998) | `{}` | `null` | 🔴 **침묵** |
| 2 | 상가 §164⑥ | 상속 | `{}` | **에러** | ✅ 차단 |
| 3 | 상가 §164⑥ | **증여** | `{}` | `null` | 🔴 **침묵** |
| 4 | 토지 §164④ | 상속(1987) | `{}` | **에러** | ✅ 차단 |
| 5 | 토지 §164④ | **증여 post-1985**(1987) | `{}` | `null` | 🔴 **침묵** |
| 6 | 토지 §164④ | 증여 pre-1985(1980) | `{}` | **에러** | ✅ 차단 |

⇒ **침묵은 3곳**이다: **주택(상속·증여) · 상가 증여 · 토지 증여 post-1985**.

**토지가 대체로 차단되는 이유**는 `pre1990Enabled` 토글이 §163⑨ 경로에서도 켜지기 때문이다 —
`hasPre1990`(`lib/calc/transfer-tax-api.ts:103` · `lib/calc/transfer-tax-validate-asset.ts:383`)이
참이 되어 기존 환산 검증(`transfer-tax-validate-asset.ts:468-486` 등급 3종·면적·1990가·양도시
기준시가 필수)이 **부수적으로** §163⑨ 부분입력까지 막는다.

**유일한 구멍이 gift + post-1985**인 것은 `hasPre1990` 정의가 그 조합을 **명시적으로 배제**하기
때문이다(`!(gift && acquisitionDate >= "1985-01-01")`). #1103이 `hasPre1990ForSec164`로 payload는
보내게 했으므로 **payload는 가는데 validate는 안 하는 비대칭**이 남았다 — 이 PR이 만든 갭이다.

⚠️ **차단이 되고 있어도 "설계된 차단"은 아니다** — 환산 모드 필수 검증에 **우연히 얹힌** 것이라,
`hasPre1990` 정의가 바뀌면 조용히 사라진다. §5의 단일 소스화는 이 의존도 끊는다.

### 3.1 빌더별 필수 필드 (실측 file:line)

| 경로 | 필수 필드 | 조기 반환 |
|---|---|---|
| **주택** `buildInheritedHouseValuationPayload` | `inhHouseValLandArea` · `…LandPricePerSqmAtTransfer` · `…LandPricePerSqmAtFirst` · `…HousePriceAtFirst` (4) | `transfer-tax-api-inheritance.ts:177` |
| ↳ 추가 조건 | 1990.8.30. **前**: 등급 3종 + `pre1990PricePerSqm_1990` **또는** `…AtInheritance`<br>1990.8.30. **後**: `…LandPricePerSqmAtInheritance` | `:206` · `:207` |
| **상가** `buildCommercialInheritanceValuationPayload` | 면적 3(`cbExclusiveArea`·`cbSharedArea`·`cbLandArea`) + 금액 5(`cbUnitPriceAtFirstOrAcq`·`cbLandPricePerSqmAtAcq`·`cbLandPricePerSqmAtFirst`·`cbBuildingStdPriceAtAcq`·`cbBuildingStdPriceAtFirst`) = **8** | `:128-139` |
| **토지** `buildPre1990LandPayload` | 등급 3종 + `acquisitionArea` + `pre1990PricePerSqm_1990` = **5** | `transfer-tax-api-helpers.ts:400` |

⚠️ **주택은 total이 가변이다** — 기준일이 1990.8.30. 前後냐에 따라 필요 필드가 달라지고,
前이면 「등급 3종+1990가」와 「상속개시일 단가」가 **택일**이다. 단순 카운트로 판정할 수 없다(§5.2).

### 3.2 유일한 차단(상속 상가)이 어떻게 되어 있나

`transfer-tax-validate-asset.ts:124-140`

```ts
const inhDate164 = asset.inheritanceStartDate || asset.acquisitionDate || "";
if (inhDate164 && inhDate164 < "2005-01-01") {
  const filled = areas164.filter(f => parseDecimal(f) > 0).length
               + amounts164.filter(f => parseAmount(f) > 0).length;
  if (filled > 0 && filled < 8) return `${label}: §164⑥ … 8개 항목을 모두 입력하거나 모두 비워두세요.`;
}
```

**`filled > 0 && filled < 8`** — "하나라도 손댔으면 끝까지" 규약. 이 패턴이 정본이다.

세 가지 문제가 함께 있다:

1. **상속 전용 블록 안에 있다** — `:117`이 `acquisitionCause === "inheritance"`로 게이트한다.
   #1103이 상가 §164⑥ **입력 UI를 증여에 열었지만 validate는 따라가지 않았다**(경로 3).
2. **필드 목록이 validate에 하드코딩됐다** — 빌더(`:128-139`)와 **따로 관리**된다. 빌더에 필드가
   늘면 validate의 `8`과 배열이 조용히 낡는다.
3. **기준일 파생이 인라인이다**(`:126`) — #1103이 도입한 `deriveSec163_9BaseDate`를 쓰지 않아,
   취득원인을 상속→증여로 바꾼 자산의 stale `inheritanceStartDate`에서 UI·API와 어긋난다.

---

## 4. 갭 정리

| # | 대상 | 현행 | 기대 |
|---|---|---|---|
| **P-1** | 주택 §164⑤~⑦ (상속·증여) | 침묵 | 부분 입력 시 차단 |
| **P-2** | 상가 §164⑥ **증여** | 침묵 | 상속과 동일 차단 |
| **P-3** | 토지 §164④ **증여 post-1985** | 침묵 | 부분 입력 시 차단 |
| **P-4** | 필드 목록 이중 관리 | 빌더·validate 각자 | **단일 소스** 공유 |
| **P-5** | 상가 차단의 기준일 파생 | 인라인(`:126`) | `deriveSec163_9BaseDate` |
| **P-6** | 토지 차단이 `hasPre1990`(환산 검증)에 **얹혀 있다** | 우연한 커버 | §163⑨ 자체 검사로 독립 |

**범위 축소**: 1차 계획의 「토지 상속·증여 전부 침묵」은 **오측정이었다**. 토지에서 신규 차단이
필요한 것은 **증여 post-1985 한 조합**이고, 나머지는 P-6(의존 제거) 성격이다.

---

## 5. 설계

### 5.1 A안(권고) — 필드 그룹을 단일 소스로 추출하고 빌더·validate가 공유

신규 `lib/calc/sec164-required-fields.ts`:

```ts
/** 한 경로의 opt-in 충족 상태. total은 조합에 따라 가변이다(주택). */
export interface Sec164FieldStatus {
  filled: number;
  total: number;
  /** 사용자에게 보일 항목 이름들 — 에러 메시지에 그대로 쓴다 */
  missing: string[];
}
export function sec164HouseStatus(asset: AssetForm): Sec164FieldStatus | null;
export function sec164CommercialStatus(asset: AssetForm): Sec164FieldStatus | null;
export function sec164LandStatus(asset: AssetForm): Sec164FieldStatus | null;
```

- **빌더**는 `status.filled === status.total`일 때만 payload를 만든다(현행 조기 반환을 이 함수로 대체).
- **validate**는 `0 < filled < total`이면 차단하고 `missing`을 메시지에 넣는다.
- `null` 반환 = 그 경로가 애초에 대상이 아님(자산종류·취득원인·기간 밖) → 검사도 payload도 없음.

> 🔴 **Do deviation (2026-08-06) — 토지 빌더는 교체하지 않았다.**
> `buildPre1990LandPayload`는 §163⑨ 경로뿐 아니라 **매매 환산 모드**(`hasPre1990`)에서도 호출된다.
> `sec164LandStatus`는 §163⑨ 대상(상속·증여)만 판정하므로 교체하면 **매매 경로의 payload가
> 사라진다**. ⇒ 주택·상가 빌더 2곳만 교체하고, 토지는 validate 쪽에서만 단일 소스를 쓴다.
> P-4(이중 관리)는 주택·상가에서 해소되고 토지는 **검사 쪽만** 단일화된다.

⇒ **필드가 늘어도 한 곳만 고치면 양쪽이 따라온다.** 상속 상가의 하드코딩 `8`이 사라진다(P-4).

⚠️ 대상 판정은 **`isSec163_9Cause`·`deriveSec163_9BaseDate`를 그대로 쓴다**(P-5) — #1103이 만든
단일 소스. 여기서 다시 파생하면 같은 병이 재발한다
(memory `feedback_shared_predicate_argument_parity`).

**⚠️ client-safe여야 한다** — §5.5의 UI 안내가 `total`을 받아 쓰므로 이 모듈은 `use client`
컴포넌트에서 import된다. `lib/calc/`는 이미 UI가 쓰는 계층이고(`transfer-163-9-base-date.ts`가
`CommercialInheritanceStdPriceSection`에서 import된다), 엔진(`lib/tax-engine/`)을 끌어오지 않는다.

#### 5.1.1 validate 배치 — 세 경로가 각각 어디에 붙나 (P-2 핵심)

현행 상가 차단은 `transfer-tax-validate-asset.ts:117`의 **상속 전용 블록 안**에 있어, #1103이 연
증여 경로는 그 블록에 **들어가지도 않는다**. 단순히 조건을 넓히면 그 블록의 다른 검증
(피상속인 취득일·상속개시일 평가액 필수)까지 증여에 적용되어 **오차단**된다.

⇒ **§164 부분입력 검사를 그 블록에서 떼어내 독립 함수로 만든다**:

```ts
// lib/calc/transfer-tax-validate-sec164.ts (신규)
export function sec164PartialInputError(asset: AssetForm, label: string): string | null;
```

- `validateAssetAcquisition` 진입부(상속 상가 블록 `:117`보다 **앞**)에서 1회 호출한다 —
  자산종류·취득원인 분기는 `sec164*Status`가 `null`로 처리하므로 호출부는 분기하지 않는다.
- 상가 상속의 기존 메시지는 이 함수가 그대로 생성한다(C-5 회귀).
- **토지는 `!hasPre1990`일 때만** 검사한다(U-3) — 환산 검증이 이미 막는 구간의 메시지를 보존한다.

### 5.2 주택의 가변 total — 카운트만으로 판정하지 않는다

기준일 1990.8.30. **前**에는 ③의 분자 경로가 둘이라 **택일**이다:

```
필수 4  +  ( 등급3종 + 1990㎡당가  OR  상속개시일 단가 )
```

⇒ `total`을 단일 숫자로 두면 "등급만 넣은 사용자"와 "단가만 넣은 사용자" 중 한쪽을 잘못 차단한다.
**택일 그룹을 별도 타입으로 표현**한다:

```ts
type FieldGroup =
  | { kind: "all"; fields: FieldSpec[] }
  | { kind: "oneOf"; groups: FieldGroup[] };   // 하나의 하위 그룹이 완성되면 충족
```

**판정 규칙(§7 U-2와 동일 — 빌더 기준)**:

1. **완성된 하위 그룹이 하나라도 있으면 충족**(`filled === total`로 본다) — 다른 그룹의 부분 입력은
   무시한다. 빌더(`:206`)가 정확히 그렇게 동작하므로 validate가 더 엄격하면 **"칸은 다 있는데
   차단"**이 된다(memory `feedback_validation_sync_8th_point`).
2. 어느 그룹도 완성되지 않았을 때만 **손댄 그룹**을 기준으로 `filled/total`·`missing`을 계산해
   누락 항목을 안내한다. 둘 다 손댔으면 `filled`가 큰 쪽을 기준으로 한다.

⚠️ 1과 2의 순서가 뒤바뀌면 **C-4b(등급 2/3 + 단가 완성)에서 잘못 차단**한다.

### 5.3 B안 — validate에만 검사 추가 (빌더는 그대로)

가장 작은 변경이지만 **P-4가 남는다**(이중 관리). 상속 상가가 이미 그 상태이고, 빌더가 바뀌면
validate가 조용히 낡는다. **권고하지 않는다.**

### 5.4 채택하지 않는 것 — "부분 입력이면 남은 값을 추정해 채운다"

「자동 안분 fallback 금지」 정책 위반이다(memory `feedback_no_silent_apportion_fallback`).
기준시가는 고시된 사실값이라 추정 대상이 아니다.

### 5.5 UI 안내 (U-1 결정: **상시 안내 추가**)

**신규 카드가 아니라 기존 문구 정정**이다 — 안내는 이미 있으나 **부정확**하다.

| 위치 | 현행 문구 | 문제 |
|---|---|---|
| `GiftHouseStdPriceSection:64` | "아래를 **입력한 경우에만** 비교하며, 미입력 시 증여 신고가액만 사용합니다" | "일부만 입력"도 되는 것처럼 읽힌다 |
| `GiftLandStdPriceSection:63` | "**등급을 입력한 경우에만** 비교하며…" | 동일 |
| `CommercialInheritanceStdPriceSection:84` | "아래 3시점 **입력 시에만** 적용되며…" | 동일 |
| `PostDeemedInputs:332`·`:356` | "…큰 금액으로 자동 적용됩니다 (§163⑨1·2호)" | opt-in·all-or-nothing 언급 **없음** |

⇒ **「모두」와 「일부만 입력하면 차단된다」를 명시**한다. 예:

> 아래 항목을 **모두** 입력한 경우에만 비교합니다. 전부 비워두면 {신고가액}만 사용하고,
> **일부만 입력하면 계산할 때 오류로 안내**합니다.

- 개수(`8개`)는 문구에 **하드코딩하지 않는다** — §5.1 `Sec164FieldStatus.total`에서 받는다(P-4).
  각 섹션 컴포넌트가 `sec164*Status(asset)`를 호출해 `total`을 얻는다(§5.1 client-safe 전제).
  `null`이면 그 섹션은 애초에 렌더되지 않으므로 안내도 없다.
- 상속 경로(`PostDeemedInputs`)에도 같은 취지를 넣어 4곳의 안내를 정렬한다.

⚠️ **진행 카운트("3/8 입력됨")는 채택하지 않았다** — 구현·테스트 범위가 커지고, 차단 메시지가
누락 항목(`missing`)을 이미 알려주므로 한계효용이 낮다.

### 5.6 결과 화면 (U-4 결정: **표시하지 않음**)

전부 비운 경우(② 미적용)는 **사용자의 명시적 선택**이고 입력 카드에 안내가 있다(§5.5).
부분 입력은 차단되어 결과에 도달하지 않는다. ⇒ 결과 화면 변경 **없음**.

---

## 6. 케이스 매트릭스

| # | 자산 | 취득원인 | 입력 상태 | 기대 |
|---|---|---|---|---|
| **C-1** | 주택 | 상속 | 4필수 중 3 | **차단** (누락 항목 명시) |
| **C-2** | 주택 | 증여 | 4필수 중 3 | **차단** |
| **C-3** | 주택 | 상속 | 4필수 + 등급 3종 중 2 (1990 前) | **차단** |
| **C-4** | 주택 | 상속 | 4필수 + **상속개시일 단가만** (1990 前) | **통과** — 택일 충족 |
| **C-4b** | 주택 | 상속 | 4필수 + **단가 완성 + 등급 2/3** (1990 前) | **통과** — 완성 그룹 우선(§5.2 규칙 1). 미완성 등급은 무시 |
| **C-5** | 상가 | 상속 | 8 중 7 | 차단 (**현행 유지** — 회귀 0) |
| **C-6** | 상가 | **증여** | 8 중 7 | **차단** (신규) |
| **C-7** | 토지 | **증여 post-1985** | 등급 2/3 | **차단** (신규 — 유일한 구멍) |
| **C-8(회귀)** | 토지 | 상속 | 등급 2/3 | 차단 — **현행 유지**(`hasPre1990` 경유) |
| **C-8b(회귀)** | 토지 | 증여 pre-1985 | 등급 2/3 | 차단 — **현행 유지** |
| **C-9(회귀)** | 전 자산 | — | **전부 비움** | 통과 — ① 단독은 정상 경로 |
| **C-10(회귀)** | 전 자산 | — | **전부 채움** | 통과 + ② 비교 수행 |
| **C-11(경계)** | 토지 | 매매 | 환산 모드 등급 2/3 | 현행 `hasPre1990` 검사 유지(§163⑨ 아님) |
| **C-12(경계)** | 상가 | 이월과세 | 8 중 7 | 검사 없음 — §97의2 승계라 대상 아님 |
| **C-13(UI)** | 4개 입력 섹션 | — | 렌더 | 안내에 **「모두」·「일부만 입력하면 오류」**가 포함된다(§5.5). 개수는 `total`에서 온다 |

---

## 7. 미확인 — ✅ **전건 해소** (2026-08-06)

| # | 항목 | 결과 |
|---|---|---|
| ~~U-1~~ | UI 안내 동반 여부 | ✅ **상시 안내 추가** — 단, 신규 카드가 아니라 **기존 문구 정정**(§5.5). 진행 카운트는 기각 |
| ~~U-2~~ | 주택 택일 그룹에서 둘 다 부분 입력 | ✅ **해소 — 아래** |
| ~~U-3~~ | `hasPre1990` 검사와 동시 발동 | ✅ **해소 — §3 재측정으로 매트릭스 정정** |
| ~~U-4~~ | 결과 화면 "② 미적용" 표시 | ✅ **표시하지 않음**(§5.6) — 결과 화면 변경 없음 |

### U-2 해소 — 빌더 로직이 답을 정한다

`transfer-tax-api-inheritance.ts:206`

```ts
if (isBefore1990 && !pre1990Payload && !landPriceAtInheritance) return {};
```

⇒ 등급 2/3(→ `pre1990Payload === undefined`)이어도 **`landPriceAtInheritance`가 있으면 payload가
생성된다**. 엔진은 단가 분기로 계산하고 **등급 2개는 그냥 무시된다**.

**규약**: 「**완성된 분기가 하나라도 있으면 통과**, 어느 분기도 완성되지 않았는데 부분 입력이
있으면 차단」. validate가 빌더보다 엄격하면 "칸은 다 있는데 차단"이 되므로 빌더 기준에 맞춘다.

⚠️ 다만 이 규약에서는 **미완성 분기의 입력이 조용히 무시되는 것이 남는다**(등급 2개). 그것까지
막으려면 빌더도 함께 엄격해져야 하는데, 그러면 "단가로 충분한 사용자"가 등급을 잘못 건드린 것만으로
차단된다. **현행 관대함을 유지**하고 U-1(안내)로 완화하는 편이 낫다.

### U-3 해소 — 동시 발동은 실재하나 문제가 아니다

`pre1990Enabled` 토글이 §163⑨ 경로에서도 켜지므로(`Pre1990LandValuationInput:177`) 토지는
**대부분 `hasPre1990` 경유로 이미 차단된다**(§3 표 4·6행). 두 검사가 겹쳐도 먼저 걸리는 쪽이
return하므로 메시지 중복은 없다.

⇒ 신규 토지 검사는 **`!hasPre1990`일 때만** 적용해 기존 메시지를 보존한다(C-8·C-8b 회귀).

---

## 8. 리스크

| # | 항목 | 대응 |
|---|---|---|
| **R-1** | **차단이 늘어 기존 사용자 흐름이 막힌다** | `filled > 0`일 때만 차단 — 안 쓰던 사용자는 영향 0. 전부 비움(C-9)은 통과 |
| **R-2** | **E2E 전건 회귀** — 차단 validation 추가는 기존 spec을 광범위하게 깨뜨린 이력이 있다 | memory `feedback_blocking_validation_full_e2e_regression`. 구현 후 **E2E 전건** 필수.<br>**위험 spec 예측**: §164 입력을 부분적으로 채우고 계산까지 진행하는 흐름 — `transfer-inheritance-post-deemed-land-164-4.spec.ts` · `split-mode-gating.spec.ts`(등급 입력) · 상가 상속 계열. **착수 시 `grep -l "inhHouseVal\|cbUnitPriceAtFirstOrAcq\|pre1990Grade" e2e/`로 목록을 먼저 확정**할 것 |
| **R-3** | 상속 상가 기존 메시지 변경 시 회귀 | C-5로 고정. 메시지 문구를 바꾸려면 기존 anchor 확인 |
| **R-4** | 주택 택일 로직 오판정 | C-4가 anchor. 단순 카운트 구현을 막는다 |

---

## 9. 결론

| | |
|---|---|
| **법령 판단** | ②는 max의 한쪽이므로 누락은 「법령상 요구되는 비교 미수행」이다 |
| **실태** | 6경로 중 **3경로가 침묵**(주택 상속·증여 / 상가 증여 / 토지 증여 post-1985). 토지가 대체로 막히는 것은 `hasPre1990`(환산 검증)에 **우연히 얹힌** 결과다 |
| **핵심** | 검사 추가보다 **필드 목록 단일 소스화**가 본질 — 그래야 빌더가 바뀔 때 validate가 따라오고, 토지 차단이 환산 검증 정의에 의존하지 않는다 |
| **범위(실적)** | 신규 파일 **2**(`sec164-required-fields.ts` · `transfer-tax-validate-sec164.ts`) + 빌더 **2곳**(주택·상가 — 토지는 매매 공용이라 제외) + validate 진입부 1곳 + 상가 블록 중복 제거 + UI 문구 4곳 + anchor **22건**(로직 17 · UI 5) |
| **14 동기화 지점** | **N/A** — 엔진 `input`/`result` 타입이 바뀌지 않는다. payload 구조는 그대로이고 **생성 조건**만 단일 소스로 옮긴다 ⇒ ⑨~⑭(Zod·body spread·Route 매핑) 변경 없음. 클라이언트는 ⑤(UI 문구)·⑧(validation)만 해당 |

---

## 10. 참고 — 왜 지금까지 드러나지 않았나

`{}` 반환은 **spread-safe**하게 설계됐다(`...buildX()`). 상위에서 아무 일도 일어나지 않으므로
**타입 오류도 런타임 오류도 없다**. payload 빌더 단위 테스트는 `AssetForm`을 완전한 형태로
구성해 넘기므로 **항상 통과한다** — 부분 입력을 넣어보는 테스트가 없었다.

⇒ 이 결함군은 [[feedback_api_trigger_without_input_path_is_noop]]와 같은 계열이다:
**"통과하는 테스트"가 "사용자가 도달할 수 있다"를 증명하지 않는다.**

---

## 11. 🔴 후속 정정 — 이 변경이 **과잉 차단**을 만들었다 (2026-08-06 · 해소 완료)

머지 후 U2-E 설계 중 probe로 발견했다. 필수 필드 목록에 **§164 전용이 아닌 공유 필드**가 들어가
있어, 그 필드만 채워도 「부분 입력」으로 오판해 차단했다.

| 자산 | 오염 필드 | 증상 |
|---|---|---|
| **토지** | `acquisitionArea`(취득 당시 면적) | 면적만 입력 → `1/5` → **차단**. 1990.8.30. 이전 상속·증여 토지 전반 |
| **상가** | `cbExclusiveArea`·`cbSharedArea`·`cbLandArea` | 면적 3종 입력 → `3/8` → **차단** |
| 주택 | — | `inhHouseVal*`은 §164⑤~⑦ 전용 섹션에만 있어 **오염 없음** |

사용자는 §164를 쓸 의사가 없는데도 등급·가격 칸을 채우거나, 다른 계산에 필요한 면적을 지워야
진행할 수 있었다. **§9의 「6경로 중 3경로 침묵」 진단은 맞았으나, 처방의 부작용을 못 봤다.**

**해소**: `FieldSpec.shared` + `Sec164FieldStatus.triggered` 도입. `isPartiallyFilled`가
**「§164 전용 칸을 하나라도 손댔는가」**를 함께 본다. 완성 요건은 불변이라 「등급을 채웠는데 면적이
빈」 경우는 그대로 차단된다. anchor `__tests__/calc/sec164-shared-field-not-trigger.anchor.test.ts` 8건
(Pre-Do로 수정 전 5건 실패 확인).

> **교훈**: all-or-nothing opt-in에서 「손댔다」의 신호는 **그 기능 전용 입력 위젯**이어야 한다.
> 필수 필드 목록과 trigger 목록은 **같지 않다** — 공유 필드는 완성 요건이지 opt-in 신호가 아니다.
> 설계: [`pre-deemed-clause-a-confirmation-criteria.engine.design.md`](pre-deemed-clause-a-confirmation-criteria.engine.design.md) §3.2.1
