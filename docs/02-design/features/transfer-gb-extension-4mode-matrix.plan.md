...
### ✅ O-4 — **§15에서 구현 완료** (2026-08-12 · 개정 3)

Q-3에서 게이트를 `!gbHasExtension`으로 통일하며 「두 안분 축이 충돌한다」고 봤으나,
실측하니 **충돌하지 않고 순차로 겹친다**. 상세는 **§15**.

---

## 15. O-4 구현 — 일부 양도 × 증축 (개정 3 · 2026-08-12)

**사용자 결정**: 「토지 일부 + 건물 전체」와 「건물 일부 + 대응 토지」를 **둘 다 지원**한다.

### 15.1 착수 전 실측 — 조합이 **도달 불가**였다

O-4는 「게이트 하나가 막고 있다」가 아니었다. **두 겹**이었다:

| 겹 | 위치 | 효과 |
|---|---|---|
| ① | `AREA_SCENARIOS_BY_ASSET_KIND`에 `general_building` **미등재** | 「면적 입력 방식」 선택지 자체가 없어 `areaScenario`가 **항상 `"same"`** |
| ② | `CompanionAcqPurchaseBlock` 안분 계산기 게이트 `!gbHasExtension` (Q-3) | 증축이면 계산기가 사라짐 |

⇒ ①이 있는 한 ②는 일반건물에서 **무의미했다**. Q-3의 「비일관 해소」 판단 자체는 유효했으나,
그 게이트가 실제로 가르는 것은 land·housing뿐이었다.

### 15.2 설계 — 면적은 「양도분 기준」 **단일 축**이다

`building`은 `partial`을 **되돌린 전례**가 있다(PR #912 · anchor A-6):
「취득·양도에 서로 다른 면적을 기준시가 곱셈에 넣어 환산비율을 왜곡했다 —
면적비가 단가비를 상쇄해 양도차익 0」.

**일반건물에는 그 회귀가 구조적으로 없다.** `gbLandArea`가 취득·양도 **양쪽**의 곱셈 인자이기 때문이다:

```
general-building-extension.ts:72   transferLandPricePerSqm    × landArea   ← 양도시
general-building-extension.ts:177  acquisitionLandPricePerSqm × landArea   ← 취득시
```

환산취득가 = 양도가 × (취득단가 × 면적)/(양도단가 × 면적) ⇒ **면적이 약분된다.**
`building`의 A-6은 `acquisitionArea`/`transferArea` **2칸**이 서로 다른 값을 받아서 생긴 것이고,
일반건물은 애초에 단일 필드라 그 입력이 불가능하다
(`AssetAreaGeneralBuilding` 주석 · `area-axis-single-field-invariant.anchor.test.ts`).

⇒ **면적 축을 새로 만들지 않는다.** 기존 필드의 **의미만** 「양도분」으로 바꾼다.

### 15.3 두 안분 축은 충돌하지 않고 순차로 겹친다

Q-3은 「면적 축과 기준시가 축이 충돌한다」고 봤으나, 실제로는 단계가 다르다:

```
① 안분 계산기: 전체 취득가액 ──(취득 당시 가치 비율)──▶ 양도분 취득가액
② 엔진 §166⑥:  양도분 취득가액 ──(취득시 기준시가 비율)──▶ 토지 · 건물1
   (건물2 = 증축분은 별도 슬롯 `gbExtensionActualAcquisitionPrice`)
```

②는 **부분 간 배분**이지 「전체 → 양도분」 축소가 아니다. 두 연산은 교환되지 않고 순차 합성된다.

### 15.4 부수토지(NBL) 판정 — 실측으로 「양도분 입력」이 정답임을 확인

`judgeAppurtenantLandExcess`가 `landArea` ÷ (`buildingFootprintArea` × 배율)로 비사업용 비율을 낸다.
면적을 양도분으로 줄이면 판정이 바뀌는데, **그 변화가 곧 정답**이다:

| 케이스 | 허용면적 | 비사업용 비율 | 해석 |
|---|---:|---:|---|
| 전체양도 토지 600 · 바닥 90 | 270 | 0.55 | 기준선 |
| 일부양도 토지 200 · **바닥 90 그대로** | 270 | **0** | 양도한 200㎡가 한도 270㎡ 안 ⇒ 전부 사업용 — **맞다** |
| 일부양도 토지 200 · **바닥 30**(같은 비율) | 90 | **0.55** | 건물도 1/3만 양도 ⇒ 비율 보존 — **맞다** |

⇒ 「토지만 일부 양도」면 바닥면적은 **건물 전체 값 그대로**, 「건물도 나눠 양도」면 바닥면적도
양도분. **사용자 결정의 두 케이스가 같은 규약 하나로 처리된다.**
⚠️ 그래서 **바닥면적 라벨은 「양도분」으로 바꾸지 않는다** — 바꾸면 첫 케이스에서 틀린 입력을 유도한다.

### 15.5 변경 5곳 (엔진 변경 **0**)

| # | 파일 | 내용 |
|---|---|---|
| 1 | `AssetAreaGeneralBuilding.tsx` | 「일부 양도」 ToggleCard + 안내 카드 + 면적 라벨 조건부(`양도분 …`). **바닥면적 라벨은 불변** |
| 2 | `AssetAreaSection.tsx` | `general_building: ["same","partial"]` **등재** + 축 A(면적 2칸) 렌더에서 일반건물 **제외** |
| 3 | 〃 `areaResetPatchForAssetKind` | `allowed.length === 0`이면 `same`으로 리셋 — stale `partial` 누수 차단 |
| 4 | `CompanionAcqPurchaseBlock.tsx` | 안분 계산기 게이트에서 `!gbHasExtension` **제거** + 증축 전용 hint |
| 5 | `GeneralBuildingBlock.tsx` | 증축 실거래가·필요경비 hint를 일부양도 시 「양도분」으로 |

**② 등재 + 축 A 제외를 함께 한 이유**: 허용 시나리오는 그 표가 **단일 소스**여야
`areaResetPatchForAssetKind`가 자산유형 전환 시 stale을 정리할 수 있다. 그러나 면적 입력은
전용 카드가 담당하므로 축 A를 함께 띄우면 **같은 면적을 두 곳에서** 받는다(그 표의 미등재 사유 주석).

**③이 필요한 이유**: 진입점이 늘면 새는 경로도 는다. 안분 계산기 게이트는 `areaScenario === "partial"`만
보므로, 일반건물에서 켠 `partial`이 상가로 넘어가면 **그 축이 없는 자산유형에서** 계산기가 뜬다
(메모리 `feedback_ui_gate_expansion_activates_latent_defect`).

### 15.6 ⑧ validate — anchor가 **dead code를 잡았다**

일부양도 실거래가 경로는 「양도분 취득가액이 구분되는가」 선택을 강제해야 한다(자동 안분 금지 정책).
그 규칙이 `transfer-tax-validate-asset.ts`에 이미 있어 처음엔 **거기에** 일반건물 분기를 넣었다.

**그것은 절대 실행되지 않는 코드였다** — 같은 파일 `:165`가 일반건물을 `validateGeneralBuildingAsset`으로
**early return**시킨다. anchor가 「미선택인데 차단되지 않는다」로 즉시 잡았다
(메모리 `feedback_route_if_chain_order_swallows_branches`와 같은 모양).

⇒ 검증을 `transfer-tax-validate-gb.ts`의 **V-9**로 옮겼다. 원래 자리에는 「여기 오지 않는다」는
포인터만 남겨 다음 사람이 같은 곳에 다시 쓰지 않게 했다.

> 종전 게이트가 `acq > tr` **2칸 비교**라 일반건물에서는 `NaN > NaN` = false —
> 그 보호가 **처음부터 죽어 있었다**. 일반건물은 토글 ON 자체가 확정 상태라 중간 상태 게이트가 없다.

### 15.7 테스트

**anchor 17건** (`__tests__/components/gb-partial-transfer-extension.anchor.test.tsx`) —
P1 토글 · P2 라벨(+🔑 바닥면적 불변) · P3 축 A 미렌더 · P4 계산기 개방 · P5 validate 강제 · P6 stale 정리.
**E2E 1건** — 토글 → 안분 계산기 → 계산 도달.

### 15.8 검증

- 신규 anchor **17/17** · E2E **6/6**(기존 5 + 신규 1)
- 회귀: `__tests__/components/` + 양도세 엔진 **517파일 5,029건** · `__tests__/calc/` + `__tests__/lib/` **263파일 2,836건**
- tsc 0 error
