...
- 작성일: 2026-08-12 · **개정 1: 2026-08-12 (자가 검토 후)**
...
### O-3 🟡 부분 혼합(토지 실가 + 건물1 환산) × 증축에서 환산 파트의 나목이 과소일 수 있다

`bothEstimated === false`라 D-10 수정 후에도 `transferExpense`가 전용 필드 입력 시에만 실린다.
그 조합에서 `bundledExpenses`는 **실가 파트에서만** 소비되므로, 환산 파트의 나목에는 양도비가
안 들어가는 것이 맞는지 아니면 안분된 몫만큼 들어가야 하는지 판단이 필요하다.
「소득세법」 제100조 제2항 후문(양도비 = 가액 비례 안분)을 따르면 **안분된 몫이 각 파트의 나목/가산에
들어가야 한다**는 해석이 유력하나, **현행과 동일**하므로 이번 변경으로 악화되지 않는다.

근본 해법은 **파트 축 상시 활성화**(`landDirectExpenses`를 0이라도 전송)인데,
증축뿐 아니라 **비증축 GB 전체(사례 31·32)** 의 §97②2호 판정 단위를 바꾸므로 범위가 크다.
