...
| ② 상속주택 | ⑦ 상속 5년 (§167의3①7호) — ~~+ 주택수 제외~~ | 🔴→✅ **D16 정정**(2026-09-18): 주택 수에서 빼던 것이 결함(3주택 299,816,000 vs 법령 354,541,000). 이제 산입 + 중과 대상 제외(양도 주택 자체 · 3주택 유일 일반주택 · 2주택 §167의10①10호), §155② 동일세대·순위 게이트 적용. 계획서 `docs/00-pm/transfer-review-4-defects.plan.md` §4 |
| ③ 공동상속 | `isCoInherited`·`isLargestCoInheritedShareholder` 속성 존재 | 🔶 동작 미확인 |
| ④ 동거봉양 | ~~배제 3 `parental_care_merge`(`:292`)~~ | ~~✅ 구현~~ → 🔴 **오판정**(2026-09-18): 2주택·먼저 양도·합가 전 취득·§154① 요건이 모두 없었다 — 3주택 141,966,000 vs 법령 354,541,000. **D9에서 배제 1(15호)로 통합**(`resolveMergeDeeming` — 비과세 E-3.5와 같은 정본). 계획서 `docs/00-pm/transfer-review-4-defects.plan.md` §1 |
| ⑤ 혼인 | ~~배제 2 `marriage_merge`(`:280`)~~ | 🟠 **부분**(2026-09-18): §154① 게이트는 있었으나 먼저 양도·합가 전 취득 검사가 없었다. **D9에서 배제 1(15호)로 통합** |
...
| **C** | 🔍 **감사** — F-4 매트릭스 🔶 6칸을 실측으로 닫는다 + F-3 mock↔seed 전수 대조 | B2 | ✅ 2026-07-31 → **§9** |
| **D** | C 결과에 따른 갭 수정 | C | 미착수 — **§9.6에 D-1~D-5 후보** |
...
## 9. Phase C — 감사 결과 (2026-07-31, 조사만)

법령: 「소득세법 시행령」 MST 286211 · 시행 2026-07-01 · 법제처 Open API 전문 조회.
수치는 throwaway probe 실측. **수정은 하지 않았다** — 아래 3건은 Phase D 재계획 대상.

### 9.1 F-6 🔴 중과 한시배제가 **2023-01-01 ~ 2024-01-09 양도분에 적용되지 않는다**

`transfer-rate-seed.ts`의 `surcharge:_default`(effective **2023-01-01**)가 `special_rules: null`인데,
한시배제를 담은 historical 레코드는 effective **2022-05-10**·**2024-01-10** 두 건이다.
`DISTINCT ON … ORDER BY effective_date DESC` 의미론상 **중간 구간을 메인 레코드가 덮는다**.

`loadFallbackTransferRates()` 실측 — `effectiveDate` / `specialRules`:

| 양도일 | 채택 레코드 | 유예 |
|---|---|---|
| 2022-12-31 | 2022-05-10 | ✅ |
| **2023-01-01 ~ 2024-01-09** | **2023-01-01** | **❌ null** |
| 2024-01-10 ~ | 2024-01-10 | ✅ |

법령상 유예 창은 2022-05-10 ~ 2026-05-09(§167의3①12의2·§167의10①12의2)이므로 **1년 9일이 통째로 빈다**.

**세액 실측**(같은 양도일에 유예 규칙만 주입한 대조군 — 기간 효과 격리 · 조정 2주택 · 양도가 20억):

| 2023-06-01 양도 | 산출세액 | 총부담 |
|---|---:|---:|
| 현행 | 777,435,000 | 855,178,500 |
| 유예 적용 | 424,335,000 | 466,768,500 |
| **차액** | | **+388,410,000 과다과세** |

⚠️ **fallback만의 문제가 아니다.** `npm run seed:tax-rates`가 메인·historical을 **같은 테이블**에
시딩하고 `preload_tax_rates()`도 동일 의미론이라 **DB 경로도 같은 공백**을 갖는다.
2023년 양도는 경정청구(§45의2, 5년) 기간 안이다.

> historical 4-B 주석의 「4-A2와 동일 종료일, 무해한 중복」은 **두 historical 레코드 사이에 낀
> 메인 2023-01-01 레코드를 고려하지 않았다.**

### 9.2 F-7 🔴 2주택에서 §167의10①2호 준용 배제 5종이 **전부 미적용** — 3주택보다 불리한 역전

§167의10①2호 = 「§167의3제1항**제2호부터 제8호까지 및 제8호의2**에 해당하는 주택」(법문 실측).
엔진의 해당 배제 블록은 `effectiveHouseCount >= 3` 게이트 안에 있다
(`multi-house-surcharge-exclusion.ts:311`).

probe 실측 — **양도 주택 자체**가 각 호에 해당할 때:

| 준용 호 | 보유 2채 | 보유 3채 |
|---|---|---|
| 6호 국가유산(문화재) | 🔴 `multi_house_2` **중과** | ✅ 배제 `cultural_heritage` |
| 4호 사원용 10년 | 🔴 중과 | ✅ 배제 |
| 5호 조특법 감면주택 | 🔴 중과 | ✅ 배제 |
| 8호의2 어린이집 5년 | 🔴 중과 | ✅ 배제 |
| 8호 저당권 실행 3년내 | 🔴 중과 | ✅ 배제 |

**주택을 적게 가진 쪽이 불리해지는 역전**이며 과다과세다. 2호·3호(장기임대·감면임대)와 7호(상속
5년)는 `countEffectiveHouses`가 주택 수에서 빼므로 결과적으로 커버된다 — 빠진 것은 위 5종이다.

### 9.3 F-8 🟠 공동상속 소수지분 주택이 **중과 주택 수에 산입**된다 (§167의3②2호 준용)

§167의10② → §167의3②2호: 「공동상속주택: 상속지분이 가장 큰 상속인의 소유로 하여 주택수를
계산」 — **기간 제한이 없다**. 엔진의 `isCoInherited`·`isLargestCoInheritedShareholder`는
`transfer-inheritance-exclusion.ts`(비과세 주택 수)에서만 읽히고, 그 단계는
`transfer-tax.ts:266`으로 **중과 판정(:182)보다 뒤**다. 중과 쪽 `countEffectiveHouses`는
상속 5년 규칙만 본다.

probe 실측(양도 h1 일반 + h2 공동상속, 상속 10년 경과 · 조정):

| h2 유형 | effective | 결과 |
|---|---|---|
| 공동상속 **소수지분** | 2 | 🔴 `multi_house_2` — §167의3②2호대로면 최대지분자 소유라 본인은 1주택 |
| 공동상속 최대지분 | 2 | ✅ 정상(산입이 맞음) |

### 9.4 F-4 매트릭스 갱신 — 6칸 종결

| §155 항 | 결론 | 근거 |
|---|---|---|
| ③ 공동상속 | 🟠 **F-8** — 중과 경로 미반영 | probe 실측 |
| ⑥ 국가유산 | 🔴 **F-7**에 포함(2주택 미적용) · 3주택은 정상 | §167의3①6호 = 국가유산주택 **자체** 양도 |
| ⑦ 농어촌주택(상속·이농·귀농) | 🔴 **전 저장소 미구현** — 비과세·중과 양쪽 | grep 0건(히트는 조특법 §99의4 감면, 별개 조문) |
| ⑧ 수도권 밖 부득이 | 🔴 **미구현** — §167의10①**4호** 전용, `isUnavoidableReason`은 3호(다른 시군)용 | `HouseInfo`에 대응 필드 없음 |
| ⑯ 공공기관 지방이전(3→5년) | 🔴 **미구현** — `deadlineYears` 산정에 분기 없음 | `resolveTemporaryTwoHouseDeadlineYears` 전문 |
| ⑳ 장기임대 + 거주주택 | ✅ **커버됨** | probe: 장기임대주택이 주택 수에서 빠져 effective=1 → 중과 미적용 |

### 9.5 V-2 — mock ↔ seed 전수 대조

`transferTaxSeeds` 10키 × `makeMockRates()`·`makeMockRatesWithHouseEngine()` 자동 diff:

| 항목 | seed | mock | 판정 |
|---|---|---|---|
| `long_term_holding.exclusions` | `["multi_house_surcharge","non_business_land","unregistered"]` | `["surcharge_applied","unregistered"]` | ⚪ **무해 — 엔진이 안 읽는다**(장특 배제는 코드 하드코딩). Zod가 파싱만 하는 dead config |
| `surcharge.rate_table.*.condition` | `regulated_area_2house` | `조정대상지역 2주택` | ⚪ 무해 — 읽는 코드 0곳 |
| `surcharge.special_rules` | `null` | 유예 객체 | 🔴 **F-6의 발현 지점** |
| `one_house_exemption.temporary_two_house.regulatedAreaDeadlineYears` | 2 | 1 | ✅ 해소 — anchor가 seed로 실행(T-B9) |
| `house_count_exclusion.lowPriceThreshold.non_capital` | 300,000,000 | 100,000,000 | 🟠 **mock이 구법** — 지방 저가주택 1억→3억 개정(§167의3①1호) 미반영. 그 mock을 쓰는 테스트는 구법을 고정 중 |
| `long_term_rental_v2`·`new_housing_matrix`·`non_business_land_judgment` | 존재 | **mock 없음** | 🟠 optional 파싱이라 **예외 없이 침묵 스킵**(R-9와 같은 구조). 개별 테스트가 별도 상수로 주입 |
