...
> **[P2b Do 확정 — rev.3 미기술 사항 2건]**
>
> **(1) 안분 비율의 분모를 파트 합계로 바꿔야 한다.** rev.3은 E1이 `calcApportionRatio`에
> 영향을 주지 않는 것처럼 서술했으나(§8 무변경 목록), 실제로는 **분모가 어긋난다**.
> 종전 분모는 `input.standardPriceAtAcquisition`(결합 총액)인데, 파트별 독립 경로에서는
> `land + building ≠ 그 총액`이 되어 비율이 틀어진다. 결합 총액이 애초에 공시되지 않으므로
> 유일하게 옳은 분모는 **파트 합계**다.
> 레거시 경로에서는 두 값이 항등이라 산출값이 **완전히 동일**하다(회귀 0):
>   · `landStd ≤ total` → `building = total − land` → 합 = total
>   · `landStd > total` → `building = 0` → 합 = landStd → 비율 1 (종전 `min(land/total, 1)`과 동일)
> → 축 B 산출을 `calcAcqStdPair()` 단일 함수로 뽑고 `calcApportionRatio`가 그것을 소비한다.
> 금액과 비율이 같은 소스에서 나오므로 어긋날 수 없다.
>
> **(2) 결합 총액이 더 이상 필수가 아니다.** 건물분 명시 입력 시 `standardPriceAtAcquisition`을
> 참조하지 않으므로, 그 값이 없어도 분리가 성립해야 한다. Zod refine
> (`transfer-tax-schema-refines.ts:58` "환산취득가 사용 시 취득시 기준시가 필수")이
> 이 정당한 조합을 400으로 막던 것을 `hasIndependentAcqStd` 우회로 해소했다.
...
> **[P3 Do 확정 — 범위 확대. rev.3의 "양도 축 범위 밖"은 부분 수정이 되어 위험]**
>
> rev.3은 양도가액 파트 필드를 선재 결함으로 보고 범위 밖으로 뒀으나, **취득 축만 스케일하면
> 상태가 더 나빠진다**. 현재는 양도·취득 파트가 **둘 다 raw(100%)** 라 서로 정합하는데,
> 취득만 지분 스케일로 내리면 `양도 100% − 취득 50%` 조합이 되어 양도차익이 폭증한다.
> 부분 수정이 무수정보다 나쁜 전형이므로 **파트 필드 전체를 함께 스케일**한다.
>
> 실측(지분 50%, throwaway probe)으로 확인한 누수 전량:
>
> | 필드 | 총액(정상) | 파트/추계(종전) | 방향 |
> |---|---|---|---|
> | 양도가액 | 5억 | 토지 6억 + 건물 4억 | 과대과세 |
> | 취득가액 | 2억 | 토지 3억 + 건물 2.5억 | 과소과세 |
> | 자본적지출 | ratio 적용 | 1천만 / 2천만 | 과소과세 |
> | `appraisalValue` | — | 4억 | 과소과세 |
> | `similarSalesValue` | — | 4.5억 | 과소과세 |
>
> **`appraisalValue`·`similarSalesValue`는 분리와 무관하다** — 지분 자산이 감정·매매사례 모드면
> `acquisitionPrice=0`이고 이 값이 취득가액이 되므로, 비분리 자산에서도 취득가가 2배가 됐다.
> rev.3 §4 E5가 진단에서 이미 지목한 필드(`appraisalValue`도 raw)라 범위 내로 본다.
>
> **기준시가는 대상이 아니다** — 소유 지분이 아니라 물건의 속성값이고, 환산 산식에서 분자·분모로
> 함께 나타나 비율이 상쇄된다. 다만 개산공제(`기준시가 × 3%`)는 지분과 무관하게 산출되는데
> 이것이 정당한지는 **미검증**(별건 — 분리·비분리 공통, 엔진에 지분 개념 자체가 없음).
>
> 스케일 적용은 `makeRatioed()` 단일 진입점으로 통일했다(`transfer-tax-api-split.ts`).
...
| V3 | `building` + 건물분 기준시가 입력 + 토지 3요소(㎡당 공시지가·면적) 중 하나라도 미입력 | "건물분 취득시 기준시가를 입력하면 토지분도 취득 당시 ㎡당 개별공시지가와 토지 면적으로 산출해야 합니다 — 둘 다 입력하세요(§99①1호 가목·나목)" |

> **[P2b Do 확정 — V3 형태 변경]** rev.3의 "한쪽만 입력 → 혼합 역산" 표현은 구현상 발생하지 않는다.
> `calcAcqStdPair`는 건물분 입력 시 총액을 **아예 참조하지 않으므로**, 혼합 역산이 구조적으로 불가능하다.
> 실제 위험은 다른 곳에 있다 — 건물분만 입력하고 토지 3요소가 비면 `calcAcqStdPair`가 null이 되어
> **분리 계산 전체가 오류 없이 비활성**된다(PR #837이 고친 §3.1과 동형). V3을 그 조건으로 재정의했다.
