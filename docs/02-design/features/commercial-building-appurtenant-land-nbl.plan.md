...
- **작성일**: 2026-08-05 (rev.9 — **전 Phase 완료**, §10)
...
- **진행 상태**: **Phase 0·A·B·C·D·E 전건 ✅ 완료** (2026-08-05)
...
anchor 8건 — `commercial-appurtenant-interaction.anchor.test.ts`

- **C-12 swap**: 양도차익·`swapApplied` 불변 + 세액만 상승 ⇒ STEP 0.35 재구성이 STEP 0.62 주입값을 보존함을 실증
- **C-13 수용**: 환산 분모·양도차익 불변
- **회귀 가드**: 기준면적 이내면 swap 단독과 **완전히 동일**
- **§101① 단서**: swap·수용 조합에서도 전량 중과가 살아남는다

**실측 발견 2건**

1. 🔴 **부칙 §9270호 §14① 중과배제** — 기존 swap·수용 anchor의 취득일 **2010-06-01**은
   「2009.3.16~2012.12.31 취득 토지 +10%p 배제」 구간이라 그대로 쓰면 중과가 아예 안 걸린다
   (`transfer-tax-rate-calc.ts:356` `isCrisisAcqExempt`). 취득일을 2013-06-01로 옮겨 검증했다.
   ⇒ **CB 부수토지 중과도 이 부칙 배제를 그대로 탄다**(토지 축 규정이므로 정합).
2. **수용 + 초과비율 1/2에서는 세액이 같다** — 수용 특례로 과세표준이 작아진 구간에서
   §104⑤ 비교과세가 일반세액을 택해 +10%p가 소멸한다. Phase 0-4(§3.3a)와 같은 현상이며
   **법령대로**다. 전량 비사업용(§101① 단서)에서는 정상적으로 세액이 오른다 — 두 케이스를 모두 anchor로 고정.

**최종 검증**: 전체 **13,514건 GREEN** · E2E 3건 · tsc 0 · lint 0 errors
...
### rev.9 (2026-08-05) — Phase E 완료 · 전 Phase 종료

| 항목 | 결과 |
|---|---|
| **C-12·C-13 상호작용** | swap·수용 특례는 **양도차익 축**, 부수토지 초과분은 **세율 축**으로 서로 독립함을 8건으로 고정. STEP 0.35 재구성이 STEP 0.62 주입값을 보존한다는 §3.2의 코드 분석이 값으로 실증됐다 |
| 🔴 **부칙 §9270호 §14①** | 기존 anchor 픽스처의 취득일(2010-06-01)이 **2009.3.16~2012.12.31 중과배제 구간**이라 재사용 시 중과가 안 걸린다. CB 부수토지 중과도 이 배제를 그대로 탄다 |
| **수용 + 부분 초과 = 세액 동일** | §104⑤ 비교과세로 일반세액이 채택되는 구간. Phase 0-4와 같은 현상이며 법령대로다. 전량 비사업용에서는 정상 상승 — 두 케이스 모두 anchor |
| **최종** | 전체 13,514건 GREEN · E2E 3건 · tsc 0 · lint 0 errors |

**전 Phase 종료.** 남은 것은 §0.3의 실무 발생 빈도 확인(Phase 0-2, 블로커 아님)과 §2.3 ㉮ 해석의 실무 재확인이다.
