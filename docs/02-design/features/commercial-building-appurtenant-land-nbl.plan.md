...
- **작성일**: 2026-08-05 (rev.2 — 자가 검토 반영, §10)
...
### 2.3 🔴 미확정 쟁점 — 구분소유 건물의 "바닥면적" (착수 조건)

§101①2호는 "**건축물의** 바닥면적"이라고만 하고 **구분소유에 대한 안분 규정을 두지 않는다**(§101 전문 실측 — 해당 문구 없음).

CB는 호(戶) 단위 양도인데, 판정 기준을 무엇으로 볼지 세 해석이 가능하다:

| 안 | 기준면적 | 비교 대상 | 비고 |
|---|---|---|---|
| ㉮ 전체-후-안분 | 건물 전체 바닥면적 × 배율 × 대지권 지분율 | 해당 호 대지권 지분면적 | 지분율이 분자·분모에 함께 걸려 사실상 건물 전체 판정과 동치 |
| ㉯ 호별 직접 | 해당 호 전용면적 × 배율 | 해당 호 대지권 지분면적 | 공유면적·계단실 등이 누락돼 기준면적 과소 |
| ㉰ 판정 자체 부인 | — | — | 구분소유는 §101①2호 적용 대상이 아니라는 해석 |

**착수 조건**: 이 쟁점이 해소되기 전에 엔진 산식을 확정하면 안 된다 — 메모리 정책 `feedback_unverified_authority_blocks_tax_change`(「본문 미확인」 근거가 판정을 가르면 착수 조건이다) 적용 대상이다.

**해소 방법**(Phase 0-1):
1. KoreanLaw MCP — 「지방세법」 §106①2호 및 §101 관련 법령해석례·조세심판례 검색 (키워드: "구분소유", "부속토지", "적용배율", "대지권")
2. 재산세 실무(집합건물 별도합산 판정) 유권해석 확인
3. 0건이면 **㉮를 채택하되 근거를 "명문부재 → 전체 판정 후 지분 안분이 자연해석"으로 문서화**하고 UI에 판정 근거를 노출

> ㉮와 ㉯는 대부분의 케이스에서 결과가 다르다. 전용면적 합계는 건물 연면적보다 작고 바닥면적과도 다르므로, 어느 쪽을 쓰느냐가 초과분 면적을 직접 바꾼다.
>
> ㉮ 채택 시 **입력 부담**: 사용자가 "건물 전체 바닥면적"과 "대지권 지분율"을 알아야 한다. 등기부·건축물대장에서 확인 가능하나 CB 폼에 현재 없는 값이다 → Phase D 입력 설계에 반영.
...
| C-6 | 미허가·미사용승인 건축물 (§101① **단서**) | — | **부속토지 전량 비사업용**(배율 무관) | ☐ |
...
| # | 작업 | 완료 기준 | 블로커 |
|---|---|---|---|
| 0-1 | §2.3 구분소유 바닥면적 기준 해소 | 해석례·심판례 인용 확보, 또는 0건 확인 후 ㉮ 채택 근거 문서화 | **🔴 예** |
| 0-2 | 실무 발생 케이스 확인 | 배율 초과 사례 1건 이상 | 아니오(우선순위 자료) |
| 0-3 | §101② 정본 브랜치 머지 | `lib/tax-engine/local-tax-zone-multiplier.ts` master 반영 | **🔴 예** |
| 0-4 | §3.3 계산방식 차이 실측 | 동일 입력 GB(카드 분할) vs CB(비율 안분) 세액 비교 → 차이·법령 정합성 문서화 (C-15) | **🔴 예** |

> **0-1·0-3·0-4 미해소 시 Phase B 진입 금지.**
...
| # | 리스크 | 대응 |
|---|---|---|
| R-1 | §2.3 미해소 상태로 산식 확정 → 세액 오류 | Phase 0-1 블로커 |
