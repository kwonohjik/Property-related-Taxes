...
| **P1 공용 primitive** | `FieldCard`·`SectionHeader`·`ToggleCard`·`RadioCardGroup` 정본 클래스 치환 + 신규 `StepTitle`(F6)·`Caption`·`SubsectionTitle` 신설·적용. **`components/ui/*` 고재사용 공유 primitive(`dialog`·`confirm-dialog`·`SaveToast`·`law-article-modal`·`address-search`·헤더배너)도 여기서 편입(A#3)** | 소(파일 수↓·영향↑) | 대표 마법사 3세목 레이아웃 확인 · `tsc` |
...
| **P3 입력 마법사** | 입력 단계 컴포넌트의 `text-[8/9/10/11px]` → `text-micro`/`text-caption`(§3.1 라우팅). **[F4] 대상 glob = `app/calc/**/*.tsx` + `components/calc/**`, 단 `components/calc/results/**` 제외(P4)**. 세목 dir 비균일(`steps/`는 transfer·stock만; 나머지는 `app/calc/{tax}/*.tsx`+`components/calc/{tax}/**` 인라인) + 추가 마법사 3종(`gift-deemed`·`inheritance` 사후관리·`family-business` 사후관리) 포함 | 대(입력 화면 한정) | 세목별 E2E baseline · 단계별 레이아웃깨짐(light·dark) |
