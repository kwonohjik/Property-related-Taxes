...
| **R-14** | 🆕 입주권·완공APT의 §163⑨ 상속·증여 평가액 **자동 산정** | R-10에서 분리. 두 자산종류 **공통 구조**라 함께 설계해야 한다 |
| **R-15** | 🆕 `transfer-tax.ts:229-235` §166 분기 override의 **안전망 0건** | R-10 뮤테이션 M-4에서 발견 — 통째로 무력화해도 anchor 10건이 전부 통과한다. 실제로 다른 값을 내는 조합(pre-deemed + ③환산 채택 + ①② 확인값)이 **미검증**. 죽은 분기인지 미커버 분기인지부터 판정할 것 |
