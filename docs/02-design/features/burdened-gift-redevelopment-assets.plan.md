...
## §12 PA 착수 실측 (2026-09-08) — **설계 3건이 stale이었다**

> Do 진입 시 인용 `file:line`을 전수 재측정했다. 설계는 2026-08-12 작성이고 그 사이
> 2026-08-13·08-25·08-27에 재개발 엔진이 세 차례 정정됐다.

| 설계 서술 | 2026-09-08 실측 | 처리 |
|---|---|---|
| **D-8** 「음수 clamp 비대칭 — `splitAptPay`가 삼키고 `computeRightPay`는 싣는다」 | ⛔ **소멸.** `96ed87b4`(E1-03, 08-25)가 `splitAptPay`의 `postApprovalGain <= 0` 조기반환을, T1-05(08-27)가 §166①2호 가목과 `calcAptReceiveSettlementGain`의 `Math.max(0,…)`를 제거했다. **4분기 전부 clamp가 없다** — 남은 조기반환은 분모 0 방어뿐 | D-8 삭제. clamp 관련 작업 없음 |
| **X-1 / PA 7 / anchor X-A1** 「`RedevelopmentBlock`의 subject 라디오에 차단 조건 추가」 | ⛔ **도달 불가.** 2026-08-13 축 일원화로 그 라디오가 **폐지**됐다. `redevSubject`의 유일한 쓰기 지점은 `redevSubjectPatchForAssetKind`(`AssetAreaRedevelopment.tsx:53`)이고 자산 종류에서 파생한다. 저장된 불일치 조합은 `calc-wizard-asset-migrate.ts:340-345`가 양방향 정규화한다 | 구현 안 함(죽은 코드 방지). 대신 그 정규화가 살아 있음을 확인 |
| 게이트 **3층** | ✅ 맞다 — ⑤ `TransferModeBlock.tsx:63` · ⑧ `transfer-tax-validate-bg.ts:28` · **엔진 `burdened-gift-eligibility.ts`** (`BURDENED_GIFT_SUPPORTED_PROPERTY_TYPES`로 export) | 셋 다 편입 + parity anchor(G-1) |

그 밖에 줄 번호가 크게 밀렸다(`computeAptPay` 267→308 · `computeRightPay` 425→548 ·
`calculateRedevelopmentTax` 56→74 · 12억 지점이 `transfer-tax-redevelopment.ts:84·86` →
**`transfer-tax-redevelopment-steps.ts:104·106`**으로 파일 이동). 구조 판정은 전부 유효했다.

### 🔴 β의 방향은 사실관계에 종속된다 — 채택 근거 ②가 일반화되지 않는다

§9 U-1은 (a)안 채택 근거로 ②「α는 세액을 **높인다**」를 들었다. **PA anchor 사실관계에서는
반대였다** (뮤테이션 실측 · `burdened-gift-redevelopment-apt.anchor.test.ts` 헤더):
...
여기서 α는 청산금 2억·필요경비 1,500만을 **물건 전체로 공제**하면서 양도가액만 채무분으로
줄여 인가후 두 분기를 음수로 만들고, 그만큼 **과소** 산정한다. P0 사례에서는 반대로 인가전
분이 음수가 되어 장기보유공제가 소멸해 **과대**였다.

⇒ **β는 「납세자에게 유리한 안」이 아니다.** 채택 근거로 남는 것은 ①**산식 정합성**과
③**안분 비율 보존**(§166②1호의 `평가액 : 청산금`은 분자·분모에 같은 `r`이 걸려 불변)이다.
②는 P0 사례에 종속된 서술이므로 **일반 근거로 인용하지 말 것**. 법적 불확실성은 (a)안이
정한 대로 결과 화면 고지로 노출한다.

### 12억 분모 — 증상이 「안분 누락」보다 나빴다

D-2는 「채무액으로 12억을 재면 고가주택 판정이 빠진다」고 적었다. 실측에서는 비과세 판정이
`isPartialExempt`로 안분을 **켠 뒤** 분모만 채무액 6억이라
`taxableRatio = (6억 − 12억) / 6억 = −1`이 되어 **과세대상 양도차익이 −471,250,000**,
비과세분이 942,500,000(전체 양도차익의 두 배)으로 찍혔다. 비교와 안분이 **같은 값**을 써야
하는 이유가 여기 있다.

---

## §13 PR(조합원입주권) 착수 실측 (2026-09-08) — 설계가 놓친 4건

설계의 D-1(세액 0 함정)·D-2(K-4 전용)·P-5(4-way 재구성)는 **전부 유효**했다. 그런데 구현 중
설계에 없던 것 넷이 나왔고, 셋은 **그대로 두면 사용자를 영구 차단**한다.

| # | 발견 | 조치 |
|---|---|---|
| 1 | **⑧ (4) 시가 모드가 「취득가액 산정방식을 선택하세요」를 요구한다.** 입주권은 K-4 고정이라 ⑤가 그 라디오를 렌더하지 않는다 ⇒ **화면에 없는 컨트롤을 요구**하는 영구 차단 | ⑧에서 입주권은 `acqMethod = "actual"`로 읽는다(④ API와 같은 값) |
| 2 | **⑥ 사이드바가 `standardPriceAtTransfer`를 본다.** 입주권은 그 칸을 쓰지 않아 채무비율이 **0%로 표시**된다(설계는 「변경 없음 — 확인 필요」였다) | `deriveRightValuationTotal` 파생을 쓴다 |
| 3 | **평가 3항의 지분 축소가 합을 깬다.** 세 항을 각각 floor하면 `floor(총액 × 지분율)`보다 최대 2원 작아지는데, 평가액 총액(`buildingStdPriceAtTransfer`)은 따로 그 값으로 줄어 **지분 자산에서만** 자기일관 검사가 깨진다 | 마지막 항(프리미엄)이 **잔액 흡수** |
| 4 | **양도시 기준시가 카드**도 숨겨야 한다. `toPropertyKind`가 입주권을 `building_non_residential`로 매핑해 **비주거 건물 공시가격 조회 UI**까지 뜬다 | `stdPriceAtTransferComesFromElsewhere`에 입주권 추가 |

### 부수 실측

- **`computeMortgageValuation`은 담보평가를 실제 채무액으로 cap한다**(`임대보증금 + min(채권최고액, 담보채권액)`). U-6 fail-fast anchor를 짤 때 **설정액만 키워서는 보충적평가를 못 넘는다** — 채무액 자체가 넘어야 한다.
- **`BurdenedGiftBlock.tsx`가 782줄**이 됐다(설계 시점 656 → 착수 시점 753). ④′를 인라인했으면 800을 넘는다 ⇒ 설계 4.8의 별도 파일 분리가 **선택이 아니라 필수**였다.
- **④ API 조립·⑫ Zod·⑬⑭는 재개발 APT 배치에서 이미 확인**돼 재사용됐다 — ④는 `assetKind` 무관하게 `transferType`만 보고 진입하고, ⑬⑭는 객체 통째 spread다.

### 🔴 남은 미결 — U-8 (R-5 입주권 §89①4호 비과세의 12억 분모)

`applyOneRightExemption`(`transfer-tax-redevelopment-transforms.ts`)은 `input.transferPrice`로
12억을 잰다. 부담부증여에서 그 값은 **채무액 B**다. PA 배치에서 `subject="apt"` 경로는
`burdenedGiftDenominator`로 고쳤으나, **입주권 경로는 고치지 않았다** — 그 함수 주석이
「국세청 해석례 근거 — **분모 = transferPrice 단일(해석 A)**」를 명시하고 있어, 해석 B로 바꾸려면
그 해석례와의 충돌을 **먼저 해소**해야 하기 때문이다(근거 없이 바꾸면 §89①4호 판정이 조용히 달라진다).

⇒ 현행 유지 + 미결 표기. 착수 조건: 해석 A의 근거 해석례를 확인하고 부담부증여 사안에서의
적용 여부를 판정할 것.
