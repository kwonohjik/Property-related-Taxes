...
- ⚠️ **단순화 명시**: `marketValue > appraisedValue`는 법령상 **동순위**(그룹A). 둘 다 입력 시 엔진은 **매매 우선**으로 tie-break(D-4 "무조건 시가" 확정과 일치). **단 §49② "평가기준일에 가장 가까운 날" 규칙은 미반영**(현 UI에 시가·감정 각각의 평가일 입력 필드 없음). → 향후 날짜 입력 도입 시 보강. **현 단계 한계로 계획서에 동결.**
