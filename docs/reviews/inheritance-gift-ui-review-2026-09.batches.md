...
## 남은 판단 2건 종결 (2026-09-11)

「제품 판단이 필요하다」며 보고만 했던 2건을 실측으로 갈랐다.

### ① `window.confirm` → `ConfirmDialog` — 모집단은 **5가 아니라 20**이었다

직전 보고에서 「호출부 5곳」이라 적었는데 **틀렸다**. 전수:

| 컴포넌트 | 호출부 | 규칙 범위(「데이터 손실 위험 액션」) |
|---|---:|---|
| `ResetButton` | **8** | ✅ 전체 입력 삭제 |
| `HomeButton` | **11** | 9곳은 「값이 **유지된 채**」 ⇒ 범위 밖 / 2곳은 「값이 **사라집니다**」 ⇒ 범위 안 |
| `stock-transfer/steps/Step1.tsx:121` | 1 | ✅ 분할 건 폐기(첫 건만 유지) |

`HomeButton` 은 9곳이 범위 밖이지만 **모달 구현이 컴포넌트 한 곳**에 있어, 범위 안 2곳만
고치려 해도 컴포넌트를 바꿔야 한다. 공용 컴포넌트가 호출부에 따라 모달 기술을 바꿀 수는
없으므로 전부 옮겼다(단 `destructive` 는 주지 않는다 — 대다수에서 파괴적이지 않다).

**가드를 함께 걸었다** — `__tests__/lib/no-native-confirm.guard.test.ts`. 규칙 문언은
전역인데 가드가 **없어서** 2026-09-05 에 결과 화면 축만 고쳐지고 나머지가 남았다.
전수를 0건으로 만든 뒤 걸었다(「몇 건인지 모른 채」 걸면 상시 빨간불이 되어 또 무력화된다).

### ② `FarmingEligibilitySection` — **배선**이 답이었다 (삭제 아님)

`git log -S` 로 이력을 읽었다. `step4-5.tsx` 에 마운트돼 있었는데(`55e22d66`), 그 파일이
**「orphan step4-5.tsx 삭제」**되면서(`a67b2871`) 화면에서 함께 사라졌다. 그 커밋의 목적은
자동채움 UI 였고 **영농 요건 입력을 없앤다는 언급이 없다** — 의도된 제거가 아니다.

아래가 전부 살아 있는 채로 **⑤(UI)만 없었다**:

| 지점 | 상태 |
|---|---|
| ① `FormState.farming?: FarmingInheritanceInput` | 살아 있음 |
| ③ normalize 의 고아 heirId 정리 | 살아 있음 |
| ④ `buildInput` 의 `farming: form.farming` | 살아 있음 |
| ⑦ 결과뷰 「Step4에서 영농상속공제 요건 입력을 활성화하면」 | **살아 있음 — 없는 UI 를 가리킴** |

⇒ `form.farming` 은 영원히 `undefined` 였다. 756줄짜리 §18의3 요건 판정(거주지 검증·상속인별
자격·§16⑭)이 통째로 닿을 수 없었다. 삭제하면 엔진 경로와 결과뷰 안내까지 함께 걷어내야
하므로 **배선이 더 작고 옳은 변경**이다 ([[feedback_ui_gate_removes_sole_input_path]]).

> ⚠️ **RTL anchor 는 배선을 절반만 증명한다.** Step4 를 직접 렌더하면 그룹 A 의 progressive
> disclosure 를 **건너뛴다** — 실제 마법사에서는 그룹을 펼쳐야 영농 블록이 보인다. E2E 를
> 처음 돌렸을 때 정확히 그것 때문에 실패했다.

### 🔴 배선 복구가 **조문 배지를 두 벌** 만들었다

전체 E2E 에서 `inheritance-law-article-badges` LAW-1 이 빨개졌다 —
`getByRole("button", { name: /§18의3 영농상속공제/ })` 가 **2건**을 잡았다.

`FarmingEligibilitySection` 이 자기 헤더에 `§18의3` 배지를 갖고 있는데, 그것이 붙는 Step4 영농
블록도 「영농상속재산가액」 칸 옆에 **같은 배지**를 단다(그룹 A 의 공통 패턴 — 금융 §22 ·
영농 §18의3 · 가업 §18의2). 따로 있을 때는 각자 옳았고, **합치자 중복이 됐다.**

⇒ 셀렉터를 `.first()` 로 느슨하게 하지 않고 **섹션 쪽 배지를 지웠다**. 그렇게 하면 그룹 A 의
패턴이 그대로 유지된다. 시행령 §16(요건)은 이 섹션에만 있는 축이라 남겼다.

> 부수 발견: 두 배지의 `legalBasis` 가 **서로 달랐다** — 섹션은 `"상증법 §18의3"`,
> Step4 는 `"상속세및증여세법 §18의3"`. 중복을 지우면서 함께 사라졌다.

⚠️ **고아를 배선할 때는 「그 컴포넌트가 자기 맥락을 스스로 그린다」는 전제를 의심할 것** —
   단독으로 쓰이던 컴포넌트는 제목·조문 배지·안내를 **자기가 그리도록** 만들어져 있고,
   그것이 새 부모의 것과 겹친다.

### 검증

앵커 16건 신규(가드 3 + RTL 13) · E2E 1건 · **뮤테이션 4축 전부 구별력**(4·3·3·2건 실패) ·
`building-standard-price.spec.ts` 의 `page.on("dialog")` 는 이제 아무것도 잡지 못하므로
`confirm-dialog-confirm` 클릭으로 정정.
