...
## 해결 현황 (2026-06-04 — 전 항목 수정·푸시 완료)

확정 31건 중 **29건 수정 완료**, 잔여 2건(H-6·H-7, 경미/부분). + 리뷰 중 발견된 §28 cutoff 절사 보너스 정정.

| 그룹 | 상태 |
|---|---|
| **HIGH** | H-1✅ H-2✅ H-3✅ H-4✅ H-5✅ · H-6⚠️(LOW 강등·기능영향0, 미적용) · H-7◐(M-9·L-9로 부분 커버) |
| **MEDIUM** | M-1✅ M-2✅ M-3✅ M-4✅ M-5✅ M-6✅ M-7✅ M-8✅ M-9✅ (전건) |
| **LOW** | L-1~L-15 전건✅ (L-7은 H-5 동반, L-10은 KoreanLaw 검증 후 현행 유지, L-14는 SC3만 실효화·SC1/2/4는 엔진 제약 명시, L-15는 정합 확인) |
| **보너스** | §28 증여세액공제 필터 cutoff 일 단위 정정 (M-9 작성 중 발견, `8bd40f9`) |

**잔여 2건** (소규모): H-6 = `listedStockCode` Zod 1줄 예방 추가(분류 결과 불변). H-7 = EstateItem Zod `.parse()` round-trip 종합 가드(M-9 JSON round-trip + L-9 deductionSchema preserve로 부분 충족).

**커밋**(master): `2dfd1c6`(docs) → `184c709`(§13) → `30ce094`(§28①) → `078c6c8`(UI/API 5건) → `220cdbf`(§30) → `8bd40f9`(§28 cutoff) → `74ce82a`(M-5) → `4275fad`(M-6) → `511548e`(M-7) → `67a4af5`(M-9) → `7f66a02`(L-1·2·3) → `ed7bd25`(L-4·5·6·8) → `ccb6cfb`(L-9·10) → `356fb21`(L-11~15).
