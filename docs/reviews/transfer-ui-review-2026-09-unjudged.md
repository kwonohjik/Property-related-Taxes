...
## 0-1. 처리 결과 (2026-09-08 · 전건 종결)

**실재 19건 + 대장 밖 B1 = 20건 전건 수정 완료.**

| 배치 | PR | 항목 |
|---|---|---|
| ① high | **#1547** | R23(정당한 사유 면제 no-op) · R21(거주개월 소실) |
| ② 세액·차단 | **#1548** | R06 · R11 · R13 · R14 · R16 |
| ③ 표시·규약 | (이 커밋) | R02 · R03 · R05 · R07 · R08 · R09 · R12 · R15 · R17 · R18 · R19 · R20 · **B1** |

⛔ **닫힘 4건(R01·R04·R10·R22)은 재제안 금지** — 아래 §1 표의 근거를 먼저 읽을 것.

### 이 작업이 뒤집은 기존 anchor 2건 (약화가 아니다)

- `amendment.test.ts` **A2** — `exempt`에서 전액을 단언해 **결함을 고정**하고 있었다.
  「가산세 ON이면 10%가 붙는다」 축은 **A2b**로 옮겨 보존했다.
- `ownership-ratio-percent-message.anchor.test.tsx` — 문구를 「공유 지분율」로 고정했는데
  같은 위젯이 「취득 지분율」로도 렌더된다. 공통 어간 「지분율」로 바꿨고
  **「분자·분모 금지」 원래 축은 그대로 유지**된다.

### ⛔ 한 번 되돌렸다가 취소한 처방 1건

R21을 **「direct↔interval 토글 부활」**로 고치려다 취소했다 — `251df37b`(2026-07-26)가
「임대·거주 기간은 최소 1건 반드시 입력해야 하므로 토글이 불필요·혼란」이라며 **의도적으로**
제거한 것이고 Step4만 예외로 남긴 것도 그때의 명시적 결정이다. 상시 표시는 그대로 두고
**direct 값의 존재와 대체 사실을 말로 밝히는** 방향으로 바꿨다.
E2E 「토글 없이 상시 표시」 단언이 그대로 통과하는 것으로 무변경을 실증했다.
