...
**처방**  V3-b와 **같은 leaf를 공유**해 고칠 것 — 코드 비교 전에 일반구를 상위 시로 접는다.

> 🔴 **정정(2026-09-02 PR-4 착수 시 실측)** — 위 처방이 제시한 판별식 `code[3:5] !== "00"`은 **틀렸다**. 256건 중 **238건**이 걸려 서울 종로구(11110)·의정부시(41150)·담양군까지 일반구로 오탐한다. 대안으로 검토한 「끝자리 ≠ 0」도 서울 광진구·인천 연수구·증평군·태안군 등 **12건**을 오탐한다. 정확한 판별은 **명칭**이다 — `fullName`이 3토큰(도 + 시 + 구)인 것이 정확히 일반구 **39건 · 13개 시**이고 반례 0건이다(수원·성남·안양·부천·안산·고양·용인·화성·청주·천안·포항·창원·전주). 같은 시의 구들은 앞 4자리가 일치하고 그 prefix를 쓰는 비일반구 코드는 **0건**이므로 「앞4자리 + '0'」이 곧 행안부 시 코드다(창원시 48120·고양시 41280·수원시 41110). 구현은 `lib/geo/sigungu-unit.ts`.

접은 뒤 시 단위 대표코드로 비교. 연접 매트릭스도 시 단위로 union해야 한다(현재 41287의 인접에 41281이 없는 것은 매트릭스가 구 단위여서다). residence.ts:64 주석의 미러 짝(farming-residence-check.ts)을 반드시 함께 고칠 것 — 한쪽만 고치면 두 세목이 갈린다.
