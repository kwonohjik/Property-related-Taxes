...
⚠️ **"2001년부터 고시"는 프로젝트 데이터의 적용 상한에서 역산한 값**이다. 국세청 고시 원문으로
최초 고시 연도를 직접 확인하지는 않았다 — **Do 착수 전 확인 필요**(§9-A).
...
| # | 확인 | 방법 |
|---|---|---|
| 0-1 | **건물 기준시가 최초 고시 연도** | 국세청 「건물 기준시가 계산방법」 고시 원문 또는 KoreanLaw. §1-1의 "2001년"은 프로젝트 데이터에서 역산한 값 |
| 0-2 | 차단 방식 (a)/(b) 결정 | §5-2 표 |
| 0-3 | `calcAcqBaseBreakdown`이 상가 용도에서 정상 동작 | throwaway probe — 1998 취득 상가 1건 실측 |

### Phase 1 — 게이트·안내 (G1·G3)

- `CommercialBuildingBlock` ③ 섹션 + `CommercialInheritanceStdPriceSection` ③ 섹션 안내 카드
- 확인 토글 + `AssetForm` 1필드 (①②③⑤⑧)
- `transfer-tax-validate-asset.ts` 차단 규칙
- **verify**: RTL — 취득 1998 시 안내·토글 노출 / 2001 시 미노출 / 미확인 상태 validate 차단.
  회귀 — 기존 `commercial-inheritance-164-6-max.spec.ts` 통과(취득 2000-12-07이므로 **이 스펙이
  P-04 경계 케이스에 걸린다** → 시드에 확인 토글 `true` 추가 필요)
