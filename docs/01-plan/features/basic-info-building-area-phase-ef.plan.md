...
## 8.8 U-10 법령 확인 — §8.7 권고를 정정한다 (rev.6)

KoreanLaw 실측. anchor 3건 추가(법령 제약 고정).

### 8.8.1 조문 원문 — 「소득세법 시행령」 제176조의2 제2항 제2호 (MST 286211, 시행 2026-07-01)

> 환산가액 = 양도당시의 실지거래가액 × (**취득당시의 기준시가** / 양도당시의 기준시가)

조문은 **면적을 언급하지 않는다.** "취득당시의 기준시가"는 법 제114조 제7항 문맥상 **양도자산의** 취득당시 기준시가이며, 일부양도에서는 양도한 부분이 그 자산이다.

### 8.8.2 🔑 조심 2018부0572 (2018.05.03, **기각**) — 분할 양도 직접 사례
...
| 재결 | **기각** — 처분 정당 |
| 판단 요지 | "일괄하여 취득한 토지의 총 취득가액은 확인되나 **각 필지별 실지취득가액이 불분명한 경우**, 각 토지의 취득가액 산정은 전체 토지의 실지취득가액을 **각 필지의 취득 당시 기준시가 비율로 안분**하여 산정하는 것이 합리적" |
| 배척된 주장 | 양도 당시 **감정평가액** 기준 안분 |

같은 방향(감정가액 안분 배척): 국심2005구1458(기각) · 국심1992서2655(기각).

### 8.8.3 도출되는 세 제약 — §8.7 권고 정정

| # | 제약 | §8.7 권고와의 관계 |
|---|---|---|
| **L-1** | 안분 기준은 **면적비가 아니라 취득 당시 기준시가 비율**이다. 2018부0572는 양도분·잔여분의 취득 당시 공시지가가 **동일**한 사안이라 결과적으로 면적비와 같아졌을 뿐이다(처분청 의견 "공시지가가 동일하며") | 🔴 **정정** — §8.7이 "면적비 안분"이라 한 것은 특수 케이스다. 부분별 단가가 다르면 갈린다(anchor 고정) |
| **L-2** | 안분은 **실지취득가액이 불분명한 경우의 보충적 방법**이다. 계약서에 구분 기재돼 있으면 그 값을 쓴다 | ✅ §8.7의 "자동 안분 금지" 방향은 맞았다. 단 근거가 "비례가 자명하지 않아서"가 아니라 **"구분 가액이 우선하므로"**다 |
| **L-3** | 취득 기준시가(환산 분자)는 **양도한 부분의** 취득 당시 기준시가다("각 필지의 취득 당시 기준시가") | ✅ §8.7 지지. 부분별 단가가 같으면 면적비 적용과 수학적으로 동치(anchor 고정) |

### 8.8.4 정정된 처방 (B4-1 최종안)

| 대상 | 처방 | 근거 |
|---|---|---|
| **취득 기준시가**(환산 분자) | **양도한 부분의** 취득 당시 기준시가 = 양도분 면적 × 그 부분의 취득 당시 단가. 부분별 단가가 다르면 그 단가 | 「소득세법 시행령」 제176조의2 제2항 + L-3 |
| **취득가액**(실거래가) | ① 계약서상 구분되면 **그 값 직접 입력**(우선) ② 불분명하면 **취득 당시 기준시가 비율로 안분** — 면적비 아님 | 조심 2018부0572 + L-1·L-2 |
| 취득 필요경비 | 「소득세법」 제100조 제2항 후문 "공통되는 취득가액과 양도비용은 해당 자산의 가액에 비례하여 안분" 유추 — **확인 필요**(U-11) |

**UI 함의**: 사용자에게 **"취득가액이 양도분·잔여분으로 구분되는가"**를 물어야 한다. 무조건 자동 안분은 L-2 우선순위를 뭉개고, 무조건 사용자 입력은 L-1의 보충 방법을 제공하지 않는다. → 3-state가 아니라 **2-state 선택 + 안분 계산기**가 자연스럽다.

**⚠️ 이 절의 "감정가액 안분 금지"는 rev.7에서 정정됐다 — §8.9.4 참조.**

---

## 8.9 U-3·U-4·U-5·U-6·U-11 검증 + rev.6 정정 (rev.7)

### 8.9.1 🔴 U-3 — 두 필드는 **개념이 다르다.** F3(통합) 부정

법령 문언이 갈린다(KoreanLaw 실측 — 소득세법 MST 280405 · 시행령 MST 286211, 시행 2026-07-01):

| 용도 | 조문 | 문언 | 곱셈 대상 |
|---|---|---|---|
| 1세대1주택 **비과세 부수토지 한도** | 「소득세법」 제104조… 아니라 **제89조 제1항 제3호** → 「소득세법 시행령」 제154조 제7항 | "이에 딸린 토지로서 **건물이 정착된 면적**에 … 배율을 곱하여" | **건물** 정착면적 |
| **비사업용토지** 주택부수토지 | 「소득세법」 제104조의3 제1항 제5호 → 「소득세법 시행령」 제168조의12 | "주택부속토지 중 **주택이 정착된 면적**에 … 배율을 곱하여" | **주택** 정착면적 |

**배율표는 동일**하다(제154조 제7항 = 제168조의12: 수도권 주·상·공 3배 / 수도권 녹지 5배 / 수도권 밖 5배 / 그 밖 10배). 그러나 **곱셈 대상이 "건물" ↔ "주택"으로 다르다** → 겸용주택에서 값이 갈린다.

**코드는 이미 이 구분을 구현하고 있다**:
```ts
// transfer-tax-mixed-use-helpers.ts:92 — 주택 정착면적은 건물 정착면적에서 파생
residentialFootprintArea: round2(asset.buildingFootprintArea * acqResRatio),
// :425 — NBL 판정은 주택 정착면적을 쓴다
const allowedArea = derived.residentialFootprintArea * multiplier;
```
`mixedResidentialFootprintOverride`("주택 정착면적 수동 지정 — 빈값=자동(정착면적 × 주택연면적비율)")도 같은 취지다.

→ **`nblHousingFootprint`와 `buildingFootprintArea`는 통합할 수 없다.** 단독주택에서는 값이 같지만 겸용주택에서 다르다. **Phase F3를 "통합"에서 "이중 입력 UX 개선"으로 격하**한다.

### 8.9.2 U-4 — 축 B 전용 결과 카드 표시는 **불필요**

| 자산 | 축 B 결과 표시 | 위치 |
|---|---|---|
| 상가 | ✅ 있음 | `CommercialBuildingValuationDetailCard.tsx:123,125` "양도시 호별총액 = ㎡당 호별고시가 × 연면적" |
| 겸용 | ✅ 있음 | `MixedUseResultCardParts.tsx:141,145` 취득시 주택·상가 연면적 |
| 종부세 서식 | ✅ 있음 | `ComprehensiveFilingFormBuppyo3.tsx:137` |
| **GB** | ❌ 없음 | `GeneralBuildingValuationDetailCard`에 연면적 표시 0건(축 C 수평투영면적만 `:395`) |

축 B의 정본 표시 경로는 **「건물 기준시가 계산서」 서식**(`components/calc/building-std-price/`)이다 — 연면적이 그 서식의 입력·출력 항목이다(`ApartmentConversionSection.tsx:94` 등). GB조차 결과 카드에 연면적을 안 보여주는데 housing에 신설할 근거가 없다.

→ **F1의 ⑦(결과 카드)은 변경 없음.** 계산서 서식 스냅샷이 담당한다.

### 8.9.3 U-5 — `land`는 축 C를 **이미 받고 있고 기본정보 승격 대상이 아니다**

`land` 자산의 정착면적은 `nblHousingFootprint`로 NBL 섹션에서 받는다(`NblSectionContainer.tsx:200` — `nblLandType === "housing_site"` 게이트). 그리고 §8.9.1에서 이 필드는 **「소득세법」 제104조의3 제1항 제5호 전용 개념**임이 확정됐다.

→ 기본정보의 범용 "건물 바닥면적"(축 C)과 **합치면 안 된다.** F1에서 `land`에 축 C를 추가하지 않는다.

### 8.9.4 🔴 U-11 + **rev.6 정정** — 안분 기준은 서열이 아니라 **신빙성 판단**이다

rev.6에서 "감정가액 안분은 3건 모두 배척"이라 썼는데 **틀렸다.** 제목의 "(기각)"이 무엇을 기각한 것인지 확인하지 않고 추론했다. 본문을 읽은 결과:

| 재결 | **채택된** 안분 기준 | 배척된 것 |
|---|---|---|
| 국심1992서2655 (1992.09.09) | **면적비** — 이유: "**기준시가가 동일하므로** 면적비율로 안분" | 청구인의 감정가액 안분(그 감정가액이 "합리성을 결여하고 비현실적") |
| 국심2005구1458 (2005.08.31) | **감정가액** — 법원 토지매각허가 감정, 신빙성 有. "2002년도 **개별공시지가보다는 감정가액으로** 안분계산하는 것이 타당" | 청구인의 **면적** 안분 |
| 조심 2018부0572 (2018.05.03) | **취득 당시 기준시가** | 청구인의 **양도 당시** 감정평가액 안분 |

**정정된 원리**:

1. 실지취득가액이 불분명하면 안분한다(3건 공통).
2. 안분 기준은 고정 서열이 아니라 **"취득 당시 각 부분의 상대적 가치를 가장 신빙성 있게 반영하는 것"**이다.
   - 취득 당시 신빙성 있는 감정가액이 있으면 **그것**(2005구1458)
   - 없으면 **취득 당시 기준시가**(2018부0572)
   - 기준시가가 부분별로 **동일하면 면적비 = 기준시가비**(1992서2655) — 면적비는 독립 기준이 아니라 특수 케이스
3. **양도 당시** 가액을 취득가액 안분 기준으로 쓰는 것은 배척(2018부0572).

**U-11 해소**: 국심2005구1458은 "취득가액 **및 필요경비**를 감정가액으로 안분"을 인정했다 → **필요경비도 취득가액과 같은 기준으로 안분**한다. 「소득세법」 제100조 제2항 후문("공통되는 취득가액과 양도비용은 해당 자산의 **가액에 비례**하여 안분계산")과 정합.

**UI 함의 (정정)**: 금지되는 것은 감정가액이 아니라 **양도 당시 가액**이다.
```
취득가액이 양도분·잔여분으로 구분되는가?
  ├ 예   → 직접 입력 (우선)
  └ 아니오 → 안분 기준 선택
       ├ 취득 당시 감정가액 (있는 경우)
       └ 취득 당시 기준시가 (기본) ← 단가 동일 시 결과가 면적비와 같다
  ※ 양도 당시 가액(감정가·실거래가) 기준 안분은 제공하지 않는다
```

### 8.9.5 U-6 — 확정: `building`도 split 토글이 열린다
...
`building`("건물(토지 제외)")에서 "토지·건물 취득일 다름"을 켤 수 있고, 그러면 `LandBuildingSplitSection.tsx:207`이 `landAreaM2: asset.acquisitionArea`를 넘긴다 — `building`의 `acquisitionArea`는 **축 B(연면적)**이다.

**β-2가 이를 해소한다**: `building`의 축 B가 `buildingFloorArea`로 이전되면 `acquisitionArea`가 빈 값이 되어 잘못된 prefill이 사라진다.

**⚠️ 남은 의문(U-12)**: "건물(토지 제외)" 자산에 토지·건물 분리가 왜 허용되는가. 라벨과 `isSplitable`이 모순이다. 세액 영향은 미측정 — β-2 후에도 별건으로 확인 필요.

---

## 9. 미검증 항목 (rev.2 → rev.7 갱신 — **U-1~U-11 전건 해소**)

| # | 항목 | 상태 |
|---|---|---|
| ~~U-1~~ | §154⑦ 한도 검증의 실제 세액 영향 | ✅ **해소** — 초과 200㎡ ↔ 0㎡로 갈린다. **세액 결함 확정** (§8.5) |
| ~~U-2~~ | `prefillForm` ↔ `restoredForm` 병합 우선순위 | ✅ **해소** — prefill 승리, 빈 값 미주입 (§3.4) |
| ~~U-3~~ | `nblHousingFootprint` ↔ `buildingFootprintArea` 개념 동일성 | ✅ **해소 — 다르다.** 법 제89조①3호 "**건물**이 정착된 면적" ↔ 법 제104조의3①5호 "**주택**이 정착된 면적". 겸용에서 갈림 → **F3 통합 부정**, "이중 입력 UX 개선"으로 격하 (§8.9.1) |
| ~~U-4~~ | 축 B(연면적) 표시가 필요한 결과 카드 지점 | ✅ **해소 — 불필요.** 정본 표시는 「건물 기준시가 계산서」 서식. GB조차 결과 카드에 연면적 미표시 → F1 ⑦ 변경 없음 (§8.9.2) |
| ~~U-5~~ | `land` assetKind에 축 C가 필요한지 | ✅ **해소 — 추가하지 않는다.** `nblHousingFootprint`로 이미 받고 있고 그 필드는 법 제104조의3①5호 전용 개념 (§8.9.3) |
| ~~U-6~~ | `building` 자산에서 split 토글이 보이는지 | ✅ **해소 — 보인다.** `isSplitable = housing \|\| building`(`CompanionAcqPurchaseBlock.tsx:116`). β-2가 해소 (§8.9.5) |
...
| ~~U-10~~ | 「소득세법 시행령」 제176조의2 제2항의 일부양도 해석 | ✅ **해소** — 조심 2018부0572(분할 양도 직접 사례). **구분 가액 우선**·안분 기준은 면적비 아님. ⚠️ "감정가액 배척"은 **rev.7에서 정정** — 취득 당시 감정가액은 오히려 우선한다(§8.9.4) |
| ~~U-11~~ | 취득 필요경비의 안분 기준 | ✅ **해소 — 취득가액과 동일 기준.** 국심2005구1458이 "취득가액 **및 필요경비**를 감정가액으로 안분" 인정 (§8.9.4) |
| **U-12** | "건물(토지 제외)" 자산에 토지·건물 분리가 허용되는 이유 — 라벨과 `isSplitable` 모순 | 🔴 미검증 — β-2 후에도 별건 확인 필요 (§8.9.5) |

**착수 조건 ✅ 전건 해소**: U-1~U-11 모두 실측 완료. F1은 즉시 착수 가능하다.

**F1 범위가 검증으로 좁아졌다**:
- ⑦(결과 카드) **변경 없음** (U-4)
- `land`에 축 C **추가하지 않음** (U-5)
- F3(NBL 정착면적 통합)은 **부정** — 법령 개념이 달라 통합 불가, "이중 입력 UX 개선"으로 격하 (U-3)
- 남은 U-12는 별건이며 β-2가 부수적으로 해소한다
...
| 날짜 | 버전 | 변경 |
|---|---|---|
| 2026-07-30 | v1.6 (rev.7) | **U-3·U-4·U-5·U-6·U-11 검증 완료 → U-1~U-11 전건 해소**(§8.9). 🔴 **U-3: 두 필드는 개념이 다르다** — 「소득세법」 제89조 제1항 제3호는 "**건물**이 정착된 면적"(비과세 한도), 제104조의3 제1항 제5호는 "**주택**이 정착된 면적"(NBL). 배율표는 동일하나 곱셈 대상이 달라 겸용에서 갈리고, 코드도 이미 `residentialFootprintArea = buildingFootprintArea × 주택연면적비율`로 구분한다 → **F3 통합 부정**. U-4: 축 B 결과 카드 표시 불필요(정본은 「건물 기준시가 계산서」 서식) → F1 ⑦ 무변경. U-5: `land`에 축 C 추가하지 않음. U-6: `isSplitable = housing \|\| building` 확인 → β-2가 해소. 🔴 **rev.6 정정 — "감정가액 안분 배척"이 틀렸다**: 국심2005구1458은 감정가액 안분을 **채택**했고(면적 안분을 부인), 국심1992서2655는 "**기준시가가 동일하므로** 면적비"였다. 정정된 원리 = 안분 기준은 고정 서열이 아니라 **취득 당시 상대가치의 신빙성 판단**이며, 금지되는 것은 **양도 당시 가액**이다. U-11: 필요경비도 취득가액과 동일 기준(2005구1458). 신규 U-12(라벨 "건물(토지 제외)" ↔ `isSplitable` 모순) |
| 2026-07-30 | v1.5 (rev.6) | **U-10 법령 확인 완료 — §8.7 권고 정정**(§8.8, anchor 3건). 「소득세법 시행령」 제176조의2 제2항 원문은 **면적을 언급하지 않는다**. 🔑 **조심 2018부0572(2018.05.03, 기각)**가 분할 양도 직접 사례 — 분할전 1,134㎡ 중 644㎡ 양도, 처분청이 **취득 당시 기준시가 비율로 안분**한 것이 정당. 도출 제약 3: **L-1 안분 기준은 면적비가 아니라 취득 당시 기준시가 비율**(2018부0572는 단가가 동일해 우연히 면적비와 같아진 사안 — §8.7의 "면적비 안분"은 특수케이스였다), **L-2 안분은 실지취득가액 불분명 시의 보충 방법**(계약서 구분 가액 우선 → 무조건 자동 안분 금지), **L-3 환산 분자는 양도한 부분의 취득 당시 기준시가**. 감정가액 안분은 3건 모두 배척(2005구1458·1992서2655 포함). UI 함의: "취득가액이 구분되는가"를 물어야 한다. 신규 U-11(취득 필요경비 안분 기준) |
