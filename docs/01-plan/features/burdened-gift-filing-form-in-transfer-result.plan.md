...
**작성일** 2026-08-12 · **개정** R3 (구현 후 — 결과뷰 경로 누락 사고 반영) · **상태** 구현 완료

> ## 🔴 R3 — 구현 후 사용자 제보로 드러난 누락 (같은 유형 2회째)
>
> R2까지의 계획은 삽입 지점을 **`TransferTaxResultView` 하나**로 잡았다. 그런데 **결과뷰는 하나가 아니다**:
>
> | 경로 | 결과뷰 | 부담부증여 도달 |
> |---|---|---|
> | 단건 | `TransferTaxResultView` | ✅ (R2에서 배선) |
> | **일반건물 일괄** | **`BundledAllocationCard`** | ✅ — **R2에서 누락** ← 사용자가 본 증상 |
> | 다건 | `MultiTransferTaxResultView` | ❌ 도달 불가(`transfer-tax-validate.ts:124-126`이 다자산+부담부증여 차단) |
> | 겸용주택 | `MixedUseResultCard` | 기준시가 계산서 자체가 없음 |
>
> route가 일반건물을 `mode: "bundled"`로 분기하므로(`route.ts:440`) 그쪽 카드가 종착지가 된다.
> **E2E 5건이 전부 통과했는데도 실제 화면엔 없었다** — spec이 `commercial_building`만 봤기 때문이다.
>
> ⚠️ **같은 실패가 한 달 안에 두 번 났다.** `BundledAllocationCard.tsx:618-621` 주석에
> 「건물 기준시가 계산서」가 2026-08-11에 똑같이 단건 뷰에만 배선돼 일반건물에서 사라졌던 기록이 있다.
> 그 주석을 읽고도 반복했다.
>
> **대응**: 섹션을 `BurdenedGiftFilingFormSection`으로 추출해 두 뷰가 **같은 것을 렌더**하게 하고,
> 렌더 조건도 공용 술어 `hasBurdenedGiftFilingForm`으로 단일화했다(화면 게이트 ↔ 선택 출력 가용 목록 일치).
> E2E에 **경로별 케이스**를 추가하고, 배선을 끈 mutation으로 안전망이 실제로 실패하는지 확인했다.
>
> **부수 누락**: `availablePrintIds`에 `gift-filing-form`을 넣지 않아 선택 출력 패널에 항목이
> 뜨지 않았다(화면 렌더만 보는 E2E라 못 잡음). BGF-6으로 고정.
