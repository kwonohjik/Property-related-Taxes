...
## 4. ✅ Phase C 결론 — `building_site`는 §168의12가 아니라 지방세령 §101 경로다
...
| 지방세령 §101①**2호** | "건축물의 부속토지 … **건축물의 바닥면적**(시설물은 수평투영면적)에 **제2항 용도지역별 적용배율**을 곱하여 산정한 면적 범위의 토지" |

→ **비주택 건축물의 부속토지는 §168의12(주택부수토지)가 아니라 지방세령 §101①2호·②의 배율로 판정**된다. 엔진이 `landType === "building_site"`를 `housing-land.ts`(§168의12) 경로로 보내는 것은 **인용·구조상 부정확**하다.
...
`urban-area.ts:92~94` 주석은 "§168의9(주택)·§168의12(건축물) **두 조문의 배율표는 현재 동일**"이라고 기록한다. 그러나:

1. 주석의 조문 대응이 실측과 다르다 — **§168의12는 "주택부수토지의 범위"**이고 건축물 부수토지 조문이 아니다(§4 표).
2. **지방세령 §101②의 배율표를 법제처 API가 반환하지 않았다**("적용배율은 다음과 같다." 뒤 표 누락 — 조문 내 표 미반환은 알려진 한계). 따라서 §168의12 배율표와 §101② 배율표가 실제로 동일한지 **미확인**이다.

→ 동일하면 §4는 **인용·구조 문제**(세액 동일), 다르면 **세액 오답**이다. 국세청 해설서·조세심판례 이미지 등 대체 소스로 §101② 표를 확보해야 한다(선례: `project_transfer_surcharge_transition_na_da`가 조문 내 표를 해설서 이미지로 확정).
...
Phase C  ✅ 완료 — building_site는 §168의12가 아니라 지방세령 §101①2호·② 경로 (§4)
         🟡 잔여: §101② 배율표 확보(법제처 API 표 미반환) → 세액 영향 여부 확정
