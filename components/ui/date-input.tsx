...
export function DateInput({ value, onChange, className, disabled }: DateInputProps) {
  ...
  /**
   * 내부에서 보낸 onChange 횟수를 추적.
   * useEffect가 실행될 때 카운터가 남아있으면 내부 변경으로 간주해 동기화를 건너뜀.
   * → backspace로 day가 비워져 onChange("") 가 호출돼도 year·month가 초기화되지 않음.
   * → 외부(폼 리셋 등)에서 value가 바뀌면 카운터가 0이므로 정상 동기화.
   */
