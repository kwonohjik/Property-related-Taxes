...
interface FormState {
  ...
  /** 수증자 본인 식별자 — 사전증여 이력 조회 시 동일인 매칭 (이름 + 생년월일) */
  doneeName: string;
...
function validateStep(step: number, form: FormState): string | null {
  if (step === 0) {
    ...
    if (!form.doneeName.trim()) return "수증자 이름을 입력하세요. (이력 조회 시 동일인 판단)";
    if (!form.doneeBirthDate) return "수증자 생년월일을 입력하세요. (이력 조회 시 동일인 판단)";
  }
  if (step === 1) {
...
function Step0({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* 수증자 본인 식별 (이름 + 생년월일) — 사전증여 이력 조회 시 동일인 판단 */}
      <div className="rounded-lg border border-sky-200 bg-sky-50/40 p-3 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs font-semibold text-sky-700">
            수증자 본인 식별 (이력 조회 시 동일인 판단용)
          </p>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-1">
            <label className="block text-[11px] font-medium text-sky-700">
              수증자 이름 <span className="text-destructive">*</span>
            </label>
            <input
              type="text"
              value={form.doneeName}
              onChange={(e) => set({ doneeName: e.target.value })}
              placeholder="예: 홍길동"
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
          </div>
          <div className="space-y-1">
            <label className="block text-[11px] font-medium text-sky-700">
              수증자 생년월일 <span className="text-destructive">*</span>
            </label>
            <DateInput
              value={form.doneeBirthDate}
              onChange={(v) => set({ doneeBirthDate: v })}
            />
          </div>
        </div>
        <p className="text-[11px] text-sky-600">
          ※ 수증자 이름과 생년월일이 모두 일치하는 과거 회차만 이력 조회 결과에 노출됩니다.
        </p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        <PriorGiftInput
          ...
          currentDoneeName={form.doneeName}
          currentDoneeBirthDate={form.doneeBirthDate}
        />
      </div>
    </div>
  );
