 ...
 *  - §102② 통산 표 (차손 → 토지 흡수)
 *
 * ## ⚠️ 현재 이 컴포넌트는 **어디에서도 렌더되지 않는다** (참조 0건 — 주석 언급만 있다)
 *
 * 단건 결과뷰가 `(result as any).generalBuildingValuationDetail`로 게이트하고 있었으나 그
 * 필드는 **집계 객체에만** 대입되고, 일반건물은 route가 `mode:"bundled"`로 분기해 그 뷰 자체가
 * 뜨지 않았다 — 어떤 입력으로도 도달 불가한 블록이라 제거했다(결과탭 코드리뷰 #066).
 *
 * ## 왜 일괄 뷰에 배선하지 않았나 — 실측으로 **중복**임을 확인했다
 *
 * 일괄 뷰는 ①saleSplitJudgment·②상속취득·③환산주택가격을 인라인으로 재구현해 두었고,
 * 남은 ④·⑤는 **일괄 신고서 양식 표가 이미 같은 값을 토지·건물 열로 보여준다**.
 * 실측(일반건물 실가 일괄, 양도 10억):
 *
 * | ④ 카드 행 | 일괄 신고서 표 (토지 / 건물) |
 * |---|---|
 * | 양도가액 | 750,000,000 / 250,000,000 |
 * | 환산취득가 | 취득가액 200,000,000 / 200,000,000 |
 * | 기타필요경비(개산공제) | 필요경비 0 / 0 |
 * | 양도차익 | 전체 양도차익 550,000,000 / 50,000,000 |
 * | 장기보유특별공제 | 110,000,000 / 10,000,000 |
 * | 양도소득금액(통산 전) | 440,000,000 / 40,000,000 |
 *
 * ⑤(§102② 통산)도 마찬가지다 — 건물 차손 격자에서 신고서 표가
 * 「양도소득금액 456,000,000 / −470,000,000 → 감면후 소득금액 0 / 0」을 보여주고,
 * 그 감소분에 §102② 근거 note가 붙는다(`FilingFormTableAggregateHelpers`).
 *
 * ⇒ 배선하면 같은 숫자를 두 표로 보여줄 뿐이고, 카드가 요구하는 prop 8개
 *   (`lthdRateLand`·`incomeLandPreOffset`·`offsetAmount` 등)를 집계 결과에서 새로 도출해야 해
 *   **오표시를 새로 만들 위험만 진다**. 그 중복을 `gb-bundled-filing-table-parity.anchor.test.ts`가
 *   고정한다 — 그 anchor가 깨지면 그때 이 카드를 되살릴 근거가 생긴다.
 *
 * ⛔ 되살리려면 위 anchor를 먼저 확인할 것. 「카드가 있으니 배선하자」는 중복을 만든다.
 */
