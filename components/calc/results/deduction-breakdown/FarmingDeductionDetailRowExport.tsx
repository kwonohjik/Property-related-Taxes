...
/**
 * FarmingDeductionDetailRowExport — FarmingDeductionDetailRow 하위 호환 re-export
 *
 * farming-section.test.tsx 가 InheritanceTaxResultView 에서
 * export function FarmingDeductionDetailRow 를 직접 import.
 * 이 파일은 InheritanceTaxResultView.tsx 에서 re-export해 경로 보존.
 */
...
function labelMatchKind(
  ...
  switch (kind) {
    case "same_district": return "동일 시·군·구";
    case "adjacent_district": return "연접 시·군·구";
    case "within_30km": return "30km 직선거리";
    case "forest_manageable_area": return "산림지 통상 경영 지역 (사용자 명시)";
    case "fail": return "자동 미충족 (사용자 명시값 적용)";
    case null: return "미입력";
  }
}

function formatKRWLocal(n: number): string {
  return n.toLocaleString();
}

/**
 * 기존 FarmingDeductionDetailRow — 인라인 상세 (▼ 펼침 없는 레거시 스타일).
 * farming-section.test.tsx 에서 직접 사용 (경로 보존 필수).
 */
    ...
    return (
      <div className="mx-4 my-2 rounded-md border border-amber-300 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800 p-2 space-y-1">
        <p className="text-caption font-semibold text-amber-800 dark:text-amber-200">
          ⚠️ 입력 자산 {formatKRWLocal(detail.appliedAssetValue)} — 자격 미충족으로 공제 0원
        </p>
  ...
  return (
    <div className="mx-4 my-2 space-y-1">
      <div className="text-caption text-gray-600 dark:text-gray-400">
        ⓘ 영농자산 {formatKRWLocal(detail.appliedAssetValue)}
        {capped && ` (30억 한도 적용 → ${formatKRWLocal(detail.cappedDeduction)})`}
      </div>
