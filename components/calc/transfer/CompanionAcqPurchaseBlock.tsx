                ...
                hint={
                  ...
                  props.asset?.areaScenario === "partial" && !isMixedUse
                    ? // 증축이면 이 칸은 「토지+원건물 **일괄**」이므로 두 축이 겹친다 —
                      //   ① 여기서 양도분을 뽑고 ② 엔진이 §166⑥으로 토지·건물1에 나눈다(O-4).
                      isBundledExtension
                      ? "일부 양도 — 양도한 부분에 대응하는 토지·원건물 일괄 취득가액을 입력하세요(아래 계산기로 산출 가능). 그 금액을 엔진이 취득시 기준시가 비율로 토지·건물1에 안분합니다. 증축분 취득가액은 아래 증축 항목에 따로 입력합니다."
