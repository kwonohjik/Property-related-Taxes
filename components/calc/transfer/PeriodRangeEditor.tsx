 ...
 *
 * 항상 표시(토글 없음). 최소 1행을 기본 노출 — 사용자는 반드시 1건 이상 입력해야 하므로
 * "활성화" 단계가 불필요. 비연속 다중 구간(공실·거주이탈 제외) 허용.
 * 법 badge·문맥 문구는 포함하지 않음(호출처 주입). 개월은 whole-month(diffMonthsClamped) 재사용.
 */

import { DateInput } from "@/components/ui/date-input";
import { ToggleCardTone } from "@/components/calc/inputs/ToggleCard";
...
export interface PeriodRangeEditorProps {
  ...
  testidPrefix: string;
  onChange: (patch: { periods: PeriodRow[] }) => void;
}
...
export function PeriodRangeEditor({
  ...
  // 최소 1행 표시(빈 상태에서도 입력 가능). 실제 저장은 사용자 입력 시 onChange로 생성.
  const rows: PeriodRow[] = periods.length > 0 ? periods : [{ start: "", end: "" }];
  const totalMonths = rows.reduce((s, p) => s + diffMonthsClamped(p.start, p.end), 0);
  ...
  const toneBg =
    tone === "emerald" ? "bg-emerald-50/40" : tone === "violet" ? "bg-violet-50/40" : "bg-sky-50/40";
  const toneBox =
    tone === "emerald"
  ...
  function setPeriod(idx: number, patch: Partial<PeriodRow>) {
    onChange({ periods: rows.map((p, i) => (i === idx ? { ...p, ...patch } : p)) });
  }
  function addPeriod() {
    onChange({ periods: [...rows, { start: "", end: "" }] });
  }
  function removePeriod(idx: number) {
    // 마지막 1행은 남겨 항상 입력 가능 상태 유지(빈 배열 → 다시 가상 1행 표시)
    onChange({ periods: rows.filter((_, i) => i !== idx) });
  }

  return (
    <div
      data-testid={`${testidPrefix}-editor`}
      className={`rounded-lg border ${toneBorder} ${toneBg} p-3 space-y-2`}
    >
      {rows.map((p, idx) => {
        ...
        return (
          <div key={idx} className={`rounded-md border ${toneBorder} bg-white p-3 space-y-2`}>
            ...
            <p className={`text-caption ${toneText}`}>이 구간: {fmtPeriodMonths(m)}</p>
          </div>
        );
      })}
      <button
