...
export function Step1({
  ...
  return (
    <div className="space-y-4">
      {/* 면적·시가표준액 — sky 카드 */}
      <ToneCard tone="sky" sectionNum={1} title="물건 상세" bodyClassName="space-y-3" noDark>
        {isHousing && (
          <div>
      ...
      <ToggleCard
        tone="amber"
        title="사치성 재산"
        description="골프장·고급주택·고급오락장·고급선박 — 중과세 적용 (지방세법 §13⑤)"
        checked={form.isLuxuryProperty}
        onCheckedChange={(v) => set("isLuxuryProperty", v)}
        trailing={
          <TaxHelp
            title="사치성 재산 정의 및 판정 기준"
            summary="사치성 재산은 표준세율 + 8%p 중과. 별장(§13⑤1호)은 2023.3.14부터 삭제."
            details={`## 사치성 재산 (지방세법 §13⑤)

## 골프장 (§13⑤2호)
「체육시설법」상 회원제 골프장용 부동산 중 구분등록 대상 토지·건축물 (대중제 골프장 제외)

## 별장 (§13⑤1호 — 2023.3.14 삭제)
주거용 건물로 상시 주거 목적이 아닌 휴양·피서·놀이 용도.
**2023.3.14 이후 취득분은 중과 미적용** (지방세법 §13⑤ 1호 삭제).
