...
/**
 * EstateBodySimple — cash·other 본체 입력
 *
 ...
 *   - other: 자산명 + 시가("시가") + 감정가
 *
 * financial 카테고리는 EstateBodyFinancial (§63④ 정밀평가 지원)로 분리됨.
 */
...
const NAME_PLACEHOLDER: Record<"cash" | "other", string> = {
  cash: "선택 입력 (예: 현금 보유)",
  other: "선택 입력",
};

const MARKET_LABEL: Record<"cash" | "other", string> = {
  cash: "현금 금액",
  other: "시가 (매매·수용·경매가액)",
};

const MARKET_HINT: Record<"cash" | "other", string> = {
  cash: "지폐·동전 실제 보유액 (§22 금융재산공제 미적용)",
  other: "평가기간(±6개월) 내 실거래가",
};

const PRIORITY_HINT: Record<"cash" | "other", string> = {
  cash: "현금 액면가 = 시가 (상증법 §60) — §22 금융재산공제 대상 아님",
  other: "시가 우선 원칙 (상증법 §60)",
};

const SUBTITLE: Record<"cash" | "other", string> = {
...
export function EstateBodySimple({ item, onUpdate }: VariantBodyProps) {
  const cat = item.category as "cash" | "other";
