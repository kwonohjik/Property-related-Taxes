/**
 * ⑫ 양도세 Zod — **단건 기본 필드 객체** (800줄 분리, 2026-09-11)
 *
 * `transfer-tax-schema.ts` 가 825줄로 정책(트리거 800 · 착지 ≤700)을 넘겨 분리했다.
 * 이 파일은 `propertyBaseShape` **하나**만 담는다 — 429줄짜리 객체 리터럴이라
 * 그 자체가 가장 넓은 이음매다. 바깥 선언을 **하나도 참조하지 않는다**(단방향 실측).
 *
 * ⚠️ **여기서 키를 빠뜨리면 Zod 가 조용히 strip 한다** — ⑫는 TypeScript 가 감지하지 못하는
 *    동기화 지점이다(루트 CLAUDE.md 14지점). 필드를 더할 때 ④⑬⑭도 함께 볼 것.
 */
...
export const propertyBaseShape = {
  ...
  mergedHouseholdFirstHouse: mergedHouseholdFirstHouseSchema.optional(),
  ...
  /**
   * 양도시 감정평가가액 — 안분 basis 서열 1순위(부가령 §64①1호 단서 · 소령 §166⑥ 차용).
   ...
   * 근거 텍스트(`saleSplitExemptionNote`)는 계산에 쓰이지 않아 **전송하지 않는다**(UI 전용).
   */
