...
/**
 * 한글 오타 복원 — **일상 어휘에 쓰이지 않는 글자만** 담는다.
 *
 * 🔴 종전엔 셰·쉐→세, 괸·곽→관, 엄·얼→업 도 들어 있었는데, 이 글자들은 정상 한국어에
 *    흔히 쓰여 멀쩡한 검색어를 훼손했다(실측):
 *      "엄격해석 원칙" → "업격해석 원칙" · "곽재우 판례" → "관재우 판례" · "존엄사" → "존업사"
 *    이 정규화는 판례 검색어에도 그대로 적용되므로 0건 응답의 원인이 된다.
 *    남긴 벚·벆·벋·뻡·볍·뱝 은 "법" 의 인접 오타로, 일반 어휘에 사실상 등장하지 않는다.
 */
...
function normalizeBasicTypos(value: string): string {
  return value.replace(/[벚벆벋뻡볍뱝]/gu, (c) => BASIC_CHAR_MAP.get(c) ?? c);
}
...
export function normalizeLawSearchText(input: string): string {
  ...
  v = v
    ...
    .replace(/[﹦=]/gu, " ")
    // §89 → "제89조" · §18의2 → "제18조의2".
    // 🔴 종전엔 §를 " 제" 로만 바꿔 "소득세법 제89" 가 됐고, 조문 라우팅 패턴은 "조" 를
    //   요구하므로 매칭에 실패해 일반 검색으로 폴백했다(실측: fallback_search_law).
    //   저장소 자체 인용 표기가 § 라 검색바에서 가장 흔한 입력 형태다.
    .replace(/§\s*(\d+)(?:\s*의\s*(\d+))?/gu, (_m, a, b) => (b ? ` 제${a}조의${b}` : ` 제${a}조`))
    .replace(/§/gu, " 제")
    .replace(/\s*[-]\s*/gu, "-")
    .replace(/\s*\.\s*/gu, " ");
