 ...
 *
 * 토지 필지 시군구(재산세 합산 단위) 자동 채움용. parseAddressRegion(조정대상지역 파서) 재사용 +
 * resolvePropertyTaxJurisdiction(재산세 과세권자 정규화)으로 일반구는 시 단위로 축약.
 *   "서울특별시 송파구 ..." → "송파구" (자치구)
 *   "경기도 성남시 수정구 ..." → "성남시" (일반구 → 시 단위 합산)
...
export function deriveSigunguFromAddress(address: string | undefined | null): string | null {
  ...
  // 재산세 합산 단위로 정규화 (일반구 "성남시 수정구" → "성남시")
  return resolvePropertyTaxJurisdiction(sigungu);
}
