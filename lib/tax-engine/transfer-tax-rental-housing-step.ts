...
import { buildEligibilityContext, buildRentalPeriodPendingNotice } from "./transfer-tax-rental-housing-judge";
...
export {
  isPrhpScenarioB,
  judgeRentalHousingEligibility,
  isPrhpScenarioAIneligible,
  buildRentalPeriodPendingNotice,
  rentalNoticesForEarlyReturn,
  canEarlyReturnPrhp,
} from "./transfer-tax-rental-housing-judge";
...
export function runRentalHousingExceptionStep(
  ...
  const rhe = calculateRentalHousingException(
    ...
    liveYears,    // 거주주택 거주연수 (B의 1호 거주요건은 context의 등록 이후 거주기간 — OH-15)
    // OH-13 — 고가주택 기준은 양도일 기준(2021-12-07 이전 9억). 12억 고정 금지.
    ...
    ...(reductionNotice || lthdNotice || periodPendingNotice || rhe.eligibility.notices?.length
      ? {
          warnings: [
            ...
            lthdNotice,
            periodPendingNotice,
            // OH-40·§7-5 판정 보류·확인 필요 고지 — 결론은 바꾸지 않는다.
            ...(rhe.eligibility.notices ?? []),
