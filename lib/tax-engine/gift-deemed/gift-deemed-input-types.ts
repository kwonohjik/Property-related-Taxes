...
export interface CapitalIncreaseInput {
   ...
   * 그 밖=주식대금 납입일, 신주인수권증서 교부 시 교부일).
   *
   * **행위시법 판정에만** 쓴다(§29③ 간주모집 시기 게이트). 산식에는 접촉하지 않는다.
   * 폼은 이미 증여일을 필수로 수집하고 있었으나 엔진까지 배선돼 있지 않았다.
   * 미입력이면 시기 게이트를 적용하지 않는다(현행 동작 유지).
   */
  giftDate?: Date;
  /**
   * 수증자(이익을 얻는 자)가 **영리법인**인가 — 「상증법」§2 9호·§4의2①상 증여세
   * **납세의무자 범위에 없다**. 같은 조 ③이 「법인세가 부과되는 경우에는 증여세를 부과하지
   * 아니한다」로 보강하고, 「법인세법 시행령」§11 8호·§88①8호나목이 자본거래 분여이익을
   * 익금에 산입하며 「법인세법 시행령」 제89조제6항이 그 계산에 「상증법」§39·「상증령」§29②를 **준용**한다.
   *
   * ⚠️ 이익 자체를 부정하는 플래그가 아니다 — 산출근거(breakdown)는 그대로 남기고 과세분만 0으로 둔다.
   * ⚠️ 미지정은 **개인**으로 본다(안전측 — 법인으로 오판하면 과소과세 방향이다).
   */
  doneeIsForProfitCorp?: boolean;
  /**
   * 「상증법」§4의2④ 요건 ㉠ — **발행법인(영리법인)이 증여받은 재산 또는 이익**에
   * 「법인세법」에 따른 법인세가 부과되었는가(비과세·감면 포함).
   *
   * ⚠️ `doneeIsForProfitCorp`(①·③)와 **수범자가 다르다** — 그쪽은 수증자 자신이
   * 영리법인인 경우이고, 이쪽은 **그 법인의 주주등**이 증여세를 면한다.
   *
   * 근거: 국세청 과세기준자문 기준-2022-법무재산-0178[법무과-4943](생산 2023.07.13.) —
   * 출자전환 고가발행으로 발행법인에 채무면제익 법인세가 부과된 사안에 §39를 적용할 수
   * 없다고 회신했다. 원문은 `docs/00-pm/gift-deemed-taxpayer-gate-4-2.plan.md` §1.
   */
  issuerGainCorporateTaxed?: boolean;
  /**
   * 「상증법」§4의2④ 요건 ㉡ — 수증자가 **그 법인의 주주등**인가.
   *
   * ⚠️ **목이 조문으로 확정하는 경우 이 값은 무시된다**(`taxpayer-gate.ts`) — 1호 다목은
   * 「주주등이 **아닌** 자」, 1호 라목·2호 전부는 「주주등」이다. 사용자 입력이 법문을
   * 이기지 못하게 한다. 사안 의존인 1호 가·나목에서만 이 값이 판정을 가른다.
   *
   * ⚠️ 미입력은 **배제하지 않는다**(요건 미입증). 미입력을 「주주다」로 읽으면 과소과세
   * 방향이다 — §39② 소액주주 `faceValueSum`과 같은 층위의 판단이다.
   */
  doneeIsShareholderOfIssuer?: boolean;
...
export interface CapShareholder {
  ...
  allocationMethod?: ShareAllocationMethod;
  /**
   * 수증자가 **영리법인**인가 — 「상증법」§2 9호·§4의2①상 증여세 **납세의무자 범위에 없다**.
   * 같은 조 ③이 「법인세가 부과되는 경우에는 증여세를 부과하지 아니한다」로 보강하고,
   * 「법인세법 시행령」§11 8호·§88①8호나목이 자본거래 분여이익을 익금에 산입하며
   * 「법인세법 시행령」 제89조제6항이 그 계산에 「상증법」§39·「상증령」§29②를 **준용**한다.
   *
   * ⚠️ **이익 자체를 0으로 만드는 플래그가 아니다.** 부정되는 것은 증여세 부과이지 이익의 존재가
   *    아니다 — 그 금액은 법인령 §89⑥ 준용 계산액으로 그대로 쓰인다. 검증내역(`delta`·
   *    `byShareholder`·`reconciliation`)은 **보존**하고 과세분만 0으로 둔다.
   * ⚠️ 미지정은 **개인**으로 본다(안전측 — 법인으로 오판하면 과소과세 방향이다).
   */
  isCorporate?: boolean;
  /**
   * 이 주주가 소유한 **주식등의 액면가액 합계액** — 「상증령」§29⑤ 소액주주 판정의 두 요건 중 하나다.
   *
   *   「상증령」§29⑤ — 「…발행주식총수등의 **100분의 1미만**을 소유하는 경우로서 주식등의
   *                    **액면가액의 합계액이 3억원 미만**인 주주등」 (두 요건 모두 strict)
   *   「상증법」§39② — 「**제1항제1호를 적용할 때** 이익을 증여한 자가 … 소액주주로서 2명 이상인
   *                    경우에는 이익을 증여한 소액주주가 **1명인 것으로 보고 이익을 계산한다**」
   *
   * 지분율 요건은 `preShares ÷ Σ preShares`로 구할 수 있으나 액면 요건은 **외부 입력 없이는
   * 판정할 수 없다**(액면가액은 정관 사항이고 평가액·인수가와 다른 축이다).
   *
   * ⚠️ **미입력은 「소액주주 아님」으로 본다.** §39②은 「소액주주인 경우」에 의제하므로 요건이
   *    입증되지 않은 주주를 의제할 근거가 없다. 미입력이면 증여자가 각각 남아 §47② 동일인
   *    합산이 덜 되는 방향이지만, 그것은 **입력 결손**이지 엔진이 메울 값이 아니다
