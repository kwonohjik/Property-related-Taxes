...
export interface DonationSplit {
  ...
  /**
   * 「상증법」§39② 소액주주 1인 의제로 **한 행에 합쳐진** 증여자 id들 (2명 이상일 때만 존재).
   * `donorId`는 그중 대표(입력 순서상 첫 번째)다 — 의제된 「1명」은 특정 실존 주주가 아니므로
   * 표시·키 용도로 실재 id 하나를 대표로 쓰고, 합쳐진 전원은 이 배열이 보존한다.
   *
   * ⚠️ 합계는 **불변**이다 — §29② 각 호 산식에 「증여자 수」가 변수로 들어가지 않고 증여자측
   *    수량은 전부 주식수의 합이라, N명을 1명으로 보아도 그 합이 같다. 이 필드가 바꾸는 것은
   *    §47②(동일인 합산) → §26·§56 누진구간에 걸리는 **증여자 단위**뿐이다.
   */
  imputedSmallShareholderIds?: string[];
}
