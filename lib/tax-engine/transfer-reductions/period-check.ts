...
const RULES: Record<TransferReductionId, PeriodRule> = {
  ...
  rental_97_4: {
    ...
    failReason: "임대 등록일이 2014.1.1 미만 — 조특법 §97의4 적용 시한 외",
  },
  rental_97_5: {
    ...
    check: (c) =>
      ...
      //      즉 아래 `rental_97_4`의 2014-01-01 **하한은 방향이 반대**이고 납세자에게 불리하다.
      //
      //   ⚠️ 그럼에도 지금 제거하지 않는다 — 2014-01-01의 출처가 조특법 §97의4 신설
      //      **법률 제12173호 부칙(적용례)의 시행일**일 가능성이 있고, 그렇다면 정정 방향은
      //      「제거」가 아니라 **축 이동(등록일 → 양도일)** 이다. 부칙 원문을 확인하지 못한
      //      상태에서 어느 쪽으로 바꾸든 근거 없는 변경이 된다.
      //      (memory `feedback_unverified_authority_blocks_tax_change`)
      //      ⇒ 부칙 확보 시 CB-01로 처리할 것. 소령 §167의3 단서의 2018-03-31 상한도
      //         그 단서의 **신설 시기**를 확인한 뒤 함께 넣는다(행위시법 — 소급 불가).
