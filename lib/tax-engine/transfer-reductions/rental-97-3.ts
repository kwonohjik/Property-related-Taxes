 ...
 * ✅ **원문 확보 (2026-09-14)** — 종전 기재(「efYd·연혁 MST NOT_FOUND로 부칙 조항 직접 인용은
 *    추후 보강」)는 **오진**이었다. 막힌 것은 도구가 아니라 조회 방식이었고, 저장소의
 *    `fetchLawVersions`/`fetchEflawArticle`(법제처 `target=eflaw`)로 과거 시행본이 조회된다.
 *    · **종전 문언 = 2022-12-08 시행본(mst 237393) §97의3①** 원문: 「…2020년 12월 31일
 *      (민간건설임대주택의 경우에는 2022년 12월 31일)까지 … 등록[…]하여 다음 각 호의 요건을
 *      모두 갖춘 경우 … 100분의 50의 공제율을 적용한다. 다만, 장기일반민간임대주택등을 10년 이상
 *      계속하여 임대한 후 양도하는 경우에는 100분의 70의 공제율을 적용한다. 1. **8년 이상**
 *      계속하여 임대한 후 양도하는 경우」
 *    · **경과조치 = 법률 제19199호 부칙 §38** — 이 파일 :110~ 주석·:129 breakdown이 이미 인용해
 *      판정에 쓰고 있다(`9b1b6edd`). 즉 근거는 진작 들어와 있었고 이 헤더만 옛 상태로 남아 있었다.
 */
