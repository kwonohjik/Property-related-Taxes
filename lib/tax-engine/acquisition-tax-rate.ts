 ...
 *
 * ── 간주취득(§15②)은 §5⑤이 별도 규정한다 (2026-09-12 확인) ──────────────────
 *
 * §5①6호는 「§11 및 §12의 **표준세율**을 100분의 2로 적용하여 산출한 취득세액」이 과세표준인데,
 * §15② 취득에는 그 표준세율이 등장하지 않는다(본문이 통째로 중과기준세율로 치환된다).
 * 그 공백을 메우는 것이 **§5⑤**(MST 285905):
 *
 * > ⑤ 제1항제6호에도 불구하고 「지방세법」 **제15조제2항에 해당하는 경우에는 같은 항에 따라
 * >   계산한 취득세액**을 제1항제6호의 과세표준으로 본다.
 *
 * ⇒ 간주취득 농특세 = **§15②로 계산한 취득세액 × 10%**.
 *
 * ⚠️ **아래 코드는 §5⑤ 전용 분기를 두지 않는다.** 간주취득에서는 `basicRate`가 곧
 *    중과기준세율(2%)이라 위 §5①6호 산식이 §5⑤과 **항등**이기 때문이다:
 *      `2% + (적용세율 − 2%) = 적용세율`
 *    2%·6%(§13①)·10%(§13⑤) 세 경우 모두 성립한다. 값이 맞는 것은 이 항등 덕이지
 *    §5⑤을 따로 구현해서가 아니므로, 한쪽이 바뀌면 조용히 어긋난다.
 *    ⇒ anchor `[AT-RST]`가 항등을 고정한다(역방향 짝 포함 — 비간주에는 성립하지 않는다).
 *
 * 면제(농특세법 §4):
 * - 전용면적 85㎡ 이하 주택 (전면 면제)
