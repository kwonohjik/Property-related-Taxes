...
export const TRANSFER = {
   ...
   *
   * 🔴 **A23(2026-09-03 신규 발견, 미해소) — 이 인용은 틀렸다.**
   * 「소득세법 시행령」 §162의2의 실제 제목·본문은 **「지하수개발ㆍ이용권 등의 양도가액」**이며
   * (토사석 채취허가 권리·지하수 개발이용권을 토지등과 함께 양도해 가액 구분이 안 될 때
   * §51⑧을 준용하는 규정) 환지 면적과 아무 관련이 없다. 조문 본문 확인으로 **오인용은 확정**됐다.
   *
   * ⚠️ **올바른 근거를 특정하지 못해 그대로 둔다** — 추측으로 다른 조문을 넣지 않는다.
   * 엔진이 실제 적용하는 산식은 `종전토지면적 × 교부면적 / 권리면적`이고
   * (`multi-parcel-transfer.ts` P-2), 이에 대응하는 명문을 §152(환지등의 정의)·§162①9호
   * 어디에서도 찾지 못했다. 국세청 집행기준·예규 계열일 가능성이 있어 별도 조사가 필요하다.
   *
   * ⚠️ **이 값은 `legalBasis`로 결과 화면에 노출되고 `LawArticleModal`이 법령 원문으로
   * 링크까지 건다** — 사용자가 누르면 「지하수개발ㆍ이용권」 조문이 뜬다.
   * 같은 오인용이 UI 6곳에도 복제돼 있다(`ParcelListInput.tsx` · `CompanionAssetCardReplot.tsx` ·
   * `AssetAreaSection.tsx` 주석 · 테스트 2곳 · E2E 주석 1곳) — 정정 시 함께 고칠 것.
   */
