...
export const NBL = {
  ...
  /**
   * 시행령 §168조의14 ① — 부득이한 사유 유예기간
   *
   * ⚠️ **도시지역 편입유예의 근거가 아니다** (COV-4·E3-05, 2026-09-02 코드리뷰).
   *    편입유예는 지목마다 근거가 다르다 — 농지 `FARMLAND_URBAN_GRACE`(§168조의8 ⑤⑥) ·
   *    목장 `PASTURE_URBAN_GRACE`(§168조의10 ⑤) · 임야 `FOREST_URBAN_GRACE`(§168조의9 ①2호 단서).
   *    종전에는 임야 편입유예 step이 이 상수를 인용해 결과 화면에 다른 조문이 표시됐다.
   */
  URBAN_GRACE:    "소득세법 시행령 §168조의14 ①",
  /** 시행령 §168조의9 ①2호 단서 — 임야 도시지역 편입 3년 (보전녹지지역 제외) */
  FOREST_URBAN_GRACE: "소득세법 시행령 §168조의9 ①2호 단서",
  ...
  /**
   * ── 무조건 의제 사유별 정확 인용 (U3-05 · E5-05, 2026-09-02 코드리뷰) ──
   *
   * 🔴 종전에는 `unconditional-exemption.ts`가 「시행령 §168조의14 ③ 1호」처럼 **법령명 없이**
   *    문자열 리터럴을 직접 들고 있었다. 결과 카드 ⑦ 「적용 법령」 칩에 그대로 나가
   *    「소득세법 시행령」인지 「지방세법 시행령」인지 화면만으로 알 수 없었고, 같은 카드의
   *    다른 칩(`NBL.CRITERIA` = 「소득세법 시행령 §168조의6」)과 표기 체계가 어긋났다.
   *    ⇒ 커밋 `7f44d95a`(「조문 인용에 법령명·법/령/규칙을 명시」) 규칙에 맞춰 상수로 이관.
   */
  UNCONDITIONAL_INHERIT_BEFORE_2007: "소득세법 시행령 §168조의14 ③ 1호",
  UNCONDITIONAL_LONG_OWNED_20Y:      "소득세법 시행령 §168조의14 ③ 2호",
  /** ③3호 가목 — 사업인정고시일 2006.12.31. 이전 */
  UNCONDITIONAL_PUBLIC_NOTICE_2006:  "소득세법 시행령 §168조의14 ③ 3호 가목",
  /** ③3호 나목 — 사업인정고시일부터 5년 이전 취득 */
  UNCONDITIONAL_PUBLIC_ACQ_5Y:       "소득세법 시행령 §168조의14 ③ 3호 나목",
  /** ③4호 가목 — 도시지역 內 농지 중 종중 소유(2005.12.31. 이전 취득) */
  UNCONDITIONAL_URBAN_FARMLAND_JONGJOONG: "소득세법 시행령 §168조의14 ③ 4호 가목",
  /** ③4호 나목 — 도시지역 內 농지 중 상속(상속개시일부터 5년 이내 양도) */
  UNCONDITIONAL_URBAN_FARMLAND_INHERIT:   "소득세법 시행령 §168조의14 ③ 4호 나목",
  /** ③5호 → 시행규칙 §83조의5④1호 — 공장 오염피해 인접토지 */
  UNCONDITIONAL_FACTORY_ADJACENT:    "소득세법 시행규칙 §83조의5 ④ 1호",
  /** ③5호 → 시행규칙 §83조의5④2호 — 2006.12.31. 이전 이농 농지 */
  UNCONDITIONAL_INONG:               "소득세법 시행규칙 §83조의5 ④ 2호",

  /**
   * ── 종중 소유(2005.12.31. 이전 취득) — **지목마다 근거 조문이 다르다** (E5-05) ──
   *
   * 🔴 종전에는 농·임·목 전부에 「§168조의14 ③ 4호 가목 · §168-8 ③ 6호 등」을 붙였다.
   *    둘 다 **농지 전용 조문**이라 임야·목장 판정의 근거가 될 수 없다(본문 실측 mst=286211).
   *
   * ⚠️ 농지를 §168의14③4호가목이 **아니라** §168의8③6호로 인용하는 이유: 4호가목은 본문이
   *    「법 §104의3①1호 **나목**에 해당하는 농지」, 즉 **도시지역 안의** 농지에 한정된다.
   *    이 레거시 분기는 도시지역 여부를 보지 않으므로 도시지역 요건이 없는 §168의8③6호가 정본이고,
   *    도시지역 농지는 앞선 `UNCONDITIONAL_URBAN_FARMLAND_JONGJOONG` 분기가 이미 처리한다.
   */
  JONGJOONG_FARMLAND: "소득세법 시행령 §168조의8 ③ 6호",
  JONGJOONG_FOREST:   "소득세법 시행령 §168조의9 ③ 8호",
  JONGJOONG_PASTURE:  "소득세법 시행령 §168조의10 ② 2호",
} as const;
