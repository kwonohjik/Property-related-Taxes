...
//
// 취득가액 = **가목 우선** — `clauseA = max(① 상증법 §60~66 평가액, ② §164④~⑦ 기준시가)`이고,
//   가목이 0일 때만 ③ 환산취득가로 간다. **3자 max가 아니다**(#1089 `af74d907`에서 재편 — 아래 ⚠️).
//   — ①② 소령 §163⑨ 본문·1호·2호 / ③ 소령 §163⑫ → §176조의2.
// 🔴 종전 주석은 이 max의 근거로 「국심2003부602·2003서3266」을 들었으나 **오인용이다**.
//    실제 번호는 **국심2003부0627·0626**(2003.7.25.)이고, 그 결정은 오히려 **③을 배척**했다 —
//    pre-deemed(1977 증여) 사안에서 처분청의 §176조의2④ 환산 경정을 **취소**하고 §163⑨ 적용을
//    인정했다(주문: 경정 · 납세자 승). "§176조의2④은 추계결정 규정으로서 실지거래가액을
//    인정 또는 확인할 수 없는 경우에 적용하는 규정"이라는 것이 이유다.
// ✅ **2003서3266도 확보했다**(2026-08-07 · 조세심판원 ID 112396 「필요경비 해당 여부(경정)」,
//    2004.4.29.) — 종전 주석의 "원문 미확보"는 사실이 아니다. **같은 방향의 두 번째 pre-deemed
//    정면 결정**이다: 1969·1977 공동상속 토지를 2001 양도한 사안에서 처분청이 §97①1호 **다목**
//    (현행 나목)으로 환산 경정하자, 심판원이 「취득가액을 같은조 제1항 제1호 **가목 단서**의 규정을
//    적용하여 실지거래가액(**의제취득일 현재 기준시가**)으로」 정정했다. 그 결과 필요경비도
//    개산공제 3%가 아니라 **실제 필요경비**(철거비·이주보상비)를 인정했다.
//    ⚠️ 「납세자에게 유리해서」 고른 것이 **아니다** — 이유는 가목 단서 적용이고, 실제 필요경비는
//       그 **결과**다(법 §97③1호). 이 결정을 「양도차익 유리선택」의 근거로 드는 것은 오독이다.
//    ⇒ **③을 같은 max에 두는 근거는 심판례가 아니다.** 심판례는 **양쪽 다 가목 우선**이다.
//       그래서 #1089가 max에서 뺐다(아래 ⚠️).
...
//    ⭐ **시점은 pre/post가 갈린다**(B-1 · 2026-08-07 확정):
//       · post-deemed: §163⑨ 본문대로 **상속개시일 또는 증여일 현재**
//       · pre-deemed : **의제취득일(1985.1.1.) 현재** — 「소득세법」 부칙(법률 제4803호) §8이
//         "1984.12.31. 이전에 취득한 것은 **1985.1.1.에 취득한 것으로 본다**"고 **취득시기 자체**를
//         의제하므로 §164④~⑦의 「취득당시」도 그날이다(조 제목: 「양도자산의 **취득시기**에 관한 의제」).
//         **조심2010서1195**가 정면 결정이다 — 처분청이 §163⑨ + 부칙으로 의제취득일 기준시가를
//         적용한 처분을 **기각**(적법). §176조의2④(나목)은 인용조차 되지 않는다.
//    ⚠️ **이 시점은 엔진이 강제하지 못한다.** 산식에 날짜 파라미터가 없고(probe 실증: `acquisitionDate`를
//       1974→1985로 바꿔도 `calculatePre1990LandValuation` 산출값 **동일**), 값은 사용자가 넣는
//       등급·단가로만 정해진다. ⇒ 통제점은 **UI 라벨**이다 — `sec164AcqTimePointLabel`
//       (`lib/calc/transfer-163-9-base-date.ts`)이 pre-deemed에서 「의제취득일(1985.1.1.)」을 요구한다.
...
//    **국심2003부0627·조심2018서0513**("체계상 환산가액 규정보다 실지거래가액 규정을 먼저 적용")도 같은 결론이다.
//    ⚠️ ③ 제외는 **세액 증가** 방향이라 선행 계획서(`…164-max.plan.md` §6 V-3)는 「예규 확보 전 착수
//    금지」로 두었으나, **#1089(`af74d907`)가 재편했다** — 아래 `clauseA`가 그 결과다. 근거는 예규가
//    아니라 위 명문 + **국심2003부0627·0626**(pre-deemed 정면 결정, §176조의2④ 환산 경정을 **취소**).
//    조심 2023서0676은 쟁점이 **§176조의2④2호 물가상승률**이라 ①②vs③을 다투지 않았다(충돌 아님).
//    🔴 **2026-08-07 근거 강도 정정**: 위 두 결정은 **③ < ①② 사안**이라 **결과로는 「가목 우선」과
//       「3자 max」를 판별하지 못한다**(2003서3266: ①② 3,199,938,815 vs ③ 2,313,478,921 ·
//       2003부0627: 환산 경정으로 세액이 늘었으므로 ③ < ①②). 지지하는 것은 **판시 논리**다 —
//       "§176조의2④은 추계결정 규정으로서 **확인할 수 없는 경우에 적용**"(0627) · 가목 채택에
//       **필요경비 효과를 연동**(3266: ③이면 개산공제만이라 납세자 손해 ⇒ 「많은 걸 고른」 게 아니다).
//       ⚠️ 실무 교재는 같은 두 결정을 **「①②③ 중 많은 가액」의 근거**로 인용한다 — 결과에 기대면
//       양쪽 다 성립하지 않는다. **③ > ①② 사안의 정면 결정은 미확보**이나, 명문(§97①1호 단서
//       「한정하여」·§163⑫)과 대법원 2006두1326이 있어 현행 구조를 되돌릴 이유는 없다.
//       계획서: docs/02-design/features/sec164-clause-a-deemed-date-timing-b1.plan.md §4.3.1·§9.2
//    ⇒ 남은 것은 하나. 3자 max가 제공하던 **오입력 방어가 실제로 사라졌다** — 「가목이 확인됐다」를
//       `clauseA > 0`(금액)으로 판정하므로 신고가액 오입력 하나가 그대로 취득가액이 된다.
//       그 판정을 **소스의 존부**로 바꾸는 설계가 U2-E다:
//       docs/02-design/features/pre-deemed-clause-a-confirmation-criteria.engine.design.md
