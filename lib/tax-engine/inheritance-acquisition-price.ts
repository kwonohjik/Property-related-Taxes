...
//
// 취득가액 = max(① 상증법 §60~66 평가액, ② §164④~⑦ 기준시가, ③ 환산취득가)
...
//    ⇒ **③을 같은 max에 두는 근거는 심판례가 아니다.** 재편 논의는 계획서 V-3.
...
//    다만 ③ 제외는 **세액 증가** 방향이고, 재편하면 현행 max가 제공하던 **오입력 방어가 사라진다**
//    (「가목이 확인됐다」를 `reported > 0`으로 판정하면 신고가액 오입력 하나가 세액을 폭증시킨다).
//    ⇒ 그 판정 기준을 설계하기 전까지 **현행 3자 max 유지**(계획서 V-3 · U-5′).
