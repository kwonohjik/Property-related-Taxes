...
export function computeGroupsAndComparison(
   ...
   *
   * ## ✅ 단일 자산에도 MAX를 무조건 도는 것이 **맞다** (2026-09-02 종결 — 종전 「미판정」 정정)
   *
   * 종전 주석은 「§104⑤은 「둘 이상」이 요건이라 문언상 미적용이 맞지만, `calcTax`가 §104① **후단**을
   * 수행하지 않아 이 MAX가 그 비교를 대신 공급하고 있다 — 어느 쪽이 정답인지 미판정」이었다.
   * **그 서술의 전제가 stale하다.** §104① 후단은 `compareWithClause1`로 **구현됐다**
   * (2026-08-11 `f58f29df` — 주석은 그보다 앞선다). 그런데 **결론(건드리지 않는다)은 옳고,
   * 이유가 둘 다 바뀐다**:
   *
   * ### ① 파트로 갈리는 자산은 「둘 이상」이 **의제로 충족**된다
   * §104⑤ 본문 **후단**: 「이 경우 제2호의 금액을 계산할 때 … **한 필지의 토지**가 제104조의3에
   * 따른 비사업용 토지와 그 외의 토지로 구분되는 경우에는 **각각을 별개의 자산으로 보아** 양도소득
   * 산출세액을 계산한다」(법제처 MST 267581 실측 · 2018.4.1. 양도분~). 겸용주택·토지건물 분리취득도
   * 같은 층위다(§94①1호상 토지+건물). ⇒ 「자산 1건이라 §104⑤ 미적용」은 **이미 기각된 독법**이고
   * **재제안 금지 항목**이다(계획서 `transfer-104-5-proviso-mixed-use-rate-gaps.plan.md` D-8).
   *
   * 실측(부분 비사토 1필지 · 수도권 일반주거 · 대지 400㎡ · 양도차익 6억): 세율군별 151,670,000 vs
   * 전체누진 **161,260,000** — 전체누진이 이긴다. 파트가 낮은 누진 구간을 두 번 타기 때문이다.
   * ⇒ 여기서 MAX를 끄면 **9,590,000원 과소과세**다.
   *
   * ### ② 파트가 없는 진짜 단일 자산에서는 MAX가 **구조적 no-op**이다
   * 그룹 세액은 전부 `assetTaxOf` → `resolveSplitAwareTax` → `calcTax`를 거치고, `calcTax`의
   * 모든 분기가 §55① 누진 이상을 낸다: 누진 호는 동일 · 8호·§104⑦은 「누진 + 가산」(항등식) ·
   * 2·3호·10호는 `compareWithClause1`이 1호와 비교해 큰 것을 취한다. 기록 1건이면
   * `groupTaxBase == generalTaxBase`이므로 `byGroups >= byGeneral`이 **항상** 성립한다.
   *
   * 🔬 인과 확인(뮤테이션): 단기 경로의 `compareWithClause1` 호출을 끄면 단일 비주택 토지
   * 19개월(40%)에서 divergence가 살아나고, 그 경계가 **정확히 과세표준 1,318,800,000**이다
   * (`0.45T − 65,940,000 > 0.4T ⟺ 0.05T > 65,940,000`). 경계값에서 차이 0.
   * ⇒ **후단이 no-op의 원인**임이 확정된다 — 주석이 쓰인 시점에는 그 서술이 사실이었다.
   *
   * ⇒ `properties.length === 1` 가드를 넣지 말 것. ①에서 과소과세가 되고, ②에서는 아무 효과가 없다.
   * anchor `aggregate-single-asset-104-5-max.anchor.test.ts`가 둘 다 잠근다.
   */
