 ...
 *
 * ✅ **F2 Phase A·B 전건 완료** (2026-09-14 실측 정정 — 종전 주석은 아래를 「후속」으로 적었고
 *    그 시점엔 맞았으나 **이미 전부 구현**돼 거짓 고지가 돼 있었다):
 * - 별표3(체육 1호 선수전용)·별표6(예비군 5호다목) — Phase A
 * - 별표4·5(사업용·종업원 체육시설) — Phase B(B-1). `SPORTS_BUSINESS_*` · `employeeSportsArea`
 * - **별표1의3**(목장 축종별 두수 기준면적) — `data/livestock-standards.ts` + `pasture.ts` 자동산출
 * - **용도지역별 배율**(비고1·3 → 「지방세법 시행령」 §101②) — `other-land-area-limit.ts` `zoneMul`
 * - **6호 휴양 3요소**(시행규칙 §83의4⑫) — `resolveAreaLimit`의 `resort` 분기
 * - **선수가산**(별표3 비고5·별표4 비고4 테니스·연식정구) — `applySportsNotes`
 *
 * ⚠️ 「미구현·후속」을 새로 적을 때는 **구현 시점에 역방향 grep**할 것 — 이 주석이 그렇게
 *    stale해졌다 — 「구현했다」와 「고지를 지웠다」는 별개 작업이다.
 */
