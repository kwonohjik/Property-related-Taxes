 ...
 *
 * ## 강원·전북특별자치도의 군도 「도의 군」과 같다 (2026-09-14 확정)
 *
 * 문언만 보면 「특별자치도」에 걸려 열거 **안**으로 읽힐 여지가 있었다. 실측 4건으로 닫았다:
 *
 * | 확인 | 내용 |
 * |---|---|
 ...
 *
 * 즉 「특별자치도」 괄호가 **행정시의 읍·면지역만** 제외한 이유는, 그 조문이 상정한
 * 특별자치도(제주)에는 **군이 존재할 수 없기** 때문이다 — 군을 열거에 넣겠다는 의사를
 * 표시한 적이 없다. 강원·전북은 종전 도의 관할구역·시군 체계를 그대로 승계했으므로
 * 그 안의 군은 **종전 「도의 군」 그대로**다. 명칭 변경만으로 납세자에게 불리하게
 * (지역기준 대상으로) 볼 근거가 없다.
 *
 * ⇒ 시도 코드로 갈리는 분기를 두지 않는다. `endsWith("군")` 한 줄이 정본이다.
 *
...
export function isUrbanCriteriaRegion(
  ...
  // 강원·전북특별자치도의 군도 **여기로 들어온다**(위 헤더 표 참조 — 시도로 갈리지 않는다).
