 ...
 *   납세자가 소명해야 한다는 독법도 성립한다. **조문 문언만으로는 갈리지 않는다.**
 *
 * ## 현행 선택 — 미상이면 유예 미적용(비사업용)
 *
 * 입증책임 배분에 관한 판례·예규를 찾지 못해(`search_decisions`로 이 단서 관련 심판례 미검출)
 * 근거 없이 유리·불리 어느 쪽으로도 바꾸지 않고 **농지·목장과 같은 기본값을 유지**한다.
 * 실무 노출도 좁다 — ⑧ 검증(`transfer-tax-validate-nbl.ts`)이 도시지역 임야(시업중·특수지구)에
 * 편입일을 **필수 입력으로 차단**하므로, 미상 조합은 API 직접 호출·구 이력 복원처럼 검증을
 * 우회하는 경로에서만 생긴다.
 *
 * ❌ 이 기본값을 바꾸려면 **판례·예규 근거를 먼저 확보할 것** — 방향은 과소과세 쪽이다.
 */
