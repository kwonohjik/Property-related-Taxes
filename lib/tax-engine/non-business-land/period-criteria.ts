 ...
 * 도시지역 편입유예 — **세 지목 공용**.
 *
 * 근거 조문은 지목마다 다르지만 **기간과 창(窓)의 의미가 같아** 한 함수로 묶는다:
 *   · 농지 「소득세법 시행령」 §168의8⑥  — 법 §104의3①1호나목 단서의 「대통령령으로 정하는 기간」
 *   · 목장 같은 영 §168의10⑤            — 법 같은 항 3호가목 괄호의 같은 기간
 *   · **임야** 같은 영 §168의9①2호 **단서** — 「도시지역으로 편입된 날부터 3년이 경과한 임야를 제외」
 *
 * 임야가 이 함수를 쓰는 것이 종전 주석에 없어 농지·목장 전용으로 읽혔다. 실측으로 확인한 근거:
 *   · 2014.03.11. 시행본 — §168의8⑥ 「**2년**」 / §168의9①2호 단서 「편입된 날부터 **2년**이 경과한」
 *   · 제26067호 [시행 **2015.02.03**] — 셋 다 **3년**으로 함께 개정
 *   ⇒ 아래 2년/3년 연혁은 임야에도 그대로 맞다.
 *
 * ⚠️ **문언 방향은 반대인데 귀결이 같다.** 농지·목장은 「N년이 **지나지 아니한**」 것을 제외(사업용)로
 * 적고, 임야는 「N년이 **경과한**」 것을 제외(비사업용)로 적는다. 둘 다 「N년 이내 → 사업용」이므로
 * `isApplied`(= 편입일 + N년 이내)를 사업용으로 해석하는 것은 세 지목 모두 옳다.
 *
 * ⚠️ **다만 편입일이 미상일 때의 기본값은 지목마다 논리가 다르다** — 임야는
 * `checkForestIncorporationGrace`를 거치게 해 그 선택을 명시했다(V5-d).
 */
...
/**
 * 임야 전용 편입유예 — 「소득세법 시행령」 §168의9①2호 **단서**.
 *
 * 기간·창은 농지·목장과 같으므로 계산은 `checkIncorporationGrace`에 위임한다(위 주석의 실측 근거).
 * 이 래퍼가 존재하는 이유는 **편입일이 미상일 때의 기본값이 지목마다 논리가 다르기 때문**이다.
 *
 * ## 무엇이 다른가 (V5-d)
 *
 * 날짜 사실이 어느 쪽에 유리한지가 반대다:
 *   · 농지·목장 — 「N년 **미경과**」가 **납세자에게 유리한 예외**다(법 §104의3①1호나목 단서·3호가목
 *     괄호). 미입증이면 예외를 세울 수 없으니 유예 미적용 = 비사업용이 자연스럽다.
 *   · 임야 — 법 §104의3①2호 본문이 「임야 = 비사업용」이고 가목이 그 예외(공익·보호육성)를 열며,
 *     이 영 §168의9①2호가 그 예외의 내용을 정한다. 그 **단서**의 「N년 **경과**」는 열린 예외를
 *     다시 닫는, 즉 **과세를 되살리는 적극적 사실**이다.
 *
 * ⇒ 문리로는 「경과」가 입증되지 않으면 단서를 적용할 수 없다고 읽을 여지가 있다.
 *   반대로 §168의9①2호 자체가 납세자에게 유리한 예외 규정이므로 「단서에 걸리지 않음」까지
 *   납세자가 소명해야 한다는 독법도 성립한다.
 *
 * ## ✅ 확정 — 미상이면 유예 미적용(비사업용) (2026-09-14)
 *
 * **후자를 채택한다.** 근거는 조문의 **원칙/예외 구조**다:
 *   · 법 §104의3①2호 **본문**이 「임야」를 비사업용으로 정하고(원칙),
 *   · 가목이 그 예외를 열며, 이 영 §168의9①2호가 예외의 내용을,
 *   · 그 **단서**가 예외의 한계를 정한다.
 *
 * ⇒ 사업용(= 예외)을 주장하는 쪽이 그 요건사실을 뒷받침할 책임을 진다. 편입일은 그 예외가
 *   유효하게 유지되는지를 정하는 요건사실이므로, **미상이면 예외를 세울 수 없어** 원칙인
 *   비사업용으로 확정한다. 농지·목장과 결과적으로 같은 기본값이지만 **경로가 다르다** —
 *   농지·목장은 「유리한 예외 미입증」, 임야는 「원칙 복귀」다.
 *
 * 실무 실증: **조심 2022서8116**(2023.9.12., 양도) — 쟁점이 「비사업용 토지에 해당하지 않는다는
 * **청구주장**의 당부」로 구성되고, 청구인이 산림경영계획인가서·산림청 회신·임대차계약서를
 * 대어 §168의9①2호가목 예외를 세웠다. 예외는 **주장하는 쪽이 자료로 세운다**.
 * ⚠️ 다만 이 재결례는 **단서(N년 경과)의 입증책임을 직접 다루지 않는다** — 구조와 실무를
 *    보여줄 뿐이므로 그 이상으로 인용하지 말 것.
 *
 * 실무 노출도 좁다 — ⑧ 검증(`transfer-tax-validate-nbl.ts:208`)이 도시지역 임야(시업중·특수지구)에
 * 편입일을 **필수 입력으로 차단**하므로, 미상 조합은 API 직접 호출·구 이력 복원처럼 검증을
 * 우회하는 경로에서만 생긴다.
 *
 * ⛔ **재검토 금지.** 「입증책임 판례·예규가 없어 보류」였던 상태는 2026-09-14에 닫혔다.
 *    뒤집으려면 단서의 입증책임을 **정면으로** 다룬 판례·예규가 필요하다(방향은 과소과세).
 */
export function checkForestIncorporationGrace(
  urbanIncorporationDate: Date | undefined,
  transferDate: Date,
  rules: NonBusinessLandJudgmentRules = DEFAULT_NON_BUSINESS_LAND_RULES,
): IncorporationGraceResult {
  return checkIncorporationGrace(urbanIncorporationDate, transferDate, rules);
}
