 ...
 *
 * 자동산출(KoreanLaw 본문 검증) — **F2 Phase A·B 완료 후 현재 상태**:
 *   · 7호 최대면적×1.2 · 4호 수용정원×200㎡ · 2호나 최저차고×1.5 · 13호 660㎡
 *   · **1호 체육** 별표3(선수전용)·별표4(사업용)·별표5(종업원) — 종목합산·비고 가산 포함
 *   · **5호다 예비군** 별표6 제2호(부대편성인원 × 시설 합산)
 *   · **6호 휴양** 시행규칙 §83의4⑫ 3요소(옥외 방목장·식물원 + 부설주차장×2 + 건축물 부속토지)
 *
 * 직접입력이 **원칙**인 것은 **2호가목(부설주차장 설치기준면적) 하나**다. 나머지 호는
 * 자동산출이 재료 부족·표 밖 코드로 실패할 때만 `standardAreaLimit`으로 **fallback**한다
 * (자동 안분 fallback 금지 원칙과 다른 층위 — 여기서는 추정하지 않고 입력으로 되돌린다).
 */
 ...
 *
 * ⚠️ **이 함수가 담당하지 않을 뿐, 배율·종목합산은 구현돼 있다** (2026-09-14 정정 — 종전 주석은
 *    둘을 「cross-statute/명문부재 → 미구현」이라 적었고 **틀렸다**):
 *    · 용도지역별 배율(비고1·3 → 「지방세법 시행령」 §101②)은 `resolveAreaLimit`의 `zoneMul`이
 *      실내 종목·별표5 실내체육시설에 적용한다(E4-03·V6-a).
 *    · 종목합산(비고2)은 `sumSportsEvents` — 5종목군 max1 + 그 외 합산.
 */
