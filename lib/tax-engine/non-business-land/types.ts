...
export interface NonBusinessLandJudgment {
   ...
   *
   * `landCategory`는 **근거 조문이 지목마다 다르기** 때문에 함께 싣는다 (U3-02) —
   * 농지는 「소득세법 시행령」 §153③, 임야는 §168조의9②이 재촌을 각각 정의한다.
   */
  residenceMatch?: {
...
export interface NonBusinessLandJudgmentRules {
   ...
   *
   * ✅ **지목 한정 없음**(2026-09-14 확정). 두 시행본 본문에 지목 구분이 없고,
   *    제26067호 **부칙 전문(제1조~제23조)**에도 §168의6 개별 적용례·경과조치가 없어
   *    부칙 제2조②(양도소득 개정규정은 시행 이후 최초 양도분부터)가 지배한다.
   *    종전의 「농·임·목 한정」은 근거 없는 분기였다 — `getThresholdRatio` 주석 참조.
   */
