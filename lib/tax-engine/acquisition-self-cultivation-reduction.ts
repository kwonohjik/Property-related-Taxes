 ...
 *
 * [P2-5] 2년 이상 직접 경작 농민의 농지 취득세 50% 감면
 *
 * 감면 요건 (지특법 §6①):
 *   1. 취득자가 직접 2년 이상 농업에 종사하거나 농지 소재지 거주
 *   2. 농지 소재지에서 20km 이내 거주 (거리 요건)
 *   3. 취득 농지 면적 20,000㎡ 이하 (면적 요건)
 *
 * 감면액: 취득세 본세 × 50%
 *
 * 감면 중복배제 (조특법 §127②):
 *   생애최초 감면과 동시 해당 시 납세자에게 유리한 1건만 적용
 */
...
export function assessSelfCultivationReduction(
  ...
  // 요건 2: 거리 요건 — 농지 소재지에서 거주지까지 30km 이내
