...
// ─── 개별 공제 계산기 — inheritance-deduction-items.ts로 분리 (800줄 정책, 재export 호환) ───
//     방향은 한쪽뿐이다: 이 파일(오케스트레이터)이 그것들을 **쓰고**, 그쪽은 여기를 참조하지 않는다.
export {
  calcBasicDeduction,
  calcSpouseDeduction,
  calcFinancialDeduction,
  calcCohabitationDeduction,
} from "./inheritance-deduction-items";
import {
  LUMP_SUM_DEDUCTION,
  SPOUSE_MAX,
  cohabitRateAndCap,
...
export function calcInheritanceDeductions(
  ...
  // G4 사용 여부: 3필드 전부 입력 시에만 number(0 포함), 미입력 시 undefined
  // 0 = 한도 이내(차감 없음), >0 = 초과 차감액, undefined = G4 미사용
  const g4Used =
  ...
  if (!cohabitGate.applicable) {
    cohabitResult = {
      ...
      breakdown: [
        {
          label: `동거주택공제 미적용 — ${cohabitGate.reason}`,
          amount: 0,
