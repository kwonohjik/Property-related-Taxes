...
export function aggregateByGroup(
  ...
  for (const [group, idxList] of groupMap) {
    ...
    if (group === "short_term") {
      ...
      // 누진 호 쪽만 빠져 있던 내부 불일치다.
      //
      // 2026-08-02 정정(계획서 **D-13** — **세액 변경**): 단서는 요건이 **둘**이고 **둘 다**
      // 충족해야 합산한다 — ⓐ「동일한 호의 세율이 적용되고」 **그리고** ⓑ「그 **적용세율이
      // 둘 이상**인 경우」. 종전에는 ⓐ(중과 tier 동일)만 보고 ⓑ를 검사하지 않아, 자산별
      // 적용세율이 **모두 같아도** 합산 1회로 갔다.
      //
      // 그 정당화였던 「자산별 floor 합산은 floor 횟수 차이로 ±N원이 어긋난다」는
      // **단일세율 호에만 성립한다** — 누진 호에서 합산 1회는 누진 **구간과 누진공제를 통째로
      // 병합**해 ±N원이 아니라 **수천만 원**을 키운다(실측 과대: `progressive`·비사토
      // 23,300,000 · 다주택 27,940,000). 「2호 본문은 자산별로 누진을 각각 태워 **구간이
      // 리셋**된다」는 위 R7 도출값 280,120,000이 이미 쓰던 규칙이고, tier가 같을 때만
      // 어긋나 있던 내부 불일치다.
      const perAsset = idxList.map((i) => assetTaxOf(i));
      ...
      // ⓑ **같은 호 안에서 자산별 적용세율이 갈리는가.** 누진 호는 과세표준 구간에 따라
      // 자산마다 한계세율이 달라질 수 있고, 그때가 단서가 예정한 국면이다.
      //   · 교재 사례1(§1.6): B주택 3억(40%) + C주택 5억(42%) — 둘 다 §104⑦3호·2년 이상.
      //     적용세율이 **둘**이라 단서가 걸려 **합산 8억**으로 계산한다(원 단위 재현 완료).
      //   · 반대로 자산별 적용세율이 **하나**뿐이면 단서가 예정한 「각 해당 호별 세율을 적용해
      //     **큰** 산출세액」이라는 절차 자체가 성립하지 않는다 ⇒ **2호 본문**(자산별 합).
      const uniformRate = perAsset.every((p) => p.rate === perAsset[0].rate);
      // `unregistered`(§104①10호 70% **단일세율**)는 ⓑ가 영원히 불충족이지만, 그 호에서는
      // 합산 1회와 자산별 합의 차이가 **실제로 floor 횟수뿐**이라 세액 판단이 갈리지 않는다.
      // 종전 floor 규약을 유지한다(바꿔봐야 ±N원 회귀만 생긴다 — Surgical).
      const mergeAllowed = !mixedTier && (!uniformRate || group === "unregistered");
      if (!mergeAllowed) {
