...
/**
 * VII-37 정상사용면적비율 범위 검증 — **엔진과 ⑧ validation 의 단일 술어**.
 *
 * 비율이므로 정의상 0 초과 1 이하다. 검증이 없으면 백분율 오입력(0.85 대신 85)이 그대로 배율이 되어
 * 기준시가가 100배가 되고, 음수는 기준시가를 음수로 만든다(실측 2026-08-26).
 * 조용한 clamp 가 아니라 **검증 오류**로 다룬다 — 형제 경로 `adjustmentFromNos` 도
 * 도메인 밖 번호를 오류로 차단하므로 그것이 이 저장소의 관행이다.
 *
 * ⚠️ 하한을 「0 초과」로 잡은 근거: 0 이면 조정률 0 → 기준시가 0원이 되어 평가로서 의미가 없고
 *    조용한 0원은 오산과 구별되지 않는다. 고시 문언이 0 을 허용한다면 완화할 것
 *    — 국세청 「건물 기준시가 계산방법」 부속 조정률 고시 **본문 미확인**.
 *
 * @returns 오류 메시지(있으면) 또는 null
 */
export function normalUseRatioError(ratio: number | undefined): string | null {
  if (ratio === undefined) return null;
  if (!Number.isFinite(ratio) || ratio <= 0 || ratio > 1) {
    return "VII 화재·멸실 정상 사용 면적비율은 0 초과 1 이하로 입력하세요. (예: 85% → 0.85)";
  }
  return null;
}
