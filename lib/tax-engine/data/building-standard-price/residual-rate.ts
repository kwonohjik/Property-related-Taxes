 ...
 * ✅ **신공법 era-B 내용연수 — 2026-08-27 고시 원문으로 확정**(종전 주석의 사실 서술 2건이 틀렸다).
 *    국세청고시 제2013-2호·2014년 고시 **제10조③(그룹별 건물구조)** 이 아래를 명시 열거한다:
 *      III(30년) … ALC조·보강블록조·와이어패널조   ·   IV(20년) … 조립식패널조·컨테이너건물
 *    ⇒ era-C −10 추정이 낸 값과 **전건 일치**한다(값 변경 없음). ERA_B_DURABLE_FIXED에 확정 등재해
 *      era-C 그룹이 바뀌어도 era-B 값이 조용히 따라 움직이지 않게 고정했다.
 *    ⚠️ 종전 주석의 「해당연도 구조지수표에도 부재해 선택 불가 … 실무 영향 없음」은 **거짓**이었다 —
 *      2013년 고시 제7조에 5종이 모두 있고(ALC 100·보강블록/와이어패널 90·조립식패널 80·컨테이너 50),
 *      2010년 고시 5.구조지수 5행(80)에는 「철골조 중 조립식 패널」이 있다. 즉 드롭다운에 노출되며
 *      내용연수가 실제 산출에 곱해진다.
 *    ⚠️ `steel_frame_eps`(철골조 중 조립식패널)만은 2010·2011·2013·2014 고시 **어느 제10조③에도
 *      열거가 없어** 여전히 잠정이다. 2010년 고시 용어의 정의 (15)가 「조립식 패널 건물(**철골조를
 *      제외함**)·컨테이너 건물 등은 경량철골조로 분류한다」고 정해, 반대해석상 철골조 계열 ⇒
 *      철골조와 같은 III(30년)으로 둔다. 2015년 고시는 미확보(2013·2014 동일에 근거한 연장 적용).
 */
...
/**
 * 신축연도 ↔ **평가시점**(상증 평가연도 · 양도 양도연도) 순서 검증 — ⑧ validation 의 단일 술어.
 *
 * 평가 시점에 건물이 **아직 없으면** 경과연수가 음수가 되는데, 위 `calcResidualRateByDurable`
 * 의 `Math.max(0, elapsedYears)` 가 그것을 0 으로 clamp 해 **신축 직후(잔가율 1.000)** 로
 * 계산한다 — 오류 없이 **잔가율 최대치**로 과대평가된다.
 *
 * 실측(2026-09-22 · rc · 용도1 · 200㎡ · 공시지가 1,100,000원/㎡):
 *
 * | 입력 | ⑧ validation | 산출 |
 * |---|---|---|
 * | 신축 2000 · 양도 2020 (정상) | 통과 | 107,800,000 (잔가율 0.64) |
 * | **신축 2025 · 양도 2020** | **통과** | **168,600,000 (잔가율 1.000)** |
 * | **신축 2025 · 상증 평가 2020** | **통과** | **168,600,000 (잔가율 1.000)** |
 *
 * 🔑 바로 아래 형제 술어 `remodelYearError` 는 이미 「대수선 > 평가연도」를 차단하고
 *    「대수선 ≥ 신축」도 요구한다. 두 조건이 겹쳐 **대수선을 입력한 경우에만** 신축 ≤ 평가가
 *    간접 강제되고 있었다 — 미입력(대부분)이면 무방비다. 그 비대칭이 이 함수의 존재 이유다.
 *
 * ⚠️ **「신축연도 > 취득연도」는 이 술어의 대상이 아니다** — 완공 전 취득이라는 별도 범주가 있다.
 *    국세청 고시 「건물 기준시가 계산방법」의 산정기준율표가 그 셀을 **의도적으로 비워** 두었고
 *    (`acq-base-rate.ts:12` 「값 없는 셀(취득<신축=완공전취득 §8④ 등) = 미수록」),
 *    실제로 취득 ≤2000 경로는 이미 「산정기준율 미수록」으로 멈춘다(실측: 신축 1995 · 취득 1990).
 *    취득 ≥2001 경로만 잔가율 1.000 으로 통과하는데(실측 118,800,000 vs 정상 95,000,000, **+25%**),
 *    그 처방은 **§8④ 본문을 확인한 뒤 별건**이다 — 법 근거 없이 정당한 입력을 막지 않는다.
 *    이 배제는 `building-std-year-order.anchor.test.tsx` §5 가 고정한다.
 *
 * @param pointLabel 사용자에게 보일 그 시점의 칸 이름("평가연도" · "양도연도")
 * @returns 오류 메시지(있으면) 또는 null
 */
export function builtYearOrderError(
  builtYear: number,
  pointYear: number,
  pointLabel: string,
): string | null {
  if (builtYear <= pointYear) return null;
  return `신축연도(${builtYear})가 ${pointLabel}(${pointYear})보다 뒤입니다. 그 시점에는 건물이 없습니다.`;
}
