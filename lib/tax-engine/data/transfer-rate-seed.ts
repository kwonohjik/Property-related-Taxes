  ...
  // ────────────────────────────────────────────────────────────────────
  {
    ...
    deduction_rules: {
      ...
      articles: [
        ...
        //
        // 🔴 **`acquisitionPeriod`의 축이 조문과 다르다** (코드리뷰 D3-04 — 미해소, 의도적).
        //   조특법 §99①은 1호가 **매매계약 체결·계약금 납부일**, 2호가 **사용승인·사용검사일**을
        //   기준일로 삼는다. 이 시드는 **취득일**로 매칭하므로 축 자체가 어긋난다.
        //   그래서 `end`를 조문상 1999-06-30으로 「고치면」 오히려 **납세자에게 불리**해진다 —
        //   「1999.6 이전 계약 + 2000~2001년 준공·취득」인 정상 대상자가 근거 없이 배제된다.
        //   ⇒ 값이 아니라 **축**을 고쳐야 하고, 그 정본은 이미 있다:
        //     `transfer-reductions/new-99.ts:36-38`(NEW_99_PERIOD_END / _NATIONAL) +
        //     `period-check.ts` + UI `New99InputForm`. 이 legacy 행은 그 정본과 중복된 구경로다.
        //   현재는 `calcReductions`가 `reductions[]`에 정본 조문이 있으면 이 매처를 **건너뛴다**
        //   (D3-03 교차 배제). 완전 해소는 legacy 경로 제거 결정이 필요하다.
