 ...
 *
 * ✅ 데이터 입력 완료 (2026-08-05 실측 정정):
 *   REGULATED_REGIONS에 **109개 지역**이 들어 있다 — 서울 전역·강남3구·용산, 경기 재지정 그룹 등.
 *   (계획서: docs/02-design/features/regulated-area-bjd-history.plan.md §5).
 *
 *   ⚠️ 종전 이 주석은 "데이터 미입력 상태 … **현재는 빈 배열**"이었다. **문서가 아니라 코드에
 *      남은 stale이라 더 위험하다** — 조정대상지역은 중과세율과 비과세 거주요건을 가르는
 *      데이터여서, "빈 배열"을 믿으면 있는 데이터를 없다고 오판하거나 중복 입력하게 된다.
 *
...
export const REGULATED_REGIONS: RegulatedRegion[] = [
  ...
  // 부천 일반구 신설(원미·소사·오정) — 시 코드 `41190` 자체는 살아 있으나(행안부 자료 [존재])
  //   **하위 법정동이 전부 일반구로 재배치**돼 `41190xxxxx` 법정동은 하나도 남지 않았다
  //   (Vworld `emd_cd:like:41190` 0건). 현행 주소검색은 41192·41194·41196을 주므로
  //   구 코드만 두면 조정지역 이력이 통째로 매칭 실패한다(계획서 D-8).
  //   부천은 **시 전역 지정**이었으므로 3개 구가 같은 이력을 그대로 물려받는다 —
  //   별칭(1:1)으로는 1:N을 표현할 수 없어 엔트리를 함께 둔다.
  ...
  //   시 코드 `41590`은 살아 있으나 하위 법정동이 전부 일반구로 재배치됐다.
  //
  ...
  //
  //   ## 과거 이력(2017~2022) 범위는 **행정 연혁**으로 확정했다 (Y-13, 2026-08-01)
  //
  //   구 `41590420 동탄면`은 폐지돼 현행 어디에도 없다(Vworld 전국 검색 0건).
  //   행안부 「법정동코드 전체자료」(폐지 코드 전량 보존 — 계획서 Y-5)로 하위 리를 확정했다:
  //
  //     동탄면 13개 리 = 반송·석우·오산·청계·영천·중·신·목·산척·장지·송·방교·금곡
  //       → 전부 동(洞)으로 전환 → 현행 동탄구 `41597102`~`114`
  //
  //   ⚠️ **능동(41597101)은 동탄면이 아니다** — 구 화성시 본토 동 계열(진안·병점·기산과
  //   같은 묶음, `41590118`). 종전 데이터에 없던 것과 일치한다.
  //   `41597104 오산동`은 이미 **폐지**됐다(현행 결번의 정체).
  //
  //   여울동(`41597115`)은 13개 리 목록에 없는 **신설** 동이나, Vworld `LT_C_LHBLPN`
  //   「화성동탄2 택지개발사업」 폴리곤과의 교차 실측에서 **99.2%**가 지구 안이라
  //   동탄면 리에서 분할된 것으로 보아 포함한다.
  //
  //   ⚠️ 지리 실측은 **종전 데이터 라벨의 오류도 드러냈다**: 종전 `41590127`·`41590128`은
  //   「동탄2 반송동/석우동」이라 적혔으나 두 동은 **동탄1 지구가 99.8%**다. 행정 연혁상
  //   동탄면 소속인 것과 개발 지구가 동탄1인 것은 서로 모순되지 않는다(반송리 → 반송동 전환
  //   후 그 땅에 동탄1신도시가 들어섰다). 2017-08-03 고시 문구는 「동탄2택지개발지구에
  //   한함」이었으므로 종전 데이터가 이미 고시보다 넓었다.
  //
  //   2026-08-01 세무 판단 — **종전 범위를 그대로 이전**한다(반송·석우 + 동탄면 12개
  //   + 여울동 = 13개). 축소하면 저장된 이력과 어긋나고, 능동은 어느 근거로도 들어오지
  //   않으므로 제외한다 — 근거 없이 넓히면 납세자에게 불리해진다.
