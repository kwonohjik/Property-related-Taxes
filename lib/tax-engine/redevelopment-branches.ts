/**
 * 재개발 §166 — **분기 실행부** (800줄 분리, 2026-09-11)
 *
 * `redevelopment.ts` 가 833줄로 정책(트리거 800 · 착지 ≤700)을 넘겨 분리했다.
 * 이음매는 **함수 단위 이동**이다:
 *
 * · 오케스트레이터 `runRedevelopment` 는 원본에 남고, 여기의 분기 3개를 부른다.
 * · 분기 전용 헬퍼 2개(`toBranchHolding`·`apportionPreApprovalExpenses`)도 함께 옮겼다 —
 *   **오케스트레이터는 그 둘을 쓰지 않는다**(실측). 원본에 남기면 「원본 → 여기 → 원본」
 *   순환이 됐을 것이다([[feedback_800line_split_playbook]]).
 * · 본문에 남은 `runRedevelopment` 언급은 **주석뿐**이라 참조가 아니다.
 */
...
export function toBranchHolding(
  from: Date,
  to: Date,
 ...
 */
export function apportionPreApprovalExpenses(
  rawExpenses: number,
 ...
 */
export function runLandContribEstimated(
 ...
 */
export function runHousingContribReceiveEstimated(
  input: RedevelopmentOrchestratorInput,
): RedevelopmentResult {
  const { redevelopment, acquisitionDate, transferDate, transferPrice } = input;
 ...
 */
export function runOriginalMember(
  input: RedevelopmentOrchestratorInput,
): RedevelopmentResult {
...
export type { RedevelopmentResult, RedevelopmentBranchDetail } from "./types/transfer-redevelopment.types";
