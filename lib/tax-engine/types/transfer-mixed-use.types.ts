...
export interface MixedUseAssetInput {
  ...
  /**
   * §155의3 **상생임대주택** — 거주기간 제한 면제 사실 (P5-c).
   *
   * 🔴 **한 필드가 겸용 경로의 두 지점을 동시에 연다.** 법문이 면제 대상으로 세 조문을
   *    열거하기 때문이다(시행령 MST 286211 실독) — 「제154조제1항, 제155조제20항제1호 및
   *    **제159조의4**를 적용할 때 … 거주기간의 제한을 받지 않는다」:
   *
   *    | 지점 | 조문 | 겸용 경로 소비처 |
   *    |---|---|---|
   *    | 비과세 거주요건 | §154① | `exemptionReqInput` → `meetsOneHouseHoldingResidence` |
   *    | 장특 표2 게이트 | §159의4 | `buildHousingPart` → `meetsTable2ResidenceRequirement` |
   *
   *    둘 중 하나만 배선하면 「비과세는 되는데 표2는 안 열리는」(또는 그 반대) 축 어긋남이
   *    조용히 생긴다. **두 술어 모두 이 필드 하나를 읽는다** — 판정을 여기서 다시 하지 않는다.
   *
   * 🔴 **§155의2(장기저당담보)는 여기에 오지 않는다.** 그 조문 ①②는 「**제154조제1항**의
   *    규정을 적용함에 있어」만 면제하고 §159의4를 열거하지 않는다. §159의4 본문 괄호가
   *    §155의2를 포함하는 것은 「**1주택**」 개념에 넣는다는 뜻이지 거주요건 면제가 아니다
   *    (단건 경로 anchor `one-house-155-2-155-3-special`가 같은 구분을 고정한다).
   *    그리고 §155의2의 §154① 면제는 **공용 술어에 일부러 넣지 않은** 축이다 —
   *    호출부 2곳에서만 주입한다(`qualifiesLongTermMortgageResidenceExemption` 주석:
   *    공용에 넣으면 일시적 2주택·혼인 합가 같은 다른 의제 경로로 면제가 **샌다**).
   *    ⇒ 겸용 × §155의2는 별개 축이며 이 필드로 해결되지 않는다.
   */
