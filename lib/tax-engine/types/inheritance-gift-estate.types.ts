...
export interface EstateItem extends EstateLocationFields, EstateItemSavingsFields, EstateItemCryptoFields {
  ...
  /**
   * 국채·공채 (상증령 §74①2호 본문·§74②**1호** 충당 1순위).
   *
   * **§73⑤ 금융재산에서 제외되는 근거**(KoreanLaw 실측 MST 283637, 시행일 2026-02-27):
   *   §73⑤ = "금전과 금융회사등이 취급하는 예금·적금·부금·계금·출자금·특정금전신탁·
   *   보험금·공제금 및 어음" — **열거주의이며 채권은 없다**(어음만 포함).
   *   따라서 국채·공채를 §74① 충당 유가증권으로 분류해도 §73⑤과 이중계상되지 않는다.
   *
   * **왜 flag인가**: §22 금융재산상속공제의 "금융재산"(금융실명법 기준 — 채권 포함)과
   *   §73⑤ 물납 요건의 "금융재산"(열거 — 채권 제외)은 **정의가 다르다**. `AssetCategory`
   *   `"financial"`("예금·펀드·채권")은 §22 기준이므로 그대로 두고, 물납 계산에서만
   *   이 flag로 §73⑤ 대상에서 빼낸다. 카테고리를 쪼개면 §22 공제 경로가 회귀한다.
   *
   * true 시 물납에서: `grossFinancialValue` 제외(요건3·한도2). 충당 분류는
   *   `isGovernmentBondListed`가 가른다(§74①2호**가목** — 아래 참조).
   *   **결정세액·§22 공제에는 영향 없다.**
   */
  isGovernmentBond?: boolean;
  /**
   * 국채·공채가 **거래소에 상장**된 것인지 (상증령 §74①2호**가목** 본문).
   * `isGovernmentBond === true`일 때만 의미가 있다.
   *
   * **가목 원문**(법제처 XML 실측 MST 283637, 시행 2026-02-27 — MCP `get_law_text`는 목
   * 단위 본문을 반환하지 않아 `target=law&JO=007400` XML의 `<목내용>`으로 확보):
   *   "가. **거래소에 상장된 것.** 다만, 최초로 거래소에 상장되어 물납허가통지서 발송일
   *    전일 현재 「자본시장과 금융투자업에 관한 법률」에 따라 처분이 제한된 경우에는
   *    그러하지 아니하다."
   *
   * 이 단서는 §74①2호 **전체**(국채·공채·주권·내국법인 채권/증권)에 걸린다 → **상장 국채는
   * 물납 충당 대상이 아니다**. §74②1호의 "국채 및 공채"는 그 관문을 통과한 것(비상장, 또는
   * 최초상장+자본시장법상 처분제한)을 뜻한다 — §74②2호가 "가목 단서에 해당하는 유가증권
   * (**제1호의 재산을 제외한다**)으로서 거래소에 상장된 것"이라며 1호를 빼는 것이 그 방증이다.
   *
   * 국고채는 통상 KRX 상장이므로 **묻지 않고 미상장으로 가정하면 납세자 유리 방향으로
   * 과대**해진다(충당 분자 과대 + §73①2호 한도2 미차감). 자동 fallback 금지 원칙에 따라
   * 사용자에게 명시적으로 받는다.
   *
   * true  → `tradableListedValue`(충당 제외 + §73①2호 한도2 차감)
   * false → `governmentBondValue`(§74②1호 충당 1순위 + 한도2 미차감)
   *
   * ※ 가목 **단서**(최초상장 + 자본시장법 처분제한)는 국채에서 사실상 성립하지 않아
   *   국채 전용 입력을 두지 않았다. 주권·증권 쪽은 `isNewlyListedDisposalRestricted`가 받는다.
   */
  isGovernmentBondListed?: boolean;
  /**
   * **최초 상장 + 자본시장법 처분제한** 유가증권인가 (상증령 §74①2호**가목 단서**).
   *
   * ## 가목 원문 (법제처 XML `<목내용>` 실측 MST 283637)
   *
   * > "가. 거래소에 상장된 것. **다만, 최초로 거래소에 상장되어 물납허가통지서 발송일 전일
   * >  현재 「자본시장과 금융투자업에 관한 법률」에 따라 처분이 제한된 경우에는 그러하지
   * >  아니하다.**"
   *
   * 상장 유가증권은 본문에서 충당 대상에서 빠지지만, 이 단서에 걸리면 **되살아난다**
   * (IPO 보호예수 주식이 전형).
   *
   * ## true일 때 물납에서 달라지는 것 — 셋 다 **납세자 유리** 방향이다
   *
   * | 축 | false(일반 상장) | true(처분제한) |
   * |---|---|---|
   * | 요건1 분자(§73①1호) | 미산입 | **산입** |
   * | 한도2 차감(§73①2호) | 차감 | **차감 안 함**(「법령에 따라 처분이 제한된 것은 제외한다」) |
   * | 충당 순서(§74②) | 대상 아님 | **2순위** |
   *
   * ⚠️ **결정세액에는 영향이 없다** — 물납 안내 카드 전용이다.
   * ⚠️ 국채·공채는 `isGovernmentBond`+`isGovernmentBondListed`가 먼저 가르므로 이 flag를
   *    함께 켜도 국채 분기가 우선한다(가목 단서가 국채에 성립하면 그때 이 flag가 쓰인다).
   */
  isNewlyListedDisposalRestricted?: boolean;
