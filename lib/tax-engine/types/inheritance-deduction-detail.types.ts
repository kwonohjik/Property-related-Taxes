...
export interface CohabitDeductionDetail {
  ...
  /**
   * §23의2① 주택부수토지(소득세 시령 §154⑦ 배율) 초과분 차감액(토지분에서만 차감, 건물분 보존).
   * - ancillaryLandArea·buildingFootprintArea·ancillaryLandRegion·ancillaryLandStdPrice 모두 입력된 경우만 존재 (undefined = G4 미사용).
   * - 0: 4필드 모두 입력됐으나 면적 한도 이내 (차감 없음).
   * - >0: 초과 토지분 차감액.
