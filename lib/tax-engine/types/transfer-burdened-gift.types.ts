...
export interface BurdenedGiftInfo {
  ...
  /**
   * **신고(증여계약) 단위 B를 자산가액 비율로 재배분한 이 카드의 채무액** — 컴패니언
   * (다른 물건) 함께 부담부증여 전용. 있으면 `computeDebtRatio`가 B로 쓴다.
   *
   * 🔑 **왜 위 두 필드를 스케일하지 않고 별도 override인가**
   *
   * 소령 §159①의 B/C는 **신고 단위 단일 비율**이고 자산 간 배분은 A가 수행한다.
   * 카드별로 재현하려면 Bᵢ = B × Aᵢ/ΣA를 실어야 하는데, 이를 「입력 채무 × 비율」로
   * 만들 수 없다:
   *
   * - 입력 채무가 **0인 자산**(무담보 물건)에는 어떤 비율을 곱해도 0 — 그 몫이 사라진다.
   * - 채무를 스케일하면 담보평가(§66)·임대평가(§61⑤) 성분이 함께 움직여 **Aᵢ 자체가
   *   바뀐다**. Aᵢ로 비율을 정하는데 비율이 Aᵢ를 바꾸는 **자기참조**가 된다.
   *
   * override는 평가를 건드리지 않으므로 Aᵢ가 고정된다.
   *
   * ⚠️ `scaleBurdenedGiftInfo`는 이 값을 **축소하지 않는다** — 다른 채무 필드와 같은
   *    규약이다. ④가 Aᵢ를 **지분 축소 후** 값으로 계산하므로 Bᵢ도 이미 그 스케일이고,
   *    엔진이 한 번 더 곱하면 이중 축소가 된다.
   *
