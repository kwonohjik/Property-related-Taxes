...
export interface PriorGift {
  ...
  giftTaxBase?: number;
  /**
   * 증여세 부과제척기간(국기법 §26의2④⑤: 10년, 무신고·부정·거짓누락 15년) 만료로
   * 증여세를 부과할 수 없는 사전증여. true면 상증법 §28① 단서 전단에 따라 상속세
   * 증여세액공제에서 제외한다(공제 합계·§28② 한도 분자 양쪽). §13 상속재산 가산은
   * 별도(상속 제척기간)이므로 그대로 유지. 기본 undefined=미만료(현행 동작).
   */
   ...
   * §24 3호·§19 분자에서 giftTaxBase 미설정 분기(branch 2)에서만 사용.
