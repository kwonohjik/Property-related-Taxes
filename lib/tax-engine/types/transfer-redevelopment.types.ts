...
export interface RedevelopmentInfo {
   ...
   *   - settlement 분만 **분할양도** 산식으로 계산 (청산금 − 취득가액 × 청산금 ÷ 평가액)
   *   - transferPrice 입력은 무시되고 settlementAmount 가 양도가액으로 의제됨
   *
   * 법령 근거:
   *   법 §88·§95①·§100 — 청산금은 종전 부동산의 **분할양도**(법규재산2012-358, 2012.11.09).
   *   평가액만 시행령 §166④1호(관리처분계획 가격)를 빌린다.
   *   ⚠️ §166①2호 **가목이 아니다** — 가목·나목은 신축주택/입주권 양도분이고,
   *      나목이 ×(평가액−청산금)÷평가액로 이 부분을 스스로 배제한다.
...
export interface RedevelopmentResult {
  ...
  /**
   * **청산금 수령분이 본 양도가액과 별개의 대가인 경우 그 금액** (표시 전용 echo).
   *
   * 완공APT 양도 + 청산금 수령 동시신고(사례 47)에서 신고 단위는 **두 개의 양도**다 —
   * 신축APT 양도(대가 = `transferPrice`)와 청산금 수령분 양도(대가 = 청산금). 그런데 폼의
   * 양도가액은 앞의 것만 담으므로, 결과탭 합계 열의 「양도가액」이 신고 단위 전체 대가보다
   * 청산금만큼 작다. 그 상태로 취득가액을 역산하면(양도가액 − 필요경비 − 양도차익)
   * **음수 취득가액**이 나온다(사례 47 실측: 2,000,000,000 − 2,100,000,000 = −100,000,000).
   *
   * ⇒ 표시 계층이 합계 양도가액에 더할 수 있도록 엔진이 그 금액을 echo한다. 이 값을 더하면
   *   역산 취득가액이 **납세자의 실제 취득가액과 정확히 일치**한다(사례 47 실측 100,000,000).
   *
   * ⚠️ 청산금 **납부**(pay)는 대가가 아니라 취득원가이므로 undefined다. `receiveOnlyMode`
   *   (청산금 분 단독 신고)도 폼 양도가액이 이미 청산금으로 치환돼 있어 undefined다.
   *   조합원입주권(subject="right") 분기는 `settlement`가 청산금액이 아니라 실지 양도가액을
   *   담으므로(사례 38 실측 320,000,000 = 양도가액) 대상이 아니다.
   *
   * 세액 무영향 — 표시 계층만 읽는다.
   */
  settlementSeparateConsideration?: number;
