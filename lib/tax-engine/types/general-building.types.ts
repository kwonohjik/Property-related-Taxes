...
export type GeneralBuildingInput = {
  ...
  // 선택적
  /**
   * 「소득세법」 §104③ 미등기양도자산 — **토지 축**.
   *
   * 🔑 **일반건물은 등기 여부를 토지·건물 각각 판단한다.** 둘은 별개 부동산이고 등기부도
   * 별도라, 건물만 미등기(무허가 신축 등)이고 토지는 등기된 조합이 실무에서 흔하다.
   * §104③의 「그 자산 취득에 관한 등기를 하지 아니하고 양도」도 자산별로 읽힌다.
   *
   * 이 플래그가 가르는 것 둘:
   *   ① 개산공제율 — §163⑥1호 단서(미등기 3/1000). `estimatedDeductionRate()` 단일 경유.
  ...
  /**
   * 「소득세법」 §104③ 미등기양도자산 — **건물 축**.
   *
   * 증축분(건물2 카드)도 이 축을 따른다 — 증축은 민법 §256 부합으로 기존 건물의 일부가 되고,
   * 등기도 별도 소유권보존등기가 아니라 표시변경등기이므로 「그 자산 취득에 관한 등기」는
   * 건물 1동 단위로 판단한다.
   */
...
export type GeneralBuildingEstimatedDeduction = {
  ...
  buildingBase?: number;
  /**
   * 개산공제율로 **실제 사용된 값**(§163⑥ 3/100 · 1호 단서 미등기 3/1000) — 토지분.
   *
   * 🔴 base echo만으로는 부족했다(2026-09-20). 표시 층 6곳이 율을 「3%」로 박아, 미등기
   * 자산에서 **적힌 산식이 적힌 값을 만들어내지 못했다** — 실측으로 base 238,000,000 ·
   * 개산공제 714,000인데 화면은 「238,000,000 × 3%」(= 7,140,000)라고 적었다. 세액은 맞고
   * 검산만 10배 어긋난 것이다. base와 같은 이유로 율도 echo한다.
   * 산출은 `estimatedDeductionRate()` 단일 경유 — 표시 층이 율을 다시 판정하지 않는다.
   */
  landRate?: number;
