...
/**
 * 800줄 분리 — LTHD 변형은 `transfer-tax-redevelopment-lthd.ts` 로 이동했다.
 *
 * ⚠️ `export … from` 만으로는 스코프에 이름이 안 들어온다 — 아래 함수들이 직접 호출하므로
 *    `import` 도 함께 둔다(재export는 외부 import 사이트 보존용).
 */
import {
  scaleLthdParts,
  zeroLthdParts,
