 ...
 * ①(영 §66④1호)은 법 §69① **본문**의 위임이라 이 경과조치의 대상이 아니다.
 *
 * ✅ **2026-09-03 — 과거 시행령 원문으로 확증** (종전에는 부존재 증명으로만 채워 두었다).
 *   **조특령 제정본(대통령령 제15976호 · 시행 1999-01-01) §66①1호**:
 *     「양도일 현재 특별시·광역시(광역시에 있는 군을 제외한다) 또는 시(…도·농복합형태의
 *      시의 읍·면지역을 제외한다)에 있는 농지중 도시계획법에 의한 주거지역·상업지역 및
 *      공업지역안에 있는 농지로서 **이들 지역에 편입된 날부터 3년이 지난 농지**. 다만,
 *      … 대규모 개발사업지역 … 단계적 사업시행 또는 보상지연으로 … 3년이 지난 농지를 제외한다.」
 *   ⇒ **3년 배제도, 소재지 요건도, 단서 예외도 제정 시부터 있었고 편입일 시점 제한이 없다.**
 *     2001-01-01 시행본도 문언 동일. 2002-01-01 시행본(대통령령 제17458호)에서는 ①에
 *     「농지소재지 거주자」 정의가 신설되며 이 호가 **②로 밀렸을 뿐** 문언은 그대로이고,
 *     같은 개정에서 **⑤(부분감면 산식)가 신설**됐다 — 즉 ①과 ②의 층위 분리가 원문에서 확인된다.
 *   제정 부칙에도 이 호의 적용례·경과조치가 없다(부칙 전문 실측 — 「제66조」 언급은 농특세법
 *   시행령 개정 참조 1건뿐). 조특령 부칙 278건 전수 probe에서도 0건이었다.
 *
 ...
 *   **1999-01-01까지 내려간다**. 목록에 없다고 「조회 불가」로 결론내지 말 것.
 *
