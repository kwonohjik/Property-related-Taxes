 ...
 * ⚠️ **차손 통산(§102②)은 이 함수가 «하지» 않지만, 들어오는 값은 이미 통산 후다.**
 *   2026-08-12에 주식 그룹(§102①2호), **2026-09-12에 기타자산 그룹(§102①1호)** 까지 통산이
 *   들어갔다. 이 함수는 `processItem`이 패치한 `taxBase`를 그대로 버킷에 담으므로 통산이
 *   자동으로 반영된다 — 차손 자산은 `taxBase`가 0이라 합계에 기여하지 않고, 차손을 흡수한
 *   이익 자산은 **줄어든 `taxBase`** 로 들어온다. 호출 순서를 바꿀 필요는 없다(§92② 순서 일치).
 *   🔑 다만 `processItem`의 기타자산 분기가 §104⑤ 호별 echo(`clause1Bucket*`·`clause9*`·
 *   `cross1045Adjustment`)를 **함께 다시 쓰지 않으면** 여기 값과 크로스 레이어 값이 어긋난다 —
 *   anchor `loss-offset-102-2.anchor.test.ts` M-10-5가 고정한다.
 *
