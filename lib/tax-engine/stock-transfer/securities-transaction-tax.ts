...
export function calcSecuritiesTransactionTax(
  ...
  if (isKOTCTrading && marketType === "unlisted") {
    ...
    rateReference = [
      ...
      `${period.kospiRef ?? STOCK_STX.STX_DECREE_5_1_KOSPI} ${period.refSuffix}`,
    ...
    // ⚠️ 「전 구간 불변」이 아니다 — 2017.4.1~2019.6.2 은 §5 2호 나목 **1천분의 3**이었고
    //    2019.6.3 개정으로 1호 나목(1천분의 1)으로 옮겨갔다. 값은 `konexNum`, 호·목은
    //    `konexRef` 가 구간별로 준다.
    num = period.konexNum;
    rateReference = `${period.konexRef ?? STOCK_STX.STX_DECREE_5_2_KONEX} ${period.refSuffix}`;
  } else {
