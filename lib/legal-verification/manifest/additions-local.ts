...
export const LOCAL_ADDITIONS: VerificationRule[] = [
  ...
  {
    /**
     * 🔴 G-09 · G-23 — 취득세 가산세의 근거. 이 엔진은 가산세를 계산하지 않으므로 결과 화면·
     * 도움말이 「미포함」을 고지하는 데 쓰인다. ②의 미신고 매각 80% 중가산은 §53·§55를
     * **배제하고** 산출세액의 180%를 보통징수하는 별도 국면이라 반드시 함께 안내해야 한다.
     */
    id: "ACQUISITION.SHORTFALL_PENALTY",
    citation: "지방세법 §21",
    keywords: [
      "부족세액의 추징 및 가산세",
      "보통징수의 방법으로 징수",
      "신고를 하지 아니하고 매각하는 경우",
      "산출세액에 100분의 80을 가산한 금액",
    ],
    keywordMode: "ALL",
  },
  ...
  {
    /**
     * 🔴 G-29 — 양도소득분 개인지방소득세의 **과세표준** 조항.
     * §103의3(세율)이 곱하는 대상이 이것이다 — 「결정세액 + 가산세」가 아니다.
     */
    id: "TRANSFER.LOCAL_TAX_BASE",
    citation: "지방세법 §103",
    keywords: [
      "양도소득에 대한 개인지방소득세 과세표준",
      "소득세법",
      "제92조에 따라 계산한 소득세의 과세표준",
      "동일한 금액으로 한다",
    ],
    keywordMode: "ALL",
  },
  {
    /**
     * 🔴 G-29 — 세액계산의 순서. 결정세액에 더하는 가산세를 **열거**하는 항이며
     * 「국세기본법」 §47의2~§47의4는 그 열거에 없다(그래서 지방소득세 대상이 아니다).
     */
    id: "TRANSFER.LOCAL_TAX_CALC_ORDER",
    citation: "지방세법 §103의2",
    keywords: [
      "세액계산의 순서",
      "산출세액",
  ...
  {
    /**
     * 🔴 G-29 — 「소득세법」 §114조의2 환산가액적용가산세분이 지방소득세에 반영되는 근거.
     *
     * 저장소는 이를 「§103의3의 과세표준 = 결정세액 + §114조의2 가산세」로 서술해 왔으나,
     * §103의3은 **세율** 조항이고 그 과세표준은 §103②(=「소득세법」 §92 과세표준)이다.
     * §114조의2분은 이 항이 「감정가액 또는 환산취득가액의 1천분의 5 … 결정세액에 더한다」로
     * 따로 정한다 — 소득세 5%의 10분의 1이라 값이 같을 뿐 근거 조문이 다르다.
     */
    id: "TRANSFER.LOCAL_TAX_CONVERTED_ACQ_ADD",
    citation: "지방세법 §103의9",
    keywords: [
      "1천분의 5에 해당하는 금액",
      "감정가액 또는 환산취득가액",
