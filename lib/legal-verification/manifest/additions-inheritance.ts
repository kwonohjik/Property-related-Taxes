...
export const INHERITANCE_ADDITIONS: VerificationRule[] = [
  ...
  },
  {
    ...
    citation: "상증법 §39",
    // ⚠️ 종전 3개는 전부 **①항 본문·1호**에서 와서, ①1호 가목의 공모 제외 괄호·①2호(고가)·
    //    ①3호(전환주식)·②(소액주주 합산)가 개정·삭제돼도 전건 통과했다. 엔진이 그 넷을 모두
    //    분기로 구현하고 있으므로(`capital-increase.ts`·`convertible-stock.ts`) 여기서 고정한다.
    keywords: [
      "새로운 주식 또는 지분",
      "보다 낮은 가액으로 발행하는 경우",
      "신주인수권",
      // ①1호 가목 괄호 — 주권상장법인 모집방법 배정은 제외(엔진 `publicOfferingExcluded`)
      "같은 법 제9조제7항에 따른 유가증권의 모집방법(대통령령으로 정하는 경우를 제외한다)으로 배정하는 경우는 제외한다",
      // ①1호 나목 — 실권주 미배정 시 「포기자의 특수관계인」이 수증자
      "실권주를 배정하지 아니한 경우에는 그 신주 인수를 포기한 자의 특수관계인이 신주를 인수함으로써 얻은 이익",
      // ①2호 — 고가 발행 축 전체의 진입 문언
      "신주를 시가보다 높은 가액으로 발행하는 경우",
      // ①3호 — 전환주식(§29②6호 = 전환시 이익 − 발행당시 이익)
      "발행 이후 다른 종류의 주식으로 전환함에 따라 얻은",
      // ② — 소액주주 2명 이상이면 1명으로 보고 계산
      "이익을 증여한 소액주주가 1명인 것으로 보고 이익을 계산한다",
      // ② 첫머리 — **적용 범위 한정**. 엔진(`capital-increase-allocation.ts`)이 1인 의제를
      //   저가에만 걸고 고가(①2호)를 제외하는 근거가 이 한 구절이다. 이 한정이 풀리면
      //   코드가 곧바로 과소적용이 되므로 문언 자체를 감시한다.
      "제1항제1호를 적용할 때 이익을 증여한 자가",
    ],
    keywordMode: "ALL",
  },
  {
  ...
  },
  {
    ...
    citation: "상증법 §45의5",
    // 🔴 종전 키워드 3개는 **2026.1.1. 개정에 대해 구별력이 0이었다**.
    //    개정(법률 제21219호)은 거래상대방을 「지배주주의 특수관계인」 →
    //    「지배주주 **및 그** 특수관계인」으로 넓혔는데, 제목·30%·법인세차감 세 문구는
    //    개정 전후 본문에 **모두** 그대로 있어 ALL 모드가 계속 통과했다.
    //    ⇒ 개정으로 실제 바뀐 구절을 건다. 개정 전 문언으로 되돌아가면 즉시 FAIL한다.
    keywords: [
      "특정법인과의 거래를 통한 이익의 증여 의제",
      "지배주주 및 그 특수관계인과",
      "주식보유비율이 100분의 30 이상인 법인",
      "특정법인이 부담한 법인세 상당액을 차감",
    ],
    keywordMode: "ALL",
  },
  {
    ...
    citation: "상증법 §45의3",
    // 종전 4개는 전부 용어(명사)라 과세요건이 바뀌어도 통과한다 — 위 §45의5와 같은 약점.
    // ①1호나목2)(대기업 추가 과세요건)의 문언을 걸어 요건 자체의 개정을 잡는다.
    keywords: [
      "수혜법인",
      "특수관계법인거래비율",
      "특수관계법인거래비율이 정상거래비율의 3분의 2를 초과하는 경우",
      "한계보유비율",
      "증여의제이익",
    ],
    keywordMode: "ALL",
  ...
  {
    // ④는 종전에 감시망 밖이었다 — 키워드 3개가 ①②⑥만 봤다. 「해당 법인의 주주등」
    // 납세의무 게이트가 이 항에 통째로 달려 있으므로 **요건·수범자·단서 셋을 각각** 고정한다.
    // 특히 단서의 조문 열거가 바뀌면 게이트가 발동하면 안 되는 유형에서 발동한다.
    ...
    keywords: [
      ...
      // §4의2④ 요건 — 「영리법인이」 받은 것이어야 한다(수증자 자신이 아니다)
      "영리법인이 증여받은 재산 또는 이익에 대하여",
      // §4의2④ 수범자 — 「해당 법인의 주주등」
      "해당 법인의 주주등에 대해서는",
      // §4의2④ 단서 — 이 열거가 바뀌면 배제 범위가 뒤집힌다
      "제45조의3부터 제45조의5까지의 규정에 따른 경우를 제외하고는",
    ],
    keywordMode: "ALL",
  },
  {
