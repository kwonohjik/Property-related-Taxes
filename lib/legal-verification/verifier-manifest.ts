...
const BASE_MANIFEST: VerificationRule[] = [
  // ── 양도소득세 ────────────────────────────────────────────────────
  {
    ...
    citation: "소득세법 §89 ①",
    // 뒤 2개는 **§89①4호(조합원입주권 비과세)** 문언 — 재개발 축의 비과세 게이트가 전적으로
    // 여기에 의존하는데 종전 키워드 4개로는 4호가 통째로 사라져도 통과했다(L1-09).
    keywords: [
      "1세대",
      "1주택",
      "비과세",
      ...
      // 뒤 2개는 **§89②**(주택+조합원입주권·분양권 보유 중 그 주택 양도 → ①3호 부적용) 문언.
      // 커버리지는 **조 단위**라 §89② 자체는 §89 키로 흡수돼 별도 규칙을 만들 수 없다 ⇒
      // 이 조의 키워드로 항을 감시한다(P0 · 2026-09-20). `legal-verification-coverage-complete`
      // 의 REQUIRED 블록이 이 두 문언을 동결한다.
  ...
  },
  {
    ...
    citation: "소득세법 §95 ②",
    /**
     * 뒤 3개는 **재개발 축 전용 문언**이다 (L1-09 · 2026-08-26 신설).
     *
     * 입주권 축의 두 핵심 판정 — 승계조합원 LTHD 배제(`REDEVELOPMENT.LTHD_RIGHT_PROVISO`)와
     * 인가전 분 한정(`LTHD_RIGHT_GAIN_LIMIT`) — 은 §95② **본문 괄호**의 두 문언에 전적으로
     * 의존한다. 그런데 종전 키워드는 「장기보유·공제액·보유기간·공제율」 넷뿐이라 그 괄호가
     * 개정·삭제돼도 전부 통과했다. 커버리지 게이트는 **조 단위** 비교라(`coverage.ts:8`)
     * 항·호 오기도 100%로 집계하므로 이쪽에서 막지 않으면 감시 공백이 남는다.
     *
     * 세 문자열 모두 2026-08-26 법제처 조문 본문(MST 280405, 시행 2026-07-01) verbatim이며
     * `fetchArticle`이 돌려주는 `fullText`에 실재함을 캐시로 실측했다.
     */
    keywords: [
      "장기보유",
      "공제액",
