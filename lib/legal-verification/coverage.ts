 ...
 *
 * 과거 3건(소득세법 §52의2·지방세법 §7의2·§107의2)은 KoreanLaw 실측으로 현행 조문
 * (조특법 §104의8·지방세법 §7·§107②5호)을 확정해 정정 완료했다.
 */
export const KNOWN_ABSENT_ARTICLES = new Set<string>([
  /**
   * 소득세법 시행령 제103조 — 본문 전체가 「삭제」다(KoreanLaw 실측 2026-08-28,
   * lawId 003956 시행 2026-07-01). 코드가 이 조문을 **근거로 쓰지는 않는다** —
   * 「§103은 삭제됐다」는 사실을 적은 주석이 인용 스캐너에 걸릴 뿐이다
   * (`DetailedStatementHelpers.ts` — 결과탭 코드리뷰 #031에서 인용을 §107②·§111③으로 정정).
   * 삭제 사실 자체를 지우면 같은 오인용이 재발하므로 주석은 남기고 여기에 명시한다.
   */
  "소득세법 시행령 제103조",
]);
...
export const UNVERIFIABLE_LAW_NAMES: Record<string, string> = {
  ...
  "지방세법 시행령 대통령령":
    "대통령령 제30939호 부칙 제2조·제3조(권리취득일 소급·상속 주택 수 특례 적용례)·제35477호 부칙 제2조(§28의2 1호 수도권 외 2억 적용례) — 부칙이라 조문 API(lawService.do target=eflaw) 대상이 아니다. 원문은 target=law + 현행 MST 부칙단위로 별도 조회",
  "부칙":
