 ...
 * 상속인별 1+페이지. 폰트 per-glyph fallback(BESSHI_FONT_STACK). react-pdf는 colgroup 미지원 →
 * flex row + flexGrow 비례폭으로 표 구성.
 *
 * Design: docs/02-design/features/inheritance-besshi-9-buppyo-2-property-valuation.ui.design.md §3-3
 */
...
function NaRow({ r }: { r: Buppyo2ItemRow }) {
  return (
    <View style={bp.row}>
      ...
      <Cell w={NA_W[2]} align="center">[ ]여 [ ]부</Cell>
...
function NaBlock({ rows }: { rows: Buppyo2ItemRow[] }) {
  ...
  return (
    <View>
      ...
      {BP2_KYE_ROWS.map((row) => {
        const v = kyeValue(row.key, t);
        return (
          <View key={row.key} style={bp.row}>
            <Cell w={4}>{row.label}</Cell>
            <Cell w={1.5} align="right">{v != null ? fmt(v) : " "}</Cell>
          </View>
        );
      })}
    </View>
    ...
    <>
      {data.map((d, idx) => (
        <Page key={d.heirId} size="A4" orientation="landscape" style={bp.page}>
          ...
          <Text style={bp.sheetTitle}>
            상속인 {CIRCLED[idx] ?? `${idx + 1}`} {d.sectionA.name || d.sectionA.relation}
          </Text>
          {d.usedLegalShareFallback && (
            <Text style={{ fontSize: 7, color: "#b45309", marginBottom: 2 }}>
              ※ 협의분할 미입력 자산은 명세(나)에서 생략되며, 계 합계에는 법정상속분 기준으로 포함됩니다.
            </Text>
          )}
          <GaBlock a={d.sectionA} />
          <NaBlock rows={d.itemRows} />
          <KyeBlock t={d.sectionTotal} />
          <Text style={bp.footer}>
