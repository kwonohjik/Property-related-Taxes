...
export const intOrUndef = (s: string): number | undefined => {
  const n = parseInt(s, 10);
  return Number.isNaN(n) ? undefined : n;
};

/** 쉼표 구분 번호 문자열 → number[] (양수만). 비면 undefined. */
export const parseNos = (s: string): number[] | undefined => {
  const nos = s
    .split(",")
...
export function buildAncillary(areas: AncillaryAreaForm, floors?: AncillaryFloorForm): AncillaryFacility[] {
...
// ─── 폼 검증은 `building-std-price-validate.ts`로 분리했다 (800줄 정책).
//     🔑 **재export하지 않는다** — 그러면 `form ↔ validate` 순환이 된다(validate가 `toEngineInput`을
//     쓴다). 소비처 6곳을 새 경로로 직접 바꿨다.
