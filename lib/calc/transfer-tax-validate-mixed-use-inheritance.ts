...
/** 상속 취득 겸용주택 전용 검증. `asset.acquisitionCause !== "inheritance"`면 즉시 null. */
export function validateMixedUseInheritanceAsset(
  ...
  if (asset.hasPartialUsageChange) {
    return `${label}: 상속 취득 + 보유 중 일부 용도변경 조합은 아직 지원하지 않습니다.`;
  }
  if (asset.transferCause === "public_expropriation") {
    return `${label}: 상속 취득 + 공익수용 특례 조합은 아직 지원하지 않습니다.`;
  }

  // 주택분 — PHD ON이면 PHD 자체 필수 검증이 이미 상속개시일 개별주택가격을 커버 →
