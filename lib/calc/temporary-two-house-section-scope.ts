 ...
 *
 * ## 🔄 ⑧은 더 이상 이 술어를 쓰지 않는다 (P6-b)
 *
 * 계산기 ③은 `mode="calc"`로 **§155⑧ + 합가**만 그리고, 나머지 특례의 입력·검증은 판정
 * 메뉴가 소유한다(`judgmentTemporaryTwoHouseVisible`). 그래서 종전의 3층 공용(⑤·④·⑧)
 * 중 **⑧이 빠졌다** — 위 표의 양방향 어긋남은 이제 화면마다 한 술어씩으로 막는다.
 *
 * ⚠️ 그래도 이 술어를 넓히면 **계산기 ⑤만** 열린다. §155⑧·합가는 2채 이상에서 의미가
 *    있으므로(§167의10①4호는 정확히 2채, §167의3⑨는 3채 이상) 현재 조건이 둘을 함께 덮는다.
 */
