...
export async function callTransferTaxAPI(form: TransferFormData): Promise<TransferAPIResult> {
   ...
   */
  /**
   * ④ **주 자산 컨텍스트** — 취득 모드 플래그 + 자산 종류별 서브페이로드.
   *
   * `transfer-tax-api-primary-context.ts`로 분리했다(800줄 정책). 구조분해로 받으므로
   * **하류 참조는 하나도 바뀌지 않는다** — 무동작 리팩터임을 그 형태가 보장한다.
   */
  const {
    isSuccessorRight,
    isSalesCase,
    ...
    bgInfo,
  } = buildPrimaryContext(form, primary, fractionalBundleMerge);
