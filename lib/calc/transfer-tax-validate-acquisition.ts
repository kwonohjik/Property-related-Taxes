/**
 * ⑧ **자산 취득가액 축 검증** — `validateAssetAcquisition` 단일 함수(+ 지목분할 보조).
 *
 * `transfer-tax-validate-asset.ts`에서 분리했다(800줄 정책). 그 파일은 **자산 진입 검증의
 * 오케스트레이션**(`validateAssetEntry`)과 날짜 순서를 맡고, 이 파일은 그중 가장 큰 축인
 * **취득가액·환산·추계 요건**을 맡는다.
 *
 * ⚠️ 방향은 한쪽뿐이다 — `validate-asset`이 이 파일을 부르고, 이 파일은 그쪽을 참조하지 않는다.
 *    (순환을 만들지 말 것 — `transfer-tax-api-asset-basics.ts`를 만든 것과 같은 규율.)
 *
