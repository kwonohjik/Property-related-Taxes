...
export function validateGbSaleAxis(
  ...
  /**
   * ⚠️ **증축 × 감정평가가액은 구분양도 밖에서도 성립하지 않는다**(2026-08-13 정정).
   *
   * 종전에는 이 차단이 `saleSplitMode === "actual"` 블록 **안**에만 있었다. 그런데 안분 방식
   * 라디오에는 별도의 `"appraisal"`(감정평가가액으로 안분) 모드가 있고, ④ 변환의
   * `saleAppraisalFields`는 모드 게이트가 없어 감정 2필드를 **항상 전송**한다. 3-way 증축
   * 엔진(`general-building-extension.ts`)은 감정평가 필드를 읽지 않고 §166⑥ 기준시가 3-way
   * 안분만 하므로, 그 조합에서 감정평가가액이 **조용히 버려졌다**
   * (실측: 감정 有·無의 안분값·결정세액이 원 단위까지 동일).
   *
   * 사유는 구분양도와 같다 — 감정은 토지·건물 두 값뿐이라 건물분을 본체·증축으로 다시 나눌
   * 근거가 없다. UI도 증축이면 감정 선택지를 비활성화한다(`GeneralBuildingSaleSplitSection`).
   *
   * 🔴 **`apportioned`는 제외한다.** `saleSplitModePatch("apportioned")`가 감정 3필드를 비우기
   *    이전에 저장된 자산에는 **잔존 감정값**이 남아 있을 수 있는데, 그 모드에서는 어차피
   *    기준시가 비율로 안분하므로 차단하면 지금 통과하는 입력을 거짓으로 막는 것이 된다.
   *    모드 미지정(레거시)도 같은 이유로 제외한다 — 화면 기본값이 `apportioned`다.
   */
  if (
    (asset.saleSplitMode === "appraisal" || asset.saleSplitMode === "actual") &&
  ...
  if (asset.saleSplitMode === "actual") {
     ...
     *
     * ⚠️ 다만 **감정평가가액과는 함께 쓸 수 없다** — 그 차단은 `"appraisal"` 모드와 함께
     *    이 블록 **밖**으로 올렸다(위 참조). 여기서 다시 기술하지 않는다.
     */
