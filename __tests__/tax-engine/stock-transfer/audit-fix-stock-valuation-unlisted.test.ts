...
describe("[ref2] MAIN weighted_avg — heavyRE 가중치 반전 근거는 §165④1(§165⑤ 아님)", () => {
  it("appliedRules에 §165⑤ 미포함, §165④1 반전 라벨 포함(세액 불변)", () => {
    ...
    /**
     * 2026-08-09 정정: 68,000 → **80,000**.
     * 반전 가중평균 (20,000×2 + 100,000×3)/5 = 68,000이지만, **80% 하한**
     * (100,000 × 0.8 = 80,000)이 더 커서 하한이 채택된다 — §165④1 단서를 취득기준시가에도
     * 적용하도록 고쳤기 때문이다.
     *
     * ⚠️ 이 테스트의 목적(**반전이 활성인가**)은 여전히 유효하다 — 반전 여부는 위 appliedRules
     *    두 줄이 단언한다. 다만 `perShareValue`로는 반전을 판별할 수 없게 됐다(정상 3:2도
     *    52,000 < 80,000이라 같은 80,000이 된다). 판별은 라벨이 맡는다.
     */
    expect(r.perShareValue).toBe(80_000);
    expect(r.acquisitionNetAssetFloorApplied).toBe(true);
  });
});
