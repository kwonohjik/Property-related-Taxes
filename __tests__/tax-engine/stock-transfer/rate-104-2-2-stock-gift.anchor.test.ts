...
describe("D-5 주식 증여 — 증여자 취득일 통산 (2025.1.1.~ 증여분)", () => {
   ...
   */
  /**
   * 🔴 **2026-08-11 갱신 — §97의2②3호 비교과세가 이 위에 얹혔다.**
   *
   * 종전 S-1은 「`carryover_gift` + 증여자 취득일 ⇒ 25%」를 고정했다. 세율 축(§104②2호)만
   * 있던 시절에는 맞았지만, **§97의2① 본체(필요경비)와 ②3호**가 구현되면서 전제가 바뀌었다:
   *
   *   ②3호 — 「①을 적용한 결정세액이 적용하지 않은 것보다 **적은 경우** ①을 적용하지 아니한다」
   *
   * 이 픽스처는 증여자 취득가액을 주지 않아 **①1호 승계 효과가 0**이다. 그러면 ①의 효과가
   * **세율 소급뿐**이라 세액이 반드시 **줄어들고**, ②3호가 발동해 이월과세가 배제된다
   * ⇒ 수증일 기산으로 돌아가 30%.
   *
   * 🔑 **주식은 단기 30% → 가목2) 20/25%의 낙차가 커서 ②3호로 배제되기 쉽다.**
   *   A가 채택되려면 취득가액 승계로 늘어난 과세표준 Δ가
   *   **Δ ≥ 0.2 × TB_B + 60,000,000** 을 넘어야 한다(TB_B > 3억 구간 · 실측 유도).
   *   이 픽스처(TB_B = 397,500,000)는 취득가액을 0으로 놓아도 Δ 상한이 1억이라 **불가능**하다.
   *
   * ⇒ 원래 의도(「§104②2호가 주식에 적용된다」)는 **S-1a**가 이어받는다.
   */
  it("S-1 승계 효과 없음(증여자 취득가액 미입력) — ②3호로 배제 ⇒ 30%", () => {
  ...
  /**
   * S-1a — **승계가 실효적이면** ②3호가 발동하지 않아 §104②2호 통산이 살아난다.
   *
   * 증여 당시 평가액 1주당 200,000(2억) · 증여자 취득가액 1주당 50,000(5천만):
   *   A: 과표 447,500,000 → 가목2) 60,000,000 + 25%×147,500,000 = **96,875,000**
   *   B: 과표 297,500,000 → 가목1) 30% = 89,250,000
   *   A ≥ B ⇒ **A 채택**(증여자 기산 25%)
   */
  it("S-1a 승계가 실효적 — §104②2호 통산 유지 ⇒ 가목2) 25%", () => {
    const r = calc({
      acquisitionCause: "carryover_gift",
      donorAcquisitionDate: D("2015-01-01"),
      perShareAcquisitionPrice: 200_000,
      donorAcquisitionPrice: 50_000,
    });
    expect(r.appliedRate).toBe(0.25);
    expect(r.calculatedTax).toBe(96_875_000);
  });
