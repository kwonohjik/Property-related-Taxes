...
//   양도가액 차액 = 5,000,000,000 − 4,000,000,000 = 1,000,000,000
//
//   🔴 정정 (리뷰 2026-08-28 #15) — 종전 기대값 244,791,666은 법문에 없는 **실효세율**
//      (산출세액 ÷ 양도차익)로 계산한 값이었다. §118의12①은 「차액 × §118의11에 따른
//      **세율**」이고, §118의11의 세율은 누진표이므로 차액이 얹힌 위치에 적용해야 한다
//      = 「출국일 기준 산출세액 − 실제 양도가액 기준 산출세액」.
//
//   과세표준① = 2,997,500,000 → 산출세액① = 734,375,000
//   과세표준② = 2,997,500,000 − 1,000,000,000 = 1,997,500,000 → 산출세액② = 484,375,000
//   조정공제 = 734,375,000 − 484,375,000 = 250,000,000
//            (두 과세표준이 같은 구간이라 차액 10억 × 25%와 일치한다)
//   경정 후 세액 = 484,375,000
// ============================================================
...
//   조정공제 = 250,000,000
//   공제한도 = 734,375,000 − 250,000,000 = 484,375,000
...
describe("ET-anchor-05: 외국납부세액공제 한도 내", () => {
  ...
  it("경정 후 세액 = 734,375,000 − 250,000,000 − 50,000,000 = 434,375,000", () => {
    expect(result.finalTaxAfterAdjustment).toBe(434_375_000);
  });
});
...
//   공제한도 = 484,375,000
//   적용 공제 = min(600,000,000, 484,375,000) = 484,375,000
// ============================================================
...
describe("ET-anchor-06: 외국납부세액공제 한도 초과", () => {
  ...
  it("외국납부세액공제 = 공제한도 = 484,375,000", () => {
    expect(result.foreignTaxCreditApplied).toBe(484_375_000);
  });
...
// GS 보강 anchor (ET-08b, ET-09, ET-11, ET-12, ET-13) →
// exit-tax-gap-anchors.test.ts 에 분리됨 (800줄 정책)
