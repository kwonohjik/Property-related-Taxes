 ...
 *
 * ## ⚠️ 아직 구현하지 않은 예외 (별도 범위)
 *
 * **사전-2022-법규재산-0684**(2022.11.28.) — 용도변경 전 기간이 §104⑦1호 **중과 주택**이라
 * §95② 괄호로 LTHD **배제 자산**이었던 경우에는 기산일이 **용도변경일**이 된다. 이는
 * 「기간 분할」이 아니라 「배제 자산이던 기간은 세지 않는다」는 **별개 규칙**이다.
 * UC-4가 그 미지원 상태를 고정한다 — 구현하면 뒤집혀야 한다.
 */
...
describe("UC — 겸용주택 용도변경 LTHD", () => {
  ...
  it("UC-4 🟡 중과 주택 기간 제외(사전-2022-법규재산-0684)는 **미구현**이다", () => {
    /**
     * 용도변경 전 기간이 §104⑦ 중과 주택이었더라도 현재는 기산일이 취득일 그대로다.
     * 예규대로면 **용도변경일**이 기산일이어야 한다 ⇒ 구현 시 이 테스트는 뒤집힌다.
     * 그때 값을 맞추지 말고 삭제·재작성할 것.
     */
    const r = run(mixedUsePdfGap());
