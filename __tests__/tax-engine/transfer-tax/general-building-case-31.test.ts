 ...
 *
 * 예제 사례 31 — 서울 동작구 사당동 132-10 근린생활시설 건물
...
describe("사례 31: 비사업용토지 판정 — 사업용", () => {
  ...
  /**
   * 2026-07-30 근거 정정 — 종전 「소득세법 시행령」 제168조의12(주택 부수토지) 배율을
   * 건물 부수토지에 잘못 적용했다. 정본은 「소득세법」 제104조의3 제1항 제4호 나목 →
   * 「지방세법」 제106조 제1항 제2호 → 「지방세법 시행령」 제101조 제1항 제2호·제2항.
   * 상업지역은 두 표 모두 3배여서 **배율·허용면적·세액은 무변경**이고 문구만 바뀐다.
   */
  it("배율 = 상업지역 3배 (「지방세법 시행령」 제101조 제2항)", () => {
    expect(out.appliedMultiplier).toBe(3);
    expect(out.multiplierDetail).toBe("상업지역 3배 (「지방세법 시행령」 제101조 제2항)");
  });
