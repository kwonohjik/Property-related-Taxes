 ...
 * 12억 초과: §89①4호 각 목 외의 부분 단서 + §95③ 안분 → taxableRatio = (15억−12억)/15억 = 0.2
 *
 * 법령 근거:
 *   - 소득세법 §89①4호 가목 본문: 1세대1입주권 비과세
 *   - 소득세법 §89①4호 각 목 외의 부분 단서: 12억 초과 안분
...
describe("36-A5-i — §89①4호 각 목 외의 부분 단서 12억 초과 안분 (pay + 실가 + 15억)", () => {
  /**
...
describe("36-A5-ii — §89①4호 각 목 외의 부분 단서 12억 초과 안분 (receive + 실가 + 15억)", () => {
  /**
   * ★ 2026-05-15 정정 후 (§166①2호 가목 법령 원문):
   *   settlement.gain = 양도가액(1,500M) − (평가액 300M − 청산금 90M) − 0
   *                   = 1,500M − 210M = 1,290,000,000
   *
   *   taxableRatio = (1,500M − 1,200M) / 1,500M = 0.2
   *   인가전 과세분 = floor(140M × 0.2) = 28,000,000
   *   청산금 분 과세분 = floor(1,290M × 0.2) = 258,000,000
   *   인가전 LTHD = floor(28M × 30%) = 8,400,000
   *   인가후 분 LTHD = 0
   *   양도소득금액 = (28M − 8.4M) + (258M − 0) = 277,600,000
   *   과세표준 = 277.6M − 2.5M = 275,100,000 (38% 구간)
   *   산출세액 = floor(275.1M × 38%) − 19,940,000 = 104,538,000 − 19,940,000 = 84,598,000
   *   지방세 = floor(84,598,000 × 10%) = 8,459,800
   */
  const redevReceive: RedevelopmentInfo = {
    ...redevInfoExemptPay(),
    settlementDirection: "receive",
  ...
  it("[36-A5-ii-03] 인가후 분 과세분 = 258,000,000 (1,290M × 0.2)", () => {
    // ★ 정정: settlement.gain = floor(1,290M × 0.2) = 258,000,000
    expect(result.redevelopmentDetail?.settlement.gain).toBe(258_000_000);
  });

  it("[36-A5-ii-04] ★ 산출세액 = 84,598,000 (275.1M × 38% − 19,940,000 — §55 2023)", () => {
    // 과세표준 = 277.6M − 2.5M = 275,100,000 → 38% 구간
    // floor(275,100,000 × 0.38) − 19,940,000 = 104,538,000 − 19,940,000 = 84,598,000
    expect(result.calculatedTax).toBe(84_598_000);
  });

  it("[36-A5-ii-05] ★ 지방소득세 = 8,459,800", () => {
    // floor(84,598,000 × 0.1) = 8,459,800
    expect(result.localIncomeTax).toBe(8_459_800);
  });
});
...
describe("36-A5-iii — §89①4호 각 목 외의 부분 단서 12억 초과 안분 (pay + 환산 + 15억)", () => {
  /**
