 ...
 *
 * · 「소득세법」 §95② 표1 — 보유기간 **3년 이상**부터 공제한다(3년 미만은 대상 자체가 아니다).
...
describe("T1-03 · §166⑤ LTHD 3년 미만 게이트 — 조합원입주권(1호)", () => {
  ...
  /**
   * 경계 — `calculateHoldingPeriod`는 §95④ 「취득일부터」 **초일을 산입**한다
   * (재산46014-205 · 국심1994경6005 · 집행기준 89-154-20 — 정본 anchor
   * `__tests__/tax-engine/holding-period-first-day-inclusion.anchor.test.ts`).
   * 3년은 취득일 응당일의 **전날**에 만료한다(민법 §160②):
   *
   * | 취득일 → 2021-06-01 | 결과 |
   * |---|---|
   * | 2018-06-03 | 3년 만료일 2021-06-02 → **years 2** (공제 0) |
   * | 2018-06-02 | 3년 만료일 2021-06-01 → **years 3** (공제 시작) |
   *
   * ⚠️ 종전(A1a 이전)에는 초일·말일 모두 불산입이라 경계가 2018-06-01 / 2018-05-31이었다 —
   *    2018-06-01 취득은 이제 3년이다. 여기서는 **현행 경계를 고정**해 조용한 이동을 막는다.
   */
  it("T1-03-03: 경계 — years 2와 years 3이 갈리는 지점 (초일 산입)", () => {
    expect(run("right", "2018-06-03", "2021-06-01", "2026-03-02").preApproval.lthd).toBe(0);
    expect(run("right", "2018-06-02", "2021-06-01", "2026-03-02").preApproval.lthd).toBeGreaterThan(0);
  });
