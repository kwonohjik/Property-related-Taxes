 ...
 *
 * ## ✅ 별건이던 양도가액 열 1원 잔차는 종결됐다 (2026-08-27)
 *
 * 완공APT+납부의 **양도가액 열**이 두 몫을 각각 floor해 합이 총액보다 1원 적었다
 * (1,026,315,789 + 473,684,210 = 1,499,999,999). 같은 함수의 **양도차익**은 이미
 * 잔액 흡수 규약을 쓰고 있었는데(`splitAptPay`) 표시 열만 규약 밖이었다 ⇒ 흡수 적용.
 * 상세는 `apt-pay-transfer-price-residual.anchor.test.ts`.
 *
 * ⚠️ 이 anchor의 행 잔차 상한(≤2원)은 **그대로 둔다** — 필요경비 안분에서 나올 수 있는
 *    잔차를 계속 감시해야 하고, 백만 단위 어긋남으로 되돌아가면 즉시 실패한다.
 */
