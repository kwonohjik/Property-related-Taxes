 ...
 *
 * 🔄 **경계 이동 (A1a — 보유기간 초일 산입 전환)**: 위 표는 종전 구현(초일·말일 불산입) 실측이다.
 *    「소득세법」 제95조 제4항 「취득일부터 양도일까지」는 **초일을 산입**하므로 15년은 응당일
 *    (2000-01-01)의 **전날 1999-12-31**에 만료한다(민법 §160② — 정본 anchor
 *    `__tests__/tax-engine/holding-period-first-day-inclusion.anchor.test.ts`). 현행 실측:
 *    1999-12-30 = 334,920,000 / 345,546,000(갈림) · **1999-12-31 = 334,920,000 / 334,920,000(같음)**.
 *    ⇒ 경계는 **1999-12-31**이다. 세액 수준(334,920,000·345,546,000)은 종전과 같고 경계일만 이틀 당겨졌다.
 *
...
describe("D-1 — 부과제척기간 안의 양도에서는 의제취득일 해석이 세액을 가르지 않는다", () => {
  ...
  it("1999-12-31 양도 — 경계 당일부터 같다 (장특 표1 15년 상한 도달 — 초일 산입 만료일)", () => {
...
describe("D-2 — 경계 前에는 갈린다 (트레이드오프를 문서로 남긴다)", () => {
   ...
   */
  it("1999-12-30 양도 — 의제 기산이 더 크다 (보유 14년 → 표1 상한 미도달)", () => {
    const { 현행, 의제 } = pair("1999-12-30");
    expect(의제).toBeGreaterThan(현행);
  });

  it("경계는 1999-12-31이다 — 하루 차이로 갈림이 사라진다 (§95④ 초일 산입 · 민법 §160②)", () => {
    // 종전(초일·말일 불산입) 경계는 2000-01-02였다 — A1a에서 이틀 당겨졌다.
    expect(pair("1999-12-30").의제).not.toBe(pair("1999-12-30").현행);
    expect(pair("1999-12-31").의제).toBe(pair("1999-12-31").현행);
  });
});
