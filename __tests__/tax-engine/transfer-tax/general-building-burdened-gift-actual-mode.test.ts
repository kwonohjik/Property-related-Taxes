 ...
 *
 * 출처: 부담부 증여.xlsx (예제 교재 사례 34 동일 케이스, 실거래가 모드)
...
describe("부담부증여 + 일반건물 실거래가 모드 — §159①1호 환산", () => {
  ...
  it("부담부증여 없이 호출 시 기존 §166⑥ 안분 (사용자 실거래가 사용) — 회귀 보장", () => {
    ...
    const land = result.apportionment.apportioned.find(a => a.assetKind === "land");
    /**
     * 🔴 **반전**(2026-08-07 P-2) — 취득가액 안분 기준이 「양도시」 → 「**취득시**」로 바뀌었다.
     *
     * 「소득세법」 제100조 제2항 본문은 「토지와 건물 등을 함께 **취득하거나** 양도한 경우 …
     * **취득 또는 양도 당시의** 기준시가 등을 고려하여 … 안분계산한다」로 **두 시점을 나란히**
     * 든다. 함께 **취득**한 것의 구분이 불분명하면 **취득 당시**다.
     * 부가령 §64①1호의 「공급계약일 현재」는 §166⑥이 준용하는 **안분계산의 방법**에 딸린 것이고,
     * **시점은 §100② 본문**이 정한다 — **시점이 아니라 방법**을 준용한다.
     *
     * 증축 경로는 2026-05-11에 같은 정정을 마쳤다(`general-building-extension.ts:194-206` —
     * 정답표 T-05로 검증). 실가 경로만 미반영이었다.
     *
     * ⚠️ 이 단언은 **Excel 정답표가 아니라 개발자 회귀 가드**다(테스트 이름의 「회귀 보장」).
     *    Excel 유래 단언(D8·E8·D9·E9·D10·E10)은 전부 **§159 분기**에 있고 이 변경에 영향받지 않는다.
     *
     * 취득시 비율 = 2,724,270,000 / (2,724,270,000 + 424,472,064) ≈ 0.865193
     * 2,500,000,000 × 0.865193 = 2,162,982,823   (종전 양도시 비율 ≈ 0.926365 → 2,315,913,382)
     */
    expect(land?.allocatedAcquisitionPrice).toBe(2_162_982_823);
  });
});
