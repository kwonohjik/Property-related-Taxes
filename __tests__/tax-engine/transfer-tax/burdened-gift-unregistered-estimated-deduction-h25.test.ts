 ...
 * 종전: computeEstimatedDeduction이 항상 3% → 미등기 자산 개산공제 10배 과대(과소과세).
...
describe("H-25 computeEstimatedDeduction 미등기 0.3%", () => {
  it("등기(기본) = 3%", () => {
    expect(computeEstimatedDeduction(100_000_000)).toBe(3_000_000);
    expect(computeEstimatedDeduction(100_000_000, false)).toBe(3_000_000);
  });
  it("미등기 = 0.3% (10배 축소)", () => {
    expect(computeEstimatedDeduction(100_000_000, true)).toBe(300_000);
  });
});
