...
//
// 장특공 표1 — 2009-03-01~2024-03-01 보유 (§95④ 초일 산입: 15년 만료일 2024-02-29 경과 → 15년,
//   표1 상한 2%×15 = 30%. 정본 anchor holding-period-first-day-inclusion.anchor.test.ts.
//   종전 초일·말일 불산입 구현은 14년·28%로 셌다 — A1a에서 반전):
//   242,500,000 × 0.30 = 72,750,000
//   양도소득금액 = 242,500,000 - 72,750,000 = 169,750,000
//
// 양도소득 기본공제 2,500,000:
//   과세표준 = 167,250,000
//
...
//   167,250,000 구간 = 38% (누진공제 19,940,000)
//   산출세액 = 167,250,000 × 0.38 - 19,940,000 = 63,555,000 - 19,940,000 = 43,615,000
...
describe("케이스 4 — 부담부증여 + 주택 (다주택 일반 과세)", () => {
  ...
  it("장특공 표1 (보유 15년, 30%) → 공제액 72,750,000", () => {
    const result = calculateTransferTax(makeCase4Input(), rates);
    // 양도차익 = 500,000,000 - 250,000,000 - 7,500,000 = 242,500,000
    expect(result.transferGain).toBe(242_500_000);
    // 엔진 holding 산정: 2009-03-01 → 2024-03-01 → §95④ 초일 산입 15년 (calculateHoldingPeriod)
    // 표1: 2% × 15 = 30% (상한) → 242,500,000 × 0.30 = 72,750,000
    expect(result.longTermHoldingDeduction).toBe(72_750_000);
    expect(result.longTermHoldingRate).toBeCloseTo(0.30, 7);
  });

  it("§55 양도연도 누진세율 자가검증 → 산출세액 = 43,615,000", () => {
    const result = calculateTransferTax(makeCase4Input(), rates);
    // 과세표준 = 169,750,000 - 2,500,000 = 167,250,000
    expect(result.taxBase).toBe(167_250_000);
    // §55 2024 누진세율: 88M~150M=35%, 150M~300M=38% (누진공제 19,940,000)
    // 167,250,000 × 0.38 - 19,940,000 = 63,555,000 - 19,940,000 = 43,615,000
