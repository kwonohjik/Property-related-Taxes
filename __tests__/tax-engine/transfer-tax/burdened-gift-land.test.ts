...
//
// 장특공 표1 (보유 12년, 24%):
//   §95④ 「취득일부터 양도일까지」 초일 산입 — 2012-01-01 → 2024-01-01(응당일) = 만 12년
//   (종전 초일불산입 구현은 11년·22%로 셌다. 정본 anchor:
//    __tests__/tax-engine/holding-period-first-day-inclusion.anchor.test.ts)
//   588,000,000 × 0.24 = 141,120,000
//
// 양도소득금액 = 588,000,000 - 141,120,000 = 446,880,000
// 과세표준 = 446,880,000 - 2,500,000 = 444,380,000
//
// §55 2024 누진세율 자가검증:
//   300M~500M = 40% (누진공제 25,940,000)
//   444,380,000 × 0.40 - 25,940,000 = 177,752,000 - 25,940,000 = 151,812,000

describe("케이스 6 — 부담부증여 + 토지 (사업용 일반 과세)", () => {
  const TRANSFER_DATE = new Date("2024-01-01");
  const ACQUISITION_DATE = new Date("2012-01-01"); // 보유 12년 (초일 산입 — 응당일 양도)
  ...
  it("양도차익 = 588,000,000 / 장특공 표1 24% = 141,120,000", () => {
    const result = calculateTransferTax(makeCase6Input(), rates);
    expect(result.transferGain).toBe(588_000_000);
    // 엔진 holding 산정: 2012-01-01 → 2024-01-01 → 초일 산입(§95④)으로 만 12년
    // 표1: 2% × 12 = 24% → 588,000,000 × 0.24 = 141,120,000
    expect(result.longTermHoldingDeduction).toBe(141_120_000);
    expect(result.longTermHoldingRate).toBeCloseTo(0.24, 7);
  });

  it("§55 양도연도 누진세율 자가검증 → 산출세액 = 151,812,000", () => {
    const result = calculateTransferTax(makeCase6Input(), rates);
    // 과세표준 = (588M - 141.12M) - 2.5M = 444,380,000
    expect(result.taxBase).toBe(444_380_000);
    // §55 2024 누진세율: 300M~500M = 40% (누진공제 25,940,000)
    // 444,380,000 × 0.40 - 25,940,000 = 177,752,000 - 25,940,000 = 151,812,000
    expect(result.calculatedTax).toBe(151_812_000);
  });
});
...
//   - 장특공 적용 (소법 §95② 표1 — 2017.1.1. 이후 보유분, 사례 가정상 케이스 6과 같은 24%)
