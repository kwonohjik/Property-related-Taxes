 ...
 *
 * 예제 사례 33 — 서울 성북구 정릉동 299-6 근린생활시설
 ...
 *   산출세액(T-24=6,480,952)·지방세(T-25=648,095)는 예제와 정확 일치.
 *   중간 계산값 1~3원 차이는 엔진 floor 패턴 vs 예제 계산기 차이로 정상.
 *
 * NBL 판정:
...
describe("사례 33: 양도차익·LTHD·양도소득금액 (통산 전)", () => {
  ...
  it("T-09 — 토지 양도차익 = 104,260,597", () => {
    ...
    // ★ landExp=6,595,232: 취득시 비율 안분 floor 결과 (예제 6,595,233과 1원 차이 — 정상)
  ...
  it("T-12 — 건물1 양도차익 = −26,527,095", () => {
    ...
    // ★ b1Exp=1,404,768: 잔액 보정 (예제 1,404,767과 1원 차이 — 정상)
...
describe("사례 33: 차손 통산·과세표준·세액 (통산 후 — A-2 결정 확정값)", () => {
  ...
  it("T-19 — 건물2 흡수액 = 2,336,344 (income 기반 pro-rata)", () => {
    ...
    // ★ A-2 확정: 엔진 실제 출력 (예제 2,336,344와 일치)
    const b2Absorbed = (b2P?.lossOffsetFromSameGroup ?? 0) + (b2P?.lossOffsetFromOtherGroup ?? 0);
