...
describe("T-IMG-1: 공익사업 수용 감면 — 부재지주 임야 쌍방실가 (이미지 사례)", () => {
  ...
  it("최종 수치 스냅샷 (회귀 앵커) — 이미지 정답 일치", () => {
    ...
    /**
     * 🔴 **2026-08-23 갱신 — 농어촌특별세 2,425,116이 더해진다.**
     *
     * 종전 85,254,495는 결정세액 77,504,087 + 지방소득세 7,750,408뿐이었다. 이미지 사례는
     * **부재지주 임야**라 「직접 경작한 토지」가 아니므로, 「농어촌특별세법 시행령」 §4①1호의
     * §77 비과세 괄호(「§69①본문에 따른 거주자가 **직접 경작한 토지**로 한정」)에 해당하지 않는다.
     * ⇒ 같은 법 §5①1호에 따라 감면세액 12,125,580 × 20% = **2,425,116**이 부과된다.
     *
     * ⚠️ 이미지에서 온 값(양도차익·장특·과세표준·산출세액·감면세액·결정세액·지방소득세)은
     *    **하나도 건드리지 않았다** — 바뀐 것은 엔진이 합산하는 `totalTax`의 구성뿐이다.
     */
    expect(r.totalTax).toBe(85_254_495 + 2_425_116);
  });
});
