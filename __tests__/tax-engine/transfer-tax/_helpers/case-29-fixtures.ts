 ...
 * Sum_A = 취득시 기준시가합 = INT(개공지_취득 × 12.57) + 건물기준_취득(총액) = 50,004,666 + 69,602,660 = 119,607,326
 * Sum_F = 최초고시시 기준시가합 = INT(11,060,632 × 12.57) + 건물기준_최초(총액) = 139,032,144 + 69,527,856 = 208,560,000
...
// 실무: 사용자가 ㎡당 단가 × 연면적 보정계수를 직접 곱해 정수(원) 입력.
// floor 정합: 기준시가합(Sum_A·Sum_F)이 정수가 되도록 건물분을 원 단위 정수로 입력
//   (= Sum − INT(개공지 × 면적)). 종전 1,001,189 × 69.52 = 69,602,659.28의 원단위 표현.
/** 취득시(2000) 건물 기준시가 — 총액 = Sum_A 119,607,326 − INT(토지분) 50,004,666 */
export const BUILDING_STD_AT_ACQ     = 69_602_660;
/** 최초고시시(2005) 건물 기준시가 — 총액 = Sum_F 208,560,000 − INT(토지분) 139,032,144 */
export const BUILDING_STD_AT_FIRST   = 69_527_856;
...
/** 취득시 토지 기준시가합 = INT(개공지_취득 × 대지면적) */
export const LAND_STD_AT_ACQ       = 50_004_666;   // INT(3,978,096 × 12.57) — floor 정합
/** 취득시 건물 기준시가합 = 건물기준_취득 (총액, 원 단위 정수) */
export const BUILDING_STD_AT_ACQ_TOTAL = 69_602_660;

/** C-01-11: 환산취득가 토지분 = INT(합계 × 토지기준시가합 / Sum_A) — floor 정합 후 56,504,755 */
export const ESTIMATED_ACQ_LAND    = 56_504_755;
/** C-01-12: 환산취득가 건물분 = 합계 − 토지분 = 135,155,041 − 56,504,755 */
export const ESTIMATED_ACQ_BUILDING = 78_650_286;
