...
//
// 장특공 표1 (보유 10년, 20%):
//   197,000,000 × 0.20 = 39,400,000
//   보유기간은 초일 산입(소득세법 §95④ 「취득일부터 양도일까지」 —
//   __tests__/tax-engine/holding-period-first-day-inclusion.anchor.test.ts)이라 응당일 양도
//   2014-01-01 → 2024-01-01은 만 10년이다. 종전 구현(초일·말일 불산입)은 9년 18%로 환산해
//   35,460,000 · 과세표준 159,040,000 · 산출세액 40,495,200을 냈다.
//
// 양도소득금액 = 197,000,000 - 39,400,000 = 157,600,000
// 과세표준 = 157,600,000 - 2,500,000 = 155,100,000
//
// §55 2024 누진세율 자가검증:
//   150M~300M = 38% (누진공제 19,940,000)
//   155,100,000 × 0.38 - 19,940,000 = 58,938,000 - 19,940,000 = 38,998,000
...
describe("케이스 9 — 부담부증여 + 건물(토지 외)", () => {
  ...
  it("양도차익 = 197,000,000 / 장특공 표1 20%", () => {
    const result = calculateTransferTax(makeCase9Input(), rates);
    expect(result.transferGain).toBe(197_000_000);
    expect(result.longTermHoldingDeduction).toBe(39_400_000);
    expect(result.longTermHoldingRate).toBeCloseTo(0.2, 7);
  });

  it("§55 양도연도 누진세율 자가검증 → 산출세액 = 38,998,000", () => {
    const result = calculateTransferTax(makeCase9Input(), rates);
    // 과세표준 = (197M - 39.4M) - 2.5M = 155,100,000
    expect(result.taxBase).toBe(155_100_000);
    // §55 2024 누진세율: 150M~300M = 38% (누진공제 19,940,000)
    // 155,100,000 × 0.38 - 19,940,000 = 58,938,000 - 19,940,000 = 38,998,000
    expect(result.calculatedTax).toBe(38_998_000);
  });
});
