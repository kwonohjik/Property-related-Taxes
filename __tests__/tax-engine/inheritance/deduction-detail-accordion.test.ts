...
describe("상속공제 detail 노출 anchor (A-1~A-12)", () => {
  ...
  describe("A-1: lumpSumComparisonDetail — 일괄 선택", () => {
    ...
    it("A-1-2 itemizedSubtotal = 350,000,000 (기초2억+자녀2명1억+배우자연로자5천)", () => {
      // 기초 200M + 자녀공제 2명×50M=100M + 연로자공제(배우자 85세 ≥ 65세) 50M = 350M
      // 일괄공제 500M > 350M → 일괄 선택
      expect(d?.itemizedSubtotal).toBe(350_000_000);
    });
