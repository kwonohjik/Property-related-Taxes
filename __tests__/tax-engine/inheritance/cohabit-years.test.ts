/**
 * G3: §23의2①1호 동거연수 계산 — anchor (legacy fallback 경로)
 *
 ...
 *
 * Phase 4: calcCohabitYears 5인수로 교체 (reasons: CohabitReason[] | undefined, excludedYearsLegacy: number)
 *   - reasons=undefined → legacy fallback (cohabitExcludedYears 수동값 사용)
 *   - reasons=[] 또는 [...] → Phase 4 구조화 배열 경로 (cohabit-reasons.test.ts에서 검증)
 *
...
describe("G3 §23의2①1호 동거연수 계산 — calcCohabitYears (legacy fallback 경로)", () => {
  // ─── 기본 계산 ─────────────────────────────────────────────
  it("G3-BASIC 13년 동거, 미성년 없음 → effectiveYears=13, meets=true", () => {
  ...
  it("G3-EXACT-10Y 정확히 10년 → effectiveYears=10, meets=true", () => {
    const r = calcCohabitYears("2013-06-01", "2023-06-01", undefined, undefined, 0);
    expect(r.effectiveYears).toBe(10);
    expect(r.meetsRequirement).toBe(true);
  });

  it("G3-SHORT 8년11개월 → floor=8 < 10, meets=false", () => {
    // 2014-07-01 ~ 2023-06-01 = 8년 11개월 → floor=8
    const r = calcCohabitYears("2014-07-01", "2023-06-01", undefined, undefined, 0);
    expect(r.rawYears).toBe(8);
    expect(r.meetsRequirement).toBe(false);
  });

  it("G3-9Y11M 9년11개월 → floor=9 < 10, meets=false", () => {
    const r = calcCohabitYears("2013-07-02", "2023-06-01", undefined, undefined, 0);
    expect(r.rawYears).toBe(9);
    expect(r.meetsRequirement).toBe(false);
  });

  // ─── 미성년 기간 제외 (2016.1.1.~ 적용) ─────────────────────
  it("G3-MINOR 2005-01-01 동거시작, 2004-01-01 생 → 성인=2023-01-01, effectiveYears=0", () => {
    ...
    const r = calcCohabitYears("2005-01-01", "2023-01-01", "2004-01-01", undefined, 0);
  ...
  it("G3-MINOR-PARTIAL 2000-01-01 동거, 2010-01-01 생 → 성인=2029-01-01, deathDate=2025-01-01", () => {
    ...
    // → differenceInYears(2025-01-01, 2029-01-01) = 음수 → 0
    const r = calcCohabitYears("2000-01-01", "2025-01-01", "2010-01-01", undefined, 0);
    expect(r.effectiveYears).toBe(0);
    expect(r.meetsRequirement).toBe(false);
  });
  ...
  it("G3-MINOR-ADULT 2000-01-01 동거, 1985-01-01 생(성인=2004-01-01) → 성인 후 동거=같음", () => {
    ...
    const r = calcCohabitYears("2000-01-01", "2023-01-01", "1985-01-01", undefined, 0);
  ...
  it("G3-PRE2016-MINOR deathDate 2015(미성년제외 개정 전) → 미성년 제외 적용 안 함", () => {
    ...
    const r = calcCohabitYears("2000-01-01", "2015-06-01", "2000-01-01", undefined, 0);
    expect(r.minorYearsDeducted).toBe(0);
  ...
  // ─── 부득이 사유 제외 (§23의2② legacy 수동 입력 경로) ───────
  it("G3-EXCLUDED legacy excludedYearsLegacy=2 → effectiveYears 감소", () => {
  ...
  it("G3-EXCLUDED-OK legacy excludedYearsLegacy=1, 총 12년 → effectiveYears=11 meets=true", () => {
    const r = calcCohabitYears("2011-01-01", "2023-01-01", undefined, undefined, 1);
  ...
  it("G3-EXCLUDED-ZERO excludedYearsLegacy=0 → usedLegacyFallback=false", () => {
    const r = calcCohabitYears("2010-01-01", "2023-01-01", undefined, undefined, 0);
    expect(r.effectiveYears).toBe(13);
    expect(r.usedLegacyFallback).toBe(false);
  });

  // ─── 복합: 미성년 + 부득이 사유 legacy ──────────────────────
  it("G3-COMBINED 2000-01-01 동거, 1990-01-01 생(성인=2009-01-01), legacy부득이1년 → effectiveYears=13", () => {
    ...
    const r = calcCohabitYears("2000-01-01", "2023-01-01", "1990-01-01", undefined, 1);
    expect(r.effectiveYears).toBe(13);
    expect(r.meetsRequirement).toBe(true);
  });
  ...
  it("G3-MEETS-EXACTLY-10-AFTER-DEDUCTIONS 미성년+부득이로 정확히 10년", () => {
    ...
    // excludedYearsLegacy=0 → effectiveYears=10 meets=true
    const r = calcCohabitYears("2000-01-01", "2023-01-01", "1994-01-01", undefined, 0);
    expect(r.effectiveYears).toBe(10);
    expect(r.meetsRequirement).toBe(true);
  });
});
