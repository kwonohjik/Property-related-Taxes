/**
 * 다건 양도세 기본공제 배분 전략 — **FIRST vs EARLIEST_TRANSFER 구분**.
 *
 * 둘은 UI에서 별개 옵션이므로 입력 순서 ≠ 양도일 순인 케이스에서 결과가 달라야 한다.
 *
 * ⛔ **`MAX_BENEFIT`(높은 세율 우선)은 폐지됐다** — §103② 법정 순서가 아니었다
 *   (2026-09-16 사용자 결정 「법문대로」). §103② 자체의 단언은
 *   `basic-deduction-103-2-order.predo.anchor.test.ts`가 담당한다.
 */
...
const eligible = [
  { idx: 0, income: 100_000_000, transferDate: new Date("2026-05-01") },
  { idx: 1, income: 100_000_000, transferDate: new Date("2026-02-01") },
];
const AVAILABLE = 2_500_000;

describe("allocateBasicDeduction — FIRST vs EARLIEST_TRANSFER", () => {
