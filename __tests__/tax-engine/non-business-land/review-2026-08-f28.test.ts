...
describe("F28 · 세액 — 두 선택지의 효과가 뒤바뀌어 있었다", () => {
  ...
  /**
   * 보유 2014-01-01 → 2024-01-01 = §95④ 초일 산입으로 **10년**(장특 20%) — 정본 anchor
   * `__tests__/tax-engine/holding-period-first-day-inclusion.anchor.test.ts`.
   *   차익 700,000,000 − 장특 140,000,000 − 기본공제 2,500,000 = 과표 557,500,000
   *   · 분리과세(전량 사업용): 557,500,000 × 42% − 35,940,000 = 198,210,000
   *   · 별도합산(80% 비사업용, §104⑤ MAX): 비사업 446,000,000 × 50% − 25,940,000 = 197,060,000
   *       + 그 외 111,500,000 × 35% − 15,440,000 = 23,585,000 → 220,645,000 (> 합산 198,210,000)
   * (종전 초일·말일 불산입 구현은 9년·18%로 227,225,000 / 204,090,000이었다 — A1a에서 반전.
   *  F28 정정 전후 두 값이 뒤바뀌는 관계는 보유기간과 무관하게 그대로다.)
   */
  it("F28-4: 별도합산 220,645,000원 · 분리과세 198,210,000원 (정정 전 정확히 반대)", () => {
    expect(tax("separate")).toBe(220_645_000); // 정정 전 198,210,000
    expect(tax("special_sum")).toBe(198_210_000); // 정정 전 220,645,000
  });
});
