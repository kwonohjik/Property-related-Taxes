 ...
 * ## 이 테스트가 잠그는 것
 *
 * **편입일 미상일 때 유예 미적용(= 비사업용)** 이라는 **확정된 선택**(2026-09-14).
 *
 * 근거는 조문의 원칙/예외 구조다 — 법 §104의3①2호 **본문**이 「임야 = 비사업용」(원칙)이고
 * 가목이 예외를 열므로, **사업용(예외)을 주장하는 쪽이 그 요건사실을 뒷받침**한다. 편입일은
 * 그 예외가 유지되는지를 정하는 요건사실이라, 미상이면 예외를 세울 수 없어 원칙으로 돌아간다.
 * 실무 실증: 조심 2022서8116(2023.9.12.) — 쟁점이 「…비사업용 토지에 해당하지 않는다는
 * **청구주장**의 당부」로 구성되고 청구인이 인가서·회신·계약서로 예외를 세웠다.
 *
 * ⛔ 뒤집으려면 단서(N년 경과)의 입증책임을 **정면으로** 다룬 판례·예규가 필요하다
 *    — 방향은 **과소과세** 쪽이다.
 */
...
describe("[V5-d] 임야 편입유예 — 미상 기본값 계약", () => {
  it("🔴 편입일 미상 → 유예 미적용 (2026-09-14 확정 · 원칙/예외 구조)", () => {
