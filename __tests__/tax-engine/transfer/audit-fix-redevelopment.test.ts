 ...
 *    법령: §166⑤1호 보유기간 = 취득일 ~ 인가일(초일 산입 — §95④ 「취득일부터」, 재산46014-205.
 *    종전 주석·구현의 「초일불산입」은 A1a에서 뒤집혔다: holding-period-first-day-inclusion.anchor.test.ts).
 *    세액 무영향(표시 전용).
 *
 * 기대값은 조문·초일 산입 달력 산술로 독립 도출(엔진 출력 베끼기 금지).
 */
...
describe("결함182 — 토지 환산 경로 분기 보유기간 잔여월/일 보존 (§166⑤1호)", () => {
  ...
  it("보유월수 = 90개월 (취득 2007-04-09 → 인가 2014-10-23, 초일 산입 만 7년 6개월)", () => {
    // 초일 산입(소득세법 §95④ — holding-period-first-day-inclusion.anchor.test.ts): 2007-04-09 기산.
    // 7년 → 2014-04-08 만료, +6개월 → 2014-10-08 만료, 잔여 2014-10-09 ~ 2014-10-23(양도일 포함) 15일.
    // 7 × 12 + 6 = 90 (기존 84 = 7×12 로 잔여 6개월 소실).
    // (종전 초일불산입 구현은 2007-04-10 기산 · 잔여 13일이었다 — 월수는 같다.)
    expect(pre.holdingMonths).toBe(90);
  });

  it("보유일수 = 15일 (2014-10-09 ~ 2014-10-23, 양 끝 포함)", () => {
    expect(pre.holdingDays).toBe(15);
  });
...
describe("결함328 — 주택 출자 입주권 환산 경로 분기 보유기간 잔여월/일 보존", () => {
  ...
  it("보유월수 = 66개월 (초일 산입 만 5년 6개월)", () => {
    // 초일 산입(§95④): 2008-04-09 기산. 5년 → 2013-04-08 만료, +6개월 → 2013-10-08 만료,
    // 잔여 2013-10-09 ~ 2013-10-23(양 끝 포함) 15일.
    // 5 × 12 + 6 = 66 (기존 60 = 5×12 로 잔여 6개월 소실). (종전 초일불산입 구현은 잔여 13일.)
    expect(pre.holdingMonths).toBe(66);
  });

  it("보유일수 = 15일 (2013-10-09 ~ 2013-10-23, 양 끝 포함)", () => {
    expect(pre.holdingDays).toBe(15);
  });
