...
describe("게이트 확대 후 세액 (D1 실증)", () => {
  ...
  it("C-06c 입주권 — 미적용 (§99①2호)", () => {
    ...
    /**
     * 특례 前 값 유지 — 이 케이스의 단언 대상은 **특례 미적용**이다(위 `toBeUndefined`).
     *
     * 🔴 2026-08-23 갱신: 종전 기대값 **594,000,000**은 개산공제를 3%로 계산한 값이었다.
     *    입주권은 법 §94①2호 **가목**이라 시행령 §163⑥**4호 = 1/100**다(3호는 나목·다목만 열거).
     *    이 anchor가 입주권 환산을 보는 **유일한 테스트**였고, 그래서 잘못된 3%를 조용히 고정하고
     *    있었다 — 율을 법대로 고치자 이 한 건만 실패했다.
     *
     *      3%: 10억 − 환산 400,000,000 − 개산공제 6,000,000 = 594,000,000  ← 종전
     *      1%: 10억 − 환산 400,000,000 − 개산공제 2,000,000 = 598,000,000  ← 현행(법령 정합)
     *
     *    특례가 적용되지 않는다는 사실 자체는 변하지 않았다.
     */
    expect(r.transferGain).toBe(598_000_000);
  });
});
