 ...
 *
 * 🔄 표의 산출세액은 A1a(보유기간 초일 산입) **이전** 실측이다. 현행 β 산출세액은 **90,819,143** —
 *    기존건물분(2009-03-01 → 2024-03-01)이 §95④ 초일 산입으로 14년 → 15년(28% → 30%)이 된
 *    차이뿐이고 양도차익 행은 그대로다(정본 anchor `holding-period-first-day-inclusion.anchor.test.ts`).
 *
...
describe("A-A2 · β 스케일 — §166 절대 금액항이 채무비율로 줄어든다", () => {
  ...
  it("세액 실측값 (mock 세율)", () => {
    // 손 도출 — §166⑤2호나목 기존건물분은 2009-03-01 → 2024-03-01 = 초일 산입 15년(표1 30%),
    // 청산금분은 인가일 2018-06-01 → 2024-03-01 = 5년(10%):
    //   장특 = 76,000,000×30% + floor(227,142,857×30%) + floor(90,857,143×10%)
    //        = 22,800,000 + 68,142,857 + 9,085,714 = 100,028,571
    //   과표 = 394,000,000 − 100,028,571 − 2,500,000 = 291,471,429
    //   산출 = floor(291,471,429 × 38%) − 19,940,000 = 90,819,143 · 지방 9,081,914 → 합계 99,901,057
    // (종전 초일·말일 불산입 구현은 기존건물분 14년·28%로 93,123,029 / 102,435,331이었다)
    const r = run();
    expect(r.calculatedTax).toBe(90_819_143);
    expect(r.totalTax).toBe(99_901_057);
  });
});
