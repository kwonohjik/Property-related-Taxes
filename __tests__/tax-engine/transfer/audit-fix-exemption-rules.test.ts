/**
 * 감사 확정 결함 회귀 테스트 (#591 백로그 R7 — 2026-07-29 현행 master 기준 재검증)
 *
 * ## 재검증 결과 — 4건 중 2건은 이미 해결돼 있었다
 *
 * | 지적 | 현행 master | 조치 |
 * |---|---|---|
 * | `inh_public_interest.lawRef` §48① → §16① | **여전히 §48①** (상수 `INH_PUBLIC_CONTRIBUTION`은 이미 있는데 rule만 미교체) | 수정 |
 * | `§46` 호번호 오기 (2호→5호, 6호→9호) | **여전히 오기** | 수정 |
 * | `gift_public_trust.lawRef` → §52 | **이미 §52** (`PUBLIC_TRUST_GIFT`) | 상수명만 정정 |
 * | `gift_disabled_trust.lawRef` → §52의2 | **이미 §52의2** (`DISABLED_TRUST`) | 상수명만 정정 |
 *
 * 감사가 쓴 상수명 `GIFT_PUBLIC_TRUST`·`DISABLED_TRUST_EXCLUSION`·`DISABLED_TRUST_LIMIT_REF`는
 * master에 존재하지 않는다 — 감사 브랜치가 제안한 이름이고, master는 다른 이름으로 같은 값을 갖는다.
 *
 * ## 원 감사 기록 — lib/tax-engine/exemption-rules.ts (+ evaluator·legal-codes)
 *
...
describe("audit-fix: exemption-rules 조문 인용 정정", () => {
  ...
  it("[AF-3] gift_public_trust.lawRef = 상증법 §52, description에 §52 포함·§46/§48 미포함", () => {
    ...
    // 상수명 정정(2026-07-29): 감사가 기대한 `GIFT_PUBLIC_TRUST`는 존재하지 않는다.
    //   master는 같은 값을 `PUBLIC_TRUST_GIFT`(§52, 증여)로 이미 보유하고 있었다 —
    //   §17(PUBLIC_TRUST, 상속)과 구분하기 위한 이름이다. **값은 감사 지적대로 이미 옳았다.**
    expect(rule?.lawRef).toBe(EXEMPTION.PUBLIC_TRUST_GIFT);
  ...
  it("[AF-4] gift_disabled_trust.lawRef = 상증법 §52의2, description/riskNote §52의2·§46의2 미포함", () => {
    ...
    // 상수명 정정: `DISABLED_TRUST_EXCLUSION` 미존재 → master의 `DISABLED_TRUST`. 값은 이미 옳았다.
    expect(rule?.lawRef).toBe(EXEMPTION.DISABLED_TRUST);
    expect(rule?.lawRef).not.toBe("상증법 §48①");
