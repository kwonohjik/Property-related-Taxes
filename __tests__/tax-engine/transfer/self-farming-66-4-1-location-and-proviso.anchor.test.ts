...
//    규정에 의한다」 — **단서만** 배제한다.
// 영 §66④1호는 본문 위임이라 대상이 아니다.
//
// ✅ **2026-09-03 — 과거 시행령 원문으로 확증**(종전에는 부존재 증명으로만 채워 두었다).
//   **제정본(대통령령 제15976호 · 시행 1999-01-01) §66①1호**가 이미
//   「양도일 현재 특별시·광역시(광역시에 있는 군을 제외한다) 또는 시(…읍·면지역을 제외한다)에
//    있는 농지중 … **편입된 날부터 3년이 지난 농지**. 다만, … 대규모 개발사업지역 …」이다 —
//   **3년 배제·소재지 요건·단서 예외가 전부 제정 시부터 있고 편입일 시점 제한이 없다.**
//   2001-01-01본 문언 동일. 2002-01-01본(대통령령 제17458호)에서 ①에 「농지소재지 거주자」
//   정의가 신설되며 이 호가 **②로 밀렸을 뿐**이고, 같은 개정에서 **⑤(부분감면 산식)가 신설**됐다.
//   제정 부칙에도 이 호의 적용례·경과조치가 없다(부칙 전문 실측). 부칙 278건 전수 probe도 0건.
//
// 🔑 조회법: `lawSearch.do?target=eflaw` 목록은 조특령을 2005-02-19부터만 주지만,
//   `lawService.do?target=eflaw&LM=조세특례제한법 시행령&efYd=YYYYMMDD`는 1999-01-01까지 간다.
//
// ## 종전 동작의 결함
//
