...
function makeEligibleAfter5Y(overrides: Partial<New993Input> = {}): New993Input {
  return {
    ...
    // 필드명 정정(2026-07-29): 감사 fixture의 `transferPrice`는 New993Input에 없다.
    //   고가주택 판정용 물건 전체 양도가액은 `wholePropertyTransferPrice`다.
    //   위 주석대로 면적(100㎡ < 165㎡)으로 이미 비고가 배제되므로 값 자체는 판정에 무관.
