...
const rentalException: NonNullable<TransferTaxInput["rentalHousingException"]> = {
  ...
  rentalUnits: [
    {
      // 2026-07-29 fixture 재구성(#591 감사 R7): 감사 브랜치(2026-07-14 베이스) 이후
      //   `RentalUnitInput`이 개편돼 옛 필드명으로는 **특례 경로에 진입조차 못 했다**.
      //   현행 필드 대응: `registrationDate` → 사업자등록일/임대사업자등록일 2분리,
      //   `rentalType: "long-8"` → `rentalCategory: "long_general"`.
      //   구성은 동작이 확인된 `rental-housing-exception/rh-b1-early-exempt-bypass.anchor.test.ts`의
      //   `rentalUnitOk`를 기준으로 맞췄다. 본 테스트의 검증 대상은 **기본공제(§103①)**이므로
      //   임대 요건은 "전부 충족"으로만 두면 충분하다.
