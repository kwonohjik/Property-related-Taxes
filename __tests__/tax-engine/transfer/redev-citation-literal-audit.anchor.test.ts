...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    hit: (l) =>
      /§?\s*95\s*②\s*단서/.test(l) &&
      // 2026-09-08: 사용자 노출 문구가 「LTHD」 → 「장기보유특별공제」로 바뀌어 세 대안이
      // **죽은 리터럴**이 됐다. 개명된 문구로 1:1 갱신해 종전 탐지 범위를 그대로 보존한다.
      // ⚠️ 셋을 「장기보유특별공제」 하나로 합치면 규칙이 넓어져 **다른 성격의 위반 4건**이
      //    함께 걸린다(분양권 배제를 §95② 단서로 인용한 주석 등 — 본문 열거 부재가 근거다).
      //    그것은 별건이므로 여기서 조용히 끌어들이지 않는다.
      /인가전|인가 전|인가후|인가 후|승계조합원|장기보유특별공제 제외|장기보유특별공제 0|장기보유특별공제 공제율 0|청산금/.test(l) &&
