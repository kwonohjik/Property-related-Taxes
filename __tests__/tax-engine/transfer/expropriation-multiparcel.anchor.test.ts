...
describe("다필지 공익수용 §164⑨ 특례 (D7)", () => {
  // 🔁 A1a(보유기간 초일 산입, 2026-09-26) — 2010-06-01 → 2020-06-01은 응당일 양도라 §95④
  //    초일 산입으로 **10년**(종전 구현 9년) ⇒ 장특 18% → 20%. 세액만 이동하고 양도차익은 불변.
  //    종전 세액: 167,892,000 → 81,107,066. anchor: `__tests__/tax-engine/holding-period-first-day-inclusion.anchor.test.ts`.
  it("C-07: 필지별 min[] 적용 — 분모 5억 → 3억, 세액 163,140,000 → 78,619,333", () => {
  ...
  it("C-19: 보상 후보 1개라도 0이면 미적용 (게이트 — 현행 총액 유지)", () => {
    ...
    // 594,000,000 × 80% − 2,500,000 = 472,700,000 × 40% − 25,940,000 (A1a 10년 20%)
    expect(r.calculatedTax).toBe(163_140_000);
  });
