...
describe("R-2 §154⑤ 단서 — 비과세 보유기간은 주거용 사용일부터 기산한다 (Phase C)", () => {
  ...
  // 🔁 OH-59(2026-09-26) — 경계를 하루 앞당겼다. 대통령령 제34265호는 **공포·시행 모두 2024-02-29**이고
  //    (MST 260889, 부칙 제1조 「공포한 날부터 시행」), 2024-03-01 시행 예외는 별표 2(간이세액표)뿐이다.
  //    §154⑤ 적용례가 부칙에 없어 시행일 이후 양도분 — 종전 기대값(2024-02-29 비과세)은 오기였다.
  it("C-2' 양도일이 2024-02-28이면 단서를 적용하지 않는다 — 취득일 기준 비과세", () => {
    const r = calculateTransferTax(
      exempt({ transferDate: new Date("2024-02-28"), ...toggle("2023-01-01") }),
      rates,
    );
    expect(r.isExempt).toBe(true);
  });

  it("★ 2024-02-29 정확일 양도부터 단서를 적용한다 (경계 — 대통령령 제34265호)", () => {
    // 주거용 사용일 2023-01-01 → 2024-02-29까지 1년 2개월이라 보유 2년 미달 → 과세.
    // 같은 입력이 하루 전(2024-02-28)에는 위 케이스처럼 비과세다.
    const r = calculateTransferTax(
  ...
  it("2024-03-01 양도도 단서를 적용한다 (종전 경계일 — 회귀 가드)", () => {
    const r = calculateTransferTax(
