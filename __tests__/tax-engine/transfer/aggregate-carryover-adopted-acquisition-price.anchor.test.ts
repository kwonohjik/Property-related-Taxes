...
describe("A-9 축2: 일반건물 토지 파트 이월과세 — 신고서 열 산술이 성립한다", () => {
  ...
  /**
   * 🔁 A1a(보유기간 초일 산입, 2026-09-26) — 못의 값이 옮겨졌다(표시 축 수정과 무관).
   *    건물 파트 2021-03-01 → 2024-03-01은 응당일 양도라 §95④ 초일 산입으로 **3년**(종전 구현 2년)
   *    ⇒ 장특 0% → 표1 6%. 토지 파트(증여자 2005-06-15 기산 18년 · 30%)는 불변.
   *    토지 320,000,000 × 70% = 224,000,000 + 건물 247,000,000 × 94% = 232,180,000
   *    − 2,500,000 = 453,680,000 × 40% − 25,940,000 = 155,532,000 (종전 161,460,000).
   *    anchor: `__tests__/tax-engine/holding-period-first-day-inclusion.anchor.test.ts`.
   */
  it("C-A8 🔒 세액 불변 — 155,532,000", async () => {
    expect((await callGb()).determinedTax).toBe(155_532_000);
  });
});
