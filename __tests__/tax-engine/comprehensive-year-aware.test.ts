...
describe("재산세 비율안분 공제 산식 (Pre-Do anchor — 시행령 산식 갭 실증)", () => {
  // YA-5: 사례1 공제 504,000 — 법정 산식은 "재산세액 × (종부세과표×재산세FMR×표준세율) ÷ 표준세율재산세액"
  //
  // 사례1 (pdf3~4 실측):
  //   ⓐ 부과 재산세: 1,650,000 (재산세 과표 9.5억×60%=5.7억 × 0.4% − 630,000)
  //   ⓑ 종부세 과표분 표준세율 재산세상당액: 2.1억 × 60%(재산세 FMR) × 0.4% = 504,000
  //   ⓒ 총 표준세율 재산세액: 1,650,000
  //   공제 = ⓐ × ⓑ/ⓒ = 504,000
  //
  // 현행 엔진 산식(단순 과표 비율: 1,650,000 × 2.1억/5.7억 = 607,894)은 법정 산식과 다름.
  // ※ 현행 시행령 공제 조항(§4의3 추정 — Do에서 조문 번호 확정)도 동일 구조이므로 연도 무관 산식 교체 대상.
  // ⚠️ 분리: 재산세 안분 산식은 연도 무관 별도 버그 (현행 2024도 부정확).
  // 시행령 §4의3 법정 산식(표준세율 기반) ≠ 현행 엔진(단순 과표 비율).
  // 기존 93개 + 재산세 테스트 광범위 재산정이 필요해 별도 PR로 분리 — 사용자 결정 대기.
  it.skip("YA-5 (사례1): 안분 공제 = 504,000 (표준세율 기반 법정 산식) — 별도 PR", () => {
    const credit = calculatePropertyTaxCreditProration(
      1_650_000,        // 재산세 부과세액
      210_000_000,      // 종부세 과세표준
      570_000_000,      // 재산세 과세표준 (9.5억 × 60%)
