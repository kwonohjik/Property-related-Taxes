...
describe("Z-4 — 공장용 건축물 부속토지는 §101①1호 본칙을 따른다", () => {
  ...
  // 🔴 계약 정정(2026-08-05) — 종전 이 자리에는 "공장입지기준면적 입력 시 그 면적을
  // 기준면적으로 쓴다"(700㎡)가 있었다. 그러나 공장입지기준면적은 「지방세법 시행령」
  // §102①1호(**분리과세**) 한도이고, §101①1호(별도합산) 본문에는 그 개념이 없다
  // (법제처 MST 287223 실측). 분리과세 한도를 별도합산 판정에 끌어다 쓴 것이라 제거했다.
  // 상세: docs/02-design/features/property-factory-base-area-102-1-fix.plan.md
  it("공장입지기준면적을 입력해도 무시하고 §101①1호 본칙을 따른다", () => {
    expect(mk({ factoryStandardArea: 700 }).baseArea).toBe(800); // 200㎡ × 4배
    expect(mk({ factoryStandardArea: 700 })).toEqual(mk({}));
  });

  it("바닥면적 × 해당 용도지역 배율 (종전에는 공업지역 4배 고정이었다)", () => {
