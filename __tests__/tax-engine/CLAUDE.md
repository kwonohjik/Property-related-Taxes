...
- **감면 시한 검증 테스트** (`reduction-period-check.test.ts` 패턴): `checkReductionPeriod(id, ctx)` 는 `ctx.contractDate` 우선, 없으면 `acquisitionDate` fallback. 매매계약일 기준 13개 조문은 반드시 `contractDate` 유무 두 케이스를 모두 작성해 fallback 동작을 anchor.
