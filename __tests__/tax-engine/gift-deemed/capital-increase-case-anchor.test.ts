 ...
 *
 * 🔄 **픽스처를 두 번 옮겼다** — 목적(값 축·등호 축을 각각 죽인다)은 그대로 두고 좌표만 옮겼다.
 *    · 2-B(2026-09-25): 저가 실권처리의 기준선이 「균등하게 증자하는 경우의 증가주식수」 기준
 *      ㉯(§29②2호 가목)로 갈라졌다.
 *    · 2-E(2026-09-25): 임계에서 `Math.floor`를 걷어냈다. 종전 좌표(㉮ 10,000)에는 **등호점이
 *      아예 존재하지 않는다** — 기준선 R이 정수일 때 등호는 `3R`이 10의 배수여야 성립하는데
 *      그 픽스처의 해가 공집합이다(k ≤ 869 ∧ k ≥ 870). ⇒ ㉮를 11,500으로 옮겨
 *      기준선 R = 10,000·㉰ 7,000에서 차액 3,000 = 정확히 30%가 되게 했다.
 */
describe("[CI-S39-RATIO-ARM] ② 실권처리 — 30% 비율 arm 단독 통과 구간", () => {
  ...
  it("차액 3,666 = 균등 ㉯ 9,666의 37.9% → 비율 arm으로 과세 (이익 110,000 ≪ 3억)", () => {
    const r = mk(6_000);
    // 검증내역의 ㉯는 **실제** 증가주식수 기준 그대로다 — 기준선만 균등으로 갈라졌다.
    expect(r.perShareAfter).toBe(10_400);
    // 30% 기준 2,899.8 ≤ 차액 3,666 < 40% 기준 3,866.4 ⇒ 기준을 40으로 올리면 미과세로 뒤집힌다
    expect(totals(r).get("B")).toBe(110_000);
  });

  it("차액 3,000 == 균등 ㉯ 10,000의 30% 기준과 «정확히 같음» → 「이상」이므로 과세 (등호 축)", () => {
    const r = mk(7_000);
    // 금액은 equity-delta — 실제 ㉯ 10,600 기준: 150주×10,600 − 100주×11,500 − 50주×7,000 = 90,000
    expect(totals(r).get("B")).toBe(90_000); // >= 를 > 로 바꾸면 0
  });

  it("차액 2,999 < 30% 기준 3,000 → 미과세 (경계 짝)", () => {
    const r = mk(7_001);
