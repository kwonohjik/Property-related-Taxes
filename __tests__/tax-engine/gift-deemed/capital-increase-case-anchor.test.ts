 ...
 *
 * 🔄 **2-B로 기준선이 바뀌어 픽스처를 재산정했다**(2026-09-25). 저가 실권처리의 30% 기준선은
 *    이제 「균등하게 증자하는 경우의 증가주식수」 기준 ㉯(§29②2호 가목)로 잡힌다 —
 *    이 픽스처는 Σentitled 100주 기준 **8,695~8,696**이고, 검증내역에 보이는 실제 ㉯(9,200대)와
 *    다르다. **등호 축을 고정한다는 목적은 그대로**이므로 폐기하지 않고 값만 옮겼다.
 */
describe("[CI-S39-RATIO-ARM] ② 실권처리 — 30% 비율 arm 단독 통과 구간", () => {
  ...
  it("차액 2,666 = 균등 ㉯ 8,666의 30.7% → 비율 arm으로 과세 (이익 80,000 ≪ 3억)", () => {
    const r = mk(6_000);
    // 검증내역의 ㉯는 **실제** 증가주식수 기준 그대로다 — 기준선만 균등으로 갈라졌다.
    expect(r.perShareAfter).toBe(9_200);
    // 30% 기준 2,599 ≤ 차액 2,666 < 40% 기준 3,466 ⇒ 기준을 40으로 올리면 미과세로 뒤집힌다
    expect(totals(r).get("B")).toBe(80_000);
  });

  it("차액 2,608 == 균등 ㉯ 8,696의 30% 기준과 «정확히 같음» → 「이상」이므로 과세 (등호 축)", () => {
    const r = mk(6_088);
    // 금액은 equity-delta — 실제 ㉯ 9,217 기준: 150주×9,217 − 1,000,000 − 50주×6,088 = 78,150
    expect(totals(r).get("B")).toBe(78_150); // >= 를 > 로 바꾸면 0
  });

  it("차액 2,607 < 30% 기준 2,608 → 미과세 (경계 짝)", () => {
    const r = mk(6_089);
