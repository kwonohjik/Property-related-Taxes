...
describe("§39①2호 나목 고가발행 — 실권주 미배정·비율가중 (§29②4 기준 30%·3억)", () => {
  // 🔄 **픽스처를 법문에 맞췄다**(2026-09-26 · 리뷰 6단계 #23).
  //    종전은 `issuedShares: 50_000` / `ratioDenomShares: 50_000`으로 **같은 수를 두 자리에**
  //    넣었다. 두 값은 법령상 다른 수량이다:
  //      · `issuedShares`   = 「증자에 의하여 **증가한** 주식수」(㉯ 산식 — §29②3호 나목)
  //      · `ratioDenomShares` = 「증자전의 지분비율대로 **균등하게 증자하는 경우의** 증자 주식총수」
  //                             (§29②4호 분모)
  //    나목은 「실권주를 **배정하지 아니한** 경우」라 그 실권주만큼 신주가 발행되지 않는다
  //    ⇒ 실제 증가 = 균등가정 총수 − 실권주수 = 50,000 − 30,000 = **20,000**.
  //    종전 픽스처는 「실권주 30,000이 소멸했는데 실제 증가가 균등가정과 같다」는,
  //    **법문상 성립할 수 없는 사실관계**를 고정하고 있었고 두 축을 구별하지도 못했다.
  //    ⚠️ 기대값 변경(60,003,000 → 75,006,000)은 엔진 거동이 바뀐 것이 **아니다** — 입력이 바뀌었다.
  it("[CI-HIGH-NR] ㉯ 11,666 · 차액 8,334 × 실권주 3만 = 2.5002억 × (특수관계 15,000 ÷ 균등 50,000) = 75,006,000", () => {
    const r = calcCapitalIncreaseGift({
      direction: "high",
      subType: "no_realloc",
      preIssuePrice: 10_000,
      preIssueShares: 100_000,
      newSharePrice: 20_000,
      issuedShares: 20_000, // 실제 증가 (균등 50,000 중 실권 30,000 미배정)
      forfeitedShares: 30_000,
      relatedAcquiredShares: 15_000,
      ratioDenomShares: 50_000, // 균등증자 가정 증자 주식총수
    });
    expect(r.applied).toBe(true);
