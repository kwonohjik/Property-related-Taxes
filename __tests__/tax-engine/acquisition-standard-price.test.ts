...
describe("calcStandardPrice — 건물 잔가율 경계값 (지방세법 시행령 §4조의2)", () => {
  // 공통 입력: 신축가격기준액 1,000,000원/㎡, 지수 1.0, 연면적 100㎡
  // 시가표준액 = 1,000,000 × 1.0 × 잔가율 × 100 = 잔가율 × 100,000,000
  const calcBuilding = (elapsedYears: number) =>
    calcStandardPrice("building", undefined, {
      propertyType: "building",
      newBuildingBasePrice: 1_000_000,
      structureIndex: 1.0,
      usageIndex: 1.0,
  ...
  it("경과 0년 미만(음수): 0으로 보정, 잔가율 1.000", () => {
    // Math.max(0, elapsedYears) 처리 확인
    expect(calcBuilding(-3).standardValue).toBe(100_000_000);
  });

  it("경과 1년 (구간 경계): 잔가율 1.000", () => {
    // y <= 1 → 1.000
    expect(calcBuilding(1).standardValue).toBe(100_000_000);
  });

  it("경과 5년 (구간 경계): 잔가율 0.870", () => {
    // 1.000 - (5 - 1) × 0.0325 = 1.000 - 0.130 = 0.870
    expect(calcBuilding(5).standardValue).toBe(87_000_000);
  });

  it("경과 10년 (구간 경계): 잔가율 0.750", () => {
    // 0.870 - (10 - 5) × 0.024 = 0.870 - 0.120 = 0.750
    expect(calcBuilding(10).standardValue).toBe(75_000_000);
  });

  it("경과 15년 (구간 경계): 잔가율 0.640", () => {
    // 0.750 - (15 - 10) × 0.022 = 0.750 - 0.110 = 0.640
    expect(calcBuilding(15).standardValue).toBe(64_000_000);
  });

  it("경과 20년 (구간 경계): 잔가율 0.540", () => {
    // 0.640 - (20 - 15) × 0.020 = 0.640 - 0.100 = 0.540
    expect(calcBuilding(20).standardValue).toBe(54_000_000);
  });

  it("경과 30년 (최저 경계): 잔가율 0.350", () => {
    // 0.440 - (30 - 25) × 0.018 = 0.440 - 0.090 = 0.350
    expect(calcBuilding(30).standardValue).toBe(35_000_000);
  });

  it("경과 31년 (최저 초과): 잔가율 0.350 (최저값 유지)", () => {
    expect(calcBuilding(31).standardValue).toBe(35_000_000);
  });
});
