...
describe("A-8 사이드바 — 엔진 축(취득가액 0 · 필요경비 전액)을 따른다", () => {
  ...
  it("대조군 — 본문(swap 미발동)은 환산취득가액을 그대로 싣는다", () => {
    ...
    // 본문 축에서 엔진이 차감한 것은 개산공제(4,500,000)다 — 폼의 자본적지출(20,000,000)이 아니다.
    // 종전에는 사이드바가 폼 입력값을 보였다(F-14에서 엔진 값으로 정정). swap 축(230,000,000)과
    // 값이 갈리므로 두 테스트가 서로의 구별력이 된다.
    expect(rows[0].expense).toBe(result.expenses);
    expect(rows[0].expense).toBe(4_500_000);
  });
});
