...
describe("D-11 §99 구 방식 — 일수 안분 산출근거", () => {
  ...
  it("echo가 있으면 일수 산식에 값이 대입된다", () => {
    ...
    const t = bodyText();
    // 분수 표기(`<Frac>`)라 분자·분모가 별도 span으로 갈리고, `bodyText()`는 그것을 이어 붙인다.
    // 단언 축(「산식이 뜨고 값이 대입된다」)은 그대로다 — `÷` 문자만 사라졌다.
    expect(t).toContain("감면대상 양도차익 = 전체 양도차익 × 감면대상 보유일수전체 보유일수");
    expect(t).toContain("100,000,000 × 1,826일3,652일 = 50,000,000");
  });
