...
/**
 * 🔴 **반전(2026-09-08 · R15)** — 문구가 「공유 지분율…」로 고정돼 있었는데, 같은 위젯이
 *    지분 분할 모드에서는 「취득 지분율」로 렌더된다(`AssetSectionAcquisition.tsx:100`).
 *    그 화면에서는 **없는 칸 이름**을 안내했다. ⑧은 `splitMode`를 모르므로 라벨을 알 수 없다
 *    ⇒ 두 라벨의 공통 어간 「지분율」만 쓴다. 경고는 해당 FieldCard 안에 붙으므로 대상이 명확하다.
 *    「분자·분모를 쓰지 않는다」는 원래 축은 아래에서 그대로 유지된다.
 */
describe("[OWN-PCT] 지분율 오류 문구 — 단일 술어", () => {
  it.each([
    ["100", "100", null],
    ["50", "100", null],
    ...
    ["150", "100", OVER_100],
    ["101", "100", "지분율은 100%를 초과할 수 없습니다 (입력값 101%)."],
    ["3", "2", "지분율은 100%를 초과할 수 없습니다 (입력값 150%)."],
    ["0", "100", "지분율은 0보다 커야 합니다."],
    ["-10", "100", "지분율은 0보다 커야 합니다."],
    ["abc", "100", "지분율은 숫자로 입력하세요."],
    ["50", "0", "지분율을 다시 입력하세요 (저장된 지분 값이 올바르지 않습니다)."],
  ])("%s/%s → %s", (n, d, expected) => {
