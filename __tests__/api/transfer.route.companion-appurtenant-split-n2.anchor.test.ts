 ...
 *
 * 📌 보유기간은 **초일 산입**(소득세법 §95④ — `holding-period-first-day-inclusion.anchor.test.ts`)이다.
 *    취득 2012-06-01 → 양도 2024-06-01(응당일) = 12년(장특 표1 24%). 종전(초일·말일 불산입) 구현은
 *    11년(22%)으로 세어 N2-03 883,849,999 · N2-04 424,335,000이었다. 아래 주석의 「수정 전」 세액
 *    (932,050,000 · 438,735,000 · 400,935,000 등)은 **종전 보유기간 기준 실측**이다.
 */
...
describe("N2 — 컴패니언 부수토지 배율초과 분리 (영 §167의5)", () => {
  ...
  it("N2-03: primary 단기(5개월) — 한도 내만 70%, 초과분은 토지 본래 보유기간 세율", async () => {
     ...
     */
    expect(agg.determinedTax).toBe(880_489_999);
  });

  it("N2-04: primary 장기(12년) — 양쪽 다 누진이라 **세액은 같고 분리만 일어난다**", async () => {
    const agg = await run({ ...primary("2012-06-01"), companionAssets: [companion()] });
    // 이 케이스는 N2 수정 전후 동일하다(당시 424,335,000 — 종전 보유기간 11년 기준).
    // 초일 산입 12년(장특 24%): 과세표준 1,061,500,000 × 45% − 누진공제 65,940,000.
    // 「세액이 안 변한다」가 곧 「분리가 안 됐다」는 뜻이 아님을 고정한다(N2-01이 분리를 본다).
    expect(agg.determinedTax).toBe(411_735_000);
  });
