...
describe("DEEMED_ALLOWED_CATEGORIES", () => {
  it("none → 8 SupportedCategory 전체 허용 (deposit·superficies 포함)", () => {
    expect(DEEMED_ALLOWED_CATEGORIES.none).toHaveLength(8);
  ...
  it("trust (§9) → 전체 허용", () => {
    expect(DEEMED_ALLOWED_CATEGORIES.trust).toHaveLength(8);
  });
