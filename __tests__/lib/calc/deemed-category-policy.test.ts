...
describe("DEEMED_ALLOWED_CATEGORIES", () => {
  it("none → 10 SupportedCategory 전체 허용 (deposit·superficies·receivable·convertible_bond 포함)", () => {
    expect(DEEMED_ALLOWED_CATEGORIES.none).toHaveLength(10);
  ...
  it("trust (§9) → 전체 허용", () => {
    expect(DEEMED_ALLOWED_CATEGORIES.trust).toHaveLength(10);
  });
