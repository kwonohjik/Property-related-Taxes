 ...
 *
 * ⚠️ **재개발·입주권은 여기 넣지 않는다** — 결함이라서가 아니라 **불변식이 다르기** 때문이다.
 *    청산금(§166①1호 「납부한」)은 설계상 스케일하지 않으므로 차익이 지분율에 정비례하지 않는다.
 *    그 축은 `redev-fractional-ownership.anchor.test.ts`(RV)가 **P/Q 정합**으로 지킨다.
 *
