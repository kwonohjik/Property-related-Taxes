...
describe("TM-7 §154① 단서 — **옮길 것이 없다**(무효과 확인)", () => {
  /**
   * ⚠️ **범위는 `temporary_two_house` 맥락뿐이다.** 판정 메뉴는 `one_house` 맥락 카드도 그리고,
   *    그 맥락의 사유별 필수 검증은 판정 메뉴 ⑧에 **있다**(OH-06 — 「옮길 것이 없다」가 그 맥락을
   *    빠뜨렸다). `one-house-judgment-c1-validation.anchor.test.ts` C1-06·C1-33이 고정한다.
   *
