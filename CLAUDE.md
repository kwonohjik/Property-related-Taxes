...
> ⚠️ **기각된 가설**(재검토 금지): 4샤드 동시 호출(오진 — `--workers=1`로도 실패) · IPv6(AAAA 레코드 없음) · 캐시(`.legal-cache/`는 `.gitignore:48`이라 CI엔 애초에 없다) · 해외 IP 전면차단 · 시간대.
...
> 📌 종전에는 샤딩 job과 `e2e-law` job으로 나뉘어 **"두 job의 합이 전건인지"** 확인해야 했다(어긋나면 조용한 미실행). 2026-08-06에 law spec을 샤드로 되돌려 그 위험은 사라졌다 — 이제 샤드 4개가 전건을 나눈다.
> ```bash
