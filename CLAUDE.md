...
>
> ⚠️ **샤드 수를 바꾸면 `--shard=N/4`의 분모도 함께** 바꿔야 한다. 어긋나면 일부 테스트가 어느 샤드에도 안 들어가 **조용히 실행되지 않는다**.

##### 🔴 법제처 API 의존 spec은 샤딩과 공존할 수 없다 — `e2e-law` 전용 job

`law-*.spec.ts` 16파일 **29건**은 법제처 Open API를 실제로 호출한다. 4샤드로 나눈 첫 실행에서 이 중 **5건이 30초~1.5분 타임아웃**으로 깨졌다(POPUP-1·HTML-1·TBL-1·CITE-1·IMP-1). 그런데 **같은 커밋의 샤딩 전 실행**에서는 **전부 5~9초에 통과**했다. ⇒ 샤드 4개가 **같은 OC 키로 동시에** 두드리면서 응답이 느려지거나 막힌 것이다 — 테스트가 깨진 게 아니라 **병렬도가 외부 API 한도를 넘었다**.

⇒ 샤딩 job은 `E2E_SKIP_LAW=1`(`playwright.config.ts` `testIgnore`)로 이들을 빼고, **`e2e-law` job이 단독·`--workers=1`**로 돌린다. 29건뿐이라 직렬로도 짧다.

> ⚠️ **두 job의 합이 전건인지 반드시 확인**한다. `testIgnore` 패턴(`**/*law-*.spec.ts`)과 job의 positional 필터(`law-`)가 어긋나면 그 spec은 **어느 job에서도 돌지 않는다**(조용한 미실행).
> ```bash
> CI=1 E2E_SKIP_LAW=1 npx playwright test --list   # 865
> CI=1 npx playwright test --list law-              #  29  → 합 894 = 전건
> ```
