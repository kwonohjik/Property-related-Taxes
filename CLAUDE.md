...
| **CI** | PR 1회 (철저) | fresh checkout + **`npm ci`** + typecheck + lint + **전체** 테스트(vitest 4샤드) + **E2E**(6샤드) |
...
##### 🔴 병목은 E2E가 아니라 vitest였다 — 전역 jsdom + 샤딩 부재 (2026-08-25)

E2E를 6샤드로 나눈 뒤 **임계 경로가 `check` job으로 옮겨갔다**. 실측(run 32738876959):

| job | 소요 | 내역 |
|---|---|---|
| **check** | **976초(16.3분)** | npm ci 17초 · typecheck 22초 · lint 59초 · **vitest 862초(88%)** |
| E2E 6샤드 | 261~476초 | 준비 54초 · 테스트 261~419초 |

**전체 벽시계 16.3분을 `check` job 하나가 결정**했다. 원인 둘을 실측으로 갈랐다:

1. **전역 jsdom** — `vitest.config.ts`가 `environment: "jsdom"`을 전역으로 걸어 순수 함수 엔진 테스트(`__tests__/tax-engine/`)까지 파일마다 jsdom을 세웠다. 전건 1488파일 CPU 1811초 중 **environment가 545초(30%)** 로, **실제 테스트 실행 89초의 6배**였다. `--environment=node`로 전건을 돌려 DOM이 실제로 필요한 파일을 세니 **1488 중 149건(10%)**, 그중 `.test.ts`는 **3건**뿐이었다. ⇒ `.test.tsx` = jsdom / `.test.ts` = node 분리로 **CPU −28%**(1811→1305초, 벽시계 223→160초). CI는 2 worker라 CPU가 곧 벽시계다.
2. **샤딩 부재** — E2E만 나뉘어 있었다. `--shard=N/4`가 CPU를 정확히 1/4로 가른다.

⇒ **`check`(typecheck+lint) / `test`(vitest 4샤드) / `e2e`(6샤드)** 3-job 구성.

**✅ 호스팅 러너 실측 (run 32782638501 · 32783795938 — PR #1276, 2회)**:

| job | 소요 | 내역 |
|---|---|---|
| typecheck + lint | 175초 | npm ci 27 · typecheck 38 · **lint 94** |
| test 4샤드 | 259~300초 | npm ci 23 · **vitest 253초** |
| E2E 6샤드 | 285~458초 | |
| 리포트 병합 | 45초 | |

**벽시계 976초(16.3분) → 461·504초(7.7~8.4분), 약 −50%.** 임계 경로는 예정대로 E2E로 옮겨갔다(최장 샤드 + 리포트 = 벽시계).

> ⚠️ **E2E 샤드는 run마다 285~458초로 흔들린다**(러너 인스턴스 변동 — 법제처 API 실패가 러너 인스턴스에 종속됐던 것과 같은 층위). **1회 측정으로 벽시계를 단정하지 말 것** — 첫 run의 461초만 보고 「7.7분」이라 적었다가 2차 504초로 정정했다.

> ⚠️ **「CPU ÷ 샤드 수」 추정은 과소평가한다** — 샤드당 155초로 예상했으나 실측 **253초(1.6배)** 였다. 샤딩은 테스트 실행을 나누지만 **transform·import 같은 고정비는 샤드마다 중복**된다(전건 import 1100초는 파일이 1/4이어도 공통 모듈만큼 반복된다). **샤드를 더 늘려도 이 고정비 아래로는 내려가지 않는다** — 다음 병목은 샤드 수가 아니라 고정비다.

> ✅ **샤드 커버리지를 실측 검증했다** — 4샤드 합집합 1488파일 = 디스크 1488파일, 샤드 간 중복 0, 누락 0(json 리포터로 파일 집합 대조). 실행 집합은 **구 설정과도 완전히 동일**하다(양쪽 다 1488파일·16366 passed, 차집합 0). 「조용한 미실행」 함정은 vitest 샤딩에도 그대로 적용되므로 **분모를 바꾸면 이 대조를 다시 할 것**.
>
> 🔴 **`--reporter=dot`의 요약은 파일 6개·테스트 65건을 적게 집계한다**(1488/16380을 1482/16315로 보고). 커버리지를 대조할 때 **dot 요약과 json을 섞어 비교하면 있지도 않은 누락이 만들어진다** — 이 작업 중 실제로 그렇게 오판해 「구 설정이 6파일을 빠뜨렸다」는 결론까지 갔다가 구 설정 json 실행으로 뒤집었다. 커버리지 대조는 **한 리포터로 통일**할 것.
>
> ⚠️ **부수 효과가 CI보다 클 수 있다** — 환경 분리는 로컬 `npm test`도 223→160초로 줄인다. 매 푸시 pre-push에서 체감된다.
>
> ⚠️ 컴포넌트를 렌더하는 `.test.ts`를 새로 만들면 `document is not defined`로 실패한다. 파일을 `.test.tsx`로 만들거나 `vitest.config.ts`의 `DOM_TS`에 추가할 것.
...
- ⚠️ **호스팅 러너는 Linux**다. Mac에서 통과하던 spec이 렌더 차이로 CI에서만 실패할 수 있다 — 원인을 확인해 고칠 것(known-failures 목록에 추가는 금지).
- **`e2e/known-failures.ts`** — **현재 0건**이다(`:41` 빈 배열). 도입 시점에는 master에도 실패하는 **16건**이 있어 제외했고 — 그대로 넣으면 CI가 상시 빨간불이 되어 게이트 구실을 못 한다(lint가 상시 실패 CI에만 있어 실질 관문이 없던 것과 같은 실패) — **2026-08-03 `bb69ea58`에서 전건 해소**됐다(그중 3건은 제품/데이터 결함, 13건은 spec rot). 목록이 비어 있으므로 지금은 **제외 없이 전건이 돈다**. **목록은 줄이기만 한다** — 새 실패를 추가하는 것은 회귀를 숨기는 것이다.
